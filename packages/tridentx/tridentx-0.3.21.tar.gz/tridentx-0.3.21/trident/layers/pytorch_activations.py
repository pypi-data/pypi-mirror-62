...
class PRelu(Layer):
    def __init__(self,num_parameters=1,init=0.25):
        ...
        self.weight = Parameter(torch.Tensor(num_parameters))
        nn.init.constant_(self.weight,0.02)
    ...
    try:
        if isinstance(fn_name,str):
            if fn_name.lower()==fn_name:
                 activation_fn = get_function(fn_name, ['trident.layers.pytorch_activations'] if fn_name in __all__ else  fn_modules)
                 return activation_fn
            else:
                try:
                    activation_fn = get_class(fn_name, ['trident.layers.pytorch_activations']if fn_name in __all__ else fn_modules)
                    return activation_fn()
                except Exception:
                    activation_fn = get_function(camel2snake(fn_name), fn_modules)
                    return activation_fn
        if getattr(fn_name, '__module__', None) == 'trident.layers.pytorch_activations':
