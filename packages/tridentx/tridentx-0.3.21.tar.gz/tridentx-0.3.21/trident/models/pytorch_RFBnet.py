...
class BasicRFB(nn.Module):
    ...
    def __init__(self, in_planes, out_planes, strides=1, scale = 0.1, visual = 1):
        super(BasicRFB, self).__init__()
        self.scale = scale
        self.out_channels = out_planes
        inter_planes = in_planes // 8
        self.branch0 = nn.Sequential(
                Conv2d_Block((1,1),2*inter_planes, strides=strides,activation='relu',normalization='batch',use_bias=False),
                Conv2d_Block((3,3), 2*inter_planes,  strides=1, padding=visual, dilation=visual, activation=None,normalization='batch',use_bias=False)
                )
        self.branch1 = nn.Sequential(
                Conv2d_Block((1,1), inter_planes,  strides=1,activation='relu',normalization='batch',use_bias=False),
                Conv2d_Block((3,3), 2*inter_planes, strides=strides, padding=(1,1),activation='relu',normalization='batch',use_bias=False),
                Conv2d_Block((3,3), 2*inter_planes,  strides=1, padding=visual+1, dilation=visual+1, activation=None,normalization='batch',use_bias=False)
                )
        self.branch2 = nn.Sequential(
                Conv2d_Block((1,1), inter_planes,strides=1,activation='relu',normalization='batch',use_bias=False),
                Conv2d_Block((3,3), (inter_planes//2)*3, strides=1, padding=1,activation='relu',normalization='batch',use_bias=False),
                Conv2d_Block((3,3), 2*inter_planes, strides=strides, padding=1,activation='relu',normalization='batch',use_bias=False),
                Conv2d_Block((3,3), 2*inter_planes, strides=1, padding=2*visual+1, dilation=2*visual+1, activation=None,normalization='batch',use_bias=False)
                )

        self.ConvLinear = BasicConv(6*inter_planes, out_planes, kernel_size=1, stride=1,activation=None,normalization='batch',use_bias=False)
        self.shortcut = BasicConv(in_planes, out_planes, kernel_size=1, strides=strides,activation=None,normalization='batch',use_bias=False)
