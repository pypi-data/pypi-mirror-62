...
resnet50_old=ResNet50(pretrained=False)
resnet50=ResNet50(pretrained=False)


pretrain_weights1 =OrderedDict()
for w in resnet50.model.weights:
    ...
    if ('norm' in item[0] or 'bn' in  item[0] ) and 'gamma'in item[0] and  item[0] not in mapping.value_list :
        ...
        for k, v in pretrain_weights.items():
            if mapping[k] == ''and ( 'conv' in k or 'block' in k)  and 'bias' in k and np.array_equal(item[1],v):
                mapping[k] = item[0]
                break
    elif 'fc' in item[0] and 'kernel' in \
            item[0] and item[0] not in mapping.value_list:
        for k, v in pretrain_weights.items():
            if mapping[k] == '' and ( 'fc' in k or 'probs' in k)  and 'kernel' in k and np.array_equal(item[1], v):
                mapping[k] = item[0]
                break

    elif 'fc' in item[0] and 'bias' in item[0] and item[0] not in mapping.value_list:
        for k, v in pretrain_weights.items():
            if mapping[k] == '' and ( 'fc' in k or 'probs' in k)   and 'bias' in k and np.array_equal(item[1], v):
                mapping[k] = item[0]
                break
...
for layer in resnet50.model.layers:
    if layer is BatchNorm:
        layer._USE_V2_BEHAVIOR=False
    w_new=[]

ws_old=resnet50_1.weights
ws=resnet50.model.weights
for i in range(len(ws)):
    w=ws[i]
    tf.keras.backend.set_value(w,pretrain_weights_dict[mapping1[w.name]])

resnet50_old.model=resnet50_1
print(resnet50_old.infer_single_image('cat.jpg',5))

print(resnet50.infer_single_image('cat.jpg',5))

test_inp=to_tensor(np.random.standard_normal((224,224,3)))
layers1=resnet50_old.model.layers
layers2=resnet50.model.layers

#resnet50.model=resnet50_m
resnet50.model.save('resnet50.h5')
