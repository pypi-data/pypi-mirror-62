...
effb5_old=torch.load('C:/Users/Allan/Downloads/efficientnet-b5.pth')
effb5=EfficientNetB5(pretrained=False)
effb5_old.cpu()
effb5.model.cpu()
effb5.model.trainable=True

paras_old=list(effb5_old.named_parameters())
paras=list(effb5.model.named_parameters())

print('paras_old',len(paras_old))
print('paras',len(paras))

buffer_old=list(effb5_old.named_buffers())
buffer=list(effb5.model.named_buffers())
...
effb5.model.eval()
effb5.model.cuda()
print(effb5.infer_single_image('cat.jpg',5))




effb5.save_onnx('efficientnet-b5.onnx')

#
# example = torch.rand(1, 3, 224, 224).cuda()
#
# traced_script_module = torch.jit.trace(effb5.model, example)
# traced_script_module.save("efficientnet-b5.pt")

effb5.model.cpu()

torch.save(effb5.model,'efficientnet-b5.pth')
effb5.model.cuda()
...
#
#
# effb5=EfficientNet(1.0, 1.1, 240, 0.2, model_name='efficientnet-b5',include_top=True,num_classes=1000)
# pretrain_weights1 =list(effb5.model.named_parameters())
# buf=list(effb5.model.named_buffers())
# buf=[ item for item in buf if  'running_mean' in  item[0] or  'running_var' in  item[0]]
# pretrain_weights1.extend(buf)
#
#
#
# effnetb5=torch.load('C:/Users/Allan/Downloads/efficientnet-b5.pth')
# pretrain_weights=OrderedDict()
#
# for k,v in effnetb5.items():
...
#
#
# for name,para in effb5.model.named_parameters():
#     if name in mapping1:
#         para.data=pretrain_weights_dict[mapping1[name]].data
#
# for name, buf in effb5.model.named_buffers():
#     if name in mapping1:
