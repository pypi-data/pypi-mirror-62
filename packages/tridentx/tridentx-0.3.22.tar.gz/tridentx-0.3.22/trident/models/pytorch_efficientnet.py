...
def EfficientNet(width_coefficient,
    ...
    if include_top:
        ...
        efficientnet.add_module('fc',Dense(num_classes,activation='softmax',name='fc'))


    model=ImageClassificationModel(input_shape=(3,default_size,default_size),output=efficientnet)
    ...
    model.preprocess_flow=[resize((default_size,default_size),keep_aspect=True),normalize(0,255),normalize([0.485, 0.456, 0.406],[0.229, 0.224, 0.225])]
    #model.summary()
    return model
...
def EfficientNetB0(include_top=True,
    ...
    if input_shape is not None and len(input_shape)==3:
        ...
        input_shape=(3, 224, 224)
    effb0 =EfficientNet(1.0, 1.0, 224, 0.2, model_name='efficientnet-b0',include_top=include_top,num_classes=classes)
    if pretrained==True:
...
def EfficientNetB4(include_top=True,
    ...
    if pretrained==True:
        ...
        recovery_model=torch.load(os.path.join(dirname,'efficientnet-b4.pth'))
        recovery_model.to(_device)
        if include_top==False:
            recovery_model.__delitem__(-1)
        else:
