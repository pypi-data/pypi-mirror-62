    ...
    def __call__(self, *input, **kwargs):
        for hook in self._forward_pre_hooks.values():
            ...
            if result is not None:
                ...
                input = result
        if self._built==False :
            if len(input)==1 and not isinstance(input[0],list):
                self.input_shape =list(input[0].size())[1:]
            else:
                if isinstance(input[0],list):
                    input=input[0]
                input_shapes=[list(inp.size())[1:] for inp in input]
...
def summary(model, input_size, batch_size=-1, device="cuda"):
    ...
    print("--------------------------------------------------------------------------------------------------------------------------------")
    line_new = "{0:^45s} {1:^25s}  {2:^25s} {3:^12s}  {4:^15s}  {5:^15s}".format("Layer (type)", "Output Shape","Weight Shape","Bias Shape", "Param #", "FLOPS #")
    print(line_new)
    print("==============================================================================")
    total_params = 0
    total_output = 0
    ...
    for layer in summary:
        ...
        line_new = "{0:^45s} {1:^25s}  {2:^25s} {3:^12s}  {4:^15}  {5:^15}".format(
            layer,
