...
class GanCallback(GanCallbacksBase):
    ...
    def __init__(self, generator=None, discriminator=None, gan_type='gan', label_smoothing=False, noisy_labels=False,
                 ...
                 g_train_frequency=1, d_train_frequency=1, noised_lr=False, **kwargs):
    ...
    def on_data_received(self, training_context):
        try:
            if training_context['gan_role'] == 'generator':
                ...
                self.D_real = to_tensor(to_numpy(self.discriminator(self.img_real)))
                ...
                if self.noised_real and training_context['current_epoch'] < self.noise_end_epoch and random.randint(0,
                                                                                                                    100) % 10 < \
        ...
        if training_context['gan_role'] == 'generator':
            ...
            try:
                ...
                if self.gan_type == 'gan':
                    ...
                    this_loss = CrossEntropyLoss()(D_fake_logit, false_label + 1).mean()
                    ...
                    training_context['tmp_metrics']['D_fake_logit'].append(to_numpy(
                        D_fake_logit).mean())  # adversarial_loss = torch.nn.BCEWithLogitsLoss()  # this_loss =
            ...
            try:
                ...
                if self.gan_type == 'gan':
                    ...
                    this_loss = (-self.D_real + self.D_fake).mean()
                elif self.gan_type == 'wgan-gp':
                    ...
                    this_loss = gp + (-self.D_real + self.D_fake).mean()

                elif self.gan_type == 'lsgan':
                    this_loss = 0.5 * (torch.mean((self.D_real - true_label) ** 2) + torch.mean(self.D_fake ** 2))
                elif self.gan_type == 'rasgan':
                    D_real_logit = sigmoid(self.D_real - self.D_fake.mean(0, True))
                    D_fake_logit = sigmoid(self.D_fake - self.D_real.mean(0, True))
                    this_loss = (CrossEntropyLoss()(D_real_logit, true_label).mean() + CrossEntropyLoss()(D_fake_logit,
                                                                                                          false_label).mean()) / 2
    ...
    def on_epoch_end(self, training_context):
        try:
            if (self.generator_first and training_context['gan_role'] == 'discriminator') or (
                ...
                # if generator_grads < 2e-3 and discriminator_grads < 2e-4 and self.discriminator_best_epoch <
                # training_context[ 'current_epoch'] - 3 and self.generator_best_epoch < training_context[
                # 'current_epoch'] - 3:  #  #     if abs(self.generator_best_metric - generator_metric) > 0.2 and
                # abs(  #             self.discriminator_best_metric - discriminator_metric) > 0.2:  #
        ...
        for k, training_item in training_items.items():
            if isinstance(training_item, ImageGenerationModel):
                if self.generatorA.name == training_item.model.name:
                    ...
                    training_item.training_context['data_feed'] = data_feed
