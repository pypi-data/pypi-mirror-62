...
__all__ = ['read_image','read_mask','save_image','save_mask','image2array','array2image','mask2array','array2mask','list_pictures','normalize','unnormalize','channel_reverse','blur','random_blur','random_crop','resize','rescale','add_noise','gray_scale','to_rgb','auto_level','random_invert_color',
...
def resize(size,keep_aspect=True,order=1):
    def img_op(*image:np.ndarray):
        ...
        if keep_aspect:
            scalex=size[0]/image[0].shape[1]
            scaley = size[1] / image[0].shape[0]
            scale=min(scalex,scaley)
            scale=(scale,scale)
            for i in range(len(results)):
                im=results[i]
                results[i] = transform.rescale(im, scale, clip=False, anti_aliasing=True, multichannel=True if len(im.shape) == 3 else False,order=0 if im.ndim== 2 else order)


            pady = ((size[1] - results[0].shape[0]) // 2, (size[1] - results[0].shape[0]) - (size[1] - results[0].shape[0]) // 2)
            padx = ((size[0] - results[0].shape[1]) // 2, (size[0] - results[0].shape[1]) - (size[0] - results[0].shape[1]) // 2)
            all_pad = (padx, pady)
            ...
            for i in range(len(results)):
                ...
                results[i] = np.pad(im, all_pad, 'edge')
...
def rescale(scale_factor,order=1):
    def img_op(*image:np.ndarray):
        ...
        for i in range(len(results)):
            im = results[i]
            results[i] = transform.rescale(im, (scale_factor,scale_factor), clip=False, anti_aliasing=True, multichannel=True if len(im.shape) == 3 else False,order=0 if im.ndim== 2 else order)
        return unpack_singleton(tuple(results))
    return img_op
...
def random_crop(w, h):
    def img_op(*image:np.ndarray):
        ...
        if width>w:
            offset_x = np.random.choice(range(width - w))
        if height>h:
            offset_y = np.random.choice(range(height-h))
        offset_x1 = np.random.choice(range(w>width)) if w>width else 0
        offset_y1 = np.random.choice(range(h>height)) if h>height else 0
        for i in range(len(results)):
            im = results[i]
            if im.ndim==3:
                returnData =np.zeros((3,h,w),dtype=np.float32)
    ...
    def img_op(*image:np.ndarray):
        ...
        for k in range(len(results)):
            ...
            if im.ndim==3:
                ...
                resized_im =transform.resize(blank, (resized_h,resized_w,blank.shape[-1]),anti_aliasing=True,clip=False, order=0 if blank.ndim== 2 else 1)
                ...
                blank = np.zeros((max_value, max_value))
                blank[i:i+height,j:j+width]=im

                resized_im = transform.resize(blank, (resized_h,resized_w),anti_aliasing=True,clip=False, order=0 if blank.ndim== 2 else 1)
                returnData= np.zeros((max(resized_h,h), max(resized_w,w)))
...
def random_blur():
    def img_op(image: np.ndarray):
        ...
        if n==0:
            ...
            image =  gaussian(image, sigma=n,multichannel=True)
        return image
    return img_op
