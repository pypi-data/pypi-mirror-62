...
class Model(ModelBase):
        ...
        if isinstance(out_var, Sequential):
            ...
            if isinstance(out, tuple):
                for i in range(len(out)):
                    self.targets[self.model.output_names[i]] = list(to_numpy(out[i]).shape[1:])
            else:
                self.targets[self.model.output_names[0]] = list(to_numpy(out).shape[1:])
        else:
    ...
    def do_on_training_start(self):
        # self.model.compile(self.optimizer,loss={**self._losses, **self._model_regs,**self._output_regs},
    ...
    def do_gradient_update(self, log_gradients=False, keep_gradients_history=False):
        ...
        # self.optimizer.apply_gradients(zip(self.training_context['grads'],self.model.trainable_variables))  # loss,
        # metrics = self.model.train_on_batch(self.training_context['current_input'], self.training_context[
        ...
        if save_path is not None:
            ...
            if 'Models' is not None and len('Models') > 1 and not os.path.exists('Models'):
                try:
                    ...
                    pass
            save_full_path = 'Models/model_{0}_epoch{1}.h5'.format(self.model.__name__,
        ...
        try:
            ...
            with tf.GradientTape() as g:
                ...
                for k, v in self._losses.items():
                    ...
                    this_loss = v(target, output) if callable(v) else v.call(target, output)  # v.call(target,output) if
                    # hasattr(v, 'call') else
                ...
                if accumulate_grads == False:
                    ...
                    grads = g.gradient(self.training_context['current_loss'], self.model.trainable_variables)
                    ...
                    if self.training_context['stop_update'] == 0:
                        ...
                        print(self.model.name, 'stop_update {0} times'.format(self.training_context['stop_update']))
                        self.training_context['stop_update'] = self.training_context['stop_update'] - 1
                    ...
                    output = self.model(test_input, training=False)
                    self.training_context['current_model'] = self.model
                    self.training_context['current_output'] = output
                    ...
                    for k, v in self._metrics.items():
                        ...
                        if is_collect_data:
                            self.training_context['metrics'][k].append(
                                v.call(test_target, output).numpy() if hasattr(v, 'forward') else v(test_target,
                                                                                                    output).numpy())
...
class ImageClassificationModel(Model):
    ...
    def infer_single_image(self, img, topk=1):
        if self.model.built:
            ...
            result = self.model.predict(to_tensor(np.expand_dims(img, 0)))
...
class ImageDetectionModel(Model):
    ...
    def infer_single_image(self, img, scale=1):
        if self.model.built:
            ...
            self.model._set_inputs(to_tensor(np.expand_dims(img, 0)))
