...
class Model(ModelBase):
            ...
            if train_data is not None:
                if len(train_data) == 1:
                    ...
                    for i in range(len(train_data)):
                        ...
                        elif data.dtype in (np.int64, np.int32) and (data_shape == outshapes[0][1:]) and data_feed[
                            'target'] < 0:
                            data_feed['target'] = i if data_feed['target'] < 0 else data_feed['target']
                            target = to_tensor(train_data[i])
    ...
    def save_model(self,save_path=None):
        ...
        if save_path is None or len(save_path)==0:
            save_path = self.training_context.get('save_path', self.save_path)
            if save_path is None or len(save_path) == 0:
                save_path = 'Models/model_{0}_epoch{1}.pth'.format(self.model.name, self.training_context['current_epoch'])
