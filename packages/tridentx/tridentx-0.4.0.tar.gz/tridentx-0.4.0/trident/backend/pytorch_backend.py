...
__all__ = ['to_numpy','to_tensor','print_network','plot_tensor_grid','summary','calculate_flops', 'Layer', 'Sequential', 'Input', 'get_device', 'load','Combine','ReplayBuffer']
...
def to_numpy(x) -> np.ndarray:
    ...
    if isinstance(x, np.ndarray):
        ...
        return x.cpu().detach().clone().numpy()
...
def to_tensor(x, dtype=torch.float32,requires_grad=None) -> torch.Tensor:
    if isinstance(x,  torch.Tensor):
        ...
        npdtype=x.dtype
        x = torch.tensor(x) if requires_grad is None else torch.tensor(x,requires_grad=requires_grad)
...
class Layer(nn.Module):
    ...
    def input_shape(self, value):
        ...
        if self._built == False :
            ...
            self.input_filters = -1 if self._input_shape.ndim==0 else int(self._input_shape[0])
    ...
    def output_shape(self, value):
        if isinstance(value, (list, tuple)) and len(value) > 0:
            ...
            raise ValueError('not valid output_shape')
    ...
    def __call__(self, *input, **kwargs):
        for hook in self._forward_pre_hooks.values():
            ...
            if result is not None:
                ...
                input = result
        if self._built==False :
            self.input_shape =input[0].size()[1:] if isinstance(input[0],torch.Tensor) else torch.tensor(-1)
            ...
            if self.output_shape is None :
                if result is not None:
                    if not isinstance(result, tuple):
                        ...
                        self.output_shape =result[0].size()[1:]
...
class Sequential(Layer):
    ...
    def add_module(self, name, module):
        ...
        if len(self._modules)>0:
            ...
            if last_module.built and last_module.output_shape is not None:
                final_output=to_list(last_module.output_shape)
        ...
        if final_output is not None:
            module.input_shape=final_output
        self._modules[name] = module
...
def summary(model, input_size, batch_size=-1, device="cuda"):
    ...
    for h in hooks:
        h.remove()
    layer_width=(max([len(layer) for layer in summary])//5+1)*5


    print("--------------------------------------------------------------------------------------------------------------------------------")
    line_new = "{0:^45s} {1:^20s}  {2:^20s} {3:^8s}  {4:^8s}  {5:^12s}".format("Layer (type)", "Output Shape","Weight ","Bias", "Param #", "FLOPS #")
    print(line_new)
    print("==============================================================================")
    total_params = 0
    total_output = 0
    ...
    for layer in summary:
        ...
        line_new = "{0:<45s} {1:<20s}  {2:<20s} {3:<8s}  {4:<8}  {5:<12}".format(
            layer,
