...
class SegTileImageCallback(VisualizationCallbackBase):
    ...
    def plot_tile_image(self,training_context):
        ...
        for i in range(len(inp)):
            input_arr.append(image2array(self.reverse_image_transform(inp[i])))
        input_arr=np.asanyarray(input_arr)
        tile_images_list.append(input_arr)

        target_arr = to_numpy(target)
        target_arr[target_arr>0]=1

        tile_images_list.append(target_arr*input_arr+(1-target_arr)*255)

        output_arr = to_numpy(output)
        output_arr[output_arr > 0] = 1
        tile_images_list.append(output_arr*input_arr+(1-output_arr)*255)
