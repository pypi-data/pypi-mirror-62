...
from ..layers.tensorflow_normalizations import get_normalization,USE_V2_BEHAVIOR,BatchNorm
...
def bottleneck(num_filters=64,strides=1,expansion = 4,conv_shortcut=True,use_bias=False,name=''):
    ...
    return ShortCut2d(Sequential(Conv2d_Block((1,1),num_filters=num_filters ,strides=strides,auto_pad=True,padding_mode=PaddingMode.zero,normalization='batch',activation='relu',use_bias=use_bias,name=name + '_0_conv'),
...
def ResNet(block, layers, input_shape=(224, 224,3), num_classes=1000, use_bias=True, zero_init_residual=False,
    ...
    resnet.add(Conv2d_Block((7,7),64,strides=2,use_bias=use_bias,auto_pad=True,padding_mode=PaddingMode.zero,normalization='batch',activation='relu',name='first_block'))
    resnet.add(MaxPool2d((3,3),strides=2,auto_pad=True,padding_mode=PaddingMode.zero,name='maxpool'))
    ...
    model.preprocess_flow=[resize((input_shape[1],input_shape[0]),keep_aspect=True),normalize( [103.939, 116.779, 123.68],[1,1,1])]
    #model.summary()
    return model
