"""Add `session_expiration_time` and `carry_over_slots_to_new_session` columns to
table `domain`.
Reason:
...
COLUMNS = [
    ...
    ("carry_over_slots_to_new_session", sa.Boolean()),
]
...
def upgrade():
    for column_name, column_type in COLUMNS:
        migration_utils.create_column(
            TABLE_NAME, sa.Column(column_name, column_type),
        )
