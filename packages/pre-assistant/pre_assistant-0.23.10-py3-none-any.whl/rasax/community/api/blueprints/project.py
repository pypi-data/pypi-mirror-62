...
async def collect_status_calls(service_dict: Dict[Text, StackService]) -> List[Dict]:
...
def blueprint() -> Blueprint:
    ...
    @inject_rasa_x_user()
    async def create_project(request, project_id, user=None):
        settings_service = SettingsService(request[REQUEST_DB_SESSION_KEY])
        print("TEAM")
        print(request.json)
        rjs = request.json
        ...
        print(rjs["username"])
        print(request)
        print(request[REQUEST_DB_SESSION_KEY])

        try:
            #project = settings_service.init_project(user["team"], project_id)
            project = settings_service.init_project(rjs["team"], project_id)
        except ProjectException as e:
            return rasa_x_utils.error(404, "ProjectCreationError", e)

        user_service = UserService(request[REQUEST_DB_SESSION_KEY])
        user_service.assign_project_to_user(rjs["username"], project_id)
    ...
    async def logs(_):
        try:
            ...
            logger.debug(e)
            return rasa_x_utils.error(500, "error collecting logs", e)
