...
def locate_interface() -> Text:
    ...
    if os.path.exists(pkg_index):
        ...
        if not os.environ.get("SKIP_FRONTEND_BUILD", "false").lower() == "true":
            ...
            print_info(
                "Using external frontend build - make sure there is a frontend "
                "available in '{}'.".format(os.path.abspath(external_frontend))
            )
...
def blueprint() -> Blueprint:
    ...
    async def ignore_404s(request: Request, exception):
        if request.path.startswith(API_URL_PREFIX):
