...
from rasa.nlu.training_data import TrainingData as NluTrainingData, Message
...
class DataService(DbService):
    ...
    def delete_entity_synonym(self, project_id: Text, entity_synonym_id: Text) -> bool:
    ...
    def delete_entity_synonym_mapped_value(
        ...
        if deleted and not entity_synonym.synonym_values:
    ...
    def _combine_response_keys_and_intents(training_data: NluTrainingData) -> None:
        ...
        for message in training_data.training_examples:
            response_key = message.get(MESSAGE_RESPONSE_KEY_ATTRIBUTE)
            intent = message.get(MESSAGE_INTENT_ATTRIBUTE)
            if response_key is not None and intent is not None:
                message.set(MESSAGE_INTENT_ATTRIBUTE, f"{intent}/{response_key}")
    ...
    def training_data_contains_retrieval_intents(
        ...
        return any(
            e.get(MESSAGE_RESPONSE_KEY_ATTRIBUTE) is not None
