...
from rasax.community.constants import CONFIG_FILE_TERMS_KEY, DEFAULT_RASA_ENVIRONMENT
...
def float_arg(
    request: Request, key: float, default: Optional[float] = None
) -> Optional[float]:
...
def default_arg(request, key, default=None):
...
def deployment_environment_from_request(
    ...
    return default_arg(request, "environment", default)
...
def zip_files(files: List[str], filename: str, basedir: Optional[str] = None) -> str:
    """Creates a zip of `files` under `filename`.

    If `basedir` is given, files will be zipped relative to it."""
...
def run_operation_in_single_sanic_worker(app: Sanic, f: Callable[[], None]) -> None:
