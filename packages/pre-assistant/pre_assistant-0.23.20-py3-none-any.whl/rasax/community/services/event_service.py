...
from rasax.community import config, utils, metrics
...
class EventService(DbService):
    ...
    def _insert_conversation_from(self, _dict: Dict[Text, Any]) -> Conversation:
        ...
        try:
            ...
            logger.warning(
                f"Conversation for '{sender_id}' could not be created since it already exists."
            )
    ...
    def add_flagged_message(self, sender_id: Text, message_timestamp: float) -> None:
        ...
        if not event:
            logger.warning(
                f"Event with timestamp '{message_timestamp}' for sender id '{sender_id}' was not found."
            )
        else:
...
def continuously_consume(
    endpoint_config: Optional[endpoints.EndpointConfig],
...
def main(should_run_liveness_endpoint: bool = True,) -> None:
    ...
    metrics.set_metrics_config_from_env()

    #endpoint_config = read_event_broker_config() if not config.LOCAL_MODE else None
    endpoint_config = read_event_broker_config()
