from typing import Text, List, Optional, Dict, Callable, Any, Union
import logging
...
def drop_primary_key(key_name: Text, batch_op: BatchOperations) -> None:
...
def rename_or_create_index(
    ...
    meta = _get_meta()
    table = Table(table_name, meta)
    existing_indexes = [index.name for index in table.indexes]

    with op.batch_alter_table(table_name) as batch_op:
        # Databases might save indexes in a lower / upper case version
        matching_index_name = _find_matching_index(old_index_name, existing_indexes)

        if matching_index_name:
            batch_op.drop_index(matching_index_name)

        existing_new_index_name = _find_matching_index(new_index_name, existing_indexes)
...
def _find_matching_index(
    index_name_to_find: Text, existing_indexes: List[Text]
) -> Optional[Text]:
    return next(
        (
            index_name
            for index_name in existing_indexes
            if index_name.lower() == index_name_to_find.lower()
        ),
        None,
    )
