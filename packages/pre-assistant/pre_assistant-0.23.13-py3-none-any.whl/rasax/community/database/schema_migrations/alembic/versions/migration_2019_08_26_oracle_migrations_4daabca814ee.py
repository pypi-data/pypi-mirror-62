...
from rasax.community.database.admin import user_role_mapping
...
migrations = {
    ...
    "single_use_token": {
        FIXED_SIZE_COLUMNS: [{"name": "token", "size": 4000}, "username"]
    },
    "story": {
    ...
    user_role_mapping.name: {FIXED_SIZE_COLUMNS: ["username", "role"]},
            ...
            with op.batch_alter_table(table_name) as batch_op:
                ...
                for column in new_types.get(FIXED_SIZE_COLUMNS, []):
                    ...
                    batch_op.alter_column(
                        column_dict["name"],
                        type_=sa.String(column_dict.get("size", 255)),
                        existing_type=sa.String(),
                    )

            # Migrate columns which should get a variable sized string type
            modifications = [
                migration_utils.ColumnTransformation(c, [sa.Text()], {"nullable": True})
                for c in new_types.get(VARIABLE_SIZED_COLUMNS, [])
            ]
            logger.debug(
