...
from rasax.community.utils import dump_yaml_to_file, dump_yaml
...
class DomainService(DbService):
    def store_domain(
        ...
        domain = Domain(
            ...
            carry_over_slots_to_new_session=session_config.get(CARRY_OVER_SLOTS_KEY),
            actions=actions,
            intents=intents,
            slots=slots,
            entities=entities,
    ...
    def validate_and_store_domain_yaml(
        ...
        self.store_domain(
            domain.cleaned_domain(), project_id, path, store_templates, username
        )
    ...
    @staticmethod
    def dump_cleaned_domain_yaml(domain: Dict[Text, Any]) -> Text:
...
def _templates(
    ...
    templates = domain.get("templates", {})
