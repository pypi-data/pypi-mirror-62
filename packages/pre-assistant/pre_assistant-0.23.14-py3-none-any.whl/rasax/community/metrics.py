...
STORY_CREATED_TTYB = "talk_to_your_bot"
STORY_CREATED_INTERACTIVE_LEARNING = "interactive_learning"
    ...
    async with aiohttp.ClientSession() as session:
        ...
        async with session.post(
            ...
            if not data.get("success"):
                raise Exception(f"Error sending metrics: {data}")
...
def _get_project_status_event(
    session: Session, project_id: Optional[Text] = config.project_name
) -> Dict[Text, Any]:
    ...
    num_lookup_table_entries = sum(len(table["elements"]) for table in lookup_tables)
...
def track_story_created(referrer: Optional[Text]) -> None:
    ...
    if path.startswith("/talk/lenient"):
        origin = STORY_CREATED_TTYB
    elif path.startswith("/talk/strict"):
        origin = STORY_CREATED_INTERACTIVE_LEARNING
    elif path.startswith("/talk"):
        origin = STORY_CREATED_TTYB
...
def metrics_collection_config(args: argparse.Namespace) -> Dict[Text, Any]:
    ...
    track_metrics_consent(settings[CONFIG_METRICS_USER], allow_metrics, args.no_prompt)
    return settings
...
def set_metrics_config_from_env() -> None:
    """Set the metrics configuration based on environment variables."""

    config.metrics_collection_config = _metrics_settings(
        config.metrics_consent_given, config.metrics_user_id
    )
