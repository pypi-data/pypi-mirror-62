...
from meeshkan.server.utils.http_utils import split_path

RoutingInfo = collections.namedtuple('RoutingInfo', ['host', 'hostname', 'path', 'scheme', 'port'])
...
class StaticRouting(Routing):
    ...
    def route(self, path: str, headers: typing.Dict, inbound_scheme='http') -> RoutingInfo:
        return RoutingInfo(path=path, host=self._host, hostname=self._url.hostname,
                           port=get_port(self._url.scheme, self._url.port), scheme=self._url.scheme)
