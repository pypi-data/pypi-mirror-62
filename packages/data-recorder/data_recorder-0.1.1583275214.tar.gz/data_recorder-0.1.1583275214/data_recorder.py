...
class TimeSeriesDataRecorder(DataRecorder):
    ...
    def as_dataframe(self, cols=None, as_delta=False, as_date=True, raw=False):
        ...
        if as_date and 'time' in df.columns:
            if as_delta:
                df['time'] = df['time'].astype(float).apply(lambda x: pd.Timedelta(x - self._start_time, unit=time_unit))
            else:
                df['time'] = df['time'].astype(float).apply(lambda x: pd.Timestamp(x, unit=time_unit))
        return df
