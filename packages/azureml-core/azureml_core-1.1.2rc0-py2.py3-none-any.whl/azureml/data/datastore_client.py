...
class _DatastoreClient:
    ...
    @staticmethod
    def _grant_msi_workspace_access(subscription_id, resource_group, workspace, account_name, role):
        ...
        url = 'https://management.azure.com/subscriptions/{subscription_id}/resourceGroups/{' \
              ...
              'account_name}/blobServices/default/providers/Microsoft.Authorization/roleAssignments/{role_assignment}'
        path_format_arguments = {
            ...
            'role_assignment': rest_client._serialize.url("role_assignment", role_assignment, 'str')
        }
        ...
        if response.status_code not in [200, 201, 409]:
            error_message = ("Please contact an owner of the storage account to manually assign the "
                             "Storage Blob Data Owner and Contributor role to the Workspace Managed Identity."
                             "Trying to grant Workspace MSI access to the Storage "
                             "Account failed with error status '{}' and error message '{}'").format(
    ...
    @staticmethod
    def _check_or_grant_access(subscription_id, resource_group, workspace, account_name):
        rest_client = _DatastoreClient._get_client(workspace, workspace._auth, None)
        url = 'https://management.azure.com/subscriptions/{subscription_id}/resourceGroups/{' \
              'resource_group}/providers/Microsoft.Storage/storageAccounts/{' \
        ...
        # Storage Blob Data Owner and Contributor roles respectively
        missing_roles = {'b7e6dc6d-f1e8-4753-8033-0f276bb0955b', 'b24988ac-6180-42a0-ab88-20f7382dd24c'}
        ...
        if response.status_code == 200:
            ...
            module_logger.warning("You do not have permissions to check whether the Workspace Managed Identity has "
                                  "access to the Storage Account. error '{}'. We will try to grant Storage Blob "
                                  "Data Owner and Contributor roles to the Workspace Managed Identity for the "
                                  "storage account.".format(rest_client._deserialize('ErrorResponse', response)))

        for role in missing_roles:
            _DatastoreClient._grant_msi_workspace_access(subscription_id, resource_group, workspace,
                                                         account_name, role)
    ...
    def register_azure_blob_container(workspace, datastore_name, container_name, account_name, sas_token=None,
        ...
        :param grant_workspace_access: Grant the Workspace Managed Identity (MSI) access to the user storage account,
            defaults to False. This should be set if the Storage account is behind a Virtual Network (VNet). If set
            to True, we will attempt to verify the Workspace Managed Identity has the correct roles assigned and
            attempt to grant any missing roles. It may take a while for the granted access to reflect.
        ...
        if grant_workspace_access:
            ...
            if not resource_group:
                raise UserErrorException("Storage Account's resource group is needed in order to grant the Workspace "
                                         "Managed Identity access to the Storage Account.")
            _DatastoreClient._check_or_grant_access(subscription_id, resource_group, workspace, account_name)
    ...
    def register_azure_sql_database(workspace, datastore_name, server_name, database_name, tenant_id, client_id,
                                    ...
                                    overwrite=False, username=None, password=None):
    ...
    def _register_azure_storage(ws, datastore_name, storage_type, container_name, account_name,
                                ...
                                blob_cache_timeout=None, grant_workspace_access=None, subscription_id=None,
                                resource_group=None):
    ...
    def _register_azure_data_lake(ws, datastore_name, store_name, tenant_id, client_id, client_secret,
        ...
        storage_dto = AzureDataLake(store_name, authority_url=authority_url, resource_uri=resource_url,
    ...
    def _get_client(ws, auth, host):
        ...
        host = host or host_env or get_service_url(
            auth, _DatastoreClient._get_workspace_uri_path(ws._subscription_id, ws._resource_group,
                                                           ws._workspace_name), ws._workspace_id)
    ...
    def _dto_to_datastore(ws, datastore):
        ...
        if datastore_type == constants.AZURE_BLOB:
            ...
            return AzureBlobDatastore(
                ...
                workspace_msi_has_access=as_section.are_workspace_managed_identities_allowed,
