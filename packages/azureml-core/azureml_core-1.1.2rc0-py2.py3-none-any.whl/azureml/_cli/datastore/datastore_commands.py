...
GRANT_WORKSPACE_ACCESS = argument.Argument(
    ...
    help="Grants Workspace Managed Service Identity (MSI) access to the storage account. "
         "Defaults to False. This should be set if the storage account is in a VNet. If set to True, we will use the "
         "current user's identity to grant the Workspace MSI access to the storage account. It may take up to 5 "
         "minutes for the granted access to take effect so the first time you run this, it can fail. If such an event "
         "occurs, please rerun this command after 5 minutes. If this error persists, please contact Azure Support.",
    default=False
)
