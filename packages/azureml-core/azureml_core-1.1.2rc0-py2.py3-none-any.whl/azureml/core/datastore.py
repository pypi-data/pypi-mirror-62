...
class Datastore(object):
    ...
    * `Create and run machine learning
    ...
    def register_azure_blob_container(workspace, datastore_name, container_name, account_name, sas_token=None,
        ...
        :param grant_workspace_access: Grants the Workspace Managed Service Identity (MSI) access to the user's storage
            account. Defaults to False. This should be set if the storage account is in VNet. If set to True, we will
            use the current user's identity to grant the Workspace MSI access to the storage account. It may take up
            to 5 minutes for the granted access to take effect so the initial register call may fail. If such event
            occurs, the register_azure_blob_container method will fail and you will need to run it again. If this error
            persists, please contact Azure Support.
