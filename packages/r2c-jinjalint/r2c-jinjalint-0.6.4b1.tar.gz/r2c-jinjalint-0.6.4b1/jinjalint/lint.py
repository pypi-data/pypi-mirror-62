...
def get_file(source, config, path):
    ...
    try:
        ...
        issue = Issue(location, "", 'Parse error: ' + str(error), 'jinjalint-parse-error')
        return [issue], None
    except RecursionError:
        location = IssueLocation(line=0, column=0, file_path=path)
        issue = Issue(location, "", 'Parse error: maximum recursion depth exceeded', 'jinjalint-recursion-error')
        return [issue], None
