...
from . import ast
from .ast import Node, Attribute
...
ANCHOR_HREF_TEMPLATE_ISSUE_MESSAGE = (
    "The 'a' tag’s 'href' attribute supports the 'javascript:' URI. "
    "If it’s present in the value of the tag the browser will "
    "execute the JavaScript when users click the element. This "
    "presents a cross site scripting risk if the variable is "
    "user controlled. To eliminate the risk use Flask’s 'url_for()' "
    "method to sanitize the template’s variable. This issue is NOT "
    "prevented by Flask’s HTML escaping: explicit sanitization is necessary."
)
...
# Hack for https://github.com/returntocorp/jinjalint/issues/3
# I think the actual fix is making sure only ast.Attribute is passed in here
def _get_attribute(node: Node, strip: bool = False) -> Tuple[str]:
    try:
        val = str(node.value).strip("\"'") if strip else str(node.value)
        return (str(node.name), val)
