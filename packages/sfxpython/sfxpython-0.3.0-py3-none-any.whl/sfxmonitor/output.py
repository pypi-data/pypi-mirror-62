...
from .datapoint import gauge, cumulative
...
class Output(object):
    ...
    def send_datapoints(self, datapoints):
        ...
        if threading.current_thread().ident == self.creator_tid:
            raise RuntimeError(
                "You should not send datapoints from the same thread that the 'configure' method on your monitor was called"
            )
        ...
        for typ, group in groupby(sorted(datapoints, key=self._datapoint_key_func), self._datapoint_key_func):
            out[typ] = [dp._as_dict() for dp in group]
