...
def calc_fsc_mrc(hf1,hf2,bin_idx,maxbin,nbin):
    import fcodes_fast
...
def map_model_fsc(half1_map, half2_map, modelf_pdb, model1_pdb, mask_map=None, map_size=None, model_resol=None):
    import fcodes_fast
    ...
    nbin,res_arr,bin_idx = fcodes_fast.resolution_grid(uc,0,maxbin,nx,ny,nz)
    res_arr = res_arr[:nbin]
    # FSC between halfmaps
    bin_fsc = calc_fsc_mrc(f_hf1,f_hf2,bin_idx,maxbin,nbin)
    ...
    for imap in [f_hf1,f_hf2]:
        bin_fsc = calc_fsc_mrc(imap,f_model1,bin_idx,maxbin,nbin)
        fsc_list.append(bin_fsc)
    # FSC between fullmap and modelfull
    bin_fsc = calc_fsc_mrc(f_ful,f_modelf,bin_idx,maxbin,nbin)
    fsc_list.append(bin_fsc)
    ################################################
    # output plots
...
def main():
    ...
    if args.hff1_map is not None:
        ...
        # FSC between halfmaps
        fsc_list = []
        bin_fsc = calc_fsc_mrc(f_hff1,f_hff2,bin_idx,maxbin,nbin)
        fsc_list.append(bin_fsc)
        calc_hf_fsc = False
