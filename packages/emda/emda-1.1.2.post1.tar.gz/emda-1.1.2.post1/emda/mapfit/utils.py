    ...
    for ibf in range(averagemaps.shape[4]):
        ...
        for imap in range(averagemaps.shape[3]):
            ...
            if imap > 0:
                ...
                t = -np.asarray(t_lst[imap-1], dtype='float') # inverse translation
                st,_,_,_ = fcodes_fast.get_st(nx,ny,nz,t)
                avgmap = utils.get_FRS(unit_cell,np.transpose(rotmat),averagemaps[:,:,:,imap,ibf],interp='cubic')[:,:,:,0]
                data2write = apply_shift(avgmap * st,com,center)
            if imap == 0: data2write = apply_shift(averagemaps[:,:,:,imap,ibf],com,center)
    ...
    '''for i in range(nmaps):
        for j in range(nmaps):
            if i == j:
                print('i,j=',i,j)
                # Diagonals
                S_mat[i,i,:] = varcovmat[i,i,:]#sgnl_var[i]
                T_mat[i,i,:] = totl_var[i]               
            elif i==0 and j > i:
                # Off diagonals
                print('i,j=',i,j)
                S_mat[i,j,:] = varcovmat[0,j,:]#covar[j-1]
                T_mat[i,j,:] = varcovmat[0,j,:]#covar[j-1]
            elif j==0 and i > j:
                print('i,j=',i,j)
                S_mat[i,j,:] = varcovmat[0,i,:]#S_mat[j,i,:]
                T_mat[i,j,:] = varcovmat[0,i,:]#T_mat[j,i,:]
            #else:
            #    print('i,j=',i,j)
            #    S_mat[i,j,:] = S_mat[j,i,:]
            #    T_mat[i,j,:] = T_mat[j,i,:]'''
    for i in range(nmaps):
        for j in range(nmaps):
            if i == j:
                ...
                # Off diagonals
                print('i,j=',i,j)
                S_mat[i,j,:] = varcovmat[i,j,:]#covar[j-1]
                T_mat[i,j,:] = varcovmat[i,j,:]#covar[j-1]
            elif i > j:
                print('i,j=',i,j)
                S_mat[i,j,:] = varcovmat[j,i,:]#S_mat[j,i,:]
                T_mat[i,j,:] = varcovmat[j,i,:]#T_mat[j,i,:]
    # Plotting
    plot_nlines_log(res_arr,[S_mat[0,0,:],S_mat[0,1,:],S_mat[1,0,:],S_mat[1,1,:]],
                            ["S11","S12","S21","S22"],'S_mat_fsc_ij.eps')
    plot_nlines_log(res_arr,[T_mat[0,0,:],T_mat[0,1,:],T_mat[1,0,:],T_mat[1,1,:]],
