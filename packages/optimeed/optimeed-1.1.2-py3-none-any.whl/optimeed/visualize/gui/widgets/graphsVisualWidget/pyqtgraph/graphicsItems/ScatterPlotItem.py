...
offset = 0.1
coords = {
    't': [(-0.5, -0.5+offset), (0, 0.5+offset), (0.5, -0.5+offset)],
...
class ScatterPlotItem(GraphicsObject):
    ...
    **Signals:**
    ...
    def __init__(self, *args, **kargs):
        ...
        profiler('setData')
        self.clicSelector = OnClicSelector()
    ...
    def pointsAt(self, pos, tol_radius=0.0):
        x = pos.x()
        y = pos.y()
        pw = self.pixelWidth()
        ph = self.pixelHeight()
        ...
        for s in self.points():
            ...
            if self.opts['pxMode']:
                s2x *= pw
                s2y *= ph
            if sx-s2x-tol_radius*pw < x < sx+s2x+tol_radius*pw and sy-s2y-tol_radius*pw < y < sy+s2y+tol_radius*pw:
                pts.append(s)
    ...
    def mouseClickEvent(self, ev):
        if ev.button() == QtCore.Qt.LeftButton:
            ...
            curr_iter = 0
            pts = list()
            while not len(pts) and curr_iter < 10:
                pts = self.pointsAt(ev.pos(), tol_radius=curr_iter)
                curr_iter += 1

            if len(pts) > 0:
                self.ptsClicked = pts
                self.sigClicked.emit(self, [self.ptsClicked[0]])
                ev.accept()
            else:
                ev.ignore()
        else:
