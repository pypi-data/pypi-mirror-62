...
class NamedExprFixer(fb.TransformerFixerBase):
    ...
    def _extract_and_replace_named_exprs(self, expr):
        new_assigns = []
        if isinstance(expr, ast.NamedExpr):
            new_assigns.append(ast.Assign(targets=[expr.target], value=expr
                .value))
            new_expr = ast.Name(id=expr.target.id, ctx=ast.Load())
        else:
            new_expr = expr
            if isinstance(expr, ast.UnaryOp):
                new_sub_assigns, new_operand = (self.
                    _extract_and_replace_named_exprs(expr.operand))
                new_assigns.extend(new_sub_assigns)
                expr.operand = new_operand
            if isinstance(expr, (ast.BinOp, ast.Compare)):
                new_sub_assigns, new_left = (self.
                    _extract_and_replace_named_exprs(expr.left))
                new_assigns.extend(new_sub_assigns)
                expr.left = new_left
            if isinstance(expr, ast.BinOp):
                new_sub_assigns, new_right = (self.
                    _extract_and_replace_named_exprs(expr.right))
                new_assigns.extend(new_sub_assigns)
                expr.right = new_right
            if isinstance(expr, ast.BoolOp):
                new_values = []
                for comp in expr.values:
                    new_sub_assigns, new_comp = (self.
                        _extract_and_replace_named_exprs(comp))
    ...
    def _update(self, nodelist, indent=0):
        i = 0
        while i < len(nodelist):
            node = nodelist[i]
            if isinstance(node, (ast.If, ast.While)):
                new_assigns, new_test = self._extract_and_replace_named_exprs(
                    node.test)
                if new_assigns and isinstance(node, ast.While):
                    loopcond_name = '__loop_condition'
                    loopcond_init_node = ast.Assign(targets=[ast.Name(id=
                        loopcond_name, ctx=ast.Store())], value=ast.
                        NameConstant(value=True, kind=None))
                    nodelist.insert(i, loopcond_init_node)
                    node.test = ast.Name(id=loopcond_name, ctx=ast.Load())
                    loopcond_assign_node = ast.Assign(targets=[ast.Name(id=
                        loopcond_name, ctx=ast.Store())], value=new_test)
                    new_ifnode = ast.If(test=ast.Name(id=loopcond_name, ctx
                        =ast.Load()), body=node.body, orelse=[])
