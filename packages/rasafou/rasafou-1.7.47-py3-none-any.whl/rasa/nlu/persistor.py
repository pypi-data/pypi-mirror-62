...
class MinioPersistor(Persistor):
    """Store models on Minio.
    ...
    def __init__(self, bucket_name: Text) -> None:
        ...
        super().__init__()
        self.minioClient = Minio('35.226.180.130:9000',
                  access_key='minio',
                  secret_key='minio123',
                  secure=False)
        self._ensure_bucket_exists(bucket_name)
        self.bucket_name = bucket_name
        print("WE ARE IN MINIO")
        #self.bucket = self.s3.Bucket(bucket_name)

    def list_models(self) -> List[Text]:
        try:
            return [
                self._model_dir_and_model_from_filename(obj.object_name.encode('utf-8'))[1]
                for obj in self.minioClient.list_objects(self.bucket_name,
                              recursive=True)
            ]
        except Exception as e:
            logger.warning(f"Failed to list models in Minio. {e}")
            return []
    ...
    def _persist_tar(self, file_key: Text, tar_path: Text) -> None:
        """Uploads a model persisted in the `target_dir` to Minio."""

        print("In _persist_tar tar_path file_key")
        print(tar_path)
        print(file_key)
        try:
            self.minioClient.fput_object(self.bucket_name, file_key, tar_path)
        except ResponseError as err:
            print(err)
    ...
    def _retrieve_tar(self, model_path: Text) -> None:
        """Downloads a model that has previously been persisted to Minio."""
        print("IN _retrieve_tar , model_path")
        print(model_path)
        #model_path = "nlu.tar.gz"
        #print(model_path)
        tar_name = os.path.basename(model_path)
        #tar_name = os.path.basename("models.tar.gz")
        print(tar_name)
        data = self.minioClient.get_object(self.bucket_name, model_path)
