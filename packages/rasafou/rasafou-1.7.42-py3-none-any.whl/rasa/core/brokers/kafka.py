...
class KafkaEventBroker(EventBroker):
    ...
    def _create_producer(self) -> None:
        ...
        if self.security_protocol == "SASL_PLAINTEXT":
            ...
            print("Finally ya ibheyem we use the right one")
            self.producer = Producer({
                'bootstrap.servers': 'pkc-43n10.us-central1.gcp.confluent.cloud:9092',
                'sasl.mechanisms': 'PLAIN',
                'security.protocol': 'SASL_SSL',
                'sasl.username': 'B4RREBVJU2D6P5B3',
                'sasl.password': 'GH8oaL7tq+m/AnTH6fwN0TrSh9T1tKPbYTC34J9pqJTZtc/G8BeOk9eCEod8wsee'
            })   

    def _publish(self, event) -> None:
        print("WE will send the following event ya bgar")
        print(event)
        result = json.dumps(event)
        print("string event")
        print(result)
        self.producer.produce(self.topic, result)
