...
def check_ascii(val, friendly_name):
    ...
    try:
        ...
        raise ValueError(
            'The string {!r} for {} cannot be UTF-8 decoded'.format(
                str(val, errors='replace').encode('ascii', 'backslashreplace'),
                friendly_name))
    except UnicodeEncodeError:
        raise ValueError(
            'The string {} for {} contains non-ASCII characters'.format(
                str(val).encode('ascii', 'backslashreplace'), friendly_name))

def escape_for_py2_exception(s):
    '''
    Helper function escapes non-ASCII characters, which aren't valid in python 2
    exception messages.
    '''
    assert isinstance(s, (str, bytes)), 'String required as argument'
    return str(str(s).encode('ascii', 'backslashreplace'))
