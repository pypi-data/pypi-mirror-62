...
from qumulo.lib.util import escape_for_py2_exception
...
class RequestError(Exception):
    ...
    def __init__(self, status_code, status_message, json_error):
        ...
        self.status_message = escape_for_py2_exception(status_message)

        json_error = {} if json_error is None else json_error

        module = json_error.get('module', 'qumulo.lib.request')
        self.module = escape_for_py2_exception(module)

        error_class = json_error.get('error_class', 'unknown')
        self.error_class = escape_for_py2_exception(error_class)

        if 'description' in json_error:
            self.description = \
                escape_for_py2_exception(json_error['description'])
        ...
        formatted = escape_for_py2_exception(formatted) or '    (no backtrace)'
        message = str('Error {}: {}: {}\nBacktrace:\n{}').format(
