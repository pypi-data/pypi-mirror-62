...
class _qstr(with_metaclass(BaseQstr, newstr)):
    ...
    3) It throws a TypeError if you specify an `encoding`/`errors` when the
    ...
    def __new__(cls,
        ...
        if isinstance(s, newstr) and (
                encoding is not DEFAULT_ENCODING or
                errors is not DEFAULT_ERRORS):
