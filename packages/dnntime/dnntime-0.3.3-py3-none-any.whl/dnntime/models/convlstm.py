...
# def build_conv_model(X, y, n_input, n_output=1, n_features=1, n_units=64,
#                     d_rate=0.15, n_epoch=10, n_batch=1, verbose=0):
def build_convlstm_model(X_train, y_train, X_test, y_test, l_subseq=1, n_row=1, n_col=1,
    ...
    conv_model.fit(X_train, y_train, epochs=n_epoch, batch_size=n_batch)
    end_time = time.time()
...
def StackedConvLSTM(n_steps, n_row, n_col, n_units, n_features, d_rate=0.5):
    model = Sequential()
    # define encoder
...
def CustomConvLSTM(n_input, n_output, n_units, n_features):
    pass
