...
# def build_gru_model(X, y, n_input, n_output=1, n_features=1, n_units=64,
#                     d_rate=0.15, n_epoch=10, n_batch=1, verbose=0):
def build_gru_model(X_train, y_train, X_test, y_test, n_input, n_output=1,
    ...
    gru_model.fit(X_train, y_train, epochs=n_epoch, batch_size=n_batch)
    end_time = time.time()

    gru_pred = gru_model.predict(X_test)

    rmse, norm_rmse = print_dynamic_rmse(y_test, gru_pred, y_train)
    mae = print_mae(y_test, gru_pred)
    mape = print_mape(y_test, gru_pred)
...
def VanillaGRU(n_input, n_output, n_units, n_features):
    model = Sequential()
