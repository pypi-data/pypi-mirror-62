...
class FronteggClient(AuditsClientMixin):
    def __init__(self,
                 client_id: str,
                 api_key: str,
                 context_callback: typing.Optional[typing.Callable[[RequestT], FronteggContext]] = None,
                 ...
                 metadata_service_url: typing.Optional[str] = None) -> None:
        """Initialize the Frontegg client.
        ...
        :param authentication_service_url: The URL for authenticating against Frontegg.
        """
        self._client = FronteggRESTClient(client_id, api_key, context_callback, base_url, authentication_service_url)

        if audits_service_url:
            self._audits_service_url = audits_service_url
        else:
            self._audits_service_url = urljoin(
                base_url, '/audits')

        if metadata_service_url:
            self._metadata_service_url = metadata_service_url
        else:
            self._metadata_service_url = urljoin(
                base_url, '/metadata')
...
__all__ = ('BaseFronteggClient', 'FronteggRESTClient')
