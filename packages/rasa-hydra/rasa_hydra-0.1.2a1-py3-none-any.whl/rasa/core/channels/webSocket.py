...
HYDRA_CHANNEL = "OMNI_WEBCHAT_00001"
...
class WebChatOutput(OutputChannel):
    ...
    async def send_video_url(
        ...
        """Sends a video to the output"""

        message = {"attachment": {"type": "video", "payload": {"src": video}}}
        await self._send_message(self.sid, message)
...
class WebChatInput(InputChannel):
    ...
    def blueprint(self, on_new_message):
        ...
        @socketio_webhook.route('/webhook/<path:path>')
        async def send_path(request, path):
            return await response.file('./static/{}'.format(path))

        @socketio_webhook.route("/webhook", methods=['GET'])
        async def index_page(request):
            if environ.get('INDEX_HTML'):
                file_path = "./static/{}".format(environ.get('INDEX_HTML'))
            else:
                file_path = "./static/index_kub.html"

            return response.html(open(file_path).read())
        ...
        async def disconnect(sid):
            ...
            output_channel = WebChatOutput(sio, sid, self.namespace, self.bot_message_evt)
