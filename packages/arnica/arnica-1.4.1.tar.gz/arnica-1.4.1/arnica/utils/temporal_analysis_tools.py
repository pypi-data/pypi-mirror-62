...
def sort_spectral_power(time, signal):
	...
	*Determine the harmonic power contribution of the signal.*
	...
	deltaf2 = 1. / ((downsampled_time[1] - downsampled_time[0]) * 2)
...
def power_representative_frequency(time, signal, threshold=0.8):
	...
	*Calculate the frequency that captures a level of spectral power.*
	...
	:returns:
	- **representative_frequency** -Frequency above which the power 
...
def plot_distributions(path="./data.txt"):
	...
	t_fusion = 1930.
	bin_edges = list(np.linspace(t_fusion - 200., t_fusion + 200., 21))
	...
	for j in range(S[1]):
		...
		xticks = list(np.linspace(t_fusion - 200., t_fusion + 200., 11))
