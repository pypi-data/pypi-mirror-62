...
class RedizConventions(object):
    ...
    @staticmethod
    def vanity_words():
        words = ['boldface','albedoes','beefalos','befooled','escalade','debacles','caseload',
                 'beadles','beefalo','caboodle','befleas','befools','belaced','boodles','caboose','codable',
        'doolees','elodeas','solaced','sofabed','seafood','fadable','abodes','cabals','f1eece',
                 'feedab1e','bef1eaed','1abe1ed','facade','defaced','daffed','cabbed','baff1ed','accede','2faced','faded','beaded','decade','ebbed',
             'acceded','effaced','ceded','cecae','feed','fade','face','deed','deaf','dada','dada007',
             'debac1e','decada1','1eafed','fab1ed','dabb1e','cab1ed','1aced','b1eed','1ead','b1ade','f1ea',
             'flee0000','beef0000','babe0000','ee100000','dad00000','deadfa11','fadab1e','1ad1ed','be1aced','class000','leadless','fleeces',
                 'deceased','seeled','sealed','scales','felled','leafless','scaled','ladles','easel000','decaf000','called','sell0000','self0000']
        return [ w.replace('s','5').replace('l','1').replace('o','0') for w in words if len(w)>=5]

    @staticmethod
    def vanity_key(username=None):
        username = random.choice(RedizConventions.vanity_words())
        username = username.replace('l', '1').replace('s', '5').replace('o','0')
        assert all([c in "0123456789abcdef-" for c in username]), 'Must be a,b,c,d,e,f,l,o,s or digits '
        remaining = 100000
        print(username)
        while remaining:
