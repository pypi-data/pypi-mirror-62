...
class RedizConventions(object):
    ...
    @staticmethod
    def vanity_words():
        words = ['f1eece','feedab1e','bef1eaed','1abe1ed','facade','defaced','daffed','cabbed','baff1ed','accede','2faced','faded','beaded','decade','ebbed',
             ...
             'flee','beef','babe','ee1','dad','deadfa11','fadab1e','1ad1ed','be1aced','class','leadless','fleeces',
                 'deceased','seeled','sealed','scales','felled','leafless','scaled','ladles','easel','decaf','called','sell','self']
        return [ w.replace('s','5').replace('l','1') for w in words]

    @staticmethod
    def vanity_key(username=None):
        username = username or random.choice(RedizConventions.vanity_words())
        username = username.replace('l', '1').replace('s', '5')
        assert all([c in "0123456789abcdef-" for c in username]), 'Must be a,b,c,d,e,f or digits '
        remaining = 100000
        print(username)
        while remaining:
