...
class Posting_itemsManager(OperationsManager):
    ...
    def createFromRequest(self, request, removed=None):
        ...
        if isinstance(items, list):
            delAttr(data, 'items')
            _data = data.copy()
            _data.update(opertype = settings.OPERS_TYPES_STACK.POSTING_TASK)
            ...
            with transaction.atomic():
                ...
                for item in items:
                    ...
                    child.update(opertype = settings.OPERS_TYPES_STACK.POSTING_DETAIL_TASK)
                    child.update(status = settings.OPERS_TYPES_STACK.POSTING_DETAIL_TASK_STATUSES.get('new'))
