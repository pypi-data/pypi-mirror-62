...
class Write_off_itemsManager(OperationsManager):
    ...
    def createFromRequest(self, request, removed=None):
        ...
        delAttr(data, 'items')
        _data = data.copy()
        _data.update(opertype=settings.OPERS_TYPES_STACK.WRITE_OFF_TASK)
        _data.update(creator_id=user_id)
        ...
        with transaction.atomic():
            ...
            for item in Tmp_buffer.objects.filter(value_off__isnull=False).exclude(value_off=0):
                child = dict()
                child.update(date=parent.date)
                child.update(creator_id=user_id)
                child.update(description=item.description)
                child.update(opertype=settings.OPERS_TYPES_STACK.WRITE_DETAIL_OFF_TASK)
                child.update(status=settings.OPERS_TYPES_STACK.WRITE_DETAIL_OFF_TASK_STATUSES.get('new'))
