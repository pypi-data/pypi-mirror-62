...
def build_keras_callback(base):
    class KerasCallback(base):  # type: ignore
        ...
        def __init__(
            self, experiment, log_params=True, log_metrics=True, log_graph=True
        ):
            """
        ...
        def on_train_begin(self, logs=None):
            ...
            if self.log_graph:
                self.experiment._set_model_graph(
                    get_keras_json_model(self.experiment, self.model), framework="keras"
                )
            self.experiment._log_other(
                "trainable_params", self.model.count_params(), framework="keras"
            )
