...
class BaseUploadProcessor(object):
    ...
    def _process_upload_by_filepath(self, upload_filepath):
        # type: (ValidFilePath) -> Optional[UploadFileMessage]
        try:
            check_max_file_size(upload_filepath, self.upload_limit, self.TOO_BIG_MSG)
        ...
        upload_filepath = self._handle_in_memory_file_upload(upload_filepath)
    ...
    def _process_upload_io(self, io_object):
        ...
        if self.copy_to_tmp:
            ...
            file_path = write_file_like_to_tmp_file(io_object)
