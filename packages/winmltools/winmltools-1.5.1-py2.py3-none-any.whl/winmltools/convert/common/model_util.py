...
def get_producer_version():
    ...
    """
    return utils.get_producer_version()
...
def get_model_version():
    ...
    """
    return utils.get_model_version()
...
def get_tensorproto_typemap():
    ...
    return {k.lower(): v.number for k, v in d.values_by_name.items()}
...
def add_zipmap(input_name, key_type, class_labels, context):
    ...
    if StrictVersion(onnx.__version__) < StrictVersion('1.2'):
        prob_output = make_map_value_info(prob_output_name, key_type, prob_value_type)
    else:
        map_proto = make_map_value_info(prob_output_name, key_type, prob_value_type)
        prob_output = make_sequence_value_info(prob_output_name, map_proto.type)
    appended_node = make_zipmap_node(context, input_name, prob_output, class_labels)
