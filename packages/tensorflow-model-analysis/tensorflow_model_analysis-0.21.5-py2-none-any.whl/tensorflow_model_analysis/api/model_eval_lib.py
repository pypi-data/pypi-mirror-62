...
from tensorflow_model_analysis.types_compat import Any, Dict, List, NamedTuple, Optional, Set, Text, Tuple, Union
...
def _check_version(version, path):
  if not version:
    raise ValueError(
...
def _serialize_eval_run(eval_config, data_location,
...
def load_validation_result(output_path):
...
def default_eval_shared_model(
    ...
    blacklist_feature_fetches = None,
    tags = None,
    model_name = '',
    eval_config = None):
...
def ExtractAndEvaluate(  # pylint: disable=invalid-name
  ...
  def update(evaluation, new_evaluation):
