...
class AbstractRunner(ABC):
    def __init__(self, starting_date = None, running_date = None, drop_token=None, dropbox_backup = True, hourly_pkl_file_name='hourly_candels.pkl',aggregated_pkl_file_name='hourly_to_mix.pkl', list_pkl_file_name = 'my_list.pkl', local_root_directory='../data/', user = 'napoleon', add_turn_over = False, skipping_size=200):
        super().__init__()
...
class EnsemblingSignalRunner(AbstractRunner):
    def runTrial(self, saver, seed, trigger, transaction_costs, normalization):
        ...
        for me_signal in self.signals_list:
            ...
            if normalization and signal_generator.is_signal_continuum(signal_type):
                return
            ...
            if cumulated_signals is None:
                if self.add_turn_over:
                    ...
                    cumulated_signals = cumulated_signals.rename(columns={'signal': 'signal'+str(counter)})
            else :
                cumulated_signals['signal'+str(counter)]=hourly_df['signal']
                if self.add_turn_over:
