...
def process_weights(w=None, df=None, s=None, n=None, low_bound=None, up_bound=None, weight_cutting_rate = 0.7, normalize = True , display=False, max_iteration = 50, trading_delay=1):
    ...
    for t in range(n, T, s):
        ...
        if normalize:
            weight_vect = normalizeWeightsVector(weight_vect, assets, low_bound, local_up_bound)
            counter=0
            while not(weight_vect.max() <= local_up_bound and weight_vect.min() >= low_bound):
        ...
        if display:
            if w[test_slice.start].max() > local_up_bound or w[test_slice.start].min() < low_bound:
                print('Invalid incoming weights')
            if newValidWeights[0].max()> local_up_bound or newValidWeights[0].min() < low_bound:
