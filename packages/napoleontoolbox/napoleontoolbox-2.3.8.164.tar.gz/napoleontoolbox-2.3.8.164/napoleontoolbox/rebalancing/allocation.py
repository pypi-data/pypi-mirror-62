...
def minSharpe(X, w0=None, normalize=False, low_bound=0., up_bound=1.0):
    T, N = X.shape
    SIGMA = np.cov(X, rowvar=False)
    if N == 0:
...
def minDrawdown(X,  w0=None, normalize=False, low_bound=0., up_bound=1.0):
    T, N = X.shape
    SIGMA = np.cov(X, rowvar=False)
    up_bound = max(up_bound, 1 / N)
...
def rolling_allocation(f, X, n=252, s=63, ret=True, drift=True, filtering_threshold = 0.7, **kwargs):
...
def rolling_ensembling(X, w_mat_predictors, w_mat_target, n=252, s=63, model_description='equireparti',ret=True, drift=True, **kwargs):
    X = pd.DataFrame(X).fillna(method='ffill')
    if ret:
        X_ = X.pct_change()
    else:
        X_ = X
    w_mat_predictors = pd.DataFrame(w_mat_predictors).fillna(0)
    idx = w_mat_predictors.index
    w_mat = pd.DataFrame(index=idx, columns=w_mat_target.columns)
    portfolio = pd.Series(100., index=idx, name='portfolio')

    roll = rolling._EnsemblingRollingMechanism(idx, n=n, s=s)

    def ensemble_process(series):
        # True if less than 50% of obs. are constant
    ...
    for slice_n, slice_s in roll():
        ...
        # print('roll_d')
        #print(roll.d)
        #if roll.d == pd.to_datetime('2019-08-23'):
        #    print('found')
        sub_X = X_.loc[slice_n].copy()
        assets = list(X.columns[sub_X.apply(ensemble_process)])

        train_X = w_mat_predictors.loc[slice_n].copy()
        train_y = w_mat_target.loc[slice_n, assets].copy()

        test_X = w_mat_predictors.loc[slice_s].copy()
        test_y = w_mat_target.loc[slice_s, assets].copy()
        ensembled_weights = {}
        for weightToModel in assets:
            #mask = [col for col in train_X.columns if weightToModel in col]
            mask = [col for col in train_X.columns]

            preds = train_X[mask]
            target = train_y[weightToModel]
            if model_description == 'gradient_boost':
                model = GradientBoostingRegressor(loss='quantile',
                                                  n_estimators=250, max_depth=3,
                                                  learning_rate=.001, min_samples_leaf=9,
                                                  min_samples_split=9)
                model.fit(preds, target)
                pred_y_grad_boost = model.predict(test_X[mask])
                pred_y = pred_y_grad_boost
            elif model_description == 'linear_regression':
                reg = LinearRegression().fit(preds,target)
                pred_y_reg_lin = reg.predict(test_X[mask])
                pred_y = pred_y_reg_lin
            elif model_description == 'equireparti':
                mask = [col for col in train_X.columns if weightToModel in col]
                pred_y_mean = test_X[mask].mean(axis = 1)
                pred_y = pred_y_mean
            elif model_description == 'random_forest':
                clf = RandomForestClassifier(n_estimators=100, max_depth=2,random_state = 0)
                clf.fit(preds, target)
                pred_y_rand_forest = clf.predict(test_X[mask])
                pred_y = pred_y_rand_forest
            # print("reg lin")
            # print(pred_y_reg_lin)
            # print("mean")
            # print(pred_y_mean)
            # print("best")
            # print(test_y[weightToModel])

            # print("pred_y")
            # print(pred_y.sum().sum())
            # print("target")
            # print(target.sum().sum())
            ensembled_weights[weightToModel] = pred_y
            ### end of modeling steps

        ensembled_weights_df = pd.DataFrame.from_dict(ensembled_weights)
        ensembled_weights_df_bis = pd.DataFrame(index=ensembled_weights_df.index, columns=ensembled_weights_df.columns)
        for index, row in ensembled_weights_df.iterrows():
            if row.sum() == 0:
                row = row + 0.00000001
            if np.min(row) < 0:
                row = row - np.min(row)
            row = (row) / row.sum()
            ensembled_weights_df_bis.loc[index] = row

        # ### renormalizing the weights
        # print(w[assets].sum())
        # if w[assets].sum() == 0:
        #     w[assets] = w[assets] + 0.0000000001
        #
        # if np.min(w) < 0:
        #     w = w - np.min(w)
        # w = (w) / w.sum()
        ...
        # w_mat.loc[slice_s, assets] =ensembled_weights_df[assets].values
        ensembled_weights_df_bis = ensembled_weights_df_bis.fillna(0)
        w_mat.loc[slice_s, assets] = ensembled_weights_df_bis.values
