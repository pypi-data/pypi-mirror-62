...
def compute_signals_weights_perf(data = None, optimal_weights = None, starting_date = None, ending_date = None, normalization = False):
    print(max(data.index))
    print(min(data.index))
    data = data[data.index >= starting_date]
    data = data[data.index <= ending_date]
    ...
    sharpe_strat_without = metrics.sharpe(hourly_df['perf_return'].dropna(), period=252 * 24, from_ret=True)
