...
class SimpleExplainer(AbstractRunner):
    def runTrial(self, saver, seed, sup, layers, epochs, n_past_features, n, s, feature_type, activation_type,
        ...
        result = self.dbx.local_supervision_npy_overwrite_and_load_array(starting_date=self.starting_date, ending_date=self.ending_date, rebal=s, lower_bound=low_bound,
        ...
        features, features_names = self.dbx.local_features_npy_ovverwrite_and_load_array(starting_date=self.starting_date,
                                                                                         ending_date=self.running_date,
    ...
    def ratio_computer(self,features_df=None, features_segmentation = {}):
        ...
        for key, features_values in extended_features_segmentation.items():
            ...
            if part_over_time_df is None:
                temp = grouped_features_df.abs().sum(axis=1)
                temp.columns = [key]
