...
class SignalParalleLauncher():
    def __init__(self,drop_token='', dropbox_backup=True, local_root_directory='../data',user='napoleon',  db_path_suffix = '_run.sqlite', triggers = [True], transaction_costs = [True], signal_idiosyncratic_paramaters=[None]):
        self.args = []
        self.counter = 1
        self.seed = 0
        for trigger in triggers:
            for transaction_cost in transaction_costs:
                for signal_type, params  in signal_idiosyncratic_paramaters.items():
                    for param in params:
                        self.args.append((self,self.seed,trigger,signal_type, json.dumps(param, sort_keys=True), transaction_cost))
