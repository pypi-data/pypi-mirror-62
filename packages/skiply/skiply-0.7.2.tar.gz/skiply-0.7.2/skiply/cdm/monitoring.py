...
class Monitoring(SkiplyBase):
    ...
    def update_monitoring_last_network_check(new_monitoring_last_network_check):
        session = db_session()
        try:
            if new_monitoring_last_network_check != None:
                self.monitoring_last_network_check = new_monitoring_last_network_check
        except Exception as e:
            print("DB Request update_monitoring_last_network_check(new_monitoring_last_network_check) Failed with error : {}".format(e))
        finally:
            session.close()
