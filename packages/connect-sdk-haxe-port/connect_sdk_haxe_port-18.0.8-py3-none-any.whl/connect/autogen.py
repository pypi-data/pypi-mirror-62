...
class connect_Env(connect_Base):
    ...
    _hx_fields = []
    _hx_methods = []
    _hx_statics = ["initConfig", "loadConfig", "isConfigInitialized", "initLogger", "isLoggerInitialized", "getConfig", "getLogger", "getApiClient", "getFulfillmentApi", "getUsageApi", "getTierApi", "getGeneralApi", "_reset", "config", "logger", "apiClient", "fulfillmentApi", "usageApi", "tierApi", "generalApi", "defaultDependencies", "dependencies", "init", "initDefaultDependencies", "createInstance"]
    _hx_interfaces = []
    _hx_super = connect_Base
...
class connect_Flow(connect_Base):
    ...
    _hx_methods = ["setup", "step", "getAssetRequest", "getTierConfigRequest", "getUsageFile", "setData", "getData", "approveByTemplate", "approveByTile", "fail", "inquire", "pend", "_run", "process", "processSetup", "loadStepDataIfStored", "getStepParam", "processStep", "prepareRequestAndOpenLogSection", "processAbortAndCloseLogSection", "abort", "logStepData", "getAssetRequestChanges", "getTierConfigRequestChanges", "getClassName"]
    ...
    def approveByTemplate(self,id):
        request = self.getAssetRequestChanges()
        tcr = self.getTierConfigRequestChanges()
    ...
    def processStep(self,step,index,lastRequestStr,lastDataStr):
        requestStr = connect_util_Util.beautifyObject(self.model.toObject(),(connect_Env.getLogger().getLevel() != connect_logger_Logger.LEVEL_DEBUG))
    ...
    def prepareRequestAndOpenLogSection(self,model):
        ...
        product = (assetRequest.asset.product if ((assetRequest is not None)) else (tierConfigRequest.product if ((tierConfigRequest is not None)) else (usageFile.product if ((usageFile is not None)) else None)))
    ...
    def logStepData(self,level,request,data,lastRequest,lastData):
        output = connect_Env.getLogger().getOutputs().iterator()
        while output.hasNext():
            output1 = output.next()
            _hx_list = connect_util_Collection().push(connect_Flow.getFormattedRequest(request,lastRequest,output1.formatter)).push(connect_Flow.getFormattedData(data,lastData,self.data,output1.formatter))
            connect_Env.getLogger()._writeToHandler(level,output1.formatter.formatList(_hx_list),output1)

    def getAssetRequestChanges(self):
        if (self.getAssetRequest() is not None):
            originalModel = python_lib_Json.loads(self.originalModelStr,**python__KwArgs_KwArgs_Impl_.fromT(_hx_AnonObject({'object_hook': python_Lib.dictToAnon})))
            diff = connect_util_Util.createObjectDiff(self.model.toObject(),originalModel)
            return connect_models_Model.parse(connect_models_AssetRequest,haxe_format_JsonPrinter.print(diff,None,None))
        else:
            return None

    def getTierConfigRequestChanges(self):
        if (self.getTierConfigRequest() is not None):
            originalModel = python_lib_Json.loads(self.originalModelStr,**python__KwArgs_KwArgs_Impl_.fromT(_hx_AnonObject({'object_hook': python_Lib.dictToAnon})))
            diff = connect_util_Util.createObjectDiff(self.model.toObject(),originalModel)
            return connect_models_Model.parse(connect_models_TierConfigRequest,haxe_format_JsonPrinter.print(diff,None,None))
        else:
            return None
    ...
    def getFormattedRequest(request,lastRequest,fmt):
        if (request != lastRequest):
            if (connect_Env.getLogger().getLevel() == connect_logger_Logger.LEVEL_DEBUG):
                ...
                requestStr = (connect_util_Util.beautifyObject(diff,False) if ((diff is not None)) else request)
                requestTitle = ("Request (changes):" if ((diff is not None)) else "Request:")
                return (("" + ("null" if requestTitle is None else requestTitle)) + HxOverrides.stringOrNull(fmt.formatCodeBlock(requestStr,"json")))
            else:
...
class connect_Processor(connect_Base):
    ...
    __slots__ = ("flows",)
    _hx_fields = ["flows"]
    _hx_methods = ["flow", "processAssetRequests", "processTierConfigRequests", "processUsageFiles", "run"]
    _hx_statics = []
    _hx_interfaces = []
    _hx_super = connect_Base
...
class connect_api_ConnectHelper:
    ...
    def postFile(resource,id = None,suffix = None,fileArg = None,fileName = None,fileContents = None):
        return connect_api_ConnectHelper.checkResponse(connect_api_ConnectHelper.connectSyncRequest("POST",connect_api_ConnectHelper.parsePath(resource,id,suffix),connect_api_ConnectHelper.getHeaders("multipart/form-data"),None,False,fileArg,fileName,None,fileContents))
    ...
    @staticmethod
    def getHeaders(contentType = None):
...
class connect_api_Query(connect_Base):
    ...
    _hx_methods = ["in_", "out", "limit", "orderBy", "offset", "ordering", "like", "ilike", "select", "equal", "notEqual", "greater", "greaterOrEqual", "lesser", "lesserOrEqual", "toString", "toPlain", "toObject", "toJson", "addRelOp"]
...
class connect_api_impl_ApiClientImpl(connect_Base):
    ...
    def logRequest(level,method,url,headers,body,response):
        ...
        while output.hasNext():
            ...
            connect_Env.getLogger()._writeToHandler(level,fmt.formatBlock((("" + ("null" if firstMessage is None else firstMessage)) + HxOverrides.stringOrNull(fmt.formatList(requestList)))),output1)
    ...
    def getFormattedData(data,title,fmt):
        if connect_util_Util.isJson(data):
            ...
            block = fmt.formatCodeBlock(connect_util_Util.beautify(data,compact),"json")
            return ((("" + ("null" if prefix is None else prefix)) + " ") + ("null" if block is None else block))
        else:
            fixedBody = StringTools.lpad(HxString.substr(data,(len(data) - 4),None),"*",len(data))
...
class connect_api_impl_TierApiImpl(connect_Base):
    ...
    def listTierConfigRequests(self,filters):
        return connect_api_ConnectHelper.get(connect_api_impl_TierApiImpl.TCR_PATH,None,None,filters)
    ...
    def pendTierConfigRequest(self,id):
        return connect_api_ConnectHelper.post(connect_api_impl_TierApiImpl.TCR_PATH,id,"pend")

    def inquireTierConfigRequest(self,id):
        return connect_api_ConnectHelper.post(connect_api_impl_TierApiImpl.TCR_PATH,id,"inquire")
    ...
    def failTierConfigRequest(self,id,data):
        return connect_api_ConnectHelper.post(connect_api_impl_TierApiImpl.TCR_PATH,id,"fail",data)

    def assignTierConfigRequest(self,id):
        return connect_api_ConnectHelper.post(connect_api_impl_TierApiImpl.TCR_PATH,id,"assign")

    def unassignTierConfigRequest(self,id):
        return connect_api_ConnectHelper.post(connect_api_impl_TierApiImpl.TCR_PATH,id,"unassign")
...
class connect_logger_ILoggerFormatter:
    ...
    __slots__ = ()
    _hx_methods = ["formatSection", "formatBlock", "formatCodeBlock", "formatList", "formatTable"]
...
class connect_logger_Logger(connect_Base):
    ...
    __slots__ = ("path", "level", "handlers", "sections")
    _hx_fields = ["path", "level", "handlers", "sections"]
    _hx_methods = ["getPath", "getLevel", "setFilename", "getFilename", "getOutputs", "openSection", "closeSection", "writeBlock", "writeCodeBlock", "writeList", "writeTable", "write", "log", "debug", "info", "notice", "warning", "error", "critical", "alert", "emergency", "_writeToHandler", "writeSections"]
    ...
    def writeBlock(self,level,block):
        ...
        while output.hasNext():
            output1 = output.next()
            self._writeToHandler(level,output1.formatter.formatBlock(block),output1)

    def writeCodeBlock(self,level,code,language):
        output = self.handlers.iterator()
        while output.hasNext():
            output1 = output.next()
            self._writeToHandler(level,output1.formatter.formatCodeBlock(code,language),output1)

    def writeList(self,level,_hx_list):
        output = self.handlers.iterator()
        while output.hasNext():
            output1 = output.next()
            self._writeToHandler(level,output1.formatter.formatList(_hx_list),output1)

    def writeTable(self,level,table):
        output = self.handlers.iterator()
        while output.hasNext():
            output1 = output.next()
            self._writeToHandler(level,output1.formatter.formatTable(table),output1)
...
class connect_logger_LoggerConfig(connect_Base):
    ...
    __slots__ = ("path_", "level_", "handlers_")
    _hx_fields = ["path_", "level_", "handlers_"]
    _hx_methods = ["path", "level", "handlers"]
    _hx_statics = []
    _hx_interfaces = []
    _hx_super = connect_Base
...
class connect_logger_MarkdownLoggerFormatter(connect_Base):
    ...
    def formatCodeBlock(self,text,language):
...
class connect_models_Model(connect_Base):
    ...
    __slots__ = ("fieldClassNames",)
    _hx_fields = ["fieldClassNames"]
    _hx_methods = ["toObject", "toString", "_setFieldClassNames", "getFieldClassName"]
        ...
        while (_g < len(_g1)):
            ...
            if python_Boot.hasField(obj,snakeField):
                ...
                if (tmp == 4):
                    ...
                    if (classObj is not None):
                        if (className != "String"):
                            Reflect.setProperty(model,field,connect_models_Model._parse(classObj,val))
                        else:
                            Reflect.setProperty(model,field,haxe_format_JsonPrinter.print(val,None,None))
                    else:
                    ...
                    if (_g2.params[0] == list):
                        ...
                        Reflect.setProperty(model,field,connect_models_Model._parseArray(classObj1,val))
                    else:
                        fieldClassName2 = model.getFieldClassName(field)
                        if (fieldClassName2 == "DateTime"):
                            Reflect.setProperty(model,field,connect_util_DateTime.fromString(val))
                        else:
                            try:
...
class connect_models_Asset(connect_models_IdModel):
    ...
    __slots__ = ("status", "externalId", "externalUid", "externalName", "product", "connection", "contract", "marketplace", "params", "tiers", "items", "configuration")
    _hx_fields = ["status", "externalId", "externalUid", "externalName", "product", "connection", "contract", "marketplace", "params", "tiers", "items", "configuration"]
...
class connect_models_AssetRequest(connect_models_IdModel):
    ...
    def update(self):
        request = connect_Env.getFulfillmentApi().updateRequest(self.id,self.toString())
        return connect_models_Model.parse(connect_models_AssetRequest,request)
...
class connect_models_Constraints(connect_models_Model):
    ...
    __slots__ = ("hidden", "required", "choices", "unique")
    _hx_fields = ["hidden", "required", "choices", "unique"]
    _hx_methods = []
    _hx_statics = []
    _hx_interfaces = []
    _hx_super = connect_models_Model
...
class connect_models_Param(connect_models_IdModel):
    ...
    __slots__ = ("name", "description", "type", "value", "valueError", "valueChoice", "title", "scope", "constraints", "valueChoices", "phase", "events", "marketplace")
    _hx_fields = ["name", "description", "type", "value", "valueError", "valueChoice", "title", "scope", "constraints", "valueChoices", "phase", "events", "marketplace"]
    _hx_methods = []
    _hx_statics = []
    _hx_interfaces = []
    _hx_super = connect_models_IdModel
...
class connect_models_TierConfigRequest(connect_models_IdModel):
    ...
    def update(self):
        request = connect_Env.getTierApi().updateTierConfigRequest(self.id,self.toString())
        return connect_models_Model.parse(connect_models_TierConfigRequest,request)

    def approveByTemplate(self,id):
        request = connect_Env.getTierApi().approveTierConfigRequest(self.id,haxe_format_JsonPrinter.print(_hx_AnonObject({'template_id': id}),None,None))
        return connect_models_Model.parse(connect_models_TierConfigRequest,request)

    def approveByTile(self,text):
        request = connect_Env.getTierApi().approveTierConfigRequest(self.id,haxe_format_JsonPrinter.print(_hx_AnonObject({'activation_tile': text}),None,None))
        return connect_models_Model.parse(connect_models_TierConfigRequest,request)

    def fail(self,reason):
        request = connect_Env.getTierApi().failTierConfigRequest(self.id,haxe_format_JsonPrinter.print(_hx_AnonObject({'reason': reason}),None,None))
        return connect_models_Model.parse(connect_models_TierConfigRequest,request)

    def inquire(self):
        request = connect_Env.getTierApi().inquireTierConfigRequest(self.id)
        return connect_models_Model.parse(connect_models_TierConfigRequest,request)

    def pend(self):
        request = connect_Env.getTierApi().pendTierConfigRequest(self.id)
        return connect_models_Model.parse(connect_models_TierConfigRequest,request)

    def assign(self):
        request = connect_Env.getTierApi().assignTierConfigRequest(self.id)
        return connect_models_Model.parse(connect_models_TierConfigRequest,request)

    def unassign(self):
        request = connect_Env.getTierApi().unassignTierConfigRequest(self.id)
        return connect_models_Model.parse(connect_models_TierConfigRequest,request)
...
class connect_models_UsageFile(connect_models_IdModel):
    ...
    __slots__ = ("name", "description", "note", "status", "createdBy", "createdAt", "uploadFileUri", "processedFileUri", "product", "contract", "marketplace", "vendor", "provider", "acceptanceNote", "rejectionNote", "errorDetails", "records", "events")
    _hx_fields = ["name", "description", "note", "status", "createdBy", "createdAt", "uploadFileUri", "processedFileUri", "product", "contract", "marketplace", "vendor", "provider", "acceptanceNote", "rejectionNote", "errorDetails", "records", "events"]
    ...
    def update(self):
        usageFile = connect_Env.getUsageApi().updateUsageFile(self.id,self.toString())
    ...
    def uploadRecords(self,records):
        sheet = connect_models_UsageFile.createSpreadsheet(records.toArray())
    ...
    @staticmethod
    def createSpreadsheet(records):
        sheet = []
        sheet.append(["record_id", "item_search_criteria", "item_search_value", "quantity", "start_time_utc", "end_time_utc", "asset_search_criteria", "asset_search_value"])
        _g = 0
        while (_g < len(records)):
            record = (records[_g] if _g >= 0 and _g < len(records) else None)
            _g = (_g + 1)
            x = [record.recordId, record.itemSearchCriteria, record.itemSearchValue, record.quantity, record.startTimeUtc, record.endTimeUtc, record.assetSearchCriteria, record.assetSearchValue]
            sheet.append(x)
    ...
    def parseSheet(sheet):
        ...
        while (_g < _g1):
            ...
            while (_g2 < _g11):
                ...
                if (elem is not None):
                    _hx_type = ("s" if (Std._hx_is(elem,str)) else "n")
                    def _hx_local_3():
                        def _hx_local_1():
                            _hx_local_0 = elem
                            if (Std._hx_is(_hx_local_0,Int) or ((_hx_local_0 is None))):
                                ...
                                raise _HxException("Class cast error")
                            return _hx_local_0
                        return python_internal_ArrayImpl.indexOf(strings,elem,None) if Std._hx_is(elem,str) else _hx_local_1()
                    value = _hx_local_3()
                    ...
                    buf_b.write(Std.string((("<v>" + Std.string(value)) + "</v>")))
                    buf_b.write("</c>")
            buf_b.write("</row>")
        buf_b.write("</sheetData>")
    ...
    def _hx_empty_init(_hx_o):
        ...
        _hx_o.note = None
        _hx_o.status = None
        _hx_o.createdBy = None
        _hx_o.createdAt = None
        _hx_o.uploadFileUri = None
...
class connect_models_UsageRecord(connect_models_Model):
    ...
    __slots__ = ("recordId", "itemSearchCriteria", "itemSearchValue", "quantity", "startTimeUtc", "endTimeUtc", "assetSearchCriteria", "assetSearchValue")
    _hx_fields = ["recordId", "itemSearchCriteria", "itemSearchValue", "quantity", "startTimeUtc", "endTimeUtc", "assetSearchCriteria", "assetSearchValue"]
    _hx_methods = []
    _hx_statics = []
    _hx_interfaces = []
    _hx_super = connect_models_Model
...
class connect_models_UsageRecords(connect_models_Model):
    _hx_class_name = "connect.models.UsageRecords"
    __slots__ = ("valid", "invalid")
    _hx_fields = ["valid", "invalid"]
    _hx_methods = []
    _hx_statics = []
    _hx_interfaces = []
    _hx_super = connect_models_Model
    ...
    def _hx_empty_init(_hx_o):
        _hx_o.valid = None
        _hx_o.invalid = None
connect_models_UsageRecords._hx_class = connect_models_UsageRecords
_hx_classes["connect.models.UsageRecords"] = connect_models_UsageRecords
...
class connect_native_PythonRequest:
    ...
    def request(method,url,headers,body,fileArg,fileName,fileContent,timeout):
        ...
        resp = (requests.request(method, url, headers=parsedHeaders, data=body.encode() if body else None, timeout=timeout) if ((fileContent is None)) else requests.request(method, url, headers=parsedHeaders, files={ fileArg: (fileName, pythonBytes) }, timeout=timeout))
...
class connect_util_DateTime:
    ...
    _hx_methods = ["getYear", "getMonth", "getDay", "getHours", "getMinutes", "getSeconds", "toString", "compare", "equals"]
...
class connect_util_Util:
    ...
    __slots__ = ()
    _hx_statics = ["beautify", "beautifyObject", "isJson", "isJsonObject", "isJsonArray", "createObjectDiff", "isArray", "isStruct", "addIdsToObject", "compactArray"]

    @staticmethod
    def beautify(text,compact):
        try:
            return connect_util_Util.beautifyObject(python_lib_Json.loads(text,**python__KwArgs_KwArgs_Impl_.fromT(_hx_AnonObject({'object_hook': python_Lib.dictToAnon}))),compact)
        except Exception as _hx_e:
            _hx_e1 = _hx_e.val if isinstance(_hx_e, _HxException) else _hx_e
            ex = _hx_e1
            return text
    ...
    @staticmethod
    def beautifyObject(obj,compact):
        if compact:
            ...
            return haxe_format_JsonPrinter.print(obj,None,"  ")
    ...
    def addIdsToObject(object,original):
        ...
        def _hx_local_0(field):
            ...
            if python_Boot.hasField(original,field):
                ...
                if (connect_util_Util.isStruct(value1) and connect_util_Util.isStruct(originalValue)):
                    ...
                    value3 = connect_util_Util.compactArray(value1,originalValue)
        ...
        Lambda.iter(python_Boot.fields(object),_hx_local_0)
        return out
    ...
    def compactArray(array,second):
        ...
        while (_g < _g1):
            ...
            if (connect_util_Util.isStruct(value) and connect_util_Util.isStruct(secondValue)):
                ...
                x1 = connect_util_Util.compactArray(value,secondValue)
                out.append(x1)
