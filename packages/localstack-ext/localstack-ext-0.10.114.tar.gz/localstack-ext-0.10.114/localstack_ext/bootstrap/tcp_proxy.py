...
class AttrDict(vSplR):
 def __init__(self,*args,**kwargs):
  vSplB(AttrDict,self).__init__(*args,**kwargs)
  self.__dict__=self
def tcp_listen(six,addr,port,blk):
 s=socket.socket(AF_INET6 if six else AF_INET,SOCK_STREAM)
 if six and vSplf(socket,'IPV6_V6ONLY'):
...
class Half(vSpln):
 def __init__(self,opt,sock,addr,vSplV):
  self.opt=opt
  self.sock=sock
  self.addr=addr
  self.vSplV=vSplV
  self.name='peer' if self.vSplV else 'client'
  self.queue=[]
  self.dest=vSplT
  self.err=vSplT
  self.ready=vSplP
 ...
 while qs:
  ...
  for q in qs:
   if q.post_wait(r,w,e):
    qs.remove(q)
def main():
 opt={'ip6':vSplP,'bind_port':2223,'addr':'192.168.43.191','port':22}
 server_loop(opt)
