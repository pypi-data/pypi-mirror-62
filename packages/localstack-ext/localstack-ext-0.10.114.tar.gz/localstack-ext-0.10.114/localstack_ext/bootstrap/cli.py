...
def import_auth():
 try:
  ...
  raise xIzFU('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=xIzFc)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=xIzFj)
def get_params(args,names=[],mandatory=xIzFc):
 result=()
 for name in names:
  value=xIzFl
  if name in args:
  ...
  if not value and mandatory:
   raise xIzFU('Please provide %s=... parameter'%name)
  result+=(value)
 if xIzFJ(result)==1:
  return result[0]
 return result
