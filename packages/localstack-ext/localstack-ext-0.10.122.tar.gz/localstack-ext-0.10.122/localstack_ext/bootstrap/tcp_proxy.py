...
class AttrDict(OMJLK):
 def __init__(self,*args,**kwargs):
  OMJLt(AttrDict,self).__init__(*args,**kwargs)
  self.__dict__=self
def tcp_listen(six,addr,port,blk):
 s=socket.socket(AF_INET6 if six else AF_INET,SOCK_STREAM)
 if six and OMJLu(socket,'IPV6_V6ONLY'):
...
class Half(OMJLf):
 def __init__(self,opt,sock,addr,OMJLB):
  self.opt=opt
  self.sock=sock
  self.addr=addr
  self.OMJLB=OMJLB
  self.name='peer' if self.OMJLB else 'client'
  self.queue=[]
  self.dest=OMJLl
  self.err=OMJLl
  self.ready=OMJLk
 ...
 while qs:
  ...
  for q in qs:
   if q.post_wait(r,w,e):
    qs.remove(q)
def main():
 opt={'ip6':OMJLk,'bind_port':2223,'addr':'192.168.43.191','port':22}
 server_loop(opt)
