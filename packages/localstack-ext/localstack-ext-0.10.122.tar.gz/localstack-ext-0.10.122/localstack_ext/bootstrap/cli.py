...
def import_auth():
 try:
  ...
  raise RQfBh('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=RQfBU)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=RQfBO)
def get_params(args,names=[],mandatory=RQfBU):
 result=()
 for name in names:
  value=RQfBx
  if name in args:
  ...
  if not value and mandatory:
   raise RQfBh('Please provide %s=... parameter'%name)
  result+=(value)
 if RQfBG(result)==1:
  return result[0]
 return result
