...
def import_auth():
 try:
  ...
  raise RqQNe('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=RqQNG)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=RqQNL)
def get_params(args,names=[],mandatory=RqQNG):
 result=()
 for name in names:
  value=RqQNM
  if name in args:
  ...
  if not value and mandatory:
   raise RqQNe('Please provide %s=... parameter'%name)
  result+=(value)
 if RqQNi(result)==1:
  return result[0]
 return result
