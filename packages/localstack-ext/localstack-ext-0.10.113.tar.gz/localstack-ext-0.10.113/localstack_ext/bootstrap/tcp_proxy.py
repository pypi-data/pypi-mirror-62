...
class AttrDict(lPFjt):
 def __init__(self,*args,**kwargs):
  lPFjo(AttrDict,self).__init__(*args,**kwargs)
  self.__dict__=self
def tcp_listen(six,addr,port,blk):
 s=socket.socket(AF_INET6 if six else AF_INET,SOCK_STREAM)
 if six and lPFjx(socket,'IPV6_V6ONLY'):
...
class Half(lPFjf):
 def __init__(self,opt,sock,addr,lPFjg):
  self.opt=opt
  self.sock=sock
  self.addr=addr
  self.lPFjg=lPFjg
  self.name='peer' if self.lPFjg else 'client'
  self.queue=[]
  self.dest=lPFjX
  self.err=lPFjX
  self.ready=lPFjn
 ...
 while qs:
  ...
  for q in qs:
   if q.post_wait(r,w,e):
    qs.remove(q)
def main():
 opt={'ip6':lPFjn,'bind_port':2223,'addr':'192.168.43.191','port':22}
 server_loop(opt)
