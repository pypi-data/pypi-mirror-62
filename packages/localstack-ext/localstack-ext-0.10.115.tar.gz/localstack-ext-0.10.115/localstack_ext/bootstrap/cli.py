...
def import_auth():
 try:
  ...
  raise ohCfN('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=ohCfv)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=ohCfG)
def get_params(args,names=[],mandatory=ohCfv):
 result=()
 for name in names:
  value=ohCfp
  if name in args:
  ...
  if not value and mandatory:
   raise ohCfN('Please provide %s=... parameter'%name)
  result+=(value)
 if ohCfb(result)==1:
  return result[0]
 return result
