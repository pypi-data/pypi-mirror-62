...
def import_auth():
 try:
  ...
  raise nxBWl('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=nxBWQ)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=nxBWY)
def get_params(args,names=[],mandatory=nxBWQ):
 result=()
 for name in names:
  value=nxBWM
  if name in args:
  ...
  if not value and mandatory:
   raise nxBWl('Please provide %s=... parameter'%name)
  result+=(value)
 if nxBWc(result)==1:
  return result[0]
 return result
