...
class FuncThread(threading.Thread):
 def __init__(self,func,params=PGHyz):
...
class RequestHandler(BaseHTTPRequestHandler):
 def do_POST(self):
  ...
  try:
   ...
   error_string=PGHyS(e)
   result=json.dumps({'error':error_string})
   self.send_response(500)
  self.send_header('Content-Length','%s'%PGHyJ(result)if result else 0)
  self.end_headers()
  if PGHyJ(result or ''):
 ...
 def handle_request(self):
  ...
  result=json.dumps(result)if PGHyI(result,PGHyi)else result
  return result
 def read_content(self):
  if PGHys(self,'data_bytes'):
   return
  content_length=self.headers.get('Content-Length')
  self.data_bytes=self.rfile.read(PGHyh(content_length))
  self.request_json={}
  try:
...
def s3_download(request):
 ...
 target_file=os.path.join(tmp_dir,request.get('file_name')or 's3file.%s'%PGHyS(uuid.uuid4()))
...
def start_ssh_forward_proxy(options):
 ...
 host=LOCAL_BIND_ADDRESS_PATTERN.replace('*',PGHyS(PGHyJ(USED_BIND_ADDRESSES)+2))
...
def create_network_interface_alias(address,interface=PGHyz):
 sudo_cmd='sudo'
 try:
...
def run_shell_cmd(command):
 try:
  ...
  error_string=PGHyS(e)
  if PGHyI(e,subprocess.CalledProcessError):
...
def run(cmd):
 ...
 return to_str(subprocess.check_output(cmd,stderr=subprocess.STDOUT,shell=PGHyM))
def log(*args):
 print(*args)
 sys.stdout.flush()
def is_mac_os():
 try:
  out=to_str(subprocess.check_output('uname -a',shell=PGHyM))
...
def get_free_tcp_port():
 ...
 return port
def to_bytes(obj):
 return obj.encode('utf-8')if PGHyI(obj,PGHyS)else obj
def to_str(obj):
 return obj.decode('utf-8')if PGHyI(obj,PGHyk)else obj
...
def main():
 logging.basicConfig()
 daemon_type=sys.argv[1]if PGHyJ(sys.argv)>1 else 'main'
 if daemon_type=='main':
...
def start_in_background():
 ...
 run(cmd,outfile=log_file,asynchronous=PGHyM)
