...
class AttrDict(HTmGC):
 def __init__(self,*args,**kwargs):
  HTmGP(AttrDict,self).__init__(*args,**kwargs)
  self.__dict__=self
def tcp_listen(six,addr,port,blk):
 s=socket.socket(AF_INET6 if six else AF_INET,SOCK_STREAM)
 if six and HTmGe(socket,'IPV6_V6ONLY'):
...
class Half(HTmGr):
 def __init__(self,opt,sock,addr,HTmGQ):
  self.opt=opt
  self.sock=sock
  self.addr=addr
  self.HTmGQ=HTmGQ
  self.name='peer' if self.HTmGQ else 'client'
  self.queue=[]
  self.dest=HTmGN
  self.err=HTmGN
  self.ready=HTmGx
 ...
 while qs:
  ...
  for q in qs:
   if q.post_wait(r,w,e):
    qs.remove(q)
def main():
 opt={'ip6':HTmGx,'bind_port':2223,'addr':'192.168.43.191','port':22}
 server_loop(opt)
