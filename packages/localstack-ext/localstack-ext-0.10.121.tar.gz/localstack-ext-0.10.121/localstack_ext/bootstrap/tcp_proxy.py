...
class AttrDict(HtwFz):
 def __init__(self,*args,**kwargs):
  HtwFo(AttrDict,self).__init__(*args,**kwargs)
  self.__dict__=self
def tcp_listen(six,addr,port,blk):
 s=socket.socket(AF_INET6 if six else AF_INET,SOCK_STREAM)
 if six and HtwFW(socket,'IPV6_V6ONLY'):
...
class Half(HtwFp):
 def __init__(self,opt,sock,addr,HtwFV):
  self.opt=opt
  self.sock=sock
  self.addr=addr
  self.HtwFV=HtwFV
  self.name='peer' if self.HtwFV else 'client'
  self.queue=[]
  self.dest=HtwFK
  self.err=HtwFK
  self.ready=HtwFI
 ...
 while qs:
  ...
  for q in qs:
   if q.post_wait(r,w,e):
    qs.remove(q)
def main():
 opt={'ip6':HtwFI,'bind_port':2223,'addr':'192.168.43.191','port':22}
 server_loop(opt)
