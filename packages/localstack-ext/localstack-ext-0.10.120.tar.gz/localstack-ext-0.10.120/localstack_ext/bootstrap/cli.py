...
def import_auth():
 try:
  ...
  raise RPrOl('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=RPrOc)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=RPrOy)
def get_params(args,names=[],mandatory=RPrOc):
 result=()
 for name in names:
  value=RPrOD
  if name in args:
  ...
  if not value and mandatory:
   raise RPrOl('Please provide %s=... parameter'%name)
  result+=(value)
 if RPrOG(result)==1:
  return result[0]
 return result
