...
def import_auth():
 try:
  ...
  raise GAWMV('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=GAWMP)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=GAWMY)
def get_params(args,names=[],mandatory=GAWMP):
 result=()
 for name in names:
  value=GAWMC
  if name in args:
  ...
  if not value and mandatory:
   raise GAWMV('Please provide %s=... parameter'%name)
  result+=(value)
 if GAWMg(result)==1:
  return result[0]
 return result
