...
class AttrDict(Xhzcw):
 def __init__(self,*args,**kwargs):
  XhzcB(AttrDict,self).__init__(*args,**kwargs)
  self.__dict__=self
def tcp_listen(six,addr,port,blk):
 s=socket.socket(AF_INET6 if six else AF_INET,SOCK_STREAM)
 if six and Xhzce(socket,'IPV6_V6ONLY'):
...
class Half(XhzcV):
 def __init__(self,opt,sock,addr,Xhzck):
  self.opt=opt
  self.sock=sock
  self.addr=addr
  self.Xhzck=Xhzck
  self.name='peer' if self.Xhzck else 'client'
  self.queue=[]
  self.dest=Xhzcf
  self.err=Xhzcf
  self.ready=Xhzcp
 ...
 while qs:
  ...
  for q in qs:
   if q.post_wait(r,w,e):
    qs.remove(q)
def main():
 opt={'ip6':Xhzcp,'bind_port':2223,'addr':'192.168.43.191','port':22}
 server_loop(opt)
