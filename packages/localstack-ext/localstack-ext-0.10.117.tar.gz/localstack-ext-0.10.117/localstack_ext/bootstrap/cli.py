...
def import_auth():
 try:
  ...
  raise mBDle('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=mBDlP)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=mBDly)
def get_params(args,names=[],mandatory=mBDlP):
 result=()
 for name in names:
  value=mBDlc
  if name in args:
  ...
  if not value and mandatory:
   raise mBDle('Please provide %s=... parameter'%name)
  result+=(value)
 if mBDln(result)==1:
  return result[0]
 return result
