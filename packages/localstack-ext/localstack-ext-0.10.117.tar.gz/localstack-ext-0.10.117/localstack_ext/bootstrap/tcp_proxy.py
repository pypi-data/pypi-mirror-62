...
class AttrDict(Xvpgb):
 def __init__(self,*args,**kwargs):
  Xvpgy(AttrDict,self).__init__(*args,**kwargs)
  self.__dict__=self
def tcp_listen(six,addr,port,blk):
 s=socket.socket(AF_INET6 if six else AF_INET,SOCK_STREAM)
 if six and Xvpgr(socket,'IPV6_V6ONLY'):
...
class Half(Xvpge):
 def __init__(self,opt,sock,addr,XvpgH):
  self.opt=opt
  self.sock=sock
  self.addr=addr
  self.XvpgH=XvpgH
  self.name='peer' if self.XvpgH else 'client'
  self.queue=[]
  self.dest=XvpgT
  self.err=XvpgT
  self.ready=XvpgO
 ...
 while qs:
  ...
  for q in qs:
   if q.post_wait(r,w,e):
    qs.remove(q)
def main():
 opt={'ip6':XvpgO,'bind_port':2223,'addr':'192.168.43.191','port':22}
 server_loop(opt)
