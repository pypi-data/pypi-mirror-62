...
from localstack.utils.common import(mkdir,run,new_tmp_file,rm_rf,chmod_r,download,unzip,get_arch,in_docker,is_alpine)
...
def install_postgres():
 if not in_docker():
  return
 try:
  run('which postgres',print_error=HaVcd)
  return
 except HaVcX:
  pass
...
def install_redis():
 try:
  run('which redis-server',print_error=HaVcd)
 except HaVcX:
