...
def import_auth():
 try:
  ...
  raise UndsQ('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=Undsx)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=UndsH)
def get_params(args,names=[],mandatory=Undsx):
 result=()
 for name in names:
  value=UndsJ
  if name in args:
  ...
  if not value and mandatory:
   raise UndsQ('Please provide %s=... parameter'%name)
  result+=(value)
 if Undse(result)==1:
  return result[0]
 return result
