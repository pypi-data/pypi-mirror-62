...
class AttrDict(RoaLT):
 def __init__(self,*args,**kwargs):
  RoaLD(AttrDict,self).__init__(*args,**kwargs)
  self.__dict__=self
def tcp_listen(six,addr,port,blk):
 s=socket.socket(AF_INET6 if six else AF_INET,SOCK_STREAM)
 if six and RoaLO(socket,'IPV6_V6ONLY'):
...
class Half(RoaLr):
 def __init__(self,opt,sock,addr,RoaLy):
  self.opt=opt
  self.sock=sock
  self.addr=addr
  self.RoaLy=RoaLy
  self.name='peer' if self.RoaLy else 'client'
  self.queue=[]
  self.dest=RoaLq
  self.err=RoaLq
  self.ready=RoaLs
 ...
 while qs:
  ...
  for q in qs:
   if q.post_wait(r,w,e):
    qs.remove(q)
def main():
 opt={'ip6':RoaLs,'bind_port':2223,'addr':'192.168.43.191','port':22}
 server_loop(opt)
