...
def import_auth():
 try:
  ...
  raise WVwFN('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=WVwFe)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=WVwFx)
def get_params(args,names=[],mandatory=WVwFe):
 result=()
 for name in names:
  value=WVwFh
  if name in args:
  ...
  if not value and mandatory:
   raise WVwFN('Please provide %s=... parameter'%name)
  result+=(value)
 if WVwFb(result)==1:
  return result[0]
 return result
