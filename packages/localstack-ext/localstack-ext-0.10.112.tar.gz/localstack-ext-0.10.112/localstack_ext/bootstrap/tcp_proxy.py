...
class AttrDict(QYiRO):
 def __init__(self,*args,**kwargs):
  QYiRE(AttrDict,self).__init__(*args,**kwargs)
  self.__dict__=self
def tcp_listen(six,addr,port,blk):
 s=socket.socket(AF_INET6 if six else AF_INET,SOCK_STREAM)
 if six and QYiRt(socket,'IPV6_V6ONLY'):
...
class Half(QYiRI):
 def __init__(self,opt,sock,addr,QYiRX):
  self.opt=opt
  self.sock=sock
  self.addr=addr
  self.QYiRX=QYiRX
  self.name='peer' if self.QYiRX else 'client'
  self.queue=[]
  self.dest=QYiRh
  self.err=QYiRh
  self.ready=QYiRg
 ...
 while qs:
  ...
  for q in qs:
   if q.post_wait(r,w,e):
    qs.remove(q)
def main():
 opt={'ip6':QYiRg,'bind_port':2223,'addr':'192.168.43.191','port':22}
 server_loop(opt)
