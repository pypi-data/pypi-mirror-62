...
def check_entities_equal(db_entity: any, new_entity: any) -> bool:
    """
    :param db_entity: any
...
def upload_to_s3_from_ram(content: Union[StringIO], filename: str, folder: str = 'yandex_files/', suffix: str = '.tsv'):
    if filename.startswith('/tmp/'):
        s3_filename = filename[5:]
    else:
        s3_filename = filename
    ...
    s3.meta.client.put_object(Bucket=BUCKET_NAME, Key=f'{folder}{s3_filename}',
                              Body=content.getvalue().encode('utf-8'), ACL='public-read')
    return {
        "status": "success",
        "s3_filename": s3_filename,
        "s3_folder": folder
