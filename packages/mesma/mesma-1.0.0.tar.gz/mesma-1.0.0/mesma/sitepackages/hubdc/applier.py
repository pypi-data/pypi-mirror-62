...
from .core import Extent, Grid, openRasterDataset, openVectorDataset, Pixel, Projection, RasterDriver, RasterSize, RasterBandDataset, RasterDataset, Resolution
from .writer import Writer, WriterProcess, QueueMock
from .progressbar import CUIProgressBar
from ..hubdc import hubdcerrors as errors
...
class ApplierInputRaster(ApplierIO):
    ...
    def fractionArray(self, categories, overlap=0, index=None):
        ...
        for category in categories:
            if category is None:
                ...
                binarizedArray.append(np.float32(tmpArray[0] == category))
    ...
    def sample(self, mask, resampleAlg=gdal.GRA_NearestNeighbour, noDataValue=None,
        ...
        ys, xs = np.indices(mask.shape[1:])[:, mask[0]]
        profiles = list()
        ...
        if len(profiles) != 0:
            profiles = np.hstack(profiles)[:, :, 0]
        else:
            profiles = np.empty((self.dataset().zsize(), 0))
        return profiles
...
class ApplierInputVector(ApplierIO):
    ...
    def dataset(self):
        ...
        if self._dataset is None:
            self._dataset = openVectorDataset(filename=self.filename(), layerNameOrIndex=self._layerNameOrIndex,
                                              update=False)
        return self._dataset
    ...
    def fractionArray(self, categories, categoryAttribute=None, oversampling=1, overlap=0):
        ...
        for category in categories:
            ...
            oversampledRaster = self._rasterize(initValue=0, burnValue=1, burnAttribute=None, allTouched=False,
                                                filterSQL=filterSQL,
                                                overlap=overlap * oversampling, dtype=np.float32,
...
class Applier(object):
    ...
    def _runProcessSubgrids(self):
        ...
        result = self.operatorType.aggregate(blockResults=blockResults, grid=self.grid(), *self.ufuncArgs,
                                             **self.ufuncKwargs)
        return result
...
class ApplierOperator(object):
    ...
    def full(self, value, bands=1, dtype=None, overlap=0):
        ...
        if isinstance(value, list):
            array = [np.full(shape=(self.subgrid().size().y() + 2 * overlap,
                                    self.subgrid().size().x() + 2 * overlap),
                             fill_value=v, dtype=dtype) for v in value]
            array = np.array(array)
        else:
            array = np.full(shape=(bands,
