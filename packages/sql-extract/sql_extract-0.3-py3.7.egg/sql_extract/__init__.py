    ...
    def __init__(self, conn, in_sql_file=None, in_out_file_location=None, in_delim=",", in_quot="\"", in_positionals=None, in_named=None, in_text=None):
        ...
        if not (in_sql_file or in_text):
            ...
            # read in sql from cmd line
                ...
                self.sql = _read_input_sql(in_text)

            # clean up string, remove semicolon if needed
            self.sql = self.sql.strip()
            self.sql = self.sql[:-1] if self.sql[-1:] == ";" else self.sql

        if in_positionals and in_named:
            raise IOError("You cannot use named and positional bind parameters.")

        self.binds = self.parse_named_binds(in_named)
        self.row_count = 0
        ...
        self.pos_vars = in_positionals if in_positionals else []
        try:
            if self.binds:
                self.cur.execute(self.sql, **self.binds)
            elif self.pos_vars:
                self.cur.execute(self.sql, self.pos_vars)
            else:
