    ...
    def get_connection(self):
        ...
        * [conn] pymysql.connect, the connection object from pool
        ...
        try:
            ...
            # re-creat a new connection if connection failed
            conn = pys.connect(
                user=self.params['user'],
                password=self.params['password'],
                host=self.params['host'],
    ...
    def refresh(self):
        ...
        try:
            ...
            while not self.pool.is_empty():
                ...
                try: # check connection will raise exception if lost connection
                    if not pys.check_connection(conn):
                        backs.append(conn)

                except Exception as e: # reconnect if lost connection to server
                    conn = pys.connect(
                        user=self.params['user'],
                        password=self.params['password'],
                        host=self.params['host'],
            ...
            # refreshing pool
            logger.message(p.INFO, msg="refresing pool finished...")
