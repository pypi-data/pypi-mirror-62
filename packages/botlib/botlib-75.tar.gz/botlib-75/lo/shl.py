...
def parse_cli(name, version=lo.__version__, opts=None, usage="", lf=None):
    ...
    lo.cfg.update(cfg)
    level(cfg.level or "error", cfg.logfile or lf or os.path.join(lo.workdir, "logs", "%s.log" % cfg.name))
    logging.debug("BOT started at %s (%s)" % (time.ctime(time.time()), lo.workdir))
    return cfg
