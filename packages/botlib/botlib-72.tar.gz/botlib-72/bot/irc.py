...
k = bot.kernels.get_first()

def init(k):
    bot = IRC()
    bot.cmds.update(k.cmds)
    bot.cfg.last()
    try:
        bot.cfg.server, bot.cfg.channel, bot.cfg.nick = k.cfg.args
        bot.cfg.save()
    except ValueError:
        pass
    if not bot.cfg.server or not bot.cfg.channel or not bot.cfg.nick:
        print("%s <server> <channel> <nick>" % k.cfg.name)
        raise lo.hdl.EINIT
    bot.start()
    return bot
...
class IRC(lo.hdl.Handler):
    ...
    def start(self):
        ...
        lo.thr.launch(self.output)

class DCC(lo.hdl.Handler):
...
def PRIVMSG(handler, event):
    if event.txt.startswith("DCC CHAT"):
        try:
            ...
            dcc.encoding = "utf-8"
            lo.thr.launch(dcc.connect, event)
            return
        except ConnectionRefusedError:
            return
