...
class SshManager(object):
    ...
    def _checkAccessAndGrantIfNecessaryIn2Steps(self, host, primaryUser, primaryPassword, finalUser, finalPw,
    ...
    def _runCommand(self, host, userSsh, command, client, ignoreStdStreams=False, txtToAdd="", timeout=SSH_CMD_TIMEOUT,
                    env=None):
        """
        run command method
            ...
            if client is None:  # no access for finalUser via SSH key
                ...
                try:
                    if twoStep:
                        ...
                        self._checkAccessAndGrantIfNecessaryIn2Steps(host, primaryUser, primaryPw, finalUser, finalPw,
                                                                     destPort)
                        client = self._checkAccessUsingKey(host, finalUser, destPort)
                    else:
                        if useKey:
