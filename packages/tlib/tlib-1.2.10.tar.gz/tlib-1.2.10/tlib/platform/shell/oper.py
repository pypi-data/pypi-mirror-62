...
def backup_file(srcpath, filename, dstpath, label=None):
    ...
    If label is None, tlib will use time.strftime('%H:%M:S')
...
class ShellExec(object):  # pylint: disable=R0903
    ...
    def __init__(self, tmpdir='/tmp/'):
        ...
        self._tmpdir = tmpdir
        self._tmpprefix = 'tlib.shell.{0}'.format(uuid.uuid4())
    ...
    def run(self, cmd, timeout):
        ...
        ::
            import tlib
            shelltool = tlib.shell.ShellExec()
...
def is_proc_alive(procname, is_whole_word=False, is_server_tag=False, filters=False):
    """
    Deprecated. Recommand using tlib.oper.is_proc_exist
    """
    # print procName
    if is_whole_word:
