...
class Controller():
    ...
    def set_save_config(self):
        ...
        while t>0:
            ...
            t=t-INTERVAL
        if t<=0:
            dialog=ErrorDialog(self,label='Error: Operation timed out setting save configuration.')
            return
    ...
    def check_remote_folder(self,dir,next_action):
        self.log('checking remote folder')
        ...
        if status=='listdirfailed':
            ...
            if not self.text_only:
                ...
                dialog=ErrorDialog(self,label='Error: Remote directory operation timed out.')
                self.log('Error: Remote directory operation timed out.')
            return False
        self.log('checking writeability')
        self.spec_commander.check_writeable(dir)
        t=300*BUFFER #Probably an excessively long timeout.
        while t>0:
            if 'yeswriteable' in self.spec_listener.queue:
                self.spec_listener.queue.remove('yeswriteable')
                self.log('writeable')
                ...
                self.spec_listener.queue.remove('notwriteable')
                dialog=ErrorDialog(self, label='Error: Permission denied.\nCannot write to specified directory.')
                self.log('notwriteable')
                return False
            time.sleep(INTERVAL)
            t=t-INTERVAL
        if t<=0:
            dialog=ErrorDialog(self,label='Error: Remote directory operation timed out.')
            self.log('Error: Remote directory writeability check timed out.')
            return False
    ...
    def finish_process(self,output_file):
        ...
        self.log(final_data_destination)
        ...
        self.log('moving data to '+final_data_destination)
        shutil.move(self.spec_temp_loc+'proc_temp.csv',final_data_destination)
        self.log('moving log to '+final_log_destination)
...
class CommandHandler():
    ...
    def wait(self):
        while True:
            ...
            self.timeout_s-=1
            if self.timeout_s<0:
                self.controller.log('super timeout')
                self.timeout()
            time.sleep(1)
...
class ProcessHandler(CommandHandler):
    ...
    def wait(self):
        while True: #self.timeout_s>0: Never going to timeout
            self.controller.log('No timeout')
            if self.timeout_s%20==0:
                self.controller.log(self.timeout_s)
            if 'processsuccess' in self.listener.queue or 'processsuccessnocorrection' in self.listener.queue or 'processsuccessnolog' in self.listener.queue:
                self.controller.log('process success!')
                warnings=''
...
class RemoteDirectoryWorker():
    ...
    def reset_timeout(self):
        self.timeout_s=300*BUFFER #Probably an excessively long timeout.
