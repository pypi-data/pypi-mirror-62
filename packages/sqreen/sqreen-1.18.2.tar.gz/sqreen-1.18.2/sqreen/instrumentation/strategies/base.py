...
class BaseStrategy(object):
    ...
    def add_callback(self, callback):
        ...
        if hasattr(callback, LIFECYCLE_METHODS["PRE"]):
            callbacks = self.callbacks[key][LIFECYCLE_METHODS["PRE"]]
            idx = bisect([c.priority for c in callbacks], callback.priority)
            callbacks.insert(idx, callback)

        # Reverse order insertion
        for method in (LIFECYCLE_METHODS["POST"], LIFECYCLE_METHODS["FAILING"]):
