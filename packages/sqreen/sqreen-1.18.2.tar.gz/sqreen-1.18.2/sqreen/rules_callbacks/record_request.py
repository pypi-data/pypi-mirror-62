...
class RecordRequest(RuleCallback):
    ...
    def _store_request(self, request):
        ...
        if self.runner is not None:
            whitelist_match = self.runner.settings.whitelist_match(request)
            if whitelist_match is not None and self.runner.whitelisted_metric:
