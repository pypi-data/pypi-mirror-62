...
class ConditionEvaluationError(ConditionError):
    """Exception raised at runtime when evaluation cannot complete."""
...
def str_included(haystack, needle, min_needle_length=0):
    ...
    if needle is found it returns True, otherwise it raises a
    ...
    if is_unicode(haystack):
        ...
        # to unicode and look for needle in it, otherwise we raise an error
        error = None
            ...
            if is_unicode(elem):
                ...
                try:
                    ...
                    if elem_safe is not None and needle in elem_safe:
                        return True
                    # if nothing was found and type is unknown we have an error
                    if error is None and not isinstance(elem, bytes):
                        error = ConditionEvaluationError(
                            "unexpected input type {!r} in list (index {})".format(type(elem), idx))
        ...
        try:
            ...
            if not isinstance(haystack, bytes):
                # raise if we don't find anything (we were not expecting this type at first)
                raise ConditionEvaluationError("unexpected input type {!r}".format(type(haystack)))
...
def coerce_operator_types(operator):
    """Convert operator arguments to unicode when possible.

    The items can be either strings, bytes, list of items and dict of items.
    In order to avoid complexity, only the first level of items are
