...
class WSGIApplicationWrapper:
    ...
    def __init__(self, middleware, app, environ, start_response, *args):
        ...
        try:
            new_args = middleware.execute_pre_callbacks((environ,))
            if new_args:
                environ = new_args[0]
            self.it = self.serve_application(environ)
        except Exception as e:
            resp = middleware.execute_failing_callbacks(e, args=(environ,))
            if resp is None:
                raise
            ...
            if not self.late_post:
                ...
                middleware.execute_post_callbacks(None, args=(environ,),
                                                  record_attack=False)
    ...
    def __iter__(self):
        try:
            ...
            if not self.orig_write_func:
                ...
                resp = self.middleware.execute_failing_callbacks(e)
                if resp is None:
                    raise
                self._start_response(resp.status, list(resp.headers.items()),
                                     sys.exc_info())
            ...
            if self.late_post:
                ...
                # been freed already
                self.middleware.execute_post_callbacks(None, record_attack=False)
        finally:
            self.close()
