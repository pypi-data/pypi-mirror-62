...
def entrypoint(project, timeout):
    ...
    if drone_build:
        logger.info(
            f"Pull request mergiata su master! Puoi seguire lo stato della build su {drone_build}"
        )
    else:
...
def select_pr(project):
    ...
    with Halo(text="Loading pull requests...", spinner="dots", color="magenta"):
        choices = [
            {"name": choice(project, pr), "value": pr}
            for pr in github.get_list_pr(project)
            if pr.mergeable
        ]
    if choices:
...
def stop_if_master_locked(project, lock):
    try:
        request = lock.status(project, "staging")
    except Exception:
...
def get_drone_build(project, sha):
    drone_build = drone.get_pr_build_url(project, sha)
    if drone_build != "":
        return drone_build
    return None
