...
class OpenAPIToPostman:
    ...
    def _generate_postman_collections(self):
        ...
        for verb, path, operation in ordered_routes:
            ...
            if global_variables:
                logger.debug(f'{verb} {path} global variables {global_variables}')
            if query:
                logger.debug(f'{verb} {path} query param variables {query}')                
            if request_url_variables:
                logger.debug(f'{verb} {path} url variables {request_url_variables}')
            if request_header:
                logger.debug(f'{verb} {path} headers {request_header}')
            if request_body:
                logger.debug(f'{verb} {path} request_body {request_body}')
            items.append(
                Item(
                    id=uuidgen(),
