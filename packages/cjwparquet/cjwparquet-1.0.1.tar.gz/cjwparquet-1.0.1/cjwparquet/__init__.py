...
def open_as_mmapped_arrow(parquet_path: Path) -> ContextManager[pyarrow.Table]:
    ...
    with tempfile.NamedTemporaryFile(
        prefix="parquet-to-arrow-",
        suffix=".arrow",
        # Python defaults to /tmp for tempfiles, but Linux normally uses
        # tmpfs for /tmp -- meaning, files are in RAM. Force /var/tmp so
        # the file is on the filesystem, not RAM.
        dir="/var/tmp",
    ) as tf:
        # raise ArrowIOError
