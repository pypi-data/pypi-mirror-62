...
test_links = [
    ...
    links.Pump(
        ...
        source_uid="junction",
        target_uid="storage",
        speed=[{"flow": 1, "head": 1, "speed": 1}, {"flow": 3, "head": 3, "speed": 1}],
        min_speed=1.0,
        created="2019-06-27T16:53:05Z",
    ),
    links.Turbine(
...
def empty_network():
    ...
    network = Network(network_name, rest_client)
    yield network
...
def test_get_nodes_filtered(network):
    remote = network.get_nodes(dt="2019-06-27T16:53:04")
    assert remote == []
...
def test_get_links_filtered(network):
    remote = network.get_links(dt="2019-06-27T16:53:04")
    assert remote == []
