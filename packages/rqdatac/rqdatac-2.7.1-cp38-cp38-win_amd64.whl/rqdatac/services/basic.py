...
from rqdatac.utils import to_date, datetime_to_int14, to_date_str, str_to_dt_time, int8_to_date, pd_version
...
def all_instruments(type=None, date=None, market="cn"):
    ...
    if len(itype) == 1:
        ...
        if "_shenwan_industry_code" in df.columns:
            if pd_version >= "0.21":
                df.drop(columns=["_shenwan_industry_code", "_shenwan_industry_name"], inplace=True)
            else:
                df.drop(["_shenwan_industry_code", "_shenwan_industry_name"], axis=1, inplace=True)
    else:
        df = pd.DataFrame(
            [
                (
