...
#post_container .fixed-hight .thumbnail{height:159px; overflow: hidden;background:url("images/load_img.png") no-repeat;background-size: cover;}
    ...
    for vfile in p.glob('*'):
        ...
        if suffix in video_set:
            ...
            try:
                ...
                nn = t.count_frames()
                frame = t.get_data(min(1800,int(nn/10)))
                imageio.imwrite(jpg_name,frame)
            except Exception as e:
                print(vfile,'\n',e)
    generate_index_html(dirname)
def generate_poster_all(dirname):
    for root,_,_ in os.walk(dirname):
        generate_poster(root)
    generate_poster(dirname)
...
if __name__=='__main__':
    generate_poster_all(sys.argv[1])