...
# The package to be installed is specified by setting the PITHY_PACKAGE environment variable,
# or else inferred from the name of the parent directory (during end-user pip install).
...
# Pip requires that the setup script is named setup.py,
# and dev mode (`-e`) requires that we place setup.py in the parent directory of each package.
# Therefore, we must the same setup.py for each package.
# We make this work by telling setup.py which package it is installing via the PITHY_PACKAGE environment variable.

base_dir = path_dir(norm_path(__file__)) or '.' # When run from pip, this is a dir with the package name, inside a temp dir.
assert not base_dir.endswith('/'), base_dir
base_name = path_name(base_dir)


package_name = environ.get('PITHY_PACKAGE')
if package_name is None:
  package_name = base_name
  print(f'PITHY_PACKAGE environment variable not set; using: {package_name!r}')
assert package_name
assert not package_name.endswith('/'), package_name
...
def main() -> None:
  ...
  cfg_path = f'setup-{package_name}.cfg'
