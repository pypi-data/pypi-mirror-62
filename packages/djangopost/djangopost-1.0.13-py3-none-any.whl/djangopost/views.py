...
def ArticleListView(request):
    ...
    promo = ArticleModel.objects.by_promo()
...
def CategoryDetailView(request, category_slug):
    ...
    article_filter = ArticleModel.objects.filter(category=category_detail).filter(is_promote=False)
    article_filter = Paginator(article_filter, 4)
    article_filter = article_filter.get_page(page)
    is_promoted = ArticleModel.objects.is_promoted().filter(category=category_detail)
    is_trending = ArticleModel.objects.is_trending().filter(category=category_detail)
    promo = ArticleModel.objects.by_promo().filter(category=category_detail)
