...
# For user defined results, object extra fields
oefEnvironmentPos = 0
oefRadialPos = 2
oefTheta = 3
oefWingName = 4
    ...
    def __setitem__(self, index, value):
        if isinstance(index, slice):
            if self.sliceCount(index) != len(value):
                raise ValueError(
                    'attempt to assign sequence of size {0} to slice of size {1}'.format(len(value), self.sliceCount(index)))
...
class ModalAnalysisSpecification(PackedStructure):
    ...
    def __init__(self, calculateShapes=True, firstMode=-1, lastMode=-1, includeCoupledObjects=False):
        PackedStructure.__init__(self)
...
class HelperMethods(object):
    ...
    def FrequencyDomainTimeHistorySampleInterval(modelHandle):
        generalHandle = HelperMethods.GetGeneralHandle(modelHandle)
        return HelperMethods.GetDataDouble(generalHandle, 'FrequencyDomainSampleInterval', -1)
    ...
    @staticmethod
    def RangeGraphPointCount(handle, varID, arclengthRange, period):
        status = ctypes.c_int()
        ...
        elif HelperMethods.IsLinePayoutNonZero(handle) and arclengthRange is not None and arclengthRange.Mode != armEntireLine:
            raise ValueError('Range graphs cannot be obtained for a given arclength range when the payout rate is non-zero in this version of OrcFxAPI.dll')
    ...
    def DiffractionIncludedBodyCount(handle):
        return HelperMethods.GetDataInteger(handle, 'NumberOfIncludedBodies', -1)

    @staticmethod
    def DiffractionFieldPointCount(handle):
        return HelperMethods.GetDataInteger(handle, 'NumberOfFieldPoints', -1)
        ...
        def hydrostaticResultsGenerator():
            for bodyIndex in range(bodyCount):
                ...
                def reinterp(index, len=1, shape=None):
                    result = HelperMethods.Reinterpreted(output[base + index * 8 : base + (index + len) * 8], numpy.float64, shape)
        ...
        def panelGeometryGenerator():
            for panelIndex in range(panelCount):
                ...
                def objectName(objectId):
                    if objectId == -1:
                        return 'Damping lid'
                    bodyIndex = HelperMethods.GetDataInteger(handle, 'BodyIndex', objectId + 1) # convert to 1-based index
        ...
        def prepareDiffractionBodyMap(fileType, count, mappings):
            ...
            def OrcaWaveBodyName(OrcaWave):
                for i in range(OrcaWave.NumberOfBodies):
                    if OrcaWave.BodyIncludedInAnalysis[i] == 'Yes':
        ...
        def GetSingleDataItem(dataName, index):
            ...
            if dataType.value == dtDouble:
                ...
                if result is not None:
                    result -= 1  # Python is 0-based, OrcFxAPI is 1-based:
                return result
...
class OrcaFlexObject(DataObject):
    ...
    def AssignWireFrameFromPanelMesh(self, filename, format, symmetry, importOrigin=None, scale=1, bodyNumber=1, importDryPanels=True):
        with HelperMethods.PanelMeshImporter(filename, format, scale, bodyNumber, importDryPanels) as importer:
            ...
            _AssignWireFrameFromPanelMesh(self.handle, importer.handle, symmetry, importOrigin, status)
            _CheckStatus(status)
...
class OrcaFlexLineObject(OrcaFlexObject):
    ...
    def RangeGraphXaxis(self, varName, arclengthRange=None, period=None):
        ...
        pointCount = HelperMethods.RangeGraphPointCount(self.handle, varID, arclengthRange, period)
...
class OrcaFlexTurbineObject(OrcaFlexObject):
...
class FatigueAnalysis(DataObject):
    ...
    def isBuiltInAttribute(self, name):
        return name in ('progressHandler', 'threadCount') \
            or DataObject.isBuiltInAttribute(self, name)
...
class Diffraction(DataObject):
    ...
    def isBuiltInAttribute(self, name):
        return name in ('progressHandler', 'progressHandlerCallback', 'calculationProgressHandler', 'threadCount') \
            or DataObject.isBuiltInAttribute(self, name)
