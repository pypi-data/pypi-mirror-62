
 

// This file is autogenerated. DO NOT EDIT

#pragma once
#include <robotpy_build.h>

#include <frc\buttons\Button.h>



#include <rpygen/frc__Trigger.hpp>

namespace rpygen {

using namespace frc;


template <typename CxxBase>
using PyBasefrc__Button = 
    Pyfrc__Trigger<
        CxxBase
    
    >
;

template <typename CxxBase>
struct Pyfrc__Button : PyBasefrc__Button<CxxBase> {
    using PyBasefrc__Button<CxxBase>::PyBasefrc__Button;



#ifndef RPYGEN_DISABLE_WhenPressed_PTCommand
    void WhenPressed(Command* command) override {
PYBIND11_OVERLOAD_NAME(void, CxxBase, "whenPressed", WhenPressed,command);    }
#endif

#ifndef RPYGEN_DISABLE_WhileHeld_PTCommand
    void WhileHeld(Command* command) override {
PYBIND11_OVERLOAD_NAME(void, CxxBase, "whileHeld", WhileHeld,command);    }
#endif

#ifndef RPYGEN_DISABLE_WhenReleased_PTCommand
    void WhenReleased(Command* command) override {
PYBIND11_OVERLOAD_NAME(void, CxxBase, "whenReleased", WhenReleased,command);    }
#endif

#ifndef RPYGEN_DISABLE_CancelWhenPressed_PTCommand
    void CancelWhenPressed(Command* command) override {
PYBIND11_OVERLOAD_NAME(void, CxxBase, "cancelWhenPressed", CancelWhenPressed,command);    }
#endif

#ifndef RPYGEN_DISABLE_ToggleWhenPressed_PTCommand
    void ToggleWhenPressed(Command* command) override {
PYBIND11_OVERLOAD_NAME(void, CxxBase, "toggleWhenPressed", ToggleWhenPressed,command);    }
#endif



};

}; // namespace rpygen
