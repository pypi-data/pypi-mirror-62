
 

// This file is autogenerated. DO NOT EDIT

#pragma once
#include <robotpy_build.h>

#include <frc/commands/ConditionalCommand.h>



#include <rpygen/frc__Command.hpp>

namespace rpygen {

using namespace frc;


template <typename CxxBase>
using PyBasefrc__ConditionalCommand = 
    Pyfrc__Command<
        CxxBase
    
    >
;

template <typename CxxBase>
struct Pyfrc__ConditionalCommand : PyBasefrc__ConditionalCommand<CxxBase> {
    using PyBasefrc__ConditionalCommand<CxxBase>::PyBasefrc__ConditionalCommand;



#ifndef RPYGEN_DISABLE_Condition_v
    bool Condition() override {
RPYBUILD_OVERLOAD_PURE_NAME(ConditionalCommand,bool, CxxBase, "_condition", Condition,);    }
#endif

#ifndef RPYGEN_DISABLE__Initialize_v
    void _Initialize() override {
PYBIND11_OVERLOAD_NAME(void, CxxBase, "_initialize", _Initialize,);    }
#endif

#ifndef RPYGEN_DISABLE__Cancel_v
    void _Cancel() override {
PYBIND11_OVERLOAD_NAME(void, CxxBase, "_cancel", _Cancel,);    }
#endif

#ifndef RPYGEN_DISABLE_IsFinished_v
    bool IsFinished() override {
PYBIND11_OVERLOAD_NAME(bool, CxxBase, "isFinished", IsFinished,);    }
#endif

#ifndef RPYGEN_DISABLE__Interrupted_v
    void _Interrupted() override {
PYBIND11_OVERLOAD_NAME(void, CxxBase, "_interrupted", _Interrupted,);    }
#endif



};

}; // namespace rpygen
