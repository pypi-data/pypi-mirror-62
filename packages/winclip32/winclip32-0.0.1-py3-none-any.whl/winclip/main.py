...
if ERROR:
    ans = ""
    while True:
        ans = input("<winclip> pywin32 not found. Do you want to install it? (y/n): ")
        if ans == "y" or ans == "n":
            break
        else:
            print("<winclip> Your response ('%s') was not one of the expected responses: y, n" % ans)
    if ans == "y":
        print("<winclip> Trying to install pywin32")
        ERROR = 0
        try:
            import subprocess, sys

            subprocess.check_call([sys.executable, "-m", "pip", "install", "pywin32"])
            print("<winclip>  Successfully installed pywin32. Restart to work")
        except:
            ERROR = 1
        if ERROR:
            raise SomethingWentWrongWhileInstallingPyWin32
        else:
            exit()
    else:
        raise CannotWorkWithOutPyWin32
...
class ClipBoardController:
    ...
    def GetDataTypesInfo(self):
        # http://timgolden.me.uk/pywin32-docs/win32clipboard__GetClipboardDataHandle_meth.html !!!!!!!!!
        # information got from http://timgolden.me.uk/pywin32-docs/win32clipboard__GetClipboardData_meth.html !!!!!!!!!
    ...
    def GetClipBoardData(self, type_key):
        ERROR = 0
        try:
        ...
        if type_key in clipboard_data_types_int:
            ...
            if ERROR:
                win32clipboard.CloseClipboard(0)
    ...
    def GetClipBoardDataHandle(self, type_key):
        ERROR = 0
        try:
            assert type(type_key) is int or type(type_key) is str
        except:
        ...
        if type_key in clipboard_data_types_int:
            try:
                win32clipboard.OpenClipboard(0)
                clipboarddata = win32clipboard.GetClipboardDataHandle(type_key)
                win32clipboard.CloseClipboard()
            except:
                ERROR = 1
            ...
            try:
                win32clipboard.OpenClipboard(0)
                clipboarddata = win32clipboard.GetClipboardDataHandle(clipboard_data_types_int[clipboard_data_types_str.index(type_key)])
                win32clipboard.CloseClipboard()
            except:
                ERROR = 1
    ...
    def IsClipBoardDataTypeAvailable(self, type_key):
        ERROR = 0
        try:
        ...
        if type(type_key) is str:
            if type_key not in clipboard_data_types_str:
                ...
                return bool(win32clipboard.IsClipboardFormatAvailable(clipboard_data_types_int[clipboard_data_types_str.index(type_key)]))
        else:
            return bool(win32clipboard.IsClipboardFormatAvailable(type_key))

    def SetClipBoardData(self, type_key, data):
        ERROR = 0
        try:
            assert type(type_key) is str or type(type_key) is int
        except:
    ...
    def CountClipBoardDataTypes(self):
        try:
