...
if ERROR:
    ans = ""
    while True:
        ans = input("<winclip32> pywin32 not found. Do you want to install it? (y/n): ")
        if ans == "y" or ans == "n":
            break
        else:
            print("<winclip32> Your response ('%s') was not one of the expected responses: y, n" % ans)
    if ans == "y":
        print("<winclip32> Trying to install pywin32")
        ERROR = 0
        try:
            import subprocess, sys

            subprocess.check_call([sys.executable, "-m", "pip", "install", "pywin32"])
            print("<winclip32>  Successfully installed pywin32. Restart to work")
        except:
            ERROR = 1
        if ERROR:
