...
def _load_images(root_path):
    ...
    Args:
        root_path: path containing image files
    ...
    for id in range(num_files):
        raw_image = tf.io.read_file(os.path.join(root_path, file_list[id]))
        image = tf.image.decode_jpeg(raw_image, channels=3)
        x[id, :, :, :] = np.expand_dims(image.numpy(), axis=0)
...
def load_data():
    ...
    """
    dataset = tf.keras.utils.get_file("UTKFace_preprocessed.tar.gz",
