...
Brainchip information:
        - Original file: https://github.com/tensorflow/tensorflow/tree/master/tensorflow/examples/speech_commands
"""
import hashlib
import math
import os.path
...
class AudioProcessor(object):
  ...
  def __init__(self,
    ...
    self.model_settings = prepare_model_settings(sample_rate, clip_duration_ms,
                                                 window_size_ms,
                                                 window_stride_ms,
                                                 feature_bin_count)
    if data_dir:
  ...
  def maybe_download_and_extract_dataset(self, data_url, dest_directory):
    ...
    if not os.path.exists(filepath):
      ...
      try:
        ...
        raise RuntimeError(f"Failed to download URL: {data_url} to folder: "
  ...
  def prepare_processing_graph(self):
    ...
    """
    @tf.function
    def processing_graph(wav_filename, foreground_volume, time_shift_padding,
    time_shift_offset, background_data, background_volume, model_settings):
  ...
  def get_data(self, how_many, offset, background_frequency,
    ...
    if self.data_dir is None:
      raise RuntimeError("Calling 'get_data' method requires to have a valid "
    ...
    for i in range(offset, offset + sample_count):
      ...
      if use_background or sample['label'] == SILENCE_LABEL:
        ...
        background_volume = 0
      input_dict['background_data'] = tf.constant(background_reshaped, dtype=tf.float32)
      input_dict['background_volume'] = tf.constant(background_volume, dtype=tf.float32)
  ...
  def get_features_for_wav(self, wav_filename):
    ...
    """
    desired_samples = self.model_settings['desired_samples']
    background_data = tf.constant(np.zeros([desired_samples, 1]), tf.float32)
    input_dict = {
    ...
    features = features.reshape((self.model_settings['spectrogram_length'],
                                 self.model_settings['fingerprint_width'],
                                 1))
    return features
  ...
  def get_augmented_data_for_wav(self,
    ...
    """Applies the feature transformation process to a wav audio file, adding
    data augmentation (background noise and time shifting).
