...
BASE_WEIGHT_PATH = 'http://data.brainchip.com/models/convnet/'

def convnet_kws(input_shape=None,
                classes=None,
                include_top=True,
                weights=None,
    ...
    """Instantiates a CNN for the "Keyword Spotting" example.

    This model is based on a succession of convolutional layers.
    The weights and activations are quantized such that it can be converted
    into an Akida model.

    This architecture was created for the "Keyword Spotting" (KWS) or "Speech
    Commands" dataset.
    ...
    Args:
        ...
        include_top (bool): whether to include the two last layers after the
        ...
        input_weight_quantization: sets weight quantization in the first layer.
            ...
            * '0' implements floating point 32-bit weights.
            * '2' through '8' implements n-bit weights where n is from 2-8 bits.
        last_layer_activ_quantization: sets activation quantization in the
            layer before the last. Takes precedence over global
            activ_quantization.
    ...
    Returns
        tf.keras.Model: a quantized Keras convolutional model for KWS
    """
    ...
    if weights == 'kws':
        if include_top and classes != 12:
            raise ValueError("If using 'weights' as 'kws' with 'include_top' as "
                             "true, classes should be 12")
        quant_params = (input_weight_quantization,
                        weight_quantization,
                        activ_quantization)
        if quant_params != (8, 4, 4):
            raise ValueError("If 'weights' is 'kws', quantization parameters "
                             "(input_weight_quantization, weight_quantization"
                             ", activ_quantization) must be (8,4,4)")
        if input_shape[-1] != 1:
    ...
    model = Model(img_input, x, name='convnet_kws')

    # Load weights.
    if weights == 'kws':
        if include_top:
            model_name = (f'convnet_kws_wq{weight_quantization}'
