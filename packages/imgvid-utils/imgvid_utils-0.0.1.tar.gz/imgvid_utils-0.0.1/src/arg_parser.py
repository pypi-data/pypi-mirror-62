...
def parse_arguments():
    ...
    parser.add_argument("--dirs_in", dest="dirs_in", nargs="*", type=str, help="List any directories you want to use.")
    parser.add_argument("--files_in", dest="files_in", nargs="*", type=str,
                        help="List any images or videos you want to use. Not compatible"
                             "with -to_imgs, dirs_in. If -to_vid, must be videos.")
    parser.add_argument("--ext_in", dest="ext_in", type=str, choices=["png", "jpg", "mp4"],
                        help="Will select files with given extension for input.")
    parser.add_argument("--ext_out", dest="ext_out", type=str, choices=["png", "jpg", "mp4"],
                        help="Outputs file with given extension."
                             " Overriden by -to_vid, -to_img, and -to_imgs")
    parser.add_argument("--dir_out", dest="dir_out", type=str, default="./output/", help="Output directory.")
    parser.add_argument("--name", dest="name", type=str, default='file', help="File name. Do not include in dir_out")
    parser.add_argument("--to_vid", dest="to_vid", action='store_true',
    ...
    parser.add_argument("--resize_down", dest="resize_down", action='store_true',
                        help="Resizes all input images to the smallest image in the set. "
                             "Computed by area of image (eg. width * height). Will override --resize_in, --resize_out."
                             " Not compatible with --resize_up, --resize_first")
    parser.add_argument("--resize_first", dest="resize_first", action='store_true',
...
class PluralizableString:
    def __init__(self, base_string: str, non_plural_end: str, plural_end: str, pluralizable: IsPlural):
        self.text: str = base_string + (plural_end if pluralizable.is_plural else non_plural_end)
...
def validate_arguments(args):
    ...
    if args.dirs_in is not None:
        ...
        if len(missing_dirs) != 0:
            ...
            raise EnvironmentError("The %s specified at %s %s not exist." % (dir_str, ", ".join(missing_dirs), do_str))
    ...
    if args.files_in is not None:
        if len(args.files_in) < args.cols * args.rows:
            raise EnvironmentError(
                ...
                "videos, %d given)" % (args.cols, args.rows, args.cols * args.rows, len(args.files_in)))
        ...
        if len(missing_files) != 0:
            ...
            raise EnvironmentError("The %s specified at %s does not exist." % (file_str, ", ".join(missing_files)))

    if (args.resize_up and args.resize_down) or (args.resize_up and args.resize_first) \
            or (args.resize_first and args.resize_down):
        raise EnvironmentError("Choose one of --resize_up, --resize_down, --resize_first.")
    ...
    if (args.to_vid and args.to_img) or (args.to_img and args.to_imgs):
        raise EnvironmentError("--to_img is not compatible with either --to_vid or --to_imgs.")
