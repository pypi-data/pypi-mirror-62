# python
from iqoapi.api import IQOptionAPI
...
from iqoapi.expiration import get_expiration_time,get_remaning_time
...
# --------for binary option detail
    ...
    def get_all_profit(self):
        ...
        return all_profit
    def get_profit_v2(self):
        all_profit = nested_dict(2, dict)
        init_info = self.get_all_init_v2()        
        for actives in init_info["turbo"]["actives"]:
            name = init_info["turbo"]["actives"][actives]["name"].split('.')[1]
            enabled = init_info["turbo"]["actives"][actives]["enabled"]
            all_profit[name]["turbo"] = ((100.0-init_info["turbo"]["actives"][actives]["option"]["profit"]["commission"])/100.0,enabled)

        for actives in init_info["binary"]["actives"]:
            name = init_info["binary"]["actives"][actives]["name"].split('.')[1]
            enabled = init_info["binary"]["actives"][actives]["enabled"]
            all_profit[name]["binary"] = ((100.0-init_info["binary"]["actives"][actives]["option"]["profit"]["commission"])/100.0,enabled)
        return all_profit
