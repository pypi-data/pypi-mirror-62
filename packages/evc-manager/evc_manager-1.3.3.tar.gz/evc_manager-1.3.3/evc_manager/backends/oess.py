...
class Oess(Backend):
    ...
    def oess_get_path(self, unis, requested=None, primary=None):
        ...
        # TODO: fix it. Request is to supposed to be the defined path.
    ...
    def move_evc(self, evc_to_move, current_primary, current_backup, nni):
        ...
        if nni in current_primary:
            warn("  Current path: %s" % current_primary)

            new_primary_path = self.oess_get_path(evc_to_move.unis,
                                                  primary=list(current_primary.split(" ")))
            warn("  New suggested primary path: %s" % new_primary_path)
            ...
            warn(copy_of_new_primary_plus_nni)
            new_backup_path = self.oess_get_path(evc_to_move.unis,
                                                 primary=copy_of_new_primary_plus_nni)
            warn("  New suggested backup path: %s" % new_backup_path)
            if new_primary_path == new_backup_path or nni in new_backup_path:
                warn("  New suggested primary path equals new backup path.")
                warn("  Changing Primary path. No Backup created.")
                # Request OESS to change primary path only.
                ...
                info('Moving EVC %s out of NNI %s' % (evc_to_move.name, nni))
            ...
            copy_of_primary_plus_nni = copy.deepcopy(current_backup)
