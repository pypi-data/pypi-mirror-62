...
from automl.client.core.common.exceptions import (
    DataException, FitException, TransformException)
from automl.client.core.common.transformer_runtime_exceptions import (
...
from automl.client.core.runtime.pickler import DefaultPickler
...
class DataTransformer(AutoMLTransformer, FeaturizationInfoProvider):
    ...
    def __init__(self,
        ...
        if task not in constants.Tasks.ALL:
            raise DataTransformerUnknownTaskException("Unknown task")
    ...
    def _check_columns_names_and_convert_types(self,
        ...
        if len(curr_columns_types_mapping) != len(columns_types_mapping):
            ...
            raise DataException(msg.
                                format(len(columns_types_mapping), len(curr_columns_types_mapping))).with_generic_msg(
        ...
        for col, col_type in curr_columns_types_mapping.items():
            if col not in columns_types_mapping:
                raise DataException("Input column not found in the fitted columns.", has_pii=False)
            ...
            for col, col_type in type_converted_columns.items():
                try:
                    ...
                    raise DataException("Error converting the input column as column does not match the "
                                        "fitted column type.", has_pii=False)
        return df
