...
from automl.client.core.common.exceptions import InvalidArgumentException, DataException
...
def compute_metrics_classification(y_pred_probs: np.ndarray,
    ...
    if y_pred_probs is None:
        raise InvalidArgumentException.create_without_pii("y_pred_probs must not be None",
                                                          target=Metric.COMPUTATION_ERROR_TARGET)
    if y_test is None:
    ...
    if num_classes is not None and class_labels is not None and num_classes != len(class_labels):
        msg = "num_classes ({}) does not equal class_labels length ({})."
        raise InvalidArgumentException(
            msg.format(num_classes, len(class_labels)), target=Metric.COMPUTATION_ERROR_TARGET).\
    ...
    if trained_class_labels is not None and len(trained_class_labels) != y_pred_probs.shape[1]:
        msg = "trained_class_labels length ({}) does not match y_pred_probs.shape[1] ({})."
        raise InvalidArgumentException(
            msg.format(len(trained_class_labels), y_pred_probs.shape[1]), target=Metric.COMPUTATION_ERROR_TARGET).\
            with_generic_msg(msg.format('[MASKED]', '[MASKED]'))
    ...
    if trained_class_labels is None:
        if class_labels is not None and len(class_labels) != y_pred_probs.shape[1]:
            msg = "class_labels length ({}) does not match y_pred_probs.shape[1] ({}) and you " \
                  "did not pass trained_class_labels."
            raise InvalidArgumentException(
                msg.format(len(class_labels), y_pred_probs.shape[1]), target=Metric.COMPUTATION_ERROR_TARGET).\
                with_generic_msg(msg.format('[MASKED]', '[MASKED]'))

        if num_classes is not None and num_classes != y_pred_probs.shape[1]:
            msg = "num_classes ({}) does not match y_pred_probs.shape[1] ({}) and you did not pass " \
                  "trained_class_labels."
            raise InvalidArgumentException(
                msg.format(num_classes, y_pred_probs.shape[1]), target=Metric.COMPUTATION_ERROR_TARGET).\
                with_generic_msg(msg.format('[MASKED]', '[MASKED]'))
...
def compute_metrics_regression(y_pred: np.ndarray,
    ...
    if y_pred is None:
        raise DataException("y_pred must not be None", target=Metric.COMPUTATION_ERROR_TARGET, has_pii=False)
    if y_test is None:
        raise DataException("y_test must not be None", target=Metric.COMPUTATION_ERROR_TARGET, has_pii=False)
...
class Metric:
    ...
    # Error Targets
    COMPUTATION_ERROR_TARGET = 'MetricsComputation'
