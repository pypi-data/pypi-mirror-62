...
from automl.client.core.common.exceptions import PipelineRunException
...
def run_pipeline(automl_settings: AutoMLBaseSettings,
    ...
    with logging_utilities.log_activity(logger, activity_name=constants.TelemetryConstants.RUN_PIPELINE_NAME,
        ...
        if results is None:
            raise PipelineRunException("Failed to train pipeline. Status: {}".format(status),
        ...
        if automl_pipeline.is_ensemble_pipeline:
            ...
            if training_type in \
                ...
                if result_full is None:
                    raise PipelineRunException("Failed while training full result. Status: {}".format(status),
                                               target=PipelineRunException.PIPELINE_OUTPUT, has_pii=False)
