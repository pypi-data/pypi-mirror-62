...
from automl.client.core.common.exceptions import AutoMLException, ErrorTypes, PipelineRunException
    ...
    def failure_reason(self) -> Optional[str]:
        ...
        for error_name in self.errors:
            if self.errors[error_name]['is_critical'] or failure_reason is None:
                ...
                failure_reason = getattr(exc, '_error_code', ErrorTypes.Unclassified)
    ...
    def error_code(self) -> Optional[str]:
        ...
        for error_name in self.errors:
            if self.errors[error_name]['is_critical'] or error_code is None:
                exc = self.errors[error_name]['exception']
                error_code = getattr(exc, 'error_type', ErrorTypes.Unclassified)
        return error_code
    ...
    def get_sanitized_output_dict(self) -> Dict[str, str]:
        ...
        if 'errors' in output_dict:
            for exception_type in output_dict['errors']:
                output_dict['errors'][exception_type]['exception'] = \
                    str(output_dict['errors'][exception_type]['exception'])
            output_dict['errors'] = json.dumps(output_dict['errors'])
