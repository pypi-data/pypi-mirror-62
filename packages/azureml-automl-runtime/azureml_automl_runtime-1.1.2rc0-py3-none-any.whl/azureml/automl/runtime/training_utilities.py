...
def check_x_y(x: DataInputType,
    ...
    if isinstance(x, pd.DataFrame):
        if len(x.columns) != len(set(x.columns)):
            raise DataException("The are duplicate column names in your raw data", has_pii=False)
...
def validate_timeseries_training_data(automl_settings: AutoMLBaseSettings,
    ...
    if automl_settings.n_cross_validations is None and X_valid is None:
        ...
        if X.shape[0] < min_points:
            ...
            """
            raise WrongShapeDataError("The data provided is insufficient for training.")
        ...
        if grain_names is None:
            ...
            pii_message = (
                ...
                "dropping that particular grain.". format(
                    ...
                    data_points))
            """
            raise WrongShapeDataError("The data provided is insufficient for some training grains.")
...
def _check_valid_pd_time(df: pd.DataFrame, time_column_name: str) -> None:
    try:
        ...
        raise DataException(
            ...
            .format(pd.Timestamp.min, pd.Timestamp.max)).with_generic_msg("Date/time is out of our usable range.")
    except ValueError:
        raise DataException(
...
def _check_time_index_duplication(df: pd.DataFrame,
    ...
    if duplicateRowsDF.shape[0] > 0:
        if grain_column_names is not None and len(grain_column_names) > 0:
            # Uncomment the code below once the PII message is scrubbed correctly
            ...
            """
            raise DuplicatedIndexException("Duplicates in time and grain combinations.", message)
        else:
            # Uncomment the code below once the PII message is scrubbed correctly
            ...
            print(duplicateRowsDF.iloc[:2, :][group_by_col])
            """
            raise DuplicatedIndexException("Duplicates in time index.", message)
...
def _check_dimensions(X: DataInputType,
    ...
    if x_shape is not None and y_shape is not None and x_shape[0] == 0:
        raise DataSamplesSizeException("X and y data do not have any samples.")
    if x_valid_shape is not None and y_valid_shape is not None and x_valid_shape[0] == 0:
        raise DataSamplesSizeException("X_valid and y_valid data do not have any samples.")
