...
class TimeSeriesTransformer(AbstractTimeSeriesTransformer, FeaturizationInfoProvider):
    ...
    def _construct_pre_processing_pipeline(self,
        ...
        if len(datetime_columns) > 0:
            default_pipeline.add_pipeline_step(TimeSeriesInternal.IMPUTE_NA_FORWARD, fillna_imputer_ffill)
            default_pipeline.add_pipeline_step(TimeSeriesInternal.IMPUTE_NA_BACKWARD, fillna_imputer_bfill)
            default_pipeline.add_pipeline_step(TimeSeriesInternal.IMPUTE_NA_FINAL, datetime_imputer_final)
