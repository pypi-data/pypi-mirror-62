...
def get_frequency_safe(time_index: Any) -> pd.DateOffset:
    ...
    try:
        ...
        raise DataException(
            ...
            "please fix it and run the experiment again. Original error: {}".format(e)).with_generic_msg(
    ...
    try:
        ...
        raise DataException(
            "Could not infer the frequency of the time index : {}".format(e)).with_generic_msg(
                'Could not infer the frequency of the time index.')
    if freq is not None:
        return freq
...
def analyze_pacf_per_grain(dataframe: pd.DataFrame,
    ...
    """
    try:
        dataframe[time_colname] = pd.to_datetime(dataframe[time_colname])
    except Exception as e:
        raise DataException(
            "The date time column contains invalid values or is absent,"
            "please fix it and run the experiment again. Original error: {}".format(e)).with_generic_msg(
                'Could not infer the frequency of the time index.')
