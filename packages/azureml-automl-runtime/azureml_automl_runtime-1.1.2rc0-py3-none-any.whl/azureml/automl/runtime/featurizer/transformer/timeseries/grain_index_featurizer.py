...
from automl.client.core.common.forecasting_exception import NotTimeSeriesDataFrameException, TransformException
    ...
    def transform(self, X):
        ...
        if len(overlap) > 0:
            message = ('Some of the existing columns in X will be ' +
                       'overwritten by the transform: {0}. ').format('[Masked]')
            ...
            if self.overwrite_columns:
                ...
                error_message = message + ("Set 'overwrite_columns' to True " +
                    ...
                    '[Masked]')
                raise TransformException(error_message, target='GrainIndexFeaturizer')
