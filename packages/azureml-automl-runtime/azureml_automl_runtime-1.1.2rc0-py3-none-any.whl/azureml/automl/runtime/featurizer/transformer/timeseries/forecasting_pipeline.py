...
from automl.client.core.common.forecasting_exception import PipelineException, ForecastingConfigException
...
class AzureMLForecastPipeline:
    ...
    def add_pipeline_step(self, name, step, prepend=False):
        ...
        if next((step for key, step in self._pipeline.steps if key == name),
            ...
            raise ForecastingConfigException.create_without_pii(Messages.PIPELINE_STEP_ADD_INVALID)
    ...
    def remove_pipeline_step(self, name):
        ...
        if next((step for key, step in self._pipeline.steps if key == name),
            ...
            raise ForecastingConfigException.create_without_pii(Messages.PIPELINE_STEP_REMOVE_INVALID)
