...
class Evaluation(EvaluationMeta):
    """Class to perform evaluation of the model.
        ...
        Examples:
            ...
            >>> evaluator.stop()
    """
    def __init__(self, model=None, debug=False, data_type='valid', tuning=False, session=None, multiprocess=False):
        ...
        if multiprocess:
            ...
            self.calculator = MetricCalculator(self.model.config)
    ...
    def test_batch(self, epoch=None, multiprocess=False):
        ...
        with progressbar.ProgressBar(max_value=self.loop_len, widgets=widgets) as bar:
            for i in range(self.loop_len):
                ...
                if multiprocess:
                    ...
                    self.calculator.append_result(result_data)
        ...
        if multiprocess:
            ...
            self.calculator.settle()
            self.calculator.display_summary()

            if self.calculator.epoch >= self.model.config.epochs - 1:
                calculator.save_test_summary(model_name)

                if tuning:
                    score = self.calculator.get_curr_score()
