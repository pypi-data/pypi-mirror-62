    ...
    def draw(self, image, image_width, image_height):
        for i in range(self.amount):
            if type(self.drawing_function) is list:
                drawing_function = choice(self.drawing_function)
            else:
                drawing_function = self.drawing_function
            point, dimensions = self.__random_dimensions(image_width, image_height)
            self.tone = self.__random_color()
            drawing_function(image, point, dimensions, self.tone, self.__random_gradient())
...
def gradient_circle(image, center, radius, color=(0, 0, 0), gradient=('2*x', '2*x', '2*x'), tangent_angle=0):
    """
     Draws a circle with a gradient according to the given traits
    :param image: The that the circle should be drawn on, an already open image as a numpy array
                 (opened with cv2.imread)
    ...
    for i in range(radius):
        ...
        circle_center = (int(center[0] + (radius - i) * math.sin(math.radians(tangent_angle))),  # x value
                         int(center[1] + (radius - i) * math.cos(math.radians(tangent_angle))))  # y value
...
def rotated_rectangle(image, start_point, dimensions, angle=360, color=(0, 0, 0), thickness=1):
    ...
    x, y = start_point
    a = [x, y]
    b, d = [x + int(math.cos(math.radians(angle)) * w), y - int(math.sin(math.radians(angle)) * w)], \
           [x + int(math.sin(math.radians(angle)) * h), y + int(math.cos(math.radians(angle)) * h)]
    vd = [d[0] - x, d[1] - y]
    vb = [b[0] - x, b[1] - y]
...
def gradient_rectangle(image, start_point, dimensions, color=(0, 0, 0), grade=-1, angle=360.0,
    ...
    """
     Draws a rectangle with a changing color
...
def create_color_image(color, width=320, height=240, color_space=None):
    """
     Creates an image that is completely black
...
class GenerateImage:
    """
    An image generating object, can create image with given shapes
    """

    def __init__(self, width, height, image_color, true_shape, false_shapes=()):
        """
         Generates random images
        :param width: width of the image in pixels
        :param height: height of the image in pixels
        :param true_shape: the shape object of the true_shape target
        :param false_shapes: a list of the false_shapes shapes
        :param image_color: the image background color
        """
        self.true_shape = true_shape
        self.false_shapes = false_shapes
    ...
    def __random_color(self):
        ...
        if cr_type is Color:
            ...
            for ranges in color_range.ranges:
                ...
                brightness.append(j[1])
            return [choice(hue), choice(saturation), choice(brightness)]

    def generate_image(self):
        self.image = create_color_image(self.__random_color(), self.width, self.height)
        for f in self.false_shapes:
            f.draw(self.image, self.width, self.height)
        self.true_shape.draw(self.image, self.width, self.height)
        return self.image


def draw_enclosing_circle(image, contour, color=(0, 0, 0), thickness=1, display_data=True):
    """
     Draws the enclosing (bounding) circle on the given contour
...
def draw_bounding_rect(image, contour, color=(0, 0, 0), thickness=1, display_data=True):
    """
     draws the straight bounding rectangle around the given contour.
