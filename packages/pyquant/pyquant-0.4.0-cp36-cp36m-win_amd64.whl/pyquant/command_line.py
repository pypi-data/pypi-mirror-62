...
def run_pyquant():
    ...
    mrm_pair_info = (
        pd.read_csv(args.mrm_map, sep="\t") if args.mrm and args.mrm_map else None
    )
    ...
    if args.label_scheme:
        label_info = pd.read_csv(
            args.label_scheme.name, sep="\t", header=None, dtype="str"
        )
        try:
            label_info.columns = ["Label", "AA", "Mass", "UserName"]
            name_mapping = dict(
                [(v["Label"], v["UserName"]) for i, v in label_info.iterrows()]
            )
        except ValueError:
            label_info.columns = ["Label", "AA", "Mass"]
            name_mapping = dict(
                [(v["Label"], v["Label"]) for i, v in label_info.iterrows()]
            )
    ...
    if args.peptide_file:
        ...
        peptides = (
            set(args.peptide) if isinstance(args.peptide, list) else set([args.peptide])
        )
    else:
        peptides = None
    ...
    input_found = None
    if args.search_file:
        results = GuessIterator(
            args.search_file.name, full=True, store=False, peptide=peptides
        )
        input_found = "ms"
    elif args.tsv:
    ...
    if args.search_file:
        ...
        pyquant_parser.error(
            "No input files provided. Please provide one via --search-file, --tsv, --scan-file, or --scan-file-dir."
        )

    if args.scan_file:
        nfunc = (
            lambda i: (
                os.path.splitext(os.path.split(i.name)[1])[0],
                os.path.abspath(i.name),
            )
            if hasattr(i, "name")
            else (os.path.splitext(os.path.split(i)[1])[0], os.path.abspath(i))
        )
        ...
        if os.path.isdir(raw_file):
            scan_filemap = dict(
                [
                    (os.path.splitext(i)[0], os.path.abspath(os.path.join(raw_file, i)))
                    for i in os.listdir(raw_file)
                    if i.lower().endswith("mzml")
                ]
            )
        else:
            scan_filemap[
                os.path.splitext(os.path.split(raw_file)[1])[0]
            ] = os.path.abspath(raw_file)
    ...
    if input_found == "tsv":
        ...
        def tsv_formatter(row):
            ...
            precursor_mass = (
                row_info[precursor_col] if precursor_col in row_info else None
            )
            rt_value = row_info[rt_col] if rt_col in row_info else None
            label = (
                name_mapping.get(row_info[label_col]) if label_col in row_info else None
            )
        ...
        if args.mva:
            ...
            if fname in replicate_file_mapper:
                ...
                seq_matches = [
                    i
                    for i in sorted(
                        seq_matches, key=operator.itemgetter(0), reverse=True
                    )
                    if i[0] != 1
                ]
        ...
        if fname not in scan_filemap:
            ...
            if fname not in scan_filemap:
                if skip:
                    continue
                sys.stderr.write(
                    "{0} not found in filemap. Filemap is {1}.".format(
                        fname, scan_filemap
                    )
                )
                return 1
    ...
    if scan_filemap and raw_data_only:
        ...
        if args.msn_ion or args.msn_peaklist:
            ...
            ions_selected = (
                [
                    sorted(list(set(map(float, ion.split(",")))))
    ...
    if not scan_filemap and input_found is None:
        sys.stderr.write(
            "No valid input entered. PyQuant requires at least a raw file or a processed dataset."
        )
        return 1
    sys.stderr.write("\nScans loaded.\n")
    ...
    if not labels:
        if ion_search:
            if args.require_all_ions:
                ...
                labels = list(
                    map(str, set([ion for ion_set in ions_selected for ion in ion_set]))
                )
        else:
            labels = [""]
    ...
    RESULT_ORDER.extend(
        [("charge", "Charge"), ("ms1", "MS{} Spectrum ID".format(msn_for_quant)),]
    )
    ...
    for label in iterator:
        ...
        if msn_for_quant == 1:
            if not args.no_mass_accuracy_correction:
                RESULT_ORDER.extend(
                    [
                        (
                            "{}_calibrated_precursor".format(label),
                            "{}Calibrated Precursor".format(label_key),
                        ),
                    ]
                )
            RESULT_ORDER.extend(
        ...
        RESULT_ORDER.extend(
            [
                (
                    "{}_intensity".format(label),
                    "{}Intensity".format(label_key),
                    partial(naninfsum, empty=0),
                ),
            ]
        )
        if args.peaks_n == 1:
            RESULT_ORDER.extend(
                [
                    (
                        "{}_peak_width".format(label),
                        "{}RT Width".format(label_key),
                        partial(naninfmean, empty=0),
                    ),
                    (
                        "{}_peak_mean".format(label),
                        "{}Peak Center".format(label_key),
                        partial(naninfmean, empty=np.NaN),
                    ),
                    (
                        "{}_mean_diff".format(label),
                        "{}Mean Offset".format(label_key),
                        partial(naninfmean, empty=np.NaN),
                    ),
                ]
            )
        ...
        if msn_for_quant == 1:
            RESULT_ORDER.extend(
                [
                    (
                        "{}_residual".format(label),
                        "{}Residual".format(label_key),
                        partial(naninfsum, empty=np.NaN),
                    ),
                    (
                        "{}_coef_det".format(label),
                        "{}R^2".format(label_key),
                        partial(naninfmean, empty=0),
                    ),
                    (
                        "{}_snr".format(label),
                        "{}SNR".format(label_key),
                        partial(naninfmean, empty=0),
                    ),
                ]
            )
        if not args.merge_labels and not args.no_ratios and label != "":
            for label2 in labels:
                ...
                if label_key != label_key2 and (
                    ref_label is None or ref_label.lower() == label_key2.lower()
                ):
                    RESULT_ORDER.extend(
                        [
                            (
                                "{}_{}_ratio".format(label, label2),
                                "{}/{}".format(label_key.strip(), label_key2.strip()),
                            ),
                        ]
                    )
                    if calc_stats:
                        RESULT_ORDER.extend(
                            [
                                (
                                    "{}_{}_confidence".format(label, label2),
                                    "{}/{} Confidence".format(
                                        label_key.strip(), label_key2.strip()
                                    ),
                                ),
                            ]
                        )
    ...
    if html:
        ...
        def table_rows(html_list, res=None):
            ...
            for col_index, (i, v) in enumerate(zip(line.split("\t"), keys)):
                ...
                html_template.substitute(
                    {
                        ...
                        "table_header": "\n".join(
                            [
                                "<th>{0}</th>".format(i)
                                for i in ["Controls", "Raw File"]
                                + [i[1] for i in RESULT_ORDER]
                            ]
                        ),
                    }
                )
    ...
    for filename in raw_files.keys():
        ...
        calc_spline = (
            not mass_accuracy_correction and not raw_data_only and not args.neucode
        )
        spline_x = []
        spline_y = []
        spline = None
        ...
        for index, scan in enumerate(raw):
            ...
            if msn_rt_window and not any(
                [(i[0] < float(rt) < i[1]) for i in msn_rt_window]
            ):
                continue
            if scan.parent:
                try:
            ...
            if not scans_to_select or str(scan_id) in scans_to_select:
                if ion_search:
                    ...
                    if msn_for_id == scan.ms_level:
                        ...
                        spectra_to_quant = (
                            find_prior_scan(msn_map, scan_id, ms_level=msn_for_quant)
                            if msn_for_quant != msn_for_id
                            else scan_id
                        )
                        d = {
                            ...
                            "id_scan": {
                                ...
                                "charge": scan.charge,
                                "mass": float(scan.mass),
                                "product_ion": float(scan.product_ion)
                                if args.mrm
                                else None,
                            },
                        }
        ...
        if calc_spline and len(spline_x):
            spline_df = pd.DataFrame(
                zip(spline_x, spline_y), columns=["Observed", "Error"]
            )
            spline_df = spline_df[
                (spline_df["Error"] < 25) & (spline_df["Error"] > -25)
            ].dropna()
            spline_df.sort("Observed", inplace=True)
            spline_df.drop_duplicates("Observed", inplace=True)
            if len(spline_df) > 10:
                spline = UnivariateSpline(
                    spline_df["Observed"].astype(float).values,
                    spline_df["Error"].astype(float).values,
                    s=1e6,
                )
            else:
                spline = None
        ...
        if ion_search or args.mva:
            ions = (
                [
                    i["id_scan"].get("theor_mass", i["id_scan"]["mass"])
                    for i in raw_scans
                ]
                if args.mva
                else raw_scans["ions"]
            ...
            for scan_id in scans_to_fetch:
                ...
                if args.msn_all_scans:
                    ...
                    for ion_index, ion_set in enumerate(ions):
                        d = {
                            ...
                            "id_scan": {
                                "id": scan_id,
                                "theor_mass": ion_set[0],
                                "rt": rt_info[ion_index] if rt_info else scan["rt"],
                                "charge": 1,
                                "mass": ion_set[0],
                                "ions_found": None,
                                "ion_set": ion_set,
                ...
                for ion_set in ions:
                    if isinstance(ion_set, float):
                        ion_set = [ion_set]
                    for ion_index, (ion, nearest_mz_index) in enumerate(
                        zip(
                            ion_set,
                            find_nearest_indices(
                                scan_mzs, np.array(ion_set, dtype=np.float)
                            ),
                        )
                    ):
                        ...
                        if peaks.get_ppm(ion, nearest_mz) < ion_tolerance:
                            if args.mva:
                                ...
                                if ion_rt is None or (
                                    rt - args.rt_window < ion_rt < rt + args.rt_window
                                ):
                                    found_ions.append(
                                        {
                                            "ion": ion,
                        ...
                        ions_found += (
                            found_ions[0] if args.require_all_ions else found_ions
                        )
                if ions_found:
                    ...
                    if msn_for_quant == msn_for_id or args.mva:
                        for ion_dict in ions_found:
                            ...
                            if charge == 0 and not args.mva:
                                ...
                                for i in range(1, 5):
                                    ...
                                    for j in range(1, 3):
                                        ...
                                        closest_mz = peaks.find_nearest_index(
                                            scan_mzs, next_peak
                                        )
                                        if (
                                            peaks.get_ppm(
                                                next_peak, scan_mzs[closest_mz]
                                            )
                                            < isotope_ppm * 1.5
                                        ):
                                    ...
                                    charge_states.append(
                                        (charge_peaks_found, i, peak_height)
                                    )
                                charge_states = sorted(
                                    charge_states,
                                    key=operator.itemgetter(0, 2),
                                    reverse=True,
                                )
                                if int(ion_dict["charge"]) not in [
                                    i[0] for i in charge_states
                                ]:
                                    continue
                                elif args.mva:
                                ...
                                if args.mva:
                                    rep_key = (
                                        ion_dict["scan_info"]["id_scan"]["rt"],
                                        ion_dict["scan_info"]["id_scan"]["mass"],
                                        charge_to_use,
                                    )
                            ...
                            if args.mva:
                                ...
                                spectra_to_quant = find_prior_scan(
                                    msn_map, scan_id, ms_level=msn_for_quant
                                )
                                ...
                                d = {
                                    ...
                                    "id_scan": {
                                        "id": scan_id,
                                        "theor_mass": ion,
                                        "rt": rt if ion_rt is None else ion_rt,
                                        "charge": charge_to_use,
                                        "mass": float(nearest_mz),
                                        "ions_found": ion_found,
                            ...
                            added.add(key)
                            if args.mva:
                                replicate_search_list[
                                    (d["id_scan"]["rt"], ion_dict["ion"])
                                ].append((spectra_to_quant, d))
                            else:
                                ion_search_list.append((spectra_to_quant, d))
                    else:
                        ...
                        d = {
                            ...
                            "id_scan": {
                                ...
                                "ions_found": ";".join(
                                    map(
                                        lambda x: "{}({})".format(ion, nearest_mz),
                                        ions_found,
                                    )
                                ),
                            },
        ...
        if ion_search or all_msn:
            raw_scans = [
                i[1] for i in sorted(ion_search_list, key=operator.itemgetter(0))
            ]
        if args.mva:
            raw_scans = []
                    ...
                    [
                        (
                            np.abs(
                                (
                                    rep_mapper.predict(
                                        np.array(j[1]["replicate_scan_rt"]).reshape(
                                            -1, 1
                                        )
                                    )
                                    if rep_mapper
                                    else j[1]["replicate_scan_rt"]
                                )
                                - ion_rt
                            ),
                            j[1],
                        )
                        for j in replicate_search_list[i]
                    ],
        ...
        quant_msn_map = (
            [i for i in msn_map if i[0] == msn_for_quant] if not args.mrm else msn_map
        )
        manager = Manager()
        ...
        for i in range(threads):
            worker = Worker(
                ...
                debug=args.debug,
                html=html,
                mono=not args.spread,
                precursor_ppm=args.precursor_ppm,
                isotope_ppm=args.isotope_ppm,
                isotope_ppms=None,
                msn_rt_map=msn_rt_map,
                reporter_mode=reporter_mode,
                reader_in=reader_in,
                reader_out=reader_outs[i],
                thread=i,
                quant_method=quant_method,
                spline=spline,
                isotopologue_limit=isotopologue_limit,
                labels_needed=labels_needed,
                quant_msn_map=quant_msn_map,
                overlapping_mz=overlapping_mz,
        ...
        exclusion_masses = (
            mrm_pair_info.loc[
                ...
                [
                    i
                    for i in mrm_pair_info.columns
                    if i.lower() not in ("light", "retention time")
                ],
            ].values.flatten()
            if args.mrm
            else set([])
        ...
        for scan_index, raw_scan_info in enumerate(raw_scans):
            ...
            if quant_scan.get("id") is None:
                ...
                if scan_to_quant is not None:
                    ...
                    msn_to_quant = find_prior_scan(
                        msn_map, scanId, ms_level=msn_for_quant
                    )
            ...
            if args.mva and rep_mapper is not None:
                target_scan["rt"] = rep_mapper.predict(
                    np.array(float(target_scan["rt"])).reshape(-1, 1)
                )[0]
            ...
            if msn_for_quant != 1:
                ...
                target_scan["theor_mass"] = (
                    target_scan.get("theor_mass", target_scan.get("mass")) - mass_shift
                )
                target_scan["precursor"] = (
                    target_scan["mass"] - mass_shift
                    if not args.neucode
                    else target_scan["theor_mass"]
                )
            # key is filename, peptide, charge, target scan id, modifications
            key = (
                filename,
                target_scan.get("peptide", ""),
                target_scan.get("charge"),
                target_scan.get("id"),
                target_scan.get("modifications"),
            )
            if resume:
        ...
        while workers or result is not None:
            try:
                ...
                for i, v in enumerate(workers):
                    if not v.is_alive():
                        ...
                        if exit_code in CRASH_SIGNALS:
                            print(
                                "thread has been killed using params {}".format(
                                    v.params
                                )
                            )
                        to_del.append(
                ...
                workers_to_add = []
                for worker_dict in sorted(
                    to_del, key=operator.itemgetter("worker_index"), reverse=True
                ):
                    ...
                    if worker_dict["exitcode"] in CRASH_SIGNALS:
                        ...
                        worker = Worker(
                            ...
                            reporter_mode=reporter_mode,
                            reader_in=reader_in,
                            reader_out=reader_outs[thread_index],
                            thread=thread_index,
                            quant_method=quant_method,
                            spline=spline,
                            isotopologue_limit=isotopologue_limit,
                            labels_needed=labels_needed,
                            quant_msn_map=[i for i in msn_map if i[0] == msn_for_quant]
                            if not args.mrm
                            else msn_map,
                            overlapping_mz=overlapping_mz,
                            min_resolution=args.min_resolution,
            ...
            if result is not None:
                ...
                if completed % 10 == 0:
                    sys.stderr.write(
                        "\r{0:2.2f}% Completed".format(completed / scan_count * 100)
                    )
                    sys.stderr.flush()
                ...
                for label_name in labels:
                    ...
                    if args.export_mzml:
                        ...
                        if args.export_mode == PER_FILE:
                            export_mapping[
                                "{}_{}.mzML".format(out_path, filename)
                            ] |= flattened_scans
                            ...
                            export_mapping[
                                "{out}_{raw}_{ms1}_{precursor}.mzML".format(
                                    **{
                                        "out": out_path,
                                        "raw": filename,
                                        "precursor": result.get(
                                            "{}_precursor".format(label_name)
                                        ),
                                        "ms1": result.get("ms1"),
                                    }
                                )
                                ...
                                peak_report.append(
                                    list(
                                        map(
                                            str,
                                            (
                                                xic_peak_info.get(i[0], "NA")
                                                for i in PEAK_REPORTING
                                            ),
                                        )
                                    )
                                )
                ...
                temp_file.write(
                    pd.io.json.dumps(
                        {"res_dict": res_dict, "html": result.get("html", {})}
                    )
                )
                temp_file.write("\n")
                temp_file.flush()
    ...
    data = pd.DataFrame.from_records(
        df_data, columns=[i for i in headers if i != "Confidence"]
    )
    header_mapping = []
    ...
    if html:
        ...
        for index, (row_index, row) in enumerate(data.iterrows()):
            ...
            to_write, html_info = table_rows(
                [
                    {
                        "table": res.strip(),
                        "html": html_data[index],
                        "keys": header_mapping,
                    }
                ]
            )
