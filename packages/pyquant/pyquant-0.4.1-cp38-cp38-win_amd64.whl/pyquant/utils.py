...
def looper(selected=None, df=None, theo=None, index=0, out=None):
    ...
    if index != len(selected):
        for i in selected[index]:
            out[index] = i
            for j in looper(
                selected=selected, df=df, theo=theo, index=index + 1, out=out
            ):
                yield j
    else:
...
def find_prior_scan(msn_map, current_scan, ms_level=None):
    ...
    for scan_msn, scan_id in msn_map:
        if scan_id == current_scan:
            return prior_msn_scans.get(
                ms_level if ms_level is not None else scan_msn, None
            )
...
def get_scans_under_peaks(rt_scan_map, found_peaks):
    ...
    for peak_isotope, isotope_peak_data in six.iteritems(found_peaks):
        ...
        for xic_peak_index, xic_peak_params in six.iteritems(isotope_peak_data):
            mean, stdl, stdr = (
                xic_peak_params["peak_mean"],
                xic_peak_params["std"],
                xic_peak_params["std2"],
            )
            left, right = mean - 2 * stdl, mean + 2 * stdr
            scans[peak_isotope][xic_peak_index] = set(
                rt_scan_map[
                    (rt_scan_map.index >= left) & (rt_scan_map.index <= right)
                ].values
            )
    return scans
    ...
    for peaks1_key, peaks2_key in combinations(new_peaks.keys(), 2):
        ...
        for peak_index1, peak1 in enumerate(peaks1):
            ...
            left_bound, right_bound = (
                mean - peak1.get("std1", 0) * 2,
                mean + peak1.get("std2", peak1.get("std1", 0)) * 2,
            )
            ...
            for peak_index2, peak2 in enumerate(peaks2):
                ...
                left_bound2, right_bound2 = (
                    mean2 - peak2.get("std1", 0) * 2,
                    mean2 + peak2.get("std2", peak2.get("std1", 0)) * 2,
                )
                # This tests whether there is a overlap.
                if (left_bound <= right_bound2) and (left_bound2 <= right_bound):
                    potential_peaks[dict_key]["overlaps"].add(
                        ((peaks2_key, peak_index2), mean2)
                    )
                    potential_peaks[dict_key]["intensities"] += area2
    peak_overlaps = [
        (key, len(overlap_info["overlaps"]), overlap_info["intensities"])
        for key, overlap_info in six.iteritems(potential_peaks)
    ]
    if not peak_overlaps and len(new_peaks) == 1:
        # there is only 1 ion w/ peaks, just pick the biggest peak
        means = [
            sorted(list(new_peaks.values())[0], key=itemgetter("total"), reverse=True)[
                0
            ].get("mean")
        ]
    else:
        ...
        if tie_breaker_time is not None:
            tie_breaker_distances = [
                (
                    (key, peak_index),
                    np.abs(tie_breaker_time - new_peaks[key][peak_index]["mean"]),
                )
...
ETNS = {
    1: {"C": 0.0110, "H": 0.00015, "N": 0.0037, "O": 0.00038, "S": 0.0075},
    2: {"O": 0.0020, "S": 0.0421},
    4: {"S": 0.00020},
}
...
def calculate_theoretical_distribution(peptide=None, elemental_composition=None):
    ...
    def dio_solve(n, loops=None, index=0, out=None):
        loops = loops or [1, 2, 4]
        if out is None:
            out = [0] * len(loops)
        if index != len(loops):
            ...
            if n == sum([a * b for a, b in zip(loops, out)]):
                yield out
    ...
    while tp < 0.999:
        ...
        for solution in dio_solve(len(dist)):
            ...
            for k, i in zip(solution, [1, 2, 4]):
                petn = ETN_P[i]
                p2.append(
                    (comb(total_atoms, k) * (petn ** k))
                    * ((1 - petn) ** (total_atoms - k))
                )
            p += np.cumprod(p2)[-1]
        tp += p
        dist.append(p)
        ...
        for label2 in mass_labels.keys():
            ...
            try:
                ...
                for label, new_label in zip([label1, label2], ["Label1", "Label2"]):
                    for col in cols:
                        nd["{} {}".format(new_label, col)] = data[
                            "{} {}".format(label, col)
                        ]

                nd.replace([-np.inf, np.inf, "NA"], np.nan, inplace=True)
                non_na_data = nd.dropna().index
                nd.loc[non_na_data, "Label1 Intensity"] = np.log2(
                    nd.loc[non_na_data, "Label1 Intensity"].astype(float)
                )
                nd.loc[non_na_data, "Label2 Intensity"] = np.log2(
                    nd.loc[non_na_data, "Label2 Intensity"].astype(float)
                )

                nd.replace([-np.inf, np.inf, "NA"], np.nan, inplace=True)
                non_na_data = nd.dropna().index
                nd.loc[non_na_data, "Label1 R^2"] = logit(
                    nd.loc[non_na_data, "Label1 R^2"].astype(float)
                )
                nd.loc[non_na_data, "Label2 R^2"] = logit(
                    nd.loc[non_na_data, "Label2 R^2"].astype(float)
                )
                ...
                sys.stderr.write(
                    "Unable to calculate statistics for {}/{}.\n Traceback: {}".format(
                        label1, label2, traceback.format_exc()
                    )
                )
...
def boolrelextrema(data, comparator, axis=0, order=1, mode="clip"):
    ...
    if np.ma.is_masked(data):
        ...
        for index, result in enumerate(
            boolrelextrema(main, comparator, axis=axis, order=order, mode=mode)
        ):
...
def merge_peaks(peaks_found, debug=False):
    ...
    for peak_width_index in range(len(peak_widths) - 1):
        ...
        if set(smaller_peaks) == set(larger_peaks) and set(smaller_minima) == set(
            larger_minima
        ):
...
def find_possible_peaks(
    ...
    possible_peaks = PEAK_METHODS[peak_find_method](
        xdata, ydata, ydata_peaks=ydata_peaks
    )
    ...
    for peak_width, peak_info in six.iteritems(possible_peaks):
        ...
        if snr or zscore:
            ...
            if local_filter_size:
                ...
                for row_peak in row_peaks:
                    selection = np.abs(
                        select_window(ydata_peaks, row_peak, local_filter_size)
                    )
                ...
                logger.debug(
                    "{} peaks lost to filtering\n{}\n".format(
                        len(row_peaks) - len(new_peaks), lost_peaks
                    )
                )
                ...
                if snr and row_peaks.any():
                    logger.debug(
                        "{} peaks lost to SNR\n".format(
                            sum(ydata_peaks[row_peaks] / ydata_peaks_std < snr)
                        )
                    )
                if zscore and row_peaks.any():
                    logger.debug(
                        "{} peaks lost to zscore\n".format(
                            sum(
                                (ydata_peaks[row_peaks] - ydata_peaks_median)
                                / ydata_peaks_std
                                < zscore
                            )
                        )
                ...
                if snr and row_peaks.any():
                    row_peaks = row_peaks[
                        np.abs(ydata_peaks[row_peaks]) / ydata_peaks_std >= snr
                    ]
                if zscore and row_peaks.any():
                    row_peaks = row_peaks[
                        (np.abs(ydata_peaks[row_peaks]) - ydata_peaks_median)
                        / ydata_peaks_std
                        >= zscore
                    ]
        ...
        if amplitude_filter != 0 and row_peaks.any():
            logger.debug(
                "{} peaks lost to amp filter\n{}\n".format(
                    sum(abs_ydata[row_peaks] < amplitude_filter),
        ...
        if percentile_filter and row_peaks.any():
            logger.debug(
                "{} peaks lost to percentile filter\n{}\n".format(
                    sum(
                        np.abs(ydata_peaks[row_peaks])
                        < np.percentile(abs_ydata, percentile_filter)
                    ),
                    row_peaks[
                        np.abs(ydata_peaks[row_peaks])
                        < np.percentile(abs_ydata, percentile_filter)
...
def find_peaks_rel_max(
    xdata, ydata, ydata_peaks=None, peak_width_start=2, peak_width_end=4, micro=False
):
    ...
    while peak_width <= peak_width_end:
        row_peaks = np.array(
            argrelextrema(np.abs(ydata_peaks), np.greater, order=peak_width)[0],
            dtype=int,
        )
        ...
        minima.extend(
            [
                i
                for i in argrelextrema(np.abs(ydata_peaks), np.less, order=peak_width)[
                    0
                ]
                if i not in minima and i not in row_peaks
            ]
        )
        minima.sort()
...
def get_cross_points(data, pad=True):
    ...
    if cross_points and pad:
        ...
        if cross_points[0] != 0:
            cross_points.insert(
                0, np.argmax(signs[: cross_points[0]] != signs[cross_points[0]])
            )
...
def find_peaks_derivative(
    ...
    ydata_peaks=None,
    min_slope=None,
    rel_peak_height=None,
    min_peak_side_width=2,
    max_peak_side_width=np.inf,
    min_peak_width=None,
    max_peak_width=np.inf,
    smooth=True,
):
    ...
    if cross_points:
        ...
        for i in range(1, len(cross_points) - 1):
            ...
            if not (
                (
                    (min_peak_side_width <= left_peak_width <= max_peak_side_width)
                    or (
                        min_peak_side_width > left
                    )  # This allows us to find peaks that start at the edges of our data
                )
                and (
                    (min_peak_side_width <= right_peak_width <= max_peak_side_width)
                    or (
                        (len(ydata) - 1) < right + min_peak_side_width
                    )  # This allows us to find peaks that start at the edges of our data
                )
            ):
            ...
            rel_peak = min(np.abs([(center_y / left_y), (center_y / right_y),]))
...
def erode_direction(peaks_found, peak_heights, start, increment, distance):
    ...
    peaks_to_remove = set([])
    while (
        (next_index >= 0 and next_index < len(peaks_found))
        and np.abs(peak - peaks_found[next_index]) < distance
        and peak_height != peak_heights[next_index]
    ):
...
def merge_close_peaks(peaks_found, ydata, distance=2):
    ...
    if peaks_found.any():
        ...
        for sort_index, peak_index in enumerate(sort_order):
            ...
            removed_peaks |= erode_direction(
                peaks_found, peak_heights, peak_index, 1, distance
            )
            removed_peaks |= erode_direction(
                peaks_found, peak_heights, peak_index, -1, distance
            )
...
def estimate_peak_parameters(
    xdata,
    ydata,
    row_peaks,
    minima_array,
    fit_negative=False,
    rel_peak_constraint=0,
    micro=False,
    bigauss_fit=False,
    fit_baseline=False,
):
    ...
    for peak_num, peak_index in enumerate(row_peaks):
        if peak_index in skip_peaks:
            continue
        next_peak = (
            len(xdata) if peak_index == row_peaks[-1] else row_peaks[peak_num + 1]
        )
        ...
        if minima_array.size:
            ...
            left_stop = (
                np.searchsorted(minima_array, last_peak) if last_peak != -1 else -1
            )
            if left == -1:
            ...
            bnds.extend(
                [
                    (-1.01, rel_peak * rel_peak_constraint)
                    if rel_peak < 0 and fit_negative
                    else (rel_peak * rel_peak_constraint, 1.01),
        ...
        if peak_values.any():
            ...
            variance = np.sqrt(
                np.average((peak_indices - average) ** 2, weights=np.abs(peak_values))
            )
            if variance == 0:
        ...
        if variance is not None:
            ...
            if fit_baseline:
                ...
                intercept = (
                    (ydata[right] - slope * xdata[right])
                    + (ydata[left] - slope * xdata[left])
                ) / 2
    ...
    if not guess:
        ...
        if fit_baseline:
            ...
            intercept = (
                (ydata[-1] - slope * xdata[-1]) + (ydata[0] - slope * xdata[0])
            ) / 2
            guess.extend([slope, intercept])
            bnds.extend([(None, None), (None, None)])
...
def subtract_baseline(y, components_to_remove=10, dampen_factor=10):
    ...
    # filter it
    filtered = (
        (fs * (1 - signal.gaussian(len(y), dampen_factor))) if dampen_factor else fs
    )
    # invert it
