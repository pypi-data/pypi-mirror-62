...
class FileMixins(object):
    def setUp(self):
        ...
        self.executable = os.path.abspath(
            os.path.join(self.base_dir, "..", "scripts", "pyQuant")
        )
        self.ecoli_search_file = os.path.join(
            self.data_dir, "Chris_Ecoli_1-2-4-(01).msf"
        )
...
class GaussianMixin(object):
    def setUp(self):
        self.amp, self.mu, self.std, self.mu2 = 1.0, 0.0, 1.0, 3.0
        self.one_gauss_params = np.array([self.amp, self.mu, self.std], dtype=np.float)
        self.two_gauss_params = np.array(
