    ...
    def run(self):
        ...
        for scan_request in iter(self.incoming.get, None):
            ...
            if not d:
                ...
                if scan is not None:
                    rt = scan.rt
                    if self.rt_window is not None and not any(
                        [(i[0] < float(rt) < i[1]) for i in self.rt_window]
                    ):
                        d = None
                    else:
                        scan_vals = np.array(scan.scans)
                        if self.spline:
                            scan_vals[:, 0] = scan_vals[:, 0] / (
                                1 - self.spline(scan_vals[:, 0]) / 1e6
                            )
                        # add to our database
                        d = {
                            "vals": scan_vals,
            ...
            if d is not None and (mz_start is not None or mz_end is not None):
                ...
                out["vals"] = out["vals"][
                    np.where(
                        (out["vals"][:, 0] >= mz_start) & (out["vals"][:, 0] <= mz_end)
                    )
                ]
