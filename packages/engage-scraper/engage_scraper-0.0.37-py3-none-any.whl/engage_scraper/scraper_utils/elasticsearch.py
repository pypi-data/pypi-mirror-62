...
class ElasticsearchUtility():
    def __init__(self):
        ...
        r = requests.head(f"http://{ES_HOSTNAME}/{ES_INDEX_NAME}:9200?allow_no_indices=false")
        self.status_code = r.status_code
        if self.status_code != requests.codes.ok:
            reqPut = requests.put(f"http://{ES_HOSTNAME}/{ES_INDEX_NAME}:9200", json=self._agenda_items)
