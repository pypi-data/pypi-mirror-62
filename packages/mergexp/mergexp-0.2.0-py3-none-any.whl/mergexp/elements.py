...
class Topology():
    ...
    be given a name.
    """
    def __init__(self, name, devices=None, nets=None):
        if not devices:
            devices = []
        if not nets:
            nets = []
...
def link_id(endpoints):
    ...
    for e in endpoints[:-1]:
        lid += "%s.%d-"%(e.endpoint.device.name, 
                        e.endpoint.device.endpoints.index(e.endpoint))

    lid += "%s.%d"%(endpoints[-1].endpoint.device.name, 
                    endpoints[-1].endpoint.device.endpoints.index(endpoints[-1].endpoint))
