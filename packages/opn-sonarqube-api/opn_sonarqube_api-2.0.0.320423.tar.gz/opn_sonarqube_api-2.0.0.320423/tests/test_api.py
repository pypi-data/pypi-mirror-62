...
from sonarqube_api import SonarAPI
from sonarqube_api.exceptions import ClientError, AuthError, ValidationError, ServerError


class SonarAPITest(TestCase):

    def setUp(self):
        self.h = SonarAPI(user='admin', password='admin')

    def test_url_building(self):
        test = SonarAPI(host='http://localhost', port=9001,
                        base_path='/testing')
        self.assertEqual(
            "http://localhost:9001/testing{}".format(test._resources.RESOURCES_ENDPOINT),
            test.get_url(test._resources.RESOURCES_ENDPOINT))

        test = SonarAPI(host='http://localhost', port=9001)
        self.assertEqual(
            "http://localhost:9001{}".format(test._resources.RESOURCES_ENDPOINT),
            test.get_url(test._resources.RESOURCES_ENDPOINT))

        test = SonarAPI(host='http://localhost')
        self.assertEqual(
            "http://localhost:9000{}".format(test._resources.RESOURCES_ENDPOINT),
            test.get_url(test._resources.RESOURCES_ENDPOINT))
    ...
    def test_get_metrics(self, mock_call):
        ...
        mock_call.assert_any_call(
            'get', self.h._metrics.METRICS_LIST_ENDPOINT, f='coverage,violations'
        )
        mock_call.assert_any_call(
            'get', self.h._metrics.METRICS_LIST_ENDPOINT, f='coverage,violations', p=2
        )
    ...
    def test_get_rules(self, mock_call):
        ...
        self.assertEqual(mock_call.call_count, 2)
        mock_call.assert_any_call(
            'get', self.h._rules.RULES_LIST_ENDPOINT, is_template='no', statuses='READY',
    ...
    def test_get_resources_metrics(self, mock_call):
        ...
        mock_call.assert_called_once_with(
            'get', self.h._resources.RESOURCES_ENDPOINT,
            metrics=','.join(self.h._resources.GENERAL_METRICS)
        )
