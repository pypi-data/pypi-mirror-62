...
def _config_app(app_config):
    ...
    for i in os.listdir(app_config['MENTAT_CORE']['__core__reporter']['event_classes_dir']):
        if os.path.isdir(os.path.join(app_config['MENTAT_CORE']['__core__reporter']['event_classes_dir'], i)):
            app_config['BABEL_TRANSLATION_DIRECTORIES'] += os.path.join(
