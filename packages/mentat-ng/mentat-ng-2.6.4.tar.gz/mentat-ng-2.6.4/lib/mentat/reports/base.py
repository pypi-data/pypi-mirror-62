...
class BaseReporter:
    ...
    * ``self.translator`` - report localization
    ...
    def _setup_renderer(self, templates_dir):
        ...
        #    lambda x: self.translator.ugettext(x),
        #    lambda s, p, n: self.translator.ungettext(s, p, n),
        #    newstyle = True
        #)
        renderer.install_gettext_callables(
            lambda x: self.translator.gettext(x),
            lambda s, p, n: self.translator.ngettext(s, p, n),
            newstyle = True
        )
    ...
    def translator(self):
        ...
        if self.locale not in self.translations:
            ...
            for dir in self.translations_dir.split(";"):
                try:
