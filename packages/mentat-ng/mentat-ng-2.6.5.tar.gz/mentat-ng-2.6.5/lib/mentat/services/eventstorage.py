...
def _bq_qbase_select_full(qname = None):
...
def _bq_qbase_select_ghost(qname = None):
    query = psycopg2.sql.SQL('SELECT * FROM events')
    if qname:
        query += psycopg2.sql.SQL(' AS {}').format(
            psycopg2.sql.Identifier(qname)
...
def _bq_qbase_count(qname = None):
...
def _bq_qbase_delete(qname = None):
...
def build_query(parameters = None, qtype = QTYPE_SELECT, qname = None):  # pylint: disable=locally-disabled,too-many-branches
    """
    ...
    if qtype == QTYPE_SELECT:
        query = _bq_qbase_select_full(qname)
    elif qtype == QTYPE_SELECT_GHOST:
        query = _bq_qbase_select_ghost(qname)
    elif qtype == QTYPE_COUNT:
        query = _bq_qbase_count(qname)
    elif qtype == QTYPE_DELETE:
        query = _bq_qbase_delete(qname)
...
class incstats_decorator(object):
