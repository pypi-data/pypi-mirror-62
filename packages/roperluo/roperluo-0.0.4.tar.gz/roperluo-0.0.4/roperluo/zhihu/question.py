        ...
        with open(file, 'w', encoding='utf-8') as f:
            for ans in self.answers:
                try:
                    ...
                    print("[ERROR] save answer txt fail.err:{}, ans:{}".format(e, ans))
        ...
        for ans in self.answers:
            try:
                ...
                for img in doc('.origin_image').items():
                    ...
                    if img_src.startswith('http') and img_src.find('pic3') == -1:  #pic3的图片没法访问
                        print('save image:{}'.format(img_src))
                        picture_name = img_src.split('/')[-1]
                        filename=os.path.join(image_dir, picture_name)
                        urllib.request.urlretrieve(img_src,filename=filename)
                        time.sleep(0.5)
            except Exception as e:
                print("[ERROR] save answer txt fail.err:{}, ans:{}".format(e, ans))
