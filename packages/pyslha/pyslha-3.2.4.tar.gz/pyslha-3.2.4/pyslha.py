...
## Python version: require >= 2.7 (including Py3)
import sys
...
def _autotype(var):
    ...
    TODO: use ast module to do this better?
    """
...
class Block(object):
    ...
    def itemlists(self):
        """Access all the block items as [key+value] lists."""
        rtn = []
        for k,v in self.items():
            klist = [ik for ik in k] if hasattr(k, "__iter__") else [k]
            vlist = [iv for iv in v] if hasattr(v, "__iter__") else [v]
            elist = klist + vlist
            rtn.append(elist)
        return rtn
...
class Process(object):
    ...
    def get_xsecs(self, sqrts=None, scale_scheme=None, qcd_order=None, ew_order=None, kappa_f=None, kappa_r=None, pdf_id=None, code=None):
        ...
        if qcd_order is not None:
            rtn = [x for x in rtn if x.qcd_order == qcd_order]
        if ew_order is not None:
...
def writeSLHAXSections(xsections, precision=8):
    ...
    for _, proc in xsections.items():
        for sqrts in proc.sqrtses:
            blockstr = "XSECTION " + _autostr(sqrts, precision) + " "
            ...
            for x in proc.get_xsecs(sqrts=sqrts):
                ...
                blockstr += "%s %s " % (_autostr(x.kappa_f, precision), _autostr(x.kappa_r, precision))
                blockstr += "%d %s" % (x.pdf_id, _autostr(x.value, precision))
                if x.code[0]:
                    blockstr += " " + x.code[0]
                if x.code[1]:
                    blockstr += " " + x.code[1]
