...
function appendMenus() {
  ...
  for(var x in elems) {
    ...
    anchoritem.setAttribute("onclick","appendItem('" + path + "')");    
...
function showEnumerations () {
  ...
  for (var i = 1; i < enumerationTableRows.length; i++) { 
...
class SchemaItem {
  ...
  getCrossReferences() {
    if (!this.CrossReferences){
      ...
      if (this.Node.childNodes[1].childNodes[3].childNodes[7]){
...
function appendPath(appendToNode, itemObj, showCopyButton) {
  ...
  if (showCopyButton) {
    ...
    showMoreNode.setAttribute('onclick', 'copyPath("' + itemObj.Path +'")');
...
function appendSource (appendToNode, itemObj) {
    ...
    if (TYPE == 'Transaction' && SUBSCHEMA == false){
      ...
      anchorNode.setAttribute('onclick', 'setVersion("Master","undefined", "' + masterVersion + '")')
...
function appendParentPath (appendToNode, itemObj) {
  ...
  else if (itemType != 'complexType' && itemType != 'simpleType')
  {
      ...
      for (var i = 0; i < ancestorsArray.length; i++) 
      {
          ...
          anchorNode.setAttribute('onclick', 'appendItem(\'' + ancestorObj.Path +'\')');
...
/*     var href = "./index.html#" + itemObj.Path;
    ...
    anchorNode.setAttribute('onclick', 'appendItem(\'' + itemObj.Type +'\')');
    ...
    //console.log("transactions: ",transactions);
     ...
     if (transactions){
      ...
      for (var i = 0; i < transactionsArray.length; i++) 
      {
          ...
          anchorNode.setAttribute('onclick', 'setVersion("Transaction","' + transaction + '", "' + getSubschemas()[transaction] + '")')
...
function appendContentsOfElement (appendToNode, itemObj, tooltip = false, heading) {
  ...
  /*                     if (childObj[19] !== "" && childObj[19] != null){
            ...
            anchorNode.setAttribute('onclick', 'appendItem(\'' + childObj.Path +'\')');
            ...
            if (tooltip == true) {
              ...
              iconNode.setAttribute('onclick', 'appendItem(\'' + childObj.Path +'\')');
...
function appendEnumerations(appendToNode, itemObj, showAllEnumerations, tooltip, showMoreButton) {
  ...
  if (enumerations) {
    ...
    if (showMoreButton && (enumerations.length > 10)) {      
      ...
      showMoreNode.setAttribute('onclick', 'showEnumerations()');
...
function appendUsedBy (appendToNode, itemObj, showAllUsedBy, showMoreButton) {
  ...
  if (usedBy && usedBy.length > 0) {
    ...
    for (var i = 0; i < usedBy.length; i++) {
      ...
      // onclick
      anchorNode.setAttribute('onclick', 'appendItem(\'' + path +'\')');
 ...
 /*       if (tooltip == true) {
        ...
        imageNode.setAttribute('class','info');
        iconNode.className = 'tooltip-parent';  
        iconNode.setAttribute('onclick', 'appendItem(\'' + path +'\')');
...
function appendCrossReferences(appendToNode, itemObj, showAllCrossReferences, tooltip, showMoreButton) {
  ...
  if (crossReferences) {
    ...
    /*   if (enumerationsDict[enumeration][5] != null && enumerationsDict[enumeration][5] != '') {
      ...
      var anchorNode = document.createElement('a');  
      anchorNode.setAttribute('onclick', 'appendItem(\'' + crossReference.getPath() +'\')');      
      ...
      if (tooltip == true) {
        ...
        imageNode.setAttribute('class','info');
        iconNode.className = 'tooltip-parent'; 
        iconNode.setAttribute('onclick', 'appendItem(\'' + crossReference.getPath() +'\')');             
        colOneNode.appendChild(iconNode);
        iconNode.appendChild(imageNode);
...
function appendCodeSnippet(appendToNode, itemObj) {
  ...
  // type attribute
  var type_link =  '<a class="xml-complextype-name" onclick="appendItem(\'' + type + '\')">' + type + '</a>';
  
  // special case for 'Comment' and 'InlineAttachment'
  if (name=="Comment" || name=="InlineAttachment" )  {
    var type_link = '<a class="xml-simpletype-name" onclick="appendItem(\'' + type + '\')">' + type + '</a>';
    code += ' type="'+ type_link +'"';
  }    
  ...
  for (var i = 0; i < childElements.length; i++) {
    ...
    var child_link =  '<a class="xml-element-name" onclick="appendItem(\'' + child_path + '\')">' + child_name + '</a>';
  ...
  for (var i = 0; i < childAttributes.length; i++) {
    ...
    var child_link =  '<a class="xml-attribute-name" onclick="appendItem(\'' + child_path + '\')">' + child_name + '</a>';
    var type_link = '<a class="xml-simpletype-name" onclick="appendItem(\'' + child_type + '\')">' + child_type + '</a>'
...
function appendJsonSchemaSnippet (appendToNode, itemObj) {
  ...
  var complex_type_link =  '<a class="xml-complextype-name" onclick="appendItem(\'' + type + '\')">' + type + '</a>';
  ...
  if (typetype == 'complexType') {
    ...
    for (var i = 0; i < childAttributes.length; i++) {
      ...
      var child_use       = childObj.getUse();
        ...
        var child_link =  '<a class="xml-attribute-name" onclick="appendItem(\'' + child_path + '\')">' + child_name + '</a>';
      var type_link =  '<a class="xml-complextype-name" onclick="appendItem(\'' + child_type + '\')">' + child_type + '</a>';
...
function appendXMLCodeSnippet(appendToNode, itemObj) { 
  ...
  if (name=="InlineAttachment")  {
    code += "&gt;U2FtcGxlIEJhc2U2NA==&lt;/xs:InlineAttatchment";
...
function insertGlossaryLinks (sentence) {
  ...
  function getLinkHTML (trimmedphrase, phrase) {
    ...
    var replacement = '<a><span onclick="appendGlossaryItem(\'' + titleCase(phrase) + '\')" class="tooltip-parent">' + trimmedphrase + '<span class="tooltiptext-under"><small>' + definition + '</small></span><span class="tooltiptext-after"/></span></a>' + phrase.replace(trimmedphrase, "");
    return replacement;
  }
