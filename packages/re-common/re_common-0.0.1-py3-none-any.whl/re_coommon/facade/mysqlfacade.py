...
from re_coommon.baselibrary import MysqlBuilderAbstract
from re_coommon.baselibrary.database.moudle import SqlMoudle
from re_coommon.baselibrary import IniConfig
from re_coommon.baselibrary.readconfig.toml_config import TomlConfig
from re_coommon.baselibrary import MLogger
...
class MysqlBuilder(MysqlBuilderAbstract):
    def __init__(self, configfile, configname, keytransformdicts=None):
        ...
        # 获取配置文件对象
        self.cf = IniConfig(configfile).builder().set_keep_keys_case().read_config()
        # 配置文件位置
    ...
    def build_port(self, port):
        self.sqlmoudle.port = int(self.cf.get_value(self.configname, port))
        return self

    def build_server_address(self, host):
        self.sqlmoudle.host = self.cf.get_value(self.configname, host)
        return self

    def build_password(self, passwd):
        self.sqlmoudle.passwd = self.cf.get_value(self.configname, passwd)
        return self

    def use_db(self, db):
        self.sqlmoudle.db = self.cf.get_value(self.configname, db)
        return self

    def build_username(self, user):
        self.sqlmoudle.user = self.cf.get_value(self.configname, user)
        return self

    def build_chart(self, chartset):
        self.sqlmoudle.charset = self.cf.get_value(self.configname, chartset)
        return self
    ...
    #     self.sqlmoudle.cursorclass = self.cf.get_value(self.configname, cursorclass)
