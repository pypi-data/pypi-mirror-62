...
from re_coommon.baselibrary.database.moudle import SqlMoudle, Sqlite3Moudle
...
class MysqlBuilder(MysqlBuilderAbstract):
    def __init__(self, configfile, configname, keytransformdicts=None):
        ...
        self.cf = IniConfig(configfile).set_keep_keys_case().read_config()
...
class Sqlite3Builder(Sqlite3BuilderAbstract):
    ...
    def build_file_path(self, dbPath='', opt="dbpath"):
        if dbPath:
            ...
            self.sqlite3moudle.database = self.cf.get_value(self.sec, opt)
        return self
