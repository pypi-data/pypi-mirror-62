...
def nlpaudiovideo(username,passw,company,email,maads_rest_url,thefile='',theserverfolder=''):   
    print("Please wait..this could take several minutes")
    if len(thefile)>0:
      files = {'file': open(thefile, 'rb')}
      data = {'mode':'nlpaudiovideo', 'username':username, 'password':passw,'company':company,'email':email,'localname':thefile,'thefolder': theserverfolder}
