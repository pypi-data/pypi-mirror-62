...
def formaturl(maindata,host,microserviceid):
    hostarr=host.split(":")
    hbuf=hostarr[0]
    hbuf=hbuf.lower()
    domain=''
    ...
    if len(microserviceid)>0:    
      mainurl=hostarr[0] + "://" + domain +"/" + microserviceid + "/?hyperpredict=" + maindata
    else:
      mainurl=hostarr[0] + "://" + domain +"/?hyperpredict=" + maindata
...
async def tcp_echo_client(message, loop,host,port,usereverseproxy,microserviceid):
    ...
    if usereverseproxy:
        geturl=formaturl(message,host,microserviceid) #host contains http:// or https://
        message="GET %s\n\n" % geturl 
    else:
        hostarr=host.split(":")
        hbuf=hostarr[0]
...
def uploadcsvfortraining(maadstoken,thefile,autofeature,removeoutliers,hasseasonality,dependentvariable,url,summer,winter,shoulder,trainingpercentage,retrainingdays,retraindeploy,shuffle,theserverlocalname,username='',passw='',company='',email=''):
...
def nlpclassify(maadstoken,iscategory,maads_rest_url,thefile='',theserverlocalname='',csvonly=0,username='',trainingpercentage=75,retrainingdays=0,retraindeploy=0,passw='',company='',email=''):
    print("Please wait..this could take several minutes")
    tstr=''
    rn=0
    if len(thefile)>0:
