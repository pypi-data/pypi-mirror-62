...
def nlp(username,passw,company,email,buffer,url,detail=20,maxkeywords=10):
    isurl=0
    ...
    try:    
        if os.path.isfile(buffer):  #pdf
            ...
            data = {'mode':'nlp1', 'username':username, 'password':passw,'company':company,'email':email,'localname':bn,'fvalue': detail,'maxkeywords': maxkeywords}
            r = requests.post(url, data=data, files=files)
        elif isurl==1:  #url
...
def nlpclassify(username,passw,company,email,thefile,iscategory,maads_rest_url,trainingpercentage=75,retrainingdays=0,retraindeploy=0,csvonly=0):
