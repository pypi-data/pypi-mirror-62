...
def parse_conf_for_url(file, regex, expected):
    file1 = open(file, 'r')
    lines = file1.readlines()
    for line in lines:
        if line.startswith(regex):
            words = line.split()
            x = re.sub("/validca.tgz", "", words[-1])
            x = re.sub("/validca.md5sum", "", x)
            x = re.sub("http://", "", x)
            pprint.header("#", False, ' ')
            pprint.info("  Url détectée: {0} ".format(x).ljust(48), False, ' ')
            if x == expected:
                pprint.success('{:>10s}'.format("OK"), True)
            else :
...
def check_https_service_config(varconfig):
    ...
    if isexistsdirectory("/etc/nginx/ssl"):
        if isexistsfile("/etc/nginx/ssl", "recup_crl_nginx.sh"):
            parse_conf_for_url("/etc/nginx/ssl/recup_crl_nginx.sh", "/usr/bin/wget -q", "validca.libriciel.fr")
