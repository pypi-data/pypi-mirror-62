...
def check_mysql_service_config(varconf):
    ...
    if not check_server_socket(mysqlserver, mysqlport):
        ...
        try:
            ...
            # Performance: how long for 1000 queries? must be inferior to 2ms
            import timeit
            loops = 1000
            setup = "import MySQLdb \n" \
            "db = MySQLdb.connect(host='" + str(mysqlserver) + \
            "', user='" + mysqluser + \
            "', passwd='" + mysqlpwd + \
            "', db='" + mysqlbase + "') \n" \
            "c = db.cursor()"
            ...
            pprint.header("#", False, ' ')
            pprint.info('Temps pour 1000 requêtes  = {:>f}'.format(total_time).ljust(46), False, '  ')
            if total_time > 2:
                pprint.error('{0:>10s}'.format(">2s, KO"), True)
            else:
                pprint.success('{0:>10s}'.format("<2s, OK"), True)
...
def check_crontab_jobs(version_ip):
    pprint.header("#", False, ' ')
    pprint.log("---- Check CRONTAB entries ----  ", True)
    version_ip_for_crontab = re.sub("\.", "", version_ip)
    if int(version_ip_for_crontab) < 459:
        version_ip_for_crontab = 450
    elif int(version_ip_for_crontab) < 467:
        version_ip_for_crontab = 460
    else:
        version_ip_for_crontab = 468
    parse_crontab("/usr/local/lib/python2.7/dist-packages/parapheur/files/crontab/" + str(version_ip_for_crontab) + ".txt", "/tmp/expected_crontab.txt")
