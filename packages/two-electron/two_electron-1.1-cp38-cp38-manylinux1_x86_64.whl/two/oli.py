...
def get_cmo(AOONEINT, SIRIFC):
    ...
    return ifc.view(Matrix)


def get_densities(SIRIFC):
    da, db = [d.view(Matrix) for d in dens.Dab(SIRIFC)]
    return da, db
...
def e2n(N, tmpdir='/tmp', hfx=1, Sg=1, Sv=1):
    ...
    S = one.read('OVERLAP',  filename=AOONEINT).unblock().unpack().view(Matrix)

    da, db = get_densities(SIRIFC)

    kN = rspvec.tomat(N, ifc, tmpdir=tmpdir).view(Matrix).T
    kn = (cmo*kN*cmo.T).view(Matrix)

    dak = (kn.T*S*da - da*S*kn.T)
    dbk = (kn.T*S*db - db*S*kn.T)*Sv

    (fa, fb), = fockab((da, db),  filename=AOTWOINT, hfx=hfx)
    fa += h
    fb += h
    (fak, fbk), = fockab((dak, dbk), filename=AOTWOINT, hfx=hfx)
