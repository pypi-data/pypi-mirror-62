...
def BO2mol(
    ...
    if l != l2:
        raise RuntimeError(
            "sizes of adjMat ({0:d}) and atomicNumList " "{1:d} differ".format(l, l2)
        )
...
def AC2mol(mol, AC, atomicNumList, charge, charged_fragments, quick):
    ...
    mol = BO2mol(
        mol, BO, atomicNumList, atomic_valence_electrons, charge, charged_fragments
    )
