...
def __getReturnAsModalHTML__(response, main=True):
    ...
    if isinstance(response, dict):
        ...
        if val_type == 'checkbox':
            ...
            return "<div class=\"md-form\"><label for=\"response-{id}\">data</label><textarea class=\"{className}\" name=\"response-{id}\" id=\"response-{id}\" rows=\"{rows}\">{value}</textarea></div>".format(className=input_className, id=response[0], value=response, rows=str( int( len(response)/100 ) + response.count('\n') ) )
            ...
            if val_type == 'url':
                return "<button class=\"btn\" data-clipboard-target=\"#response-{id}\"><img src=\"{% static 'core/img/clippy.svg' %}\" alt=\"Copy to clipboard\"></button><div class='md-form'><input class=\"{className}\" type=\"text\" id=\"response-{id}\" value=\"{input_value}\"/></div>".format(className=input_className, input_value=response, id=response[-20:])
            return "<div class='md-form'><input class=\"{className}\" type=\"text\" id=\"response-{id}\" value=\"{input_value}\"/></div>".format(className=input_className, input_value=response)
    return res
