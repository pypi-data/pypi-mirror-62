        ...
        for key, val in response.items():
            if val:
                ...
                try:
                    ...
                    val_type_temp = str
                if val_type_temp == str:
                    try:
                        val_type_temp = type(dict(val))
                    except:
                        pass

                val_type = 'text'
                if val_type_temp == int:
                    res += __getReturnAsModalHTML__(int(val), main=False)
                elif val_type_temp == list:
                    ...
                    val_type = Utils.getValueType(val)
                    try:
                    ...
                    if val_type == 'checkbox':
                        ...
                        if 'http' in val and ('.jpg' in val or '.png' in val or '.ico' in val or '.gif' in val):
                            res += "<div class='md-form'><label for=\"{id}\">{input_label_desc}</label><img src=\"{input_value}\" name=\"{id}\" /></div>".format(id=key, input_label_desc=key, className=input_className, input_value=val)
                        else:
