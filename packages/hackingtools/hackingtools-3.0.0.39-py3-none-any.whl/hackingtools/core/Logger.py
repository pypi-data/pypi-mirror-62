...
if not Utils.amIdjango(__file__):
    path_log = os.path.join(os.path.split(os.path.split(os.path.split(os.path.split(__file__)[0])[0])[0])[0], path_log)

logging.basicConfig(filename=path_log, filemode='a', format='%(asctime)s %(levelname)s - %(message)s', datefmt='%Y-%m-%d %H:%M:%S')
