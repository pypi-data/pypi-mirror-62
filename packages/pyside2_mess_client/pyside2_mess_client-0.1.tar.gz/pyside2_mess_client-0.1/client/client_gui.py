...
from PySide2.QtGui import QStandardItemModel, QStandardItem, QBrush, QColor, QIcon
from PySide2.QtWidgets import QMainWindow, qApp, QMessageBox
from PySide2.QtCore import Slot, Qt

from ui.client_gui_settings import AddContactDialog, DelContactDialog
from ui.client_UIs import Ui_MainClientWindow
...
class ClientMainWindow(QMainWindow):
    ...
    конвертированого файла client_UIs.py
    '''
    ...
    def set_disabled_input(self):
        ...
        """
        # Надпись  - получатель.
        self.ui.label_new_message.setText('Для выбора получателя дважды кликните на нем '
                                          'в окне контактов.')
    ...
    def add_contact_window(self):
        ...
        select_dialog.ui.buttonBox.accepted.connect(lambda: self.add_contact_action(select_dialog))
        select_dialog.show()

    # Функция - обработчик добавления, сообщает серверу, обновляет таблицу и список контактов
    def add_contact_action(self, item):
        """
        Функция - обработчик добавления, сообщает серверу, обновляет таблицу
        и список контактов
        :param item:
        :return:
        """
    ...
    def add_contact(self, new_contact):
        ...
        except UnknownServerError as err:
    ...
    def delete_contact_window(self):
        ...
        remove_dialog.ui.buttonBox.accepted.connect(lambda: self.delete_contact(remove_dialog))
        remove_dialog.show()
    ...
    def on_send(self):
        ...
        Забираем текст, очищаем поле ввода, делаем кнопку неактивной, отправляем сообщение
        на сервер
        """
        ...
        try:
            self.sock.send_mess(self.current_chat, message_text_encrypted_base64.decode('ansi'))
            pass
        except UnknownServerError as err:
    ...
    def message(self, message):
        ...
        encrypted_message = base64.b64decode(message[settings.MESSAGE_TEXT])
        ...
        sender = message[settings.SENDER]
        ...
        if sender == self.current_chat:
            ...
            if self.database.check_contact(sender):
                ...
                if self.messages.question(self, 'Новое сообщение',
                                          f'Получено новое сообщение от {sender}, открыть чат '
                                          f'с ним?', QMessageBox.Yes, QMessageBox.No) == \
                        QMessageBox.Yes:
    ...
    def sig_205(self):
        """
        Слот сообщения 205 - требование сервером обновить справочники доступных пользователей
        и контактов. Может получиться так, что удалён текущий собеседник, надо проверить это и
        закрыть чат с предупреждением, иначе просто обновить список контактов, без выдачи
    ...
    def make_connection(self, trans_obj):
        ...
        trans_obj.all_users_up.connect(self.sig_205)
