    ...
    for key, val in allTextures.items():
        ...
        if abspath[0]=='/' or abspath[0]=='\\':
           abspath = abspath[1:]
            ...
            abspath = abspath[3:]
        hashfile = os.path.join(os.path.join('.tmp/hash', apputil.platformName(platform)), abspath)
        #hashfile = os.path.normpath(key.replace(sourceDir, os.path.join('.tmp/hash', apputil.platformName(platform)), 1))
...
def deploy(projectFolder, userID, appName, appVersion, unit = 1.0, assetSource=None, assetDestination=None):
    ...
    convertImages(allTextures, assetSource, deployTmp+'unpackimage/ios/assets', core.TARGET_PLATFORM_IOS)
    convertImages(allTextures, assetSource, deployTmp+'unpackimage/android/assets', core.TARGET_PLATFORM_ANDROID)
