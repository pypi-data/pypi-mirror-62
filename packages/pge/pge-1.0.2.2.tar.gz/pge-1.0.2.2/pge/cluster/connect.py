...
def conductance(gr):
    ...
    for i in np.arange(1, gr.size()):
        for nodes in list(combinations(gr.get_ids(), i)):
            if not nx.is_connected(gr.subgraph(nodes).get_nx_graph()):
                continue
            res.append(phi(gr, nodes))
            res_n.append(nodes)

    ind = np.argmin(res)
    return res[ind], res_n[ind]
...
def diweak_conductance(gr, node, c):
    ...
    st = max([int(c), 2])
    end = min([gr.size()+1, st + int(gr.size()/2)])
    for i in np.arange(st, end):
        ...
        for nodes in list(combinations(gr.get_ids(), i)):
            ...
            subs = gr.subgraph(nodes).get_nx_graph()
            if not nx.is_connected(subs):
                continue
            if nx.diameter(subs) < c and i != 1:
                continue

            for j in np.arange(1, i):
                for nodes_ in list(combinations(nodes, j)):
                    if not nx.is_connected(gr.subgraph(nodes_).get_nx_graph()) and j != 1:
                        continue
                    ph = phi(gr, nodes_, [nd for nd in nodes if nd not in nodes_])
                    rs1.append(ph)
