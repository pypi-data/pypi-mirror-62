...
"""
copy from https://github.com/aliyun/django-oss-storage/
因为源代码pypi有bug。所以copy下来改着用了
"""
import six
import logging
import os
...
logger = logging.getLogger("oss_storage")
...
class OssStorage(Storage, ABC):
    ...
    def __init__(
        ...
        self.access_key_id = (
            access_key_id if access_key_id else _get_config("OSS_ACCESS_KEY_ID")
        )
        self.access_key_secret = (
            access_key_secret
            if access_key_secret
            else _get_config("OSS_ACCESS_KEY_SECRET")
        )
        self.end_point = _normalize_endpoint(
            end_point if end_point else _get_config("OSS_ENDPOINT")
        )
        self.bucket_name = (
            bucket_name if bucket_name else _get_config("OSS_BUCKET_NAME")
        )
        self.expire_time = (
            expire_time
            if expire_time
            else int(_get_config("OSS_EXPIRE_TIME", default=60 * 60 * 24 * 30))
        )
        ...
        self.bucket_acl = settings.BUCKET_ACL_TYPE
        # try:
    ...
    def _open(self, name, mode="rb"):
        ...
        try:
            ...
            logger.info(
                "content length: %d, requestid: %s", obj.content_length, obj.request_id
            )
