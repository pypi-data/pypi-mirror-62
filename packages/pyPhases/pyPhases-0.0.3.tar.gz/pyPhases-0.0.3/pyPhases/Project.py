...
class Project:
    ...
    def registerPublisher(self, publisher):
        self.decorators.append(publisher)
    ...
    def getDataId(self, dataName: str, version: str):
        return dataName + "-" + version

    def registerExporter(self, exporter):
        self.exporters.append([exporter])

    def getExporterForIntance(self, instance):
        return self.getExporterForType(type(instance))

    def getExporterForType(self, type):
        for exporter in self.exporters:
            if exporter.checkType(type):
                return exporter
        return None
    ...
    def registerData(self,
        ...
        if (self.dataStorage == None or save == False):
            return

        # save to storage layer
        exporter = self.getExporterForIntance(data)

        if (exporter == None):
            Warning("No exporter for datatype (" + type(data).__name__ +
