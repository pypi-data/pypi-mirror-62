...
class AbstractPropertyManager(ABC):
    ...
    @abstractmethod
    def __init__(self, task_name: str, root_keys: list, knowledge_keys: list):
        ...
        if isinstance(knowledge_keys, list) and len(knowledge_keys):
            ...
            self._knowledge_catalog = ['schema']
        root_keys += ['description', 'version', 'connectors', 'intent', 'snapshot', 'run_book',
    ...
    def persist_properties(self, only_branch: bool=None):
        ...
        """
        only_branch = only_branch if isinstance(only_branch, bool) else False
        if only_branch:
    ...
    def set_version(self, version: str):
        """ Sets the version of the contact and resets all the handlers versions if reset_connector is True """
    ...
    def remove_run_book(self, book_name: str):
    ...
    def get_intent(self, level: [int, str]=None, intent: str=None):
        """Returns the parameterised intent contract in full or if level give and exists, returns the level contract"""
        if isinstance(level, (int, str)):
            if self.is_key(self.join(self.KEY.intent_key, str(level))):
                if isinstance(intent, str):
                    if self.is_key(self.join(self.KEY.intent_key, str(level), intent)):
                        body = self.get(self.join(self.KEY.intent_key, str(level), intent), {})
                        return {intent: body}
                else:
                    return self.get(self.join(self.KEY.intent_key, str(level)))
            return {}
        if self.is_key(self.KEY.intent_key):
    ...
    def set_intent(self, intent_param: dict, level: [int, str]=None, replace_intent: bool=None):
        ...
        level = 0 if not isinstance(level, (int, str)) else level
        # remove any repeats of this exact intent
        self.remove_intent(intent_param)
        for label in intent_param.keys():
            if str(level) == "-1":
                if not self.get_intent():
                    self.set(self.join(self.KEY.intent_key, 0), intent_param)
                    return
                level = 0
                while True:
                    if not self.is_key(self.join(self.KEY.intent_key, level, label)):
                        self.set(self.join(self.KEY.intent_key, level), intent_param)
                        break
                    level += 1
            else:
                if self.is_key(self.join(self.KEY.intent_key, level, label)):
                    if replace_intent:
                        self.remove(self.join(self.KEY.intent_key, level, label))
                        self.set(self.join(self.KEY.intent_key, level), intent_param)
                else:
                    self.set(self.join(self.KEY.intent_key, level), intent_param)
        return
    ...
    def remove_intent(self, intent_param: [str, dict]=None, level: [int, str]=None):
        ...
        def _delete_intent(inst, _level: str, _intent: [str, dict]):
            ...
            for _key, _values in clean_dict.copy().items():
                key_values = inst.get(inst.join(inst.KEY.intent_key, _level, _key))
                if key_values is None:
                    return
                if len(_values) > 0 and str(key_values) != str(_values):
                    return
                inst.remove(inst.join(inst.KEY.intent_key, _level, _key))
                if not inst.get(inst.join(inst.KEY.intent_key, _level)):
                    inst.remove(inst.join(inst.KEY.intent_key, _level))
            return

        if str_level is not None and intent_param is not None:
        ...
        if str_level is not None:
            ...
            return
        if intent_param is not None:
            for key in sorted(self.get(self.KEY.intent_key).keys()):
                _delete_intent(inst=self, _level=key, _intent=intent_param)
            return
        return

    def report_intent(self, levels: [int, str]=None) -> dict:
        """ generates a report on all the intent contract

        :return: a dictionary with key as the header and list of values
        """
        report = {'level': [], 'intent': [], 'parameters': []}
        if isinstance(self.get_intent(), dict):
            if isinstance(levels, (int, str, list)):
                levels = self.list_formatter(levels)
            else:
                levels = sorted(self.get_intent().keys())
            for level in levels:
                for intent_param in sorted(self.get(self.join(self.KEY.intent_key, level))):
                    params = []
                    for key, value in self.get(self.join(self.KEY.intent_key, level, intent_param)).items():
                        params.append("{}={}".format(key, value))
                    report['level'].append(level)
    ...
    def set_snapshot(self, suffix: str=None) -> str:
        ...
        if isinstance(snap_dict, dict) and snap_dict.get('snapshot') is not None:
            snap_dict.pop('snapshot')
    ...
    def get_connector_contract(self, connector_name: str) -> ConnectorContract:
        ...
        kwargs = self.get(self.join(self.KEY.connectors_key, connector_name, 'kwargs'))
        if not isinstance(kwargs, dict):
            kwargs = {}
        _connector_key = self.join(self.KEY.connectors_key, connector_name)
        return ConnectorContract(uri=self.get(self.join(_connector_key, 'uri')),
                                 module_name=self.get(self.join(_connector_key, 'module_name')),
                                 handler=self.get(self.join(_connector_key, 'handler')),
                                 version=self.get(self.join(_connector_key, 'version')),
                                 **kwargs)
    ...
    def set_connector_contract(self, connector_name: str, connector_contract: ConnectorContract, aligned: bool=None):
        ...
        self._create_abstract_properties()
        self.set(self.join(self.KEY.connectors_key, connector_name, 'uri'), connector_contract.uri_raw)
        self.set(self.join(self.KEY.connectors_key, connector_name, 'module_name'), connector_contract.module_name)
        self.set(self.join(self.KEY.connectors_key, connector_name, 'handler'), connector_contract.handler)
        self.set(self.join(self.KEY.connectors_key, connector_name, 'version'), connector_contract.version)
        self.set(self.join(self.KEY.connectors_key, connector_name, 'modified'), 0)
        self.set(self.join(self.KEY.connectors_key, connector_name, 'kwargs'), connector_contract.kwargs)
    ...
    def set_connector_version(self, connector_name: str, version: str) -> bool:
        ...
        if self.has_connector(connector_name):
            ...
            self.set(self.join(self.KEY.connectors_key, connector_name, 'version'), version)
            return True
        return False
    ...
    def modify_connector_uri(self, connector_name: str, old_pattern: str, new_pattern: str) -> bool:
        ...
        if self.has_connector(connector_name=connector_name):
            self.remove_connector_handler(connector_name=connector_name)
            old_uri = self.get(self.join(self.KEY.connectors_key, connector_name, 'uri'))
            new_uri = old_uri.replace(old_pattern, new_pattern, 1)
            self.set(self.join(self.KEY.connectors_key, connector_name, 'uri'), new_uri)
            return True
        return False
    ...
    def modify_connector_aligned(self, connector_name: str, template_contract: ConnectorContract) -> bool:
        ...
        if self.get(self.join(self.KEY.connectors_key, connector_name, 'aligned'), False):
            self.remove_connector_handler(connector_name=connector_name)
            uri = self.get(self.join(self.KEY.connectors_key, connector_name, 'uri'), None)
            _, file = os.path.split(uri)
            uri = os.path.join(template_contract.uri, file)
            self.set(self.join(self.KEY.connectors_key, connector_name, 'uri'), uri)
            self.set(self.join(self.KEY.connectors_key, connector_name, 'module_name'), template_contract.module_name)
            self.set(self.join(self.KEY.connectors_key, connector_name, 'handler'), template_contract.handler)
            self.set(self.join(self.KEY.connectors_key, connector_name, 'version'), template_contract.version)
            self.set(self.join(self.KEY.connectors_key, connector_name, 'kwargs'), template_contract.kwargs)
            return True
        return False
    ...
    def report_connectors(self, connector_filter: [str, list]=None) -> dict:
        ...
        for name_key in self.get(self.KEY.connectors_key).keys():
            ...
            if isinstance(connector_contract, ConnectorContract):
                ...
                rtn_dict['uri'].append(connector_contract.address_raw)
                rtn_dict['module_name'].append(connector_contract.module_name)
                rtn_dict['handler'].append(connector_contract.handler)
    ...
    def has_knowledge(self, catalog: [str, list]=None) -> bool:
        """Test if the contract has a knowledge catalog"""
        if isinstance(catalog, (list, str)):
            catalog = self.list_formatter(catalog)
        else:
            catalog = self._knowledge_catalog
        for key in catalog:
            section = self.get(eval("self.KEY.{}.{}_key".format(self._KNOWLEDGE_ROOT, key)))
            if isinstance(section, dict) and len(section) > 0:
                return True
        return False

    def set_knowledge(self, catalog: str, label: [str, list], text: str, constraints: list=None):
        ...
        if catalog not in self.get(self.KEY.knowledge_key).keys():
            raise ValueError("The catalog name '{}' is not recognised for this component."
                             "Please select a catalog from '{}'".format(catalog, self.knowledge_catalog))
    ...
    def bulk_upload_knowledge(self, data: dict, catalog: str, constraints: list=None):
