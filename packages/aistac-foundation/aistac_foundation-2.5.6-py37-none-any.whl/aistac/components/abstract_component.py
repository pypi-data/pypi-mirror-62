...
class AbstractComponent(ABC):
    ...
    def _add_templates(cls, property_manager: AbstractPropertyManager, save: bool=None, source_path: str=None,
        ...
        source_module = source_module if isinstance(source_module, str) else 'aistac.handlers.python_handlers'
        source_handler = source_handler if isinstance(source_handler, str) else 'PythonSourceHandler'
        connector_contract = ConnectorContract(uri=source_path, module_name=source_module, handler=source_handler)
        ...
        persist_module = persist_module if isinstance(persist_module, str) else 'aistac.handlers.python_handlers'
        persist_handler = persist_handler if isinstance(persist_handler, str) else 'PythonPersistHandler'
        connector_contract = ConnectorContract(uri=persist_path, module_name=persist_module, handler=persist_handler)
