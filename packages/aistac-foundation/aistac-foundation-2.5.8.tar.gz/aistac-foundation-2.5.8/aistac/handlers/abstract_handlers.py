...
class HandlerFactory(object):
    ...
    def instantiate(connector_contract: ConnectorContract) -> [AbstractSourceHandler, AbstractPersistHandler]:
        ...
        if handler not in dir(module):
            raise ImportError("The handler '{}' could not be found in the module '{}'".format(handler, module_name))
        ...
        instance = eval('module.{}(connector_contract)'.format(handler), globals(), local_kwargs)
        if not isinstance(instance, AbstractSourceHandler):
            raise TypeError("The handler '{}' is not a subset of the AbstractHandler Class".format(handler))
        return instance
