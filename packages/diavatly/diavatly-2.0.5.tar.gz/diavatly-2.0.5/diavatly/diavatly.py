...
def model_map(polygons=None,
    ...
    if res.any():
        if log:
            ...
            if not levels.any():
                itv = 10 ** np.linspace(min(zb), max(zb), 12)  # Creates a linear space between the two extremes values
        ...
        if not levels.any():
            ...
            boundaries = np.copy(itv) # To define ticks
        ...
        ticks = [round(v, 2) for v in boundaries]
