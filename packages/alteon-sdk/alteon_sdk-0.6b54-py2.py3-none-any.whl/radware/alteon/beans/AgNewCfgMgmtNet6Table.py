...
class EnumAgMgmtNet6Protocol(BaseBeanEnum):
    ssh = 1
    telnet = 2
    sshTelnet = 3
    http = 4
...
class EnumAgMgmtNet6Delete(BaseBeanEnum):
    other = 1
    delete = 2


class AgNewCfgMgmtNet6Table(DeviceBean):
    def __init__(self, **kwargs):
        self.Index = kwargs.get('Index', None)
        self.Subnet = kwargs.get('Subnet', None)
        self.Prefix = kwargs.get('Prefix', None)
        self.Protocol = EnumAgMgmtNet6Protocol.enum(kwargs.get('Protocol', None))
        self.Delete = EnumAgMgmtNet6Delete.enum(kwargs.get('Delete', None))
