...
class EnumSlbVirtServiceUdpAge(BaseBeanEnum):
    enabled = 1
    disabled = 2


class EnumSlbVirtServiceSessEntryMode(BaseBeanEnum):
    single = 1
    multiple = 2


class EnumSlbVirtServiceAlwaysOn(BaseBeanEnum):
    enabled = 1
    disabled = 2


class EnumSlbVirtServiceSendRST(BaseBeanEnum):
    reset = 1
    drop = 2


class EnumSlbVirtServiceClsOnSlowage(BaseBeanEnum):
    none = 1
    client = 2
    server = 3
    both = 4
...
class SlbNewCfgEnhVirtServicesFifthPartTable(DeviceBean):
    def __init__(self, **kwargs):
        ...
        self.UdpAge = EnumSlbVirtServiceUdpAge.enum(kwargs.get('UdpAge', None))
        self.SessEntryMode = EnumSlbVirtServiceSessEntryMode.enum(kwargs.get('SessEntryMode', None))
        self.SecPol = kwargs.get('SecPol', None)
        self.AlwaysOn = EnumSlbVirtServiceAlwaysOn.enum(kwargs.get('AlwaysOn', None))
        self.SendRST = EnumSlbVirtServiceSendRST.enum(kwargs.get('SendRST', None))
        self.ClsOnSlowage = EnumSlbVirtServiceClsOnSlowage.enum(kwargs.get('ClsOnSlowage', None))
