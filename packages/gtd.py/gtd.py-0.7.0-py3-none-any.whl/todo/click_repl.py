...
def bootstrap_prompt(prompt_kwargs, group):
    ...
    defaults = {'history': InMemoryHistory(), 'completer': ClickCompleter(group), 'message': u'> '}
...
def repl(old_ctx, prompt_kwargs=None, allow_internal_commands=True):
    """
    Cannibalized from click-repl. I love the library and use it for other projects, but for
    this use-case it's not the best fit:
    - Executing system commands with gtd is very far out of scope
    - The internal commands are not customizable, which they need to be for the experience of using
      the gtd repl to feel the same as using the command-line interface.
