...
class AutoMLBaseSettings:
    ...
    def __init__(self,
        ...
        if isinstance(featurization, FeaturizationConfig):
            ...
            self.featurization = featurization.lower()
        ...
        self.preprocess = False if featurization == FeaturizationConfigMode.Off else True
    ...
    def _verify_settings(self, logger=None):
        ...
        if self.is_timeseries:
            if self.featurization != FeaturizationConfigMode.Off:
                logger.warning('Timeseries use its own featurization process, ignoring featurization parameter.')
            if self.task_type == constants.Tasks.CLASSIFICATION:
                raise ValueError('Timeseries do not support classification yet.'
                                 'Received \"{0}\".'.format(self.task_type))
        ...
        if self.is_timeseries:
            try:
                from automl.client.core.common import import_utilities
    ...
    def _filter(self, black_list_keys: Optional[List[str]]) -> Dict[str, Any]:
        dict_copy = dict([(k, v) for k, v in self.__dict__.items()
                          if black_list_keys is None or k not in black_list_keys])

        # instead of moving featurization flag entirely, mark it auto/off/customized
        featurization = self._get_featurization_config_mode()
        if featurization:
            dict_copy['featurization'] = featurization
