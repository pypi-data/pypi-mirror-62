...
class Imager(UVW):
    ...
    def plot_dirty(self, output='plot', **kwargs):
        """
        """
        astroplt = AstroPlot(
    ...
    def clean(self, maxiter=200, gain=0.1, output='plot', **kwargs):
        ...
        self.make_psf(npix=self.npix*2, fi=fi)
    ...
    def make_nearfield(self, radius=300, npix=100):
        """
            :param radius:
            ...
            :type npix: int, optional
        """
        #antpos = self.array.enu # <================ MAKE IT WORK !!!
        #ant_pos = t.getcol('POSITION')
        #ant_loc = EarthLocation.from_geocentric(ant_pos[:, 0], ant_pos[:, 1], ant_pos[:, 2], unit=u.m)
        #enu_loc = pyuvdata.utils.ENU_from_ECEF(ant_pos.T, nenufarloc.lat.rad, nenufarloc.lon.rad, 142).T
        antpos = np.loadtxt('/Users/aloh/Desktop/ant_pos_enu.csv', delimiter=',', usecols=(1, 2, 3))
        # make it work if one Mini-Array is missing !!!
        antpos = antpos[vis.array.ant1[0, :], :]
