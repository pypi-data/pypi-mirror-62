...
def main(args=None):
    ...
    utils.write_command(os.path.join("cmdlogs/","command_predict_" + timestr+".txt"), "cryolo_predict.py " + ' '.join(sys.argv[1:]))
...
def picking_postprocessing(picking_results, export_size, min_distance):
    ...
    for picking_result_micrograph in picking_results:
        ...
        for box in picking_result_micrograph["boxes"]:
            box.meta["boxsize_estimated"] = (int(box.w*image_width), int(box.h*image_height))
