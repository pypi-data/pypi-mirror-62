...
class RuntimeClient(threading.Thread):
    ...
    def __init__(self, send_delay = DEFAULT_SEND_DELAY):
        ...
        self.stats_detailed_que = queue.Queue()
        self.stats_detailed_customer_que = queue.Queue()
    ...
    def run(self):
        ...
        while self.should_work:
            try:
                ...
                self._check_and_publish(self.stats_detailed_que, self._publish_stats_detailed_entries)
                self._check_and_publish(self.stats_detailed_customer_que, self._publish_stats_detailed_customer_entries)
                last_sent = time.time()
    ...
    def _publish_stats_detailed_entries(self , entry):
        try:
            self.channel.basic_publish(
                exchange=MQ_RUNTIME_EXCHANGE, routing_key=MQ_RUNTIME_STATS_DETAILED_ADD,
                body=entry)
        except Exception as e:
            LOG.error(e, exc_info=True)
    ...
    def _publish_stats_detailed_customer_entries(self , entry):
        try:
            self.channel.basic_publish(
                exchange=MQ_RUNTIME_EXCHANGE, routing_key=MQ_RUNTIME_STATS_DETAILED_CUSTOMER_ADD,
                body=entry)
        except Exception as e:
            LOG.error(e, exc_info=True)
    ...
    def add_stats_detailed(self, worker_type, worker_id, customer_id, job_id, stats_name, stats_values, job_name = None):
        self.stats_detailed_que.put_nowait(
            create_stats_job_entry(worker_type, worker_id, customer_id, job_id, stats_name, stats_values, job_name = job_name)
        )

    def add_stats_detailed_customer(self, worker_type, worker_id, customer_id, stats_name, stats_values):
        self.stats_detailed_customer_que.put_nowait(
            create_stats_customer_entry(worker_type, worker_id, customer_id, stats_name, stats_values)
        )