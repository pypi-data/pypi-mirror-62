...
from trains_agent.definitions import (
    ...
    INVALID_WORKER_ID,
    PROGRAM_NAME,
    DEFAULT_VENV_UPDATE_URL, ENV_TASK_EXECUTE_AS_USER, ENV_K8S_HOST_MOUNT)
...
from trains_agent.helper.base import (
    ...
    ExecutionInfo,
    HOCONEncoder, error, get_python_path, is_linux_platform, rm_file)
...
class Worker(ServiceCommandSection):
    ...
    def _get_docker_cmd(worker_id, docker_image, docker_arguments,
        ...
        if os.environ.get(ENV_K8S_HOST_MOUNT):
            ...
            # Notice! currently we have to disable host_apt_cache, due to lock issues
            host_apt_cache = None
        ...
        if not standalone_mode:
            # Disable apt cache lock: make sure we do not lock the apt-get
            update_scheme += "echo 'APT::Periodic::Update-Package-Lists \"0\";' >> /etc/apt/apt.conf.d/20auto-upgrades ; "
