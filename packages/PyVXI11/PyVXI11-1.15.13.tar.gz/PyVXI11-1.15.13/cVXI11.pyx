...
# distutils: sources = VXI11_clnt.c VXI11_xdr.c VXI11_svc.c createAbtChannel.c cPMAP.cpp
...
revision: $Revision: 9191f6782927 $ $Date: 2020/02/28 00:48:54 $
HGDate: $HGdate: Fri, 28 Feb 2020 09:48:54 +0900 $
HGTag: "$HGTag: 1.15.13-9191f6782927 $"
"""
...
cdef class Device_RemoteFunc:
    ...
    def __cinit__(self,hostAddr, hostPort, progNum, progVers, progFamily):
...
cdef class Device_WriteParms:
    ...
    property data_len:
        def __get__(self): 
            return self.thisptr.data.data_len
...
cdef class Device_EnableSrqParms:
    ...
    property handle:
        def __get__(self): 
            return self.thisptr.handle.handle_val
...
cdef class Device_DocmdParms:
    ...
    property data_in:
        def __get__(self):
            return self.thisptr.data_in.data_in_val
    ...
    property data_in_len:
        def __get__(self): 
            return self.thisptr.data_in.data_in_len
...
cdef class Vxi11Device:
    ...
    property host: # strange behaviour. need debug. use hostName, instead.
        def __get__(self):
            return self.host
    ...
    def __init__(self, char * host, char *device=b"gpib0,0",
        ...
        if ( device_has_async_port(host)):
            ...
            vxi11logger.info( "No IRQ channel was found. Ignore the message above.")
            if (res.abortPort != 0):
    ...
    def createSVCThread(self):
        ...
        def run_svc(): # we can define this function elsewhere.
            with nogil:# we must rlease Python GIL here. Otherwise main thread cannot getback GIL again.
                svc_run() # provided by rpc library.
            return
        ...
        vxi11logger.info("start server @ {info}".format(info=self.svc_thread))
    ...
    def create_intr_chan(self, host=None, intr_port=None, proto=DEVICE_TCP):
        ...
        vxi11logger.info("intr_host/port: {host} @ {port}".format(host=self.intr_host, port=self.intr_port))
    ...
    def __del__(self):
        ...
        if (self.clnt != NULL) and (self.svc_thread != None):
            with nogil:
                res=destroy_intr_chan_1(NULL, self.clnt)
            vxi11logger.info("intr_chan was destroyed")
            self.destroy_link()
    ...
    def lock(self,flags=0,lock_timeout=0):
        parm=Device_LockParms(slef.lid, flags, lock_timeout)
        # parm.thisptr.lid=self.lid
...
cdef public void *device_intr_srq_1_svc(c_Device_SrqParms *argp, c_svc_req *rqstp) nogil:
    ...
    with gil:
        ...
        try:
            ...
            if lock.locked():
                lock.release()
                #vxi11logger.info( "SRQ received:") #,int(rqstp.rq_prog))
        except:
            raise
            #return result
    return result 
...
def test_dso():
    #dev=Vxi11Device(b"169.254.254.254",device=b"gpib0,7")
    dev=Vxi11Device(b"169.254.254.254",device=b"inst0")
    return dev
...
class IEEE488_2Device(Vxi11Device):
    def __init__(self, char * host, char *device=b"inst0",
                 char *proto=b"tcp",lock_timeout=0,lockDevice=0):
        Vxi11Device.__init__(self, host, device,
...
def device_has_async_port(host):
    ...
    #             return False
    # return False
    if (pmap_getport(host, device_async.prog, device_async.version) == 0):
        return False
    else:
        return True
...
def pmap_getport(host, program, version=1, protocol=IPPROTO_TCP):
    return cPMAP_getport(host, program, version, protocol)

cdef _pmap_getmaps(host):
   """ 
