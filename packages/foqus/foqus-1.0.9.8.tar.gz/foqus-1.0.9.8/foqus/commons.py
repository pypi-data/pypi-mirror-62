...
def download_or_remove(urls, each_url_product, download_operation=1, customer_name=None, customer_type=None,
        ...
        if download_operation == 1:
            ...
            if USE_AZURE:
                if fichier.split('/')[-2] == project_name:
                    upload_folder_into_azure(local_path=path_out, directory_path_azure=input_azure)

                else:
                    upload_folder_into_azure(local_path=path_out,
                                             directory_path_azure=input_azure + fichier.split('/')[-2])
        else:
    ...
    data_product = db.get_product_details(replace_special_caractere(customer_name).replace(' ', '_'), each_url_product,
                                          project_name)
...
def download_commit_db_images(url_images, url_produits):
    if url_images:
        for image in url_images:
            try:
                download_or_remove(image.get('images', ''), "", image.get('status'), image.get("customer_name"),
        ...
        for product in url_produits.keys():
            try:
                download_or_remove(url_produits.get(product, {}).get('images', ''), product,
                                   url_produits.get(product, {}).get('status'),
