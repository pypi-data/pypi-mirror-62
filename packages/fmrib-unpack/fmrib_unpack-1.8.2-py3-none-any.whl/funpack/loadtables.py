...
def loadVariableTable(datafiles,
    ...
    :arg indexes:     Dict of ``{filename : [index]}`` mappings, specifying
                      the number of the column to use as the index. Defaults
    ...
    # with the index column(s)
    # from each file dropped
    cols = list(finfo[2])
    for i, df in enumerate(datafiles):
        dfcols  = cols[i]
        dfidx   = indexes.get(df, [0])
        cols[i] = [c for c in dfcols if c.index not in dfidx]
