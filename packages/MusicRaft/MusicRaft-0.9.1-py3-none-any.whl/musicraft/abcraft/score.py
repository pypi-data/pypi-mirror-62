...
from .. import (Shared, dbg_print, QtCore, QtGui, QtWidgets, WebView, WithMenu)
...
class Score(QtWidgets.QGraphicsView, WithMenu):
    ...
    def __init__(self):
        ...
        dbg_print('**** Score', self.height(), self.width())
        ...
        self.svgView.setFixedHeight(1024)
        self.svgView.setFixedWidth(1024)
        dbg_print('*** svgView', self.svgView.height(), self.svgView.width())
        ...
        self.proxyWidget.setMinimumHeight(480)
        self.proxyWidget.setMinimumWidth(640)
        self.scene().addItem(self.proxyWidget)
        #dbg_print('**** Proxy', self.proxyWidget.height(), self.proxyWidget.width())
        self.scene().update()
        dbg_print('**** Scene', self.scene().height(), self.scene().width())
        dbg_print("!Score.__init__")
    ...
    def showWhichPage(self, which=0, force=False):
        ...
        self.which = which
        self.svgView.load(QtCore.QUrl('file:///' + svg_file.fileName()))
