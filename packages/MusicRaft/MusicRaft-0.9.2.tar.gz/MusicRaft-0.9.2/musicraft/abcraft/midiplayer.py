...
class MidiPlayer(QtWidgets.QWidget):
    ...
    def add_midi_menu(self):
        self.midi_menu = QtWidgets.QMenu("&Midi")

        self.select_midi_output_action = Shared.raft.myQAction("Select Midi &Output",
                                                               triggered=self.select_midi_output_)
        self.midi_menu.addAction(self.select_midi_output_action)


        self.start_midi_action = Shared.raft.myQAction("Start &Midi", shortcut="Ctrl+M",
                                                       triggered=self.start_midi)
        self.midi_menu.addAction(self.start_midi_action)


        self.pause_midi_action = Shared.raft.myQAction("Pause M&idi", shortcut="Ctrl+,",
                                                       triggered=self.pause_midi)
        self.midi_menu.addAction(self.pause_midi_action)

        Shared.raft.menuBar().addMenu(self.midi_menu)
    ...
    def open_output(self, name=None, open_not_close=True):
        ...
        dbg_print('MidiPlayer.open_output succeeded, name', self.outputPort)
