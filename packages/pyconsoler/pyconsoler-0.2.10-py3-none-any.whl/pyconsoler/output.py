...
def bordered_text(*text: str, hor_spacing=2,centered=True) -> Tuple[str]:
    ...
    text = [str(txt) for txt in text]

    border_width = max((len(txt) + 2 * hor_spacing for txt in text))

    _itms.append(f"┏{border_width * '━'}┓")
    if centered:
        for txt in text:
            _itms.append(f"┃{txt.center(border_width)}┃")
    else:
        for txt in text:
            _itms.append(f"┃ {txt.ljust(border_width-1)}┃")
