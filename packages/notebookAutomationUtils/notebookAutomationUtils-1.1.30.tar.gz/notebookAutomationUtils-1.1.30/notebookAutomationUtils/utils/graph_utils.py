...
class GraphUtils:
    ...
    def visualize_using_boxplot(self, obj_: object, numcols: int,release_criteria: int, ylimit=None):
      ...
      if (numcols == 1): 
        ...
        sns.boxplot(ax=ax1, x="entitytype", y="EPT", data=obj_[obj_['appVersion']==self.last_release], order=objects_to_colors.keys(), palette=objects_to_colors.values())
        sns.boxplot(ax=ax2, x="entitytype", y="EPT", data=obj_[obj_['appVersion']==self.this_release], order=objects_to_colors.keys(), palette=objects_to_colors.values())
        plt.ylim(0,None)
