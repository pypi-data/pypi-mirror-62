...
ROUTES_URL = (
    ...
    "bruxelles/527/20200208/original/routes.txt"  # pragma: no mutate
)  # pragma: no mutate
...
class ODStibMivb:
    ...
    async def get_gtfs_line_data(self, id_):
        ...
        if self._gtfs_line_data is None:
            self._gtfs_line_data = await self.get_gtfs_data()
        try:
            return self._gtfs_line_data[str(id_)]
        except KeyError:
            raise ValueError("unknown line id")

    async def get_gtfs_data(self):
        """
        ...
        try:
            if self._session is None:
                ...
                data = await self.get_gtfs_response(self._session)
            return extract_gtfs_data(data.decode())
        # pylint: disable=W0703
        except HttpException:
            return extract_gtfs_data(CSV_FILE)

    async def get_gtfs_response(self, session):
        """
...
def extract_gtfs_data(data):
    """extract the gtfs csv file and put it in a dict"""
    gtfs_line_data = {}
