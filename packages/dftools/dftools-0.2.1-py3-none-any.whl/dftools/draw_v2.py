...
if __name__ == "__main__":
    ...
    fig, ax = plt.subplots()
    df.plot("errorbar", "x", "y", yerr="yerr", ax=ax)
    fig.savefig("test.pdf", bbox_inches='tight')

#def errorbar(df, xlabel, ylabel, xerr=None, yerr=None, ax=None, **kwargs):
#    """
#    Create a matplotlib error bar
#    """
#    # labels can be an index
#    df_no_index = df.reset_index()
#
#    errorbar_kw = {}
#    if xerr is not None:
#        if not isinstance(xerr, str) and isinstance(var, collections.Iterable):
#            errorbar_kw["xerr"] = (df[xerr[0]], df[xerr[1]])
#        else:
#            errorbar_kw["xerr"] = df[xerr]
#    if yerr is not None:
#        if not isinstance(yerr, str) and isinstance(var, collections.Iterable):
#            errorbar_kw["yerr"] = (df[yerr[0]], df[yerr[1]])
#        else:
#            errorbar_kw["yerr"] = df[yerr]
#    errorbar_kw.update(kwargs)
#
#    ax.errorbar(df[xlabel], df[ylabel], **errorbar_kw)
#
#def poisson_errobar(df, xlabel, ylabel, ax=None, **kwargs):
#    pass
#
#def legend_data_mc(
