...
###############################################################################################################
    ...
    def closest_downstream(posa): #Determine which guide RNA is closest downstream of the 3' base coding for the amino acid.
        ...
        for x in gRNA.iloc[:, 1]:
            ...
            if diff > 0 and diff < small: #iterate through the list of guide RNAs and their positions to determine the cut site resistance that is the smallest distance away from the 3' base of the amino acid.
                small = diff
                position = posg
...
################################ MODIFY IF ADDITTIONAL PAMS ARE TO BE INCLUDED ################################
    ...
    #Generate a new column containing the PAM adjacent to the guide RNA
    def pamcolumn(entry):
...
###############################################################################################################
    ...
    guides["5' gRNA Base Position"] = guides["5' Base"].apply(closest_upstream) #Determine the closest guide RNA cut site to the 5' base of the amino acid codon
    
    guides["3' gRNA Base Position"] = guides["3' Base"].apply(closest_downstream) #Determine the closest guide RNA cut site to the3' base of the amino acid codon
    ...
    # Get names of indexes for which column gRNA sequence has value 0
    indexNamesfive = guides[ guides["5' gRNA Sequence"] == 0]. index
    indexNamesthree = guides[ guides["3' gRNA Sequence"] == 0].index

    # Delete these row indexes from dataFrame
    guides.drop(indexNamesfive , inplace=True) #Remove rows where the gRNA sequence is 0
    guides.drop(indexNamesthree , inplace=True) #Remove rows where the gRNA sequence is 0
    ...
    guides["Distance of 5' Cut Site from Amino Acid (bp)"] = - guides["Distance of 5' Cut Site from Amino Acid (bp)"] #Inverse sign of the distance between the amino acid and cut site
