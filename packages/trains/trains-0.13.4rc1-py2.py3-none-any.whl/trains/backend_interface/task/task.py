...
from ...storage.helper import StorageError, remote_driver_schemes
...
class Task(IdObjectBase, AccessMixin, SetupUploadMixin):
    ...
    def _get_output_destination_suffix(self, extra_path=None):
        ...
        if output_dest and output_dest.lower().startswith((remote_driver_schemes)):
            safe_string = "'[]{}()$^,.; -_+-=/"
        else:  # If we are saving the model / artifacts in local path, you can not have '/' in the task name
            safe_string = "'[]{}()$^,.; -_+-="
        return '/'.join(quote(x, safe=safe_string) for x in
