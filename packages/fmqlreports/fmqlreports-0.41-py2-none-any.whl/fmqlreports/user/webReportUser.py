...
from ..webReportUtils import ensureWebReportLocations, TOP_MD_TEMPL, SITE_DIR_TEMPL, keyStats, flattenFrequencyDistribution, roundFloat, reduce200
...
def webReportUser(stationNo, deidentify=False):
    ...
    BLURB_TEMPL = """Of the <span class='yellowIt'>{:,}</span> users known to {}, a minority <span class='yellowIt'>{}</span> ("__Active Users__") signed in between {} and {}, the most recent year for which data is available.
...
There are <span class='yellowIt'>{}</span> active Local Users with <span class='yellowIt'>{}</span> signons.\n\n""".format(reportAbsAndPercent(len(activeLocalUserRefs), totalActiveUsers), reportAbsAndPercent(totalLocalSignOns, totalSignOns))
    ...
    for loa in sorted(comboST["level_of_assurance"]["byValueCount"], key=lambda x: comboST["level_of_assurance"]["byValueCount"][x], reverse=True):
        tbl.addRow([
            "__{}__".format(loa), 
            reportAbsAndPercent(comboST["level_of_assurance"]["byValueCount"][loa], comboST["_total"])
        ])
    mu += tbl.md() + "\n\n"
