...
""".format(stationNo) 
    ...
    usedFactors = set(re.sub(r'\_', '/', subTypeInfo["health_factor"]["byValueCount"].keys()[0].split(" [")[0]) + " [" + subTypeInfo["health_factor"]["byValueCount"].keys()[0].split(" [")[1] for subTypeInfo in st9000010_23s)
    subTypeInfoById = dict((re.sub(r'\_', '/', subTypeInfo["health_factor"]["byValueCount"].keys()[0].split(" [")[0]) + " [" + subTypeInfo["health_factor"]["byValueCount"].keys()[0].split(" [")[1], subTypeInfo) for subTypeInfo in st9000010_23s)
    ...
    maxFactorLabel = sorted(usedFactorCounts.keys(), key=lambda x: usedFactorCounts[x], reverse=True)[0].split(" [")[0]
    unassignedActiveFactors = factors - set(usedFactorCounts.keys())
    unassignedActiveCategories = categories - set(usedCategoryCounts.keys())
