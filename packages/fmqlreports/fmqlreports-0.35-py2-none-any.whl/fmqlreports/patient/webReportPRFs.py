...
TODO v1.1:
- as doing: introduce singleValue, singleValueCount on prop which enforces singletons ie/ reduces need for .keys()
...
def webReportPRFs(stationNo):
    ...
    activeFlagUse = dict((st26_13["flag_name"]["byValueCount"].keys()[0], st26_13["_total"]) for st26_13 in stActive26_13s) 
...
def webReportFlagTypes(stationNo, activeFlagUse):
    ...
    for resource in resourceIter:
        ...
        if len(set(resource.keys()) - (set(["_id", "type", "label", "status"]).union(set(props)))):
