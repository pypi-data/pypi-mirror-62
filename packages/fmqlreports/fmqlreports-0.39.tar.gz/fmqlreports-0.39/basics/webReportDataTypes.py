...
def muIntroduction(stationNo, typeInfoById):
    ...
    meta = metaOfVistA(stationNo, forceNew=True)
    name = "VistA" if "name" not in meta else meta["name"]
    mu = "{} [{}] ".format(name, stationNo)
    if "cutDate" in meta:
        mu += "was cut on _{}_ and ".format(meta["cutDate"])
