...
def webReportYR3DICOM(stationNo):
    ...
    print "FIRST CHECK: overlap bold alg wrong - had to manually fix in PDF + "\n" spaces and things run in as does TITLE[sop class oid]
    print x
    ...
    for subTypeInfo in subTypeInfosByOT["DICOM IMAGE"]:
        ...
        if "dicom_sop_class" in subTypeInfo:
            sopClassRef = subTypeInfo["dicom_sop_class"]["byValueCount"].keys()[0] # only one
        ...
        if "spec_subspec_index" not in subTypeInfo:
            ...
            specSubSpecIndexLabel = subTypeInfo["spec_subspec_index"]["byValueCount"].keys()[0].split(" [")[0]
        ...
        if specSubSpecIndexLabel in ["RADIOLOGY", "NUCLEAR MEDICINE"]:
            ...
            if not (
                ...
                re.match(r'RADIOLOGY', subTypeInfo["parent_data_file"]["byValueCount"].keys()[0]) and
                ...
                subTypeInfo["capture_application"]["byValueCount"].keys()[0] == "D:DICOM Gateway" and
                ...
                set(subTypeInfo["status"]["byValueCount"].keys()) == set(["1:Viewable"])
...
""".format(
    ...
    """
    if set(countBySpecIndexBySOPClassNRNCA.keys()) == set(["1.2.840.10008.5.1.4.1.1.77.1.4"]):
