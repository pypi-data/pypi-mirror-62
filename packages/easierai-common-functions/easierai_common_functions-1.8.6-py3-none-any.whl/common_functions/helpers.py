...
def compose_model_params(is_classifier):
    ...
    if is_classifier:
        ...
        model_params['num_classes'] = config['INFERENCE']['num_classes']
    else:
...
def save_model(eslib, id, metadata, dict={}, inference_type=constants.ESTIMATOR, _id=None, save_tflite=False, calibration_data=None, save_tpu=False):
    ...
    try:
        ...
        if save_tflite and calibration_data is not None:
            ...
            if success:
                uploaded = upload_file_to_minio(config['ELASTIC']['index_models'], filename, constants.TF_LITE_EXTENSION, id, config['ML']['algorithm'])
                if not uploaded:
        ...
        if save_tpu and calibration_data is not None:
            ...
            if success:
                uploaded = upload_file_to_minio(config['ELASTIC']['index_models'], filename + "_edgetpu", constants.TF_LITE_EXTENSION, id, config['ML']['algorithm'])
                if not uploaded:
