...
def add(ctx, application, env_name, org_id=None,
        config_type=None,
        mount_src_path=None,
        username=None,
        password=None,
        ...
        domain=None,
        hostname=None,
        file_store_uri=None, filename=None, **kwargs):
    ...
    app = apps.get_app(ctx, org_id, application,
                       maintained=True)
    ...
    return apiclient.application_api.env_config_post(
        app['id'], env_name, _config)


def update(ctx, application, env_name, id, org_id=None,
           config_type=None, mount_path=None, file_store_uri=None,
           **kwargs):
    token = context.get_token(ctx)
    if not token:
        access_token = access.get_access_token(ctx)
    ...
    return apiclient.application_api.env_config_put(
        app['id'], env_name, id, _update)
...
def delete(ctx, application, env_name, id, org_id=None, **kwargs):
    ...
    return apiclient.application_api.env_config_delete(
        app['id'], env_name, id, org_id)
...
def query(ctx, application, env_name, org_id=None, **kwargs):
    ...
    apiclient = context.get_apiclient(ctx, token)
    return apiclient.application_api.env_config_get_all(
        app['id'], env_name, org_id)


class EnvVarConfigObj():
    def __init__(self, ctx, application, env_name,
                 org_id=None, secret=False):
        self.ctx = ctx
    ...
    def write_env_list(self, data):
        ...
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            if self.env_config:
                ...
                delete(self.ctx, self.application, self.env_name,
                       self.env_config.id, self.org_id)
            self.env_config = add(
                self.ctx, self.application, self.env_name,
                org_id=self.org_id,
