...
def update(ctx, org_id, auto_create=None, issuer=None, contact_id=None,
           subdomain=None, external_id=None):
    token = context.get_token(ctx)
    if not token:
        access_token = access.get_access_token(ctx)
        token = access_token.get()
...
def add(ctx, organisation, issuer, contact_id=None, auto_create=True,
        subdomain=None):
    token = context.get_token(ctx)
    if not token:
        access_token = access.get_access_token(ctx)
    ...
    uri = '/v1/orgs'
    resp = requests.post(context.get_api(ctx) + uri,
                         headers=headers,
                         data=json.dumps(data),
                         verify=context.get_cacert(ctx))
    response.validate(resp)
...
def add_suborg(ctx, organisation, contact_id=None, auto_create=True,
               subdomain=None):
    token = context.get_token(ctx)
    if not token:
        access_token = access.get_access_token(ctx)
...
def delete_suborg(ctx, suborg_id):
    ...
    uri = '/v1/orgs/{}/orgs/{}'.format(context.get_org_id(ctx, token),
                                       suborg_id)
    resp = requests.delete(context.get_api(ctx) + uri,
                           headers=headers,
                           verify=context.get_cacert(ctx))
    response.validate(resp)
