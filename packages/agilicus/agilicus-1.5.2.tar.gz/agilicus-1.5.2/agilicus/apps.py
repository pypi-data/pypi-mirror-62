...
def query(ctx, org_id, maintained=None, **kwargs):
    ...
    resp = requests.get(context.get_api(ctx) + uri,
                        headers=headers,
...
def get_app(ctx, org_id, application, **kwargs):
    for app in json.loads(query(ctx, org_id, **kwargs)):
...
def delete(ctx, id, org_id=None, **kwargs):
    ...
    response = requests.delete(context.get_api(ctx) + uri,
                               headers=headers,
                               verify=context.get_cacert(ctx))
...
def add(ctx, name, org_id, category):
    ...
    uri = '/v2/applications'
    response = requests.post(context.get_api(ctx) + uri,
                             headers=headers,
                             json=application,
                             verify=context.get_cacert(ctx))
    return response.text
...
def add_role(ctx, id, role_name):
    ...
    resp = requests.put(context.get_api(ctx) + uri,
                        headers=headers,
                        json=application,
                        verify=context.get_cacert(ctx))
    response.validate(resp)
...
def _build_rule(rules, name, method, path, query_parameters: list,
                json_pointers: list, host=None, rule_name=None):
    rule = {}
    rule["name"] = name
    if host:
        rule["host"] = host
    ...
    if json_pointers:
        ...
        for pointer in json_pointers:
            json_pointer.append({"pointer": pointer[0],
...
def add_rule(ctx, app_name, role_name, method, path,
             query_parameters: list, json_pointers: list,
             rule_name=None, host=None, org_id=None):
    token = context.get_token(ctx)
    if not token:
        access_token = access.get_access_token(ctx)
        token = access_token.get()
    ...
    new_rule = _build_rule(rules, rule_name, method, path, query_parameters,
                           json_pointers, host, rule_name)
...
def get_env(ctx, id, env_name, org_id=None, **kwargs):
    token = context.get_token(ctx)
    if not token:
        access_token = access.get_access_token(ctx)
        token = access_token.get()
    ...
    return apiclient.application_api.get_environment(
        id, env_name, org_id).to_dict()


def update_env(ctx, id, env_name, org_id=None,
               version_tag=None,
               config_mount_path=None,
               config_as_mount=None, config_as_env=None,
               secrets_mount_path=None,
               secrets_as_mount=None, secrets_as_env=None,
    ...
    if not org_id:
        org_id = context.get_org_id(ctx, token)
    apiclient = context.get_apiclient(ctx, token)
    app_env = apiclient.application_api.get_environment(
        id, env_name, org_id)
    ...
    apiclient.application_api.put_environment(
        id, env_name, environment=app_env)
...
def update_assignment(ctx, env_name, app_id, org_id,
                      sub_org_id, unassign=False, admin_org_id=None):
    token = context.get_token(ctx)
    if not token:
        access_token = access.get_access_token(ctx)
        token = access_token.get()
    ...
    response = requests.put(context.get_api(ctx) + uri,
                            headers=headers,
                            json=application,
                            verify=context.get_cacert(ctx))
    return response.text
...
def get_application_services(ctx, org_id=None, **kwargs):
    ...
    if not org_id:
        org_id = context.get_org_id(ctx, token)
    apiclient = context.get_apiclient(ctx, token)
    return apiclient.app_services_api.get_application_services(
        org_id)
...
def get_application_service(ctx, id, org_id=None, **kwargs):
    ...
    if not org_id:
        org_id = context.get_org_id(ctx, token)
    apiclient = context.get_apiclient(ctx, token)
    return apiclient.app_services_api.get_application_service(
        id, org_id)


def add_application_service(ctx,
                            name, hostname, port,
                            org_id=None,
                            ipv4_addresses=None,
...
def update_application_service(ctx,
                               id,
                               name=None, hostname=None, port=None,
                               org_id=None,
                               ipv4_addresses=None,
                               name_resolution=None,
                               protocol=None):
    ...
    service = apiclient.app_services_api.get_application_service(
        id, org_id)
...
def add_application_service_assignment(ctx,
    ...
    assignment = agilicus.ApplicationServiceAssignment(
        app_id=_app['id'], environment_name=environment_name,
        org_id=_app['org_id'])
...
def delete_application_service_assignment(ctx,
                                          app_service_name, app_name,
                                          environment_name,
                                          org_id=None):
    token = context.get_token(ctx)
    ...
    for assignment in service.assignments:
        if assignment.app_id == _app['id'] and  \
           assignment.environment_name == environment_name and \
           assignment.org_id == _app['org_id']:
            pass
        else:
