...
def _create_token(ctx, user_id, duration, aud, hosts=[], roles={},
                  org_id=None):
    ...
    response = requests.post(context.get_api(ctx) + '/v1/tokens',
                             headers=headers,
...
def get_token(ctx, user_id, org_id, duration, hosts):
    ...
    aud = ['urn:api:agilicus:gateway',
           'urn:api:agilicus:users',
           'urn:api:agilicus:applications']
    return _create_token(ctx, user_id, duration, aud=aud, hosts=hosts,
                         org_id=org_id).text


def create_token(ctx, user_id, roles, duration, audiences, org_id):
    result = _create_token(ctx, user_id, duration,
                           aud=audiences,
                           roles=roles,
                           org_id=org_id)
...
def query_tokens(ctx, limit=None, expired_from=None,
                 expired_to=None, issued_from=None,
                 issued_to=None, org_id=None, jti=None):
    token = context.get_token(ctx)
    if not token:
        access_token = access.get_access_token(ctx)
        token = access_token.get()
