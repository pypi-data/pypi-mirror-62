...
@click.pass_context
def cli(ctx, token, api, cacert, client_id,
        issuer, org_id, header):
...
def output_tokens_list(tokens_list):
    table = PrettyTable(['jti', 'roles', 'iat', 'exp', 'aud', 'user',
                         'session', 'revoked'])
    for token in tokens_list:
        table.add_row([token['jti'],
                       json.dumps(token['roles'], indent=2),
                       token['iat'],
                       token['exp'],
                       json.dumps(token['aud'], indent=2),
                       token['sub'], token['session'], token['revoked']])
    table.align = 'l'
    print(table)
...
def list_tokens(ctx, org_id, **kwargs):
    output_tokens_list(json.loads(tokens.query_tokens(ctx, org_id=org_id,
                                                      **kwargs)))
...
def token_get(ctx, user_id, duration, hosts, **kwargs):
    ...
    token = tokens.get_token(ctx, user_id, user['org_id'], duration,
                             hosts, **kwargs)
...
def output_list_users(orgs_by_id, users_list):
    table = PrettyTable(['id', 'First Name', 'Last Name',
                         'Email', 'External_ID', 'Organisation'])
    for entry in users_list:
        ...
        table.add_row([entry['id'],
                       entry['first_name'],
                       entry['last_name'],
                       entry['email'], entry['external_id'], org_name])
    table.align = 'l'
    print(table)
...
def list_users(ctx, organisation, org_id, **kwargs):
    ...
    output_list_users(org_by_id,
                      json.loads(users.query(ctx,
                                             org_id,
                                             **kwargs)))
...
def output_entry(entry):
    ...
    for k, v in list(entry.items()):
        if k == 'nbf' or k == 'exp' or k == 'iat':
            _t = datetime.fromtimestamp(v, timezone.utc).strftime('%Y-%m-%d %H:%M:%S %z (%Z)')  # noqa
...
def add_user(ctx, first_name, last_name, email, org_id, **kwargs):
    output_entry(users.add_user(ctx,
                                first_name,
                                last_name,
                                email,
                                org_id,
...
def update_user(ctx, user_id, org_id, **kwargs):
    output_entry(users.update_user(ctx,
                                   user_id=user_id,
                                   org_id=org_id,
                                   **kwargs))
...
def output_list_orgs(orgs_list):
    ...
    for entry in orgs_list:
        ...
        if 'subdomain' not in entry:
            subdomain = None
        table.add_row([entry['id'], entry['organisation'], entry['issuer'],
                       subdomain])
    table.align = 'l'
    print(table)
...
def add_group_member(ctx, group_id, org_id, member, member_org_id):
    users.add_group_member(ctx, group_id, member,
...
@click.pass_context
def update_org(ctx, org_id, auto_create, issuer, contact_id,
               subdomain, external_id, **kwargs):
    orgs.update(ctx, org_id,
...
@click.option('--subdomain', default=None)
@click.pass_context
def add_org(ctx, organisation, issuer, contact_id,
            auto_create, subdomain, **kwargs):
    output_entry(orgs.add(ctx, organisation, issuer,
                          contact_id,
                          auto_create,
                          subdomain=subdomain,
                          **kwargs))
...
@click.option('--subdomain', default=None)
@click.pass_context
def add_sub_org(ctx, organisation, contact_id,
                auto_create, subdomain, **kwargs):
    output_entry(orgs.add_suborg(ctx, organisation,
                                 contact_id,
                                 auto_create,
                                 subdomain,
                                 **kwargs))
...
def list_applications(ctx, organisation, org_id, **kwargs):
    ...
    if not org_id and organisation:
        if organisation in org_by_name:
            ...
            Exception("No such organisation found: {}".format(organisation))
    output_list_apps(org_by_id, json.loads(apps.query(ctx,
                                                      org_id, **kwargs)))
...
@click.pass_context
def list_environments(ctx, organisation, org_id, filter,
                      **kwargs):
    # get all orgs
    org_by_id, org_by_name = orgs.get_org_by_dictionary(ctx, org_id)
    if not org_id and organisation:
        if organisation in org_by_name:
            ...
            Exception("No such organisation found: {}".format(organisation))
    table = PrettyTable(['Name', 'Assignments', 'Services'],
...
def list_application_services(ctx, **kwargs):
    table = PrettyTable(['id', 'name', 'hostname', 'ipv4_addresses',
                         'name_resolution', 'port', 'protocol'])
    services = apps.get_application_services(ctx, **kwargs)
    for obj in services:
        service = obj.to_dict()
        table.add_row([service['id'],
...
def add_application_service(ctx, name, hostname, port, org_id, **kwargs):
    output_entry(apps.add_application_service(
        ctx, name, hostname, port,
        org_id=org_id, **kwargs).to_dict())
...
def add_application_service_assignment(ctx, **kwargs):
    output_entry(apps.add_application_service_assignment(
        ctx, **kwargs))
...
def delete_application_service_assignment(ctx, **kwargs):
    apps.delete_application_service_assignment(
        ctx, **kwargs)
...
def show_application_service(ctx, id, org_id, **kwargs):
    output_entry(apps.get_application_service(ctx, id, org_id=org_id,
                                              **kwargs).to_dict())
...
@click.option('--config_as_mount', help="json string",
              default=None)
@click.option('--config_as_env', help="json string",
              default=None)
...
@click.pass_context
def update_environment(ctx, app_id, env_name, org_id,
                       version_tag,
                       config_mount_path,
                       config_as_mount, config_as_env,
                       secrets_mount_path,
                       secrets_as_mount, secrets_as_env,
                       **kwargs):
    apps.update_env(
        ctx, app_id, env_name, org_id,
        version_tag,
        config_mount_path,
        config_as_mount, config_as_env,
        secrets_mount_path,
        secrets_as_mount, secrets_as_env, **kwargs)
...
@click.pass_context
def assign_application(ctx, env_name, app_id, org_id, assigned_org_id,
                       admin_org_id):
    output_entry(json.loads(
        apps.update_assignment(ctx, env_name, app_id, org_id,
                               assigned_org_id,
...
@click.option('--hostname', default=None)
@click.pass_context
def rules_from_csv(ctx, application_id, role_name, file_name,
                   org_id, hostname):
    result = csv_rules.add_rules_to_app(ctx, application_id, role_name,
                                        file_name, org_id, hostname)
    output_entry(result)
...
@click.option('--query-param', "-q", type=click.Tuple([str, str]),
              multiple=True)
@click.option('--json-pointer', '-j', type=click.Tuple([str, str]),
              multiple=True)
@click.option('--rule-name', default=None)
@click.option('--host', default=None)
@click.option('--org_id', default=None)
@click.pass_context
def add_rule(ctx, app_name, role_name, method,
             path_regex, query_param, json_pointer, **kwargs):
    apps.add_rule(ctx, app_name, role_name, method,
                  path_regex, query_param, json_pointer, **kwargs)
...
def list_rules(ctx, **kwargs):
    table = PrettyTable(['role', 'name', 'host', 'method',
                         'path', 'query_param', 'json_body'])
...
@click.option('--role', '-r', type=click.Tuple([str, str]),
              multiple=True)
...
def list_files(ctx, **kwargs):
    ...
    table = PrettyTable(['id', 'name', 'tag', 'created', 'last_accessed',
                         'size'])
    table.align = 'l'
    for _file in _files:
        table.add_row([_file['id'], _file['name'], _file['tag'],
                       _file['created'],
                       _file['last_access'],
                       _file['size']])
    print(table)
...
def list_config(ctx, **kwargs):
    ...
    table = PrettyTable(['id', 'config_type', 'host', 'src_mount', 'domain',
                         'share', 'username', 'password', 'dest_mount',
                         'file_store_uri'])
    table.align = 'l'
    for config in configs:
        table.add_row([config.id, config.config_type,
...
@click.option('--config_type',
              type=click.Choice(['CONFIGMAP_MOUNT',
                                 'CONFIGMAP_ENV',
                                 'SECRET_MOUNT',
                                 'SECRET_ENV',
                                 'MOUNT_SMB',
                                 'FILE_MOUNT']), prompt=True)
...
@click.option('--secret', default=False)
@click.pass_context
def add_env_var(ctx, environment_name,
                environment_value, **kwargs):
    envVar = env_config.EnvVarConfigObj(ctx, **kwargs)
...
@click.option('--secret', default=False)
@click.pass_context
def delete_env_var(ctx, environment_name,
                   **kwargs):
    envVar = env_config.EnvVarConfigObj(ctx, **kwargs)
