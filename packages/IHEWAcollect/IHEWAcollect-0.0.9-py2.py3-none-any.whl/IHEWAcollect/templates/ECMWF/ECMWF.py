...
def DownloadData(Dir, Var, Startdate, Enddate, latlim, lonlim, Waitbar, cores,
    ...
    out_dir = os.path.join(Dir, "Weather_Data", "Model", "ECMWF", TimeCase, Varname_dir,
                           "mean")
    ...
    string9 = 'ei'
    string10 = '%s/%s/%s/%s' % (
    latlim_corr[1], lonlim_corr[0], latlim_corr[0], lonlim_corr[1])  # N, W, S, E

    # Download data by using the ECMWF API
    print('Use API ECMWF to collect the data, please wait')
    Download.API(Dir, DownloadType, string1, string2, string3, string4, string5,
    ...
    for date in Dates:
        ...
        Date_good[np.logical_and(Data_time >= hours_from_start_begin, Data_time < (
