...
class MainWindow(QtG.QMainWindow):
    def __init__(self, font, images_path=None, boxes_path=None, parent=None):
        ...
        #self.lower_size_thresh_line.textChanged.connect(self.conf_thresh_label_changed)
        ...
        #self.upper_size_thresh_line.textChanged.connect(self.conf_thresh_label_changed)
    ...
    def write_box_files(self):
        ...
        for filename, rectangles in self.box_dictionary.items():
            ...
            if self.is_cbox:
                rectangles = [box for box in rectangles if self.check_if_should_be_visible(box)]
    ...
    def check_if_should_be_visible(self, box):
        return box.confidence > self.current_conf_thresh and box.est_size <= self.upper_size_thresh and box.est_size >= self.lower_size_thresh

    def draw_all_patches(self, rect):
        for box in rect:
            should_be_visible = self.check_if_should_be_visible(box)
...
def start_boxmanager(image_dir,box_dir):
...
def run(args=None):
    ...
    start_boxmanager(image_dir,box_dir)
