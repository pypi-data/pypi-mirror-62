...
class Task():
    ...
    def replace_volume(self, src_mountpoint, dst_mountpoint):
        ...
        # delete the fstab entry corresponding to destination volume
        dst_vol_fs_device = self.process.get_fsdevice_from_mountpoint(dst_mountpoint)
        if dst_vol_fs_device is None:
            return False
        if not self.process.remove_entry_from_fstab(dst_vol_fs_device):
            return False
        ...
        # daemon reload. TODO: is it needed?
        ...
        return True if self.delete_volume(dst_vol_info[0]) else False
    ...
    def unmap_dellsc_volume(self, wwn):
        ...
        if raw_devices is None:
            ...
            LOG.error('The raw devices %s of %s have been deleted. Result: %s.',
                      raw_devices, alias, result_delete_rawdevs)
    ...
    def clone_volume(self, volume_wwn, replay_label, clone_name, target_mountpoint):
        ...
        if not self.map_dellsc_volume(view_volume_wwn, target_mountpoint):
            return False
