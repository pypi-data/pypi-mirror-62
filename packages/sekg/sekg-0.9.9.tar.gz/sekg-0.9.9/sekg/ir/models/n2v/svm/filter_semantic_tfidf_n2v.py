...
class FilterSemanticTFIDFNode2VectorModel(DocumentSimModel):
    ...
    def compute_query_graph_vec(self, query):
        ...
        sort_vec = np.argsort(-tfidf_full_entity_score_vec)
        sort_vec_index = sort_vec[0:self.TOP_N]
        tfidf_full_entity_score_vec = tfidf_full_entity_score_vec[sort_vec_index]
        vectorized_simvec_index2entity_id = np.vectorize(self.doc_index2doc_id)
        index_array = vectorized_simvec_index2entity_id(sort_vec_index)
        valid_id_scores_list = np.array((index_array, tfidf_full_entity_score_vec)).T

        # index_scores = [(index, score) for index, score in enumerate(tfidf_full_entity_score_vec) if
        #                 score > self.MIN_NODE_SIM_SCORE]

        # valid_id_scores_list = [(self.doc_index2doc_id(index), score) for (index, score) in index_scores]
        valid_id_scores_map = {}
