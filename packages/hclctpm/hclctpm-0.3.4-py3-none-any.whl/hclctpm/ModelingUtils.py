...
def HPSelectMultiRandomInitializeTrainModel(model_params, Dtrain, Dval): 
    ...
    for i in range(model_params['num_inits']): 
        ...
        if model_params['model_name'] == 'DPCM': 
            list_raninit_models.append(DefineKerasSimpleTCModel(Dtrain['D_tre'].shape[1], 1, Dtrain['D_tre'].shape[0], activation=tf.nn.tanh)) 
            #list_raninit_models.append(DefineKerasDPCMModel(Dtrain['D_tre'].shape[1], 1, Dtrain['D_tre'].shape[0], activation=tf.nn.tanh, quantile=model_params['quantile'], temp=model_params['temp']))
        elif model_params['model_name'] == 'SimpleTC':
            list_raninit_models.append(DefineKerasSimpleTCModel(Dtrain['D_tre'].shape[1], 1, Dtrain['D_tre'].shape[0], activation=tf.nn.tanh)) 
        ...
        list_raninit_models[i].compile(loss={main_output_str:custom_loss, 'score_output':dummy_loss}, 
        ...
        history = list_raninit_models[i].fit([\
    ...
    if model_params['model_name'] == 'DPCM': 
        return_scores = model_struct.predict([Dtest['Dt'], Dtest['Dt'], \
                                 Dtest['ct'], Dtest['ct'], Dtest['ot'], Dtest['ot']], \
                                             batch_size=None) #Dtest['D_tre'].shape[0]) 
