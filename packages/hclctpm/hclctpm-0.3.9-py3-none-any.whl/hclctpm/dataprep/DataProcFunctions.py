...
def CLDataPreparation(dataspecs): 
    ### Prepare data for training before passing in to modeling APIs 
    
    ### returns of the function specifies treatment and control splits 
    ### of data as well as treatment label (w), cost (c), gain (o), 
    ### intensity (int, if asked to return), the third metric M3 (d2d, 
    ### if asked to return) 
    ### in e.g. Da/Db, the a, b represent user/product matching segmentations 
    ### of the feature space 
    
    Da_tre, Da_unt, Db_tre, Db_unt, \
    Dva_tre, Dva_unt, Dvb_tre, Dvb_unt, \
    Dta_tre, Dta_unt, Dtb_tre, Dtb_unt, \
    o_tre, o_unt, \
    ov_tre, ov_unt, \
    ot_tre, ot_unt, \
    c_tre, c_unt, \
    ...
    Dta, Dtb, \
    wt, ot, ct, \
    int_tre, int_unt, intv_tre, intv_unt, intt_tre, intt_unt, \
    int, intv, intt, \
    d2d_tre, d2d_unt, d2dv_tre, d2dv_unt, d2dt_tre, d2dt_unt, \
    d2d, d2dv, d2dt \
    = LoadDataFromPklMatchingD2Dist(dataspecs['data_filename'], \
                                    frac = dataspecs['sample_frac'], \
                                    use_python3=True, \
                                    save_cf_data=False, \
                                    with_intensity=dataspecs['use_intensity'])
    
    ### concatenate a, b segments of feature space to form combined features 
    ...
    ### this code segment aligns sample size lengths across treatment and control 
    ### [TODO] implement minibatched training with equal treatment/control samples 
    min_sample_size = np.minimum(D_tre.shape[0], D_unt.shape[0]) 
    Dtrain = dict() 
    Dtrain['D'] = D 
    Dtrain['o'] = o 
    Dtrain['c'] = c 
    Dtrain['w'] = w     
    Dtrain['D_tre'] = D_tre[0:min_sample_size,:]; 
    Dtrain['D_unt']=D_unt[0:min_sample_size,:]; 
    Dtrain['c_tre']=c_tre[0:min_sample_size]; 
    Dtrain['c_unt']=c_unt[0:min_sample_size]; 
    Dtrain['o_tre'] = o_tre[0:min_sample_size]; 
    Dtrain['o_unt'] = o_unt[0:min_sample_size] 
    
    min_sample_size = np.minimum(Dv_tre.shape[0], Dv_unt.shape[0]) 
    Dval = dict() 
    Dval['D_tre'] = Dv_tre[0:min_sample_size,:]; 
    Dval['D_unt']=Dv_unt[0:min_sample_size,:]; 
    Dval['c_tre']=cv_tre[0:min_sample_size]; 
    Dval['c_unt']=cv_unt[0:min_sample_size]; 
    Dval['o_tre'] = ov_tre[0:min_sample_size]; 
    Dval['o_unt'] = ov_unt[0:min_sample_size] 
    
    min_sample_size = np.minimum(Dt_tre.shape[0], Dt_unt.shape[0]) 
    Dtest = dict() 
    Dtest['D_tre'] = Dt; 
    Dtest['D_unt'] = Dt;
    Dtest['c_tre'] = ct; 
...
def LoadDataFromPklMatchingD2Dist(filename, frac = 1.0, use_python3 = False, save_cf_data=False, with_intensity=False): 
    ...
    print('\n') 
    print('loading pkl data from ' + filename) 
    ...
    if 'nX_a_tr' in Dd: 
        Da = Dd['nX_a_tr']; Db = Dd['nX_b_tr'];
    else:
        Da = Dd['nX_tr']; Db = Da 
    w = Dd['w_tr']; ni = Dd[neg_cost_str + '_tr']; o = Dd['values_tr']; c = -1.0 * ni
    if 'nX_a_va' in Dd:
        Dva = Dd['nX_a_va']; Dvb = Dd['nX_b_va'];
    else:
        Dva = Dd['nX_va']; Dvb = Dva
    wv = Dd['w_va']; niv = Dd[neg_cost_str + '_va']; ov = Dd['values_va']; cv = -1.0 * niv
    if 'nX_a_te' in Dd: 
        Dta = Dd['nX_a_te']; Dtb = Dd['nX_b_te'];
    else:
        Dta = Dd['nX_te']; Dtb = Dta 
    wt = Dd['w_te']; nit = Dd[neg_cost_str + '_te']; ot = Dd['values_te']; ct = -1.0 * nit
    ...
    if with_intensity == True: 
        int_ = Dd['treatintensity_tr'] 
        if int_.shape[-1] == 1:
            int_ = np.reshape(int_, (len(int_), )) 
    ...
    int_ = int_[np.where(filter==True)[0]] 
    ...
    print('Loaded Training Data, printing averages of cost-treatment, cost-control, gain-treatment, gain-control ... :') 
    print("{0:.3f}".format(np.average(c_tre)) + ", {0:.3f}".format(np.average(c_unt)) + ", {0:.3f}".format(np.average(o_tre)) + ", {0:.3f}".format(np.average(o_unt))) 
    print('Loaded Validation Data, printing averages of cost-treatment, cost-control, gain-treatment, gain-control ... :') 
    print("{0:.3f}".format(np.average(cv_tre)) + ", {0:.3f}".format(np.average(cv_unt)) + ", {0:.3f}".format(np.average(ov_tre)) + ", {0:.3f}".format(np.average(ov_unt))) 
    print('Loaded Test Data, printing averages of cost-treatment, cost-control, gain-treatment, gain-control ... :') 
    print("{0:.3f}".format(np.average(ct_tre)) + ", {0:.3f}".format(np.average(ct_unt)) + ", {0:.3f}".format(np.average(ot_tre)) + ", {0:.3f}".format(np.average(ot_unt))) 
    
    prefix = filename.split('/')[2].split('.')[0]
    if save_cf_data: 
        SaveCFDataD2Dist(D, w, o, c, d2d, Dv, wv, ov, cv, d2dv, Dt, wt, ot, ct, d2dt, prefix) 
    
    return Da_tre, Da_unt, Db_tre, Db_unt, Dva_tre, Dva_unt, Dvb_tre, Dvb_unt, Dta_tre, Dta_unt, Dtb_tre, Dtb_unt, o_tre, o_unt, ov_tre, ov_unt, ot_tre, ot_unt, c_tre, c_unt, cv_tre, cv_unt, ct_tre, ct_unt, Da, Db, w, o, c, Dva, Dvb, wv, ov, cv, Dta, Dtb, wt, ot, ct, int_tre, int_unt, intv_tre, intv_unt, intt_tre, intt_unt, int_, intv, intt, d2d_tre, d2d_unt, d2dv_tre, d2dv_unt, d2dt_tre, d2dt_unt, d2d, d2dv, d2dt 
