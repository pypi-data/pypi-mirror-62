...
# In[1]: 
import pandas as pd, numpy as np, pickle as pkl, sys, os, tensorflow as tf 
sys.path.append('../dataprep/') 
sys.path.append('../models/') 
from ..dataprep.QueryFunctions import * 
from ..models.pyMLModelDefinitions import * 
from ..dataprep.DataProcFunctions import * 
...
def ut(): 
    ...
    prefix = 'ponfare_run4_for_analysis' 
...
""" 
    ...
    D_tre = D_tre[0:min_sample_size,:]; D_unt=D_unt[0:min_sample_size,:]; c_tre=c_tre[0:min_sample_size]; c_unt=c_unt[0:min_sample_size]; o_tre = o_tre[0:min_sample_size]; o_unt = o_unt[0:min_sample_size] 

    list_simpleTCModel = []
    
    for i in range(num_modeling_inits): 
        ## re-implement in Keras and TF 2.0 
        list_simpleTCModel.append(DefineKerasSimpleTCModel(D_tre.shape[1], 1, D_tre.shape[0], activation=tf.nn.tanh)) 
        opt = tf.keras.optimizers.Adam(learning_rate=0.01)
        if i == 0: 
            main_output_str = 'tf_op_layer_main_output'
        else:
            main_output_str = 'tf_op_layer_main_output_'+str(i)
        
        list_simpleTCModel[i].compile(loss={main_output_str:custom_loss, 'score_output':dummy_loss}, #tf.keras.losses.mean_absolute_error * min_sample_size,
                              loss_weights={main_output_str:1.0, 'score_output':0.0},
                              optimizer=opt) #, 
        #metrics=['accuracy', custom_metric]) 
    
        y_dummy_train = np.zeros((min_sample_size, 1)) 
        history = list_simpleTCModel[i].fit([D_tre, D_unt, c_tre, c_unt, o_tre, o_unt], [y_dummy_train, y_dummy_train], 
                                            validation_data=[[D_tre, D_unt, c_tre, c_unt, o_tre, o_unt], [y_dummy_train, y_dummy_train]], 
                                            batch_size=D_tre.shape[0], 
                                            epochs=num_optimize_iterations,
                                            validation_freq=200,
                                            verbose=1)

"""        
    dobjt, ddumo, dumh, dumhu, dsaver = TFV2SimpleTCModelDNN(gs_drm[i], Dt_tre, Dt_unt, ot_tre, ot_unt, ct_tre, ct_unt, 'test', num_hidden) 
    test_result = sess.run(dobjt) 
    print('test CPIT:') 
    print(test_result) 
