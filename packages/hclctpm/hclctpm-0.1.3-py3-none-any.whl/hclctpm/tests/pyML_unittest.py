...
""" 
    ...
    for i in range(num_modeling_inits): 
        ...
        simpleTCModel = DefineKerasSimpleTCModel(D_tre.shape[1], 1, D_tre.shape[0], activation=tf.nn.tanh) 
        opt = tf.keras.optimizers.Adam(learning_rate=0.01) 
        simpleTCModel.compile(loss={'tf_op_layer_main_output':custom_loss, 'score_output':dummy_loss}, #tf.keras.losses.mean_absolute_error * min_sample_size,
                              loss_weights={'tf_op_layer_main_output':1.0, 'score_output':0.0},
                              optimizer=opt) #, 
        #metrics=['accuracy', custom_metric]) 
    
        y_dummy_train = np.zeros((min_sample_size, 1)) 
        history = simpleTCModel.fit([D_tre, D_unt, c_tre, c_unt, o_tre, o_unt], [y_dummy_train, y_dummy_train], 
