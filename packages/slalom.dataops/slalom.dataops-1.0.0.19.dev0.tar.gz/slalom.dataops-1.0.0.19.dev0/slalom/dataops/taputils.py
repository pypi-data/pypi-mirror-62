...
PLUGIN_INDEX_FILE = "./singer_index.yml"
...
def _get_taps_dir():
    return os.environ.get("TAP_CONFIG_DIR", ".")
...
def plan(tap_name, tap_dir=None, config_file=None, config_dir=None, rescan=None):
    """
...
def build_image(tap_or_plugin_alias, target_alias=None, push=False):
    name, source, alias = _get_plugin_info(tap_or_plugin_alias)
    _build_plugin_image(name, source=source, alias=alias, push=push)
    if target_alias:
        name, source, alias = _get_plugin_info(target_alias)
        _build_plugin_image(name, source=source, alias=alias, push=push)
        _build_composite_image(tap_alias=tap_or_plugin_alias, target_alias=target_alias)


def _get_plugins_list():
    yml_doc = yaml.safe_load(io.get_text_file_contents("./singer_index.yml"))
...
def _build_all_standalone(source_image=None, plugins=None, push=False):
    plugins = plugins or _get_plugins_list()
    for name, source, alias in plugins:
        image_name = _build_plugin_image(
            name, source=source, alias=alias, source_image=source_image, push=push
        )


def _get_plugin_info(alias, plugins=None):
    plugins = plugins or _get_plugins_list()
    for name, source, alias in plugins:
        if (alias or name) == alias:
            return (name, source, alias)
    raise ValueError(f"Could not file a plugin called '{alias}'")


def _build_plugin_image(plugin_name, source, alias, source_image=None, push=False):
    source = source or plugin_name
    alias = alias or plugin_name
    image_name = f"{IMAGE_BASE}:{alias}"
    build_cmd = f"docker build"
    if source_image:
        build_cmd += f" -a source_image={source_image}"
    build_cmd += (
...
def _build_all_composite(source_image=None, plugins=None, push=False):
    plugins = plugins or _get_plugins_list()
    for tap in plugins:
        tap_alias = tap[2] or tap[0]
        for target in plugins:
            target_alias = target[2] or target[0]
            if tap_alias.startswith("tap-") and target_alias.startswith("target-"):
                image_name = _build_composite_image(tap_alias, target_alias, push=push)


def _build_composite_image(tap_alias, target_alias, source_image=None, push=False):
    if tap_alias.startswith("tap-"):
...
def build_all_images(push=False):
    _build_all_composite(push=push)
    _build_all_standalone(push=push)
