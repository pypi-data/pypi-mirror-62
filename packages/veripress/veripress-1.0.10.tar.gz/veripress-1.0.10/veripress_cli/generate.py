...
from veripress_cli.helpers import (
    copy_folder_content, remove_folder_content, makedirs
)
...
@cli.command('generate', short_help='Generate static pages.',
             help='This command will generate all HTML pages '
                  'of the chosen instance.')
@click.option('--app-root', default='/',
              prompt='Please enter the application root '
                     '(used as prefix of generated url)',
              help='The application root of your VeriPress instance. '
                   'For example, if you want to access the site '
                   'through "http://example.com/blog/", '
...
def generate_command(app_root):
    ...
    # mark as 'GENERATING_STATIC_PAGES' status,
    # so that templates can react properly,
    # e.g. remove search bar
    app.config['GENERATING_STATIC_PAGES'] = True

    do_generate()
    click.echo('\nOK! Now you can find all static pages '
               'in the "_deploy" folder.')


@cli.command('clean', short_help='Clean all generated static pages.',
             help='This command will clean all generated HTML pages '
                  'of the chosen instance.')
def clean_command():
    deploy_dir = get_deploy_dir()
...
def do_generate():
    ...
    with app.app_context():
        ...
        index_page_count = int(
            math.ceil(len(posts) / app.config['ENTRIES_PER_PAGE']))
        # ignore '/page/1/', which will be generated separately later
        for i in range(2, index_page_count + 1):
    ...
    with app.test_client() as client:
        ...
        for url in all_urls:
            resp = client.get(url)
            file_path = os.path.join(get_deploy_dir(),
                                     url.lstrip('/').replace('/', os.path.sep))
            if url.endswith('/'):
