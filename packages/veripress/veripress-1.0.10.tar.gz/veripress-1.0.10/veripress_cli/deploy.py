...
@click.option('--email', '-e',
              prompt='Please enter your email (for git config)',
...
def setup_command(repo, name, email):
    ...
    os.system('git -C "{}" remote add origin git@github.com:{}/{}.git'.format(
        deploy_dir, user, repo))
...
def deploy_command():
    ...
    if os.system(
            'git -C "{}" diff --quiet --exit-code'.format(deploy_dir)) == 0 \
            and os.system(
        'git -C "{}" diff --quiet --cached --exit-code'.format(
            deploy_dir)) == 0:
        click.echo('There are no changes to be deployed.')
        return

    dt = datetime.now()
    os.system('git -C "{}" commit -m "Updated on {} at {}"'.format(
        deploy_dir, dt.strftime('%Y-%m-%d'), dt.strftime('%H:%M:%S')))
    os.system(
        'git -C "{}" push --set-upstream origin master'.format(deploy_dir))
