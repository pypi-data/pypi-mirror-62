...
class Strategy:
    ...
    def addsignal(self, signal):
        ...
        if isinstance(signal.index[0], int):
            ...
            signal = signal.pivot(
                values='weight', index='date', columns='stockid')
    ...
    def _get_turnover(signal, delta):
        ...
        Returns:
            [type] -- [description]
        """
        t = signal.diff().abs().sum(axis=1).sum() / delta * 252
        return t
    ...
    def run(self):
        ...
        ret_asset, hold_asset = self._get_ret(
            self._signal['asset'], self.commission)
        ret_future, hold_future = self._get_ret(
            self._signal['future'], self.fcommission)
        ...
        if self.benchmark:
            ...
            self.benchmark = 'benchmark'
            ret_all[self.benchmark] = self.fixed_rate
        ...
        # 保存最新持仓
        self.result.hold = hold_asset.append(hold_future)
        # 保存基准
        self.result.benchmark = self.benchmark
        # 保存换手率
        delta = len(ret_all)
        self.result.stats.turnover = self._get_turnover(
            self._signal['asset'], delta) + self._get_turnover(self._signal['future'], delta)
        # 增加其他属性
        stats = Stats(ret_all['hedge'].pct_change().fillna(0), ret_all[self.benchmark].pct_change().fillna(0))
        stats.run()
        self.result.stats.update(stats.stats)
