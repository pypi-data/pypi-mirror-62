...
def classify(features, labels, model = 'all', resample_method = None, scoring = 'roc_auc_ovo', cv = 10, n_iter = 10):
    ...
    Args:
        ...
        model: Options are: 'rf' - Random Forest
                            ...
                            'dt' - Decision Tree
                            'svm_sgd' - Support Vector Machines with Stochastic 
    ...
    def which_model(X, y, model = 'all'):
        ...
        Gradient Boosting, Decision Tree, Linear SVM with Stochastic 
        Gradient Descent learning, and Logistic Regression with Stochastic 
        Gradient Descent learning on the entire dataset (with cross-validation) 
        ...
        if model == 'all':
            ...
            pipelines = []
            pipelines.append(('Random Forest', skl_pipeline([('Standardization', StandardScaler()),
                                                        ('RF', RandomForestClassifier(random_state = 9999))])))
            pipelines.append(('Gradient Boosting', skl_pipeline([('Standardization', StandardScaler()),
                                                        ('GBM', GradientBoostingClassifier(random_state = 9999))])))
            pipelines.append(('Decision Tree', skl_pipeline([('Standardization', StandardScaler()),
                                                        ('DT', DecisionTreeClassifier(random_state = 9999))])))
            pipelines.append(('Support Vector Machines (SGD)', skl_pipeline([('Standardization', StandardScaler()),
                                                        ('SVMSGD', CalibratedClassifierCV(SGDClassifier(loss = 'hinge', random_state = 9999), cv = None, method = 'sigmoid'))])))
            pipelines.append(('Logistic Regression (SGD)', skl_pipeline([('Standardization', StandardScaler()),
                                                        ('LOGSGD', CalibratedClassifierCV(SGDClassifier(loss = 'log', random_state = 9999), cv = None, method = 'sigmoid'))])))
            ...
            elif 'Support Vector Machines (SGD)' in selected_model:
                return SGDClassifier(loss = 'hinge')
            elif 'Logistic Regression (SGD)' in selected_model:
                return SGDClassifier(loss = 'log')

        elif model == 'rf':
            rf_pipe = skl_pipeline([('Standardization', StandardScaler()), ('RF', RandomForestClassifier(random_state = 9999))])
            ...
            gb_pipe = skl_pipeline([('Standardization', StandardScaler()), ('GBM', GradientBoostingClassifier(random_state = 9999))])
            ...
            dt_pipe = skl_pipeline([('Standardization', StandardScaler()), ('DT', DecisionTreeClassifier(random_state = 9999))])
            ...
            svm_pipe = skl_pipeline([('Standardization', StandardScaler()), ('SVMSGD', CalibratedClassifierCV(SGDClassifier(loss = 'hinge', random_state = 9999), cv = None, method = 'sigmoid'))])
            svm_score = cross_val_score(svm_pipe, X, y, cv = cv, scoring = scoring)
            print('\nSupport Vector Machines (SGD) score: {:.4f} ± {:.4f}'.format(np.mean(svm_score), np.std(svm_score)))
            return SGDClassifier(loss = 'hinge')
        
        elif model == 'log_sgd':
            log_pipe = skl_pipeline([('Standardization', StandardScaler()), ('LOGSGD', CalibratedClassifierCV(SGDClassifier(loss = 'log', random_state = 9999), cv = None, method = 'sigmoid'))])
    ...
    def train(selected_model, X_train, y_train, resample_method = None):
        ...
        if resample_method == None:
            ...
            if selected_model.__class__.__name__ == 'GradientBoostingClassifier':
                ...
                pipe = skl_pipeline([('Standardization', StandardScaler()), ('clf', selected_model)])
            ...
            elif 'hinge' in selected_model.get_params().values():
                
                start = time()

                pipe = skl_pipeline([('Standardization', StandardScaler()), ('clf', CalibratedClassifierCV(selected_model, cv = None, method = 'sigmoid'))])

                svm_grid = {'clf__base_estimator__loss': ['hinge'],
            ...
            if selected_model.__class__.__name__ == 'GradientBoostingClassifier':
                ...
                pipe = imbl_pipeline([('Standardization', StandardScaler()), ('SMOTETOMEK', SMOTETomek()), ('clf', selected_model)])
                ...
                pipe = imbl_pipeline([('Standardization', StandardScaler()), ('SMOTETOMEK', SMOTETomek()), ('clf', CalibratedClassifierCV(selected_model, cv = None, method = 'sigmoid'))])
            ...
            if selected_model.__class__.__name__ == 'GradientBoostingClassifier':
                ...
                pipe = imbl_pipeline([('Standardization', StandardScaler()), ('SMOTENN', SMOTEENN()), ('clf', selected_model)])
                ...
                pipe = imbl_pipeline([('Standardization', StandardScaler()), ('SMOTENN', SMOTEENN()), ('clf', CalibratedClassifierCV(selected_model, cv = None, method = 'sigmoid'))])
    ...
    def evaluate(clf, X_test, y_test):
        ...
        if 'hinge' in clf.best_estimator_.get_params().values():
            model_name = 'Support Vector Machines (SGD)'
            y_probas = clf.predict_proba(X_test)
        elif 'log' in clf.best_estimator_.get_params().values():
        ...
        if 'hinge' in selected_model.get_params().values() or 'log' in selected_model.get_params().values():
