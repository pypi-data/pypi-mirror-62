...
class EarMasaCob:
    ...
    @staticmethod
    def execute(class_list: np.array, rmse_band: np.array, spectral_angle_band: np.array,
                constraints_band: np.array = None):
        """
        ...
        if constraints_band is None:
            print("No Constraints band found. COB not calculated. "
                  "The Square Array was not run in reset mode. This can influence the EAR and COB values.")
        elif constraints_band.shape[0] != constraints_band.shape[1]:
            raise Exception("Constraints band is not square")
        else:
            if 2 in constraints_band or 5 in constraints_band:
                print("The Square Array was not run in reset mode. This can influence the EAR and COB values.")
        ...
        if constraints_band is not None:
...
2019-05 [Python] Warning remains, but 'non-reset' mode is allowed. CoB is not calculated in that case.
"""
