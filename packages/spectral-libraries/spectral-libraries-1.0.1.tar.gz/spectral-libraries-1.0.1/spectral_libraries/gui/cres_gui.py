...
from vipertools.io.exports import EmittingStream
from vipertools.gui.logo_gui import LogoWidget
from vipertools.scripts.cres import Cres
...
class CresWidget(QMainWindow):
    ...
    def _image_browse(self, path):
        ...
        self.image = np.asarray([x.values()['y'] for x in spectral_library.profiles()]).T
        ...
        self.spectrumDropDown.addItems(['Select ...'] + spectra_names)
    ...
    def _library_browse(self, path):
        ...
        # set the library reflectance scale factor
        library = np.asarray([x.values()['y'] for x in self.spectral_library.profiles()]).T
        self.libraryScaleValue.setValue(detect_reflectance_scale_factor(library))
    ...
    def _get_class_from_list(self, class_name):
        ...
        class_list = np.asarray([x.metadata(class_name) for x in self.spectral_library.profiles()], dtype=str)
        self.class_list = np.asarray([x.lower() for x in class_list])
        unique_classes = np.unique(self.class_list)

        # check state for each endmember as unchecked
        spectrum_names = np.asarray([x.name().strip() for x in self.spectral_library.profiles()])
        n_models = 1
        for i, cl in enumerate(unique_classes):
            self.spectrum_names_per_class[cl] = {}
            names = spectrum_names[np.where(self.class_list == cl)[0]]
    ...
    def _shade_browse(self, path):
        ...
        self.shade = np.asarray([x.values()['y'] for x in spectral_library.profiles()]).T
    ...
    def _calculate_sma(self):
        ...
        if self.image is None:
            print('Choose an library with image spectra.', file=sys.stderr)
            return
        if self.spectral_library is None:
            print('Choose a spectral library.', file=sys.stderr)
            return
        if self.spectrumDropDown.currentIndex() == 0:
            print('Choose one spectrum from the image spectra.', file=sys.stderr)
            return
        if self.classDropDown.currentIndex() == 0:
            print('Choose a class from the library metadata drop down list.', file=sys.stderr)
            return

        image_spectrum = self.image[:, self.spectrumDropDown.currentIndex() - 1] / self.imageScaleValue.value()
        library = np.asarray([x.values()['y'] for x in self.spectral_library.profiles()]).T
        spectrum_names = [x.name().strip() for x in self.spectral_library.profiles()]

        if len(image_spectrum) != library.shape[0]:
            print('The image and library have different number of bands. CRES not possible.', file=sys.stderr)
            return

        if self.shade.size != 0 and self.shade.shape[0] != library.shape[0]:
            print('The library and shade spectrum have different number of bands. CRES not possible.', file=sys.stderr)
            return
        ...
        self.cres_object.execute(signal=image_spectrum,
                                 library=library / self.libraryScaleValue.value(),
    ...
    def _calculate_index(self):
        ...
        if sum(target_fractions) != 1:
            print('The target fractions should sum to 100.', file=sys.stderr)
            return
    ...
    def _set_filter_rmse(self, checked: bool = False):
        if checked:
            self._filter_rmse(value=self.filterRMSEValue.value())
        else:
            self._filter_rmse()
    ...
    def _set_filter_min_frac(self, checked: bool = False):
        if checked:
            self._filter_min_frac(value=self.filterMinFracValue.value())
        else:
            self._filter_min_frac()
    ...
    def _set_filter_max_frac(self, checked: bool = False):
        if checked:
            self._filter_max_frac(value=self.filterMaxFracValue.value())
        else:
            self._filter_max_frac()
    ...
    def _export(self):
        ...
        if not cres_selection:
            print("No spectra selected.", file=sys.stderr)
            return
        ...
        cres_library = self.spectral_library.speclibFromFeatureIDs(fid_list)
        cres_library.exportProfiles(path=output_path)
