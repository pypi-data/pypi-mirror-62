...
class BaseTestCheckout(BaseTestCase):
    ...
    def setUp(self):
        ...
        # Patch SMTP Lib
        smtplib_patcher = patch('smtplib.SMTP')
        PatchedSMTP = smtplib_patcher.start()
    ...
    def setup_defaults(self):
        ...
        # Add emails to party
        Party.write([registered_user.party], {
            'contact_mechanisms': [('create', [
                {'type': 'email', 'value': 'test@ol.in'},
            ])],
        })

        Party.write([registered_user2.party], {
            'contact_mechanisms': [('create', [
        ...
        Address.create([
            {
                ...
                'name': registered_user.party.name,
            }, {
                'party': registered_user2.party.id,
                'name': registered_user2.party.name,
            },
        ])

        uom, = Uom.search([('symbol', '=', 'cm')])
    ...
    def _create_pricelists(self):
        ...
        party_pl_margin = Decimal('1')
        guest_pl_margin = Decimal('1')
...
class TestCheckoutSignIn(BaseTestCheckout):
    ...
    def test_0010_check_cart(self):
        ...
        with app.test_client() as c:
            ...
            c.post(
                '/en/cart/add', data={
                    'product': template1.products[0].id, 'quantity': 5
                }
            )
            rv = c.get('/en/cart')
            self.assertEqual(rv.status_code, 200)
    ...
    def test_0020_guest_no_email(self):
        ...
        with app.test_client() as c:
            ...
            rv = c.post(
                '/en/checkout/sign-in', data={'checkout_mode': 'account'}
            )
    ...
    def test_0030_guest_valid(self):
        ...
        with app.test_client() as c:
            ...
            rv = c.post(
                '/en/checkout/sign-in', data={'email': 'new@m9s.biz'}
            )
            self.assertEqual(rv.status_code, 302)
...
class TestCheckoutShippingAddress(BaseTestCheckout):
    ...
    def test_0020_guest_adds_address(self):
        ...
        country = Country(available_countries[0])
...
class TestSale(BaseTestCheckout):
    ...
    def test_0010_test_sale_json_ld(self):
        ...
        with app.test_client() as c:
            login(c, 'email@example.com', 'password')
