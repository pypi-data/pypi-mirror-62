...
class Autolink(object):
    ...
    def auto_link_with_json(self, json_obj, options = {}):
        ...
        # map JSON entity to ttext entity
    ...
    def _link_to_text_with_symbol(self, entity, symbol, text, href, attributes = {}, options = {}):
        tagged_symbol = '<%s>%s</%s>' % (
            options.get('symbol_tag'),
            symbol,
            options.get('symbol_tag')
        ) if options.get('symbol_tag') else symbol
        text = self._html_escape(text)
        tagged_text = '<%s>%s</%s>' % (
            options.get('text_with_symbol_tag'),
            text,
            options.get('text_with_symbol_tag')
        ) if options.get('text_with_symbol_tag') else text
