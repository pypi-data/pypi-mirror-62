...
class ESA:
    ...
    def _process_document(self, document):
        ...
        timeit.Timer('document_vec, terms_document = self._to_vector(document, lemma)').timeit()
        document_vec, terms_document = self._to_vector(document, lemma)
        result = {}

        if self._sim == "cos":
            timeit.Timer('result_vector= Similarity.cosine_similarity(document_vec, self._mat_2)').timeit()
