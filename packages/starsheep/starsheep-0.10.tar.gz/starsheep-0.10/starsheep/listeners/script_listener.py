    ...
    def call_script(self, **kwargs):
        ...
        if self.context.models[model.get_model()].fields is not None:
            for field_name in self.context.models[model.get_model()].fields:
                variables['DINEMIC_CONTEXT_FIELD_' + field_name] = getattr(model, field_name).get()
        ...
        if self.context.models[model.get_model()].lists is not None:
            for list_name in self.context.models[model.get_model()].lists:
                for list_item in range(getattr(model, list_name).length()):
                    variables['DINEMIC_CONTEXT_LIST_' + list_name + '_' + str(list_item)] = getattr(model, list_name).at(list_item)
        ...
        if self.context.models[model.get_model()].dicts is not None:
            for dict_name in self.context.models[model.get_model()].dicts:
                for dict_item in getattr(model, dict_name).keys():
                    variables['DINEMIC_CONTEXT_DICT_' + dict_name + '_' + dict_item] = getattr(model, dict_name).get(dict_item, '')
        ...
        for obj_id in related_objects:
            ...
            if self.context.models[m.get_model()].fields is not None:
                ...
                for field_name in self.context.models[m.get_model()].fields:
                    variables['DINEMIC_RELATED_' + obj_id + '_FIELD_' + field_name] = getattr(m, field_name).get()
            ...
            if self.context.models[m.get_model()].lists is not None:
                ...
                for list_name in self.context.models[m.get_model()].lists:
                    for list_item in range(getattr(m, list_name).length()):
                        variables['DINEMIC_RELATED_' + obj_id + '_LIST_' + list_name + '_' + str(list_item)] = getattr(m, list_name).at(list_item)
            ...
            if self.context.models[m.get_model()].dicts is not None:
                ...
                for dict_name in self.context.models[m.get_model()].dicts:
                    for dict_item in getattr(m, dict_name).keys():
                        variables['DINEMIC_RELATED_' + obj_id + '_DICT_' + dict_name + '_' + dict_item] = getattr(m, dict_name).get(dict_item, '')
