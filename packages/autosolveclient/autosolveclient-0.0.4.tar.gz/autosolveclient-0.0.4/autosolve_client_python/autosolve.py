import pika, requests, threading, json, asyncio
...
class AutoSolve:
    ...
    def establish_rabbit_session(self):
        ...
        if valid == 401:
            message = "Invalid API or Access Key"
            self.log(message, self.ERROR)
            raise AuthenticationError(message)
        else:
    ...
    def check_input_values(self):
        if self.validate_inputs():
            ...
            message = "Input value for access_token is invalid or client_key/api_key are not set"
            self.log(message, self.ERROR)
            raise InputValueException(message)
...
def InputValueException(exception):
    print(exception)
