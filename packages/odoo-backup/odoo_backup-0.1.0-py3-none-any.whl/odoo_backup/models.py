...
class Server():
    ...
    def list_backups(self, database: str):
        ...
        if not database in self.bases:
            print(f"Database {self.name}:{database} doesn't exist in config file.")
            exit(0)
    ...
    def check(self):
        ...
        for base in self.bases:
            ...
            if not backups:
                print(f"\t\t{base} (no backup available)")
                break
            backups_datetimes, backups_sizes = list(zip(*backups))
            total_size = sum(backups_sizes)
            print(f"\t\t{base} (total size: {naturalsize(total_size)})")
            latest_id = backups_datetimes.index(max(backups_datetimes))
            print(f"\t\t\tLatest backup: {backups_datetimes[latest_id]} ({naturalsize(backups_sizes[latest_id])})")
            oldest_id = backups_datetimes.index(min(backups_datetimes))
            print(f"\t\t\tOldest backup: {backups_datetimes[oldest_id]} ({naturalsize(backups_sizes[oldest_id])})")

    def backup(self, name: str):
        if not name in self.bases:
            print(f"Database {self.name}:{name} doesn't exist in config file.")
            exit(0)
        print(f"Backup of {self.name}:{name} started")
        if not os.path.isdir(os.path.join(DATA_DIR, self.name)):
            os.mkdir(os.path.join(DATA_DIR, self.name))
        if not os.path.isdir(os.path.join(DATA_DIR, self.name, name)):
            os.mkdir(os.path.join(DATA_DIR, self.name, name))
        res = requests.post(
            f'http://{self.ip}:{self.port}/web/database/backup',
            data=dict(
        ...
        print(f"Backup succesfull at {now} (size: {naturalsize(len(res.content))})")
