...
if __name__ == '__main__':
    ...
    parser.add_argument('oas3spec_url', type=str,
                        help='URL to fetch the OpenAPI3 spec file from, e.g. https://dev.myapi.example/openapi.json')
        ...
        for method, methodvalues in pathvalues.items():
            ...
            if method == 'post':
                ...
                if 'requestBody' in methodvalues.keys() and 'parameters' in methodvalues.keys():
                    pathvars = methodvalues.get("parameters")
                    postvars = methodvalues.get("requestBody", {}).get("content", {}).get("application/json", {}).get("schema", {}).get(
                        "properties", {})
                    print("--------------------------------------------")
