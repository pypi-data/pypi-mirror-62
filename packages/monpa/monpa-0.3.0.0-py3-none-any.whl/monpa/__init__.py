...
def query_model(model, input_text_tokens, args):
    ...
    # word to ID
    input_text_ids = _in_tokenizer.convert_tokens_to_ids(input_text_tokens)
    all_input_ids = torch.tensor([input_text_ids], dtype=torch.long)
    all_attention_mask = torch.tensor([input_mask], dtype=torch.long)
    all_token_type_ids = torch.tensor([segment_ids], dtype=torch.long)

    predictions = model(all_input_ids, all_attention_mask, all_token_type_ids)
