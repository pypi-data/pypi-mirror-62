...
# Cell
def plot_missing(df: DataFrame, f:Callable[[DataFrame], DataFrame]=pd.isna,
    ...
    cols = df.columns
    n = len(cols)
    fig, ax = plt.subplots(figsize=(15,n*0.3))
    outliers = f(df).astype(int)
    cax = ax.matshow(outliers.T, interpolation=None, aspect='auto')
    #Color Bar
    cb = fig.colorbar(cax)
    cb.set_ticks(range(outliers.max().max()+1))
    cb.set_label('outlier values')
    #Matrix plot
...
# Cell
def comp(l:list, numbered=True):
    "Compose a list of outlier functions, return different values if `numbered=True`"
    def _inner(df):
        n = len(l)
        if numbered:
            return reduce(operator.add, [(i+1)*f(df).astype(int) for i, f in enumerate(l)])
        else:
            return reduce(operator.add, [f(df).astype(int) for f in l])
    return _inner
