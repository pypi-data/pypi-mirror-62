...
def _get_config(root_path='.', is_console=True, module_name=''):
    global __is_debug, __log_name, __is_console
    __is_console = is_console
    config = {
        'version': 1,
...
def log_init_config(module='unknown', root_path='.', monit_queue='', use_module_name='', sub_module='', is_dict=False):
    global __logger, __module, __monit_queue, __is_debug, __log_name, __project, __is_dict, __sub_module
    try:
...
def log_info(msg, **kwargs):
    global __logger, __is_dict, __is_console
    if __logger is None:
        return
    if __is_dict:
