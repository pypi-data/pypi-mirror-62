"""
  Implements utility methods used to determine information and run
  commands across different platforms.
"""
...
def cleanse_path(path, windows_long_path=True):
    ...
    # Get rid of extra/unnecessary /../ or /./ sections of the path
...
def copytree(src, dst, symlinks=False, update=False, mirror=False, permissions=False):
    if is_windows():
        robocopy(src, dst, update=update, mirror=mirror, permissions=permissions)
    else:
        rsync(src, dst, symlinks=symlinks, update=update, mirror=mirror, permissions=permissions)
...
def robocopy(src, dst, mirror=False, update=False, permissions=False):
    ...
    cmdline = 'robocopy "%s" "%s" %s %s /R:0 /MT /NP /NS /NC /NFL /NDL /NJH /NJS' % (src, dst, mode, upd)
    (ret_code, output, err) = spawn(cmdline, capture_output=True, raise_on_error=False)
    if ret_code > 3:
        raise Exception(
...
def rsync(src, dst, symlinks=False, mirror=False, update=False, permissions=False):
