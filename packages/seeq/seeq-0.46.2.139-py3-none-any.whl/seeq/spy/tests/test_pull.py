...
def test_omit_dates():
    ...
    assert 25 <= len(df) <= 35
    expected_start = pytz.utc.localize(pd.datetime.utcnow()) - pd.Timedelta(hours=1)
    expected_end = pytz.utc.localize(pd.datetime.utcnow())
    ...
    start = pd.datetime.now() - pd.Timedelta(hours=2)
    df = spy.pull(search_results, start=start, grid=None)

    assert 50 <= len(df) <= 70
    expected_start = pytz.utc.localize(pd.datetime.utcnow() - pd.Timedelta(hours=2))
    assert abs(pd.Timedelta(df.index[0].tz_convert('UTC') - expected_start).total_seconds()) < margin_in_seconds
    assert abs(pd.Timedelta(df.index[-1].tz_convert('UTC') - expected_end).total_seconds()) < margin_in_seconds
