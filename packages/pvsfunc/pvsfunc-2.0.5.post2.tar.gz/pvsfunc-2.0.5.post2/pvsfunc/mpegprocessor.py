        ...
        if self.fileid == "V_MPEG2":
            ...
            if self.fileext != "d2v":
                ...
                if not os.path.exists(d2v_path):
                    ...
                    mpg_path = os.path.join(
                        os.path.dirname(d2v_path),
                        "".join(c for c in unicodedata.normalize("NFD", os.path.splitext(os.path.basename(d2v_path))[0]) if unicodedata.category(c) != "Mn")
                    ) + ".mpg"
