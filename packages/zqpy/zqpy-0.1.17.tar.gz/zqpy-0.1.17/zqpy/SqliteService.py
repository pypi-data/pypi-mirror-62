...
class SqliteServiceClass(object):
    ...
    def UpdateData(self, tabName, key, value, fields, values):
        ...
        try:
            ...
            strValue = ''
            for index in range(0, len(fields)):
                strValue = strValue + ('%s=%s,' if index!=len(fields)-1 else '%s=%s')%(fields[index], values[index])   
    ...
    def IsExistsData(self, tabName, key, value):
        '''数据是否存在于表中'''
        try:
            if not self.cursor:
                return False
            selectData = self.SelectData(tabName, [], key, value)
