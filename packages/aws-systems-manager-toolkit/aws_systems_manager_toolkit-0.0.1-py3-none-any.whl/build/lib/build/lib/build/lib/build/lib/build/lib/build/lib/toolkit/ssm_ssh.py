...
def start_session(arg_list):
    ...
    if os.name == 'nt':
        ...
        executable = "{}\system32\WindowsPowerShell\{}1.0\powershell.exe".format(os.environ['SYSTEMROOT'],'v')
...
def get_destination(args):
    ...
    logger.debug(f"destination: {destination}  --  remote command: \"{remote}\"")
    return destination
...
def main():
    ...
    try:
        ...
        if destination:
            ...
            if instance: 
                vars(args[0])["params"] = args[0].params.replace(target, instance)
            else:
                quit(1)
