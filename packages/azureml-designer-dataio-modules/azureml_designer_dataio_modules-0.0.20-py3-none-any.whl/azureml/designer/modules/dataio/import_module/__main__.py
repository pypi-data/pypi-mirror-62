...
from azureml.dataprep.api.errorhandlers import UnexpectedError
...
from azureml.studio.internal.error import ErrorMapping, IncorrectAzureStorageOrKeyError, \
    CouldNotOpenFileError, CouldNotDownloadFileError, InvalidDatasetError, \
    IncorrectAzureMLDatastoreError, ErrorDatabaseConnectionError, ParameterParsingError
...
class ImportTabularDataModule:
    ...
    def get_datastore_type(workspace, datastore_name):
        ...
        try:
            ...
            logger.error(
                f"{get_time_as_string()} Exception in get_datastore_type(): {str(azureml_datastore_error)}.")
    ...
    def process_input_dataset_request_dto(cls, input_dataset_request_dto):
        ...
        try:
            ...
            logger.error(
                f"{get_time_as_string()} Process input dataset request dto error: {str(err)}")
            raise err
    ...
    def generate_dataflow(cls) -> 'Dataflow':
        ...
        try:
            ...
            request = cls.process_input_dataset_request_dto(
                cls.input_dataset_request_dto)
            cls.dataset = Dataset._client()._register_from_request(
                cls.workspace, request, as_pending=False, exist_ok=True, update_if_exists=True)
            data_definition = cls.dataset.get_definition()
            logger.debug(
                f'Dataflow for registered requestDto:\n{data_definition.to_json()}')
            logger.debug(
                f"{get_time_as_string()} Register input request dto is done and registered dataset id: "
                f"{cls.dataset.id}")
            return data_definition
        except HttpOperationError as http_error:
            error_message = str(http_error)
            if bool(re.search('Datastore(.*)Not found', error_message)):
                ErrorMapping.rethrow(
                    http_error, IncorrectAzureStorageOrKeyError(
                        request['dataPath']['datastoreName']))
            elif 'Requested file(s) does not exist or specified credential does not have access to read' \
                    in error_message:
                ErrorMapping.rethrow(
                    http_error, CouldNotOpenFileError(
                        file_name=request['dataPath']['relativePath']))
            elif 'Could not connect to specified database' in error_message:
                ErrorMapping.rethrow(
                    http_error, ErrorDatabaseConnectionError())
            elif 'The provided path is not valid or the files could not be accessed. Invalid path' in error_message:
                raise UserError(
                ...
                ErrorMapping.rethrow(
                    http_error, CouldNotDownloadFileError(
                        file_url=request['dataPath']['httpUrl']))
            else:
                ErrorMapping.rethrow(http_error, ValueError(
                    f"Unknown HttpOperationError '{error_message}'."))
        except Exception as err:
            logger.error(
                f"{get_time_as_string()} Exception in register from request: {str(err)}")
            raise err
        ...
        try:
            ...
            if data_flow is not None:
                if cls.datastore_type in ['AzureSqlDatabase', 'AzurePostgreSql']:
                    # call Dataflow.to_pandas_dataframe(extended_types=True)
                    data_frame = Dataflow.to_pandas_dataframe(
                        data_flow, extended_types=True)
                else:
                    data_frame = Dataflow.to_pandas_dataframe(data_flow)

                if data_frame is None:
                    ErrorMapping.throw(
                        InvalidDatasetError(
                            dataset1=cls.dataset.id))
                schema = DataFrameSchema.data_frame_to_dict(data_frame)
        except UnexpectedError as azureml_dataprep_unexpectederr:
            raise azureml_dataprep_unexpectederr
...
def main(args):
    ...
    logger.debug(f'Received argument --datastore_type: {datastore_type}.')
    logger.debug(
    ...
    logger.debug(f'Received argument --output_dir: {output_dir}.')
