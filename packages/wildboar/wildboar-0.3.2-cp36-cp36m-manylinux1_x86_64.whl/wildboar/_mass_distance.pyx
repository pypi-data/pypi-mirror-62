...
cdef void simple_z_norm(Shapelet s, double* out) nogil:
...
cdef int simple_fft(size_t offset,
                    size_t stride,
                    size_t length,
                    double* T,
                    size_t out_offset,
                    size_t out_stride, # out_length = length - 1
                    complex* out) nogil:
    cdef int n = length
    cdef int rank = 1
    cdef fftw_plan p
...
cdef int simple_ifft(size_t offset,
                     size_t stride,
                     size_t length,
                     complex* T,
                     size_t out_offset,
...
cpdef void test1(np.ndarray data, size_t ts):
    ...
    cdef size_t offset = ts * td.sample_stride + td.dim_stride
