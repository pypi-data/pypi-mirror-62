...
class TestACTypes(unittest.TestCase):
    ...
    def test_inherited_instantiation(self):
        ...
        self.assertIsInstance(self.ex1, Example1)
        self.assertIsInstance(self.ex2, Example2)
        self.assertNotIsInstance(self.ex1, Example2)
        self.assertNotIsInstance(self.ex2, Example1)
        self.assertIs(type(self.ex1), Example1)
        self.assertIs(type(self.ex2), Example2)
        self.assertIsNot(type(self.ex1), _ACBaseType)
        self.assertIsNot(type(self.ex2), _ACBaseType)

    def test_classinfo(self):
        self.assertIsNotNone(Example1.get_classinfo())
        self.assertEqual(len(Example1.get_classinfo().fields), 0)
        self.assertIsNotNone(Example2.get_classinfo())
        self.assertEqual(len(Example2.get_classinfo().fields), 3)
        self.assertEqual(len(Example2.get_classinfo().value_validators), 1)
        self.assertIsNot(self.ex3.get_classinfo(), self.ex2.get_classinfo())

    def test_fields(self):
        self.assertIs(Example2.get_classinfo().fields['type'], str)
        self.assertIs(self.ex2.get_classinfo().fields['name'], str)
        self.assertIs(Example2.get_classinfo().fields['example'], Optional[Example2])
        self.assertIsNot(Example2.get_classinfo().fields['example'], _ACBaseType)

        with self.assertRaises(KeyError):
            self.assertIs(Example2.get_classinfo().fields['nothing'], str)
        with self.assertRaises(KeyError):
            self.ex1.new_attribute = 15
        with self.assertRaises(TypeError):
            Example2("Sample Name")
    ...
    def test_complex_attributes(self):
        ...
        self.ex2.example = Example2(name='First', type=PROJECTMAP, example={'name': 'Second', 'type': LAYOUTBOOK})
    ...
    def test_failed_deserialization(self):
        with self.assertRaises(TypeError):
            Example2(**{'name': 'Only Name'})
        with self.assertRaises(TypeError):
            Example2(**{'type': PROJECTMAP})
        with self.assertRaises(TypeError):
            Example2(**{'name': 15, 'type': PROJECTMAP})
        with self.assertRaises(TypeError):
            Example2(**{'name': 'Name', 'type': {'a': 1}})
        with self.assertRaises(TypeError):
            Example2(**{'name': 'Name', 'type': PROJECTMAP, 'invalid': 15})
        with self.assertRaises(TypeError):
            Example2(**{'name': 'Name', 'type': PROJECTMAP, 'example': {'name': 'Name2'}})
        with self.assertRaises(TypeError):
            Example2(**{'name': 'Name', 'type': PROJECTMAP, 'example': {'name': 'Name', 'type': 'Invalid'}})
        with self.assertRaises(ValueError):
            Example2(**{'name': 'Name', 'type': 'Invalid'})

    def test_nested_lists(self):
        self.ex3 = Example3(guid='260D42E3-AA2E-4DAD-BFFA-F2B5021501CC',
                            numbers=[2, 4, 6, 8],
