...
import json
from quant_trade_framework.core.constant import Exchange, Interval
from quant_trade_framework.core.object import BarData, TickData
from quant_trade_framework.core.utility import get_file_path
...
def init_mysql(settings: dict):
    keys = {"mysql.database", "mysql.user", "mysql.password", "mysql.host", "mysql.port"}
...
def init_models(db: Database, driver: Driver):
    ...
    class DbLogData(ModelBase):
        ...
        blacktestid: str = BigAutoField()
        log_level: str = CharField()
        log_info: str = CharField()
        datetime: datetime = DateTimeField()
