...
@click.option('--sim_start', type=float, default=0,
              help='simulation step in seconds')
@click.option('--sim_step', type=float, default=0.1,
              help='simulation step in seconds')
@click.option('--duration', type=float, default=100,
              help='Duration of the simulation in seconds')
@click.option('--use_linear_gs', type=bool, default=True,
              help='Use linear gear swifting strategy to calculate gear limits')
@click.option('--use_cubic', type=bool, default=False,
              help='Calculate cubic splines based on speed/acceleration '
                   'points per gear')
@click.option('--plot', type=bool, default=False,
              help='plots driver model work flow')
def run_simulation(db, car_id, ds, v_start, gs, v_des, sim_start, sim_step,
                   duration, use_linear_gs, use_cubic, plot):
    """
