...
def upload(ctx, path, name, cluster_name, compress):
    ...
    if cluster_name:
        resume_command = """spell upload {} --name '{}'
                        --cluster-name '{}' {}""".format(compress_param, name, cluster_name, path)
    else:
    ...
    if compress:
        click.echo("Warning: Using compress for upload will require additional "
    ...
    t.start()
    try:
        for result in pool.imap_unordered(lambda x: client.upload_file(*x), uploads,
    ...
    def attempt_complete_upload(attempt_number=1):
        if attempt_number > 3:
            raise ExitException("Unable to complete upload after 3 attempts! Please contact Spell support for help")
