...
class Player:
    def __init__(self, command_filename, env=os.environ.copy(), speed=1):
        self.master_fd = None
        self.paused = False
        ...
        commands = open(command_filename, "r").read().split("\n")
        # Strip all blank lines except the last one.
        self.commands = list(filter(None, commands)) + [""]
...
def main():
    ...
    Player(args.command_file, speed=args.speed).play()
