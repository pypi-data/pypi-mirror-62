...
from .action import SchedulerAction

logger = logging.getLogger(__name__)


class SamplingScheduler(object):
    ...
    def __init__(self,
                 *actions: Union[str, callable, SchedulerAction],
                 time_deltas: List[float] = None,
                 sequence_name: str = None,
                 ):
        """
        Scheduler for direct inject sequence.
        ...
        if actions is not None:
            ...
            for action in actions:
                self.add_default_action(action)
    ...
    def actions(self) -> List[callable]:
        """list of actions which the scheduler will perform when triggered"""
    ...
    @property
    def next_time_point(self) -> ActionTimePoint:
    ...
    @classmethod
    def get_instance_by_uuid(cls, uuid: str) -> "SamplingScheduler":
        """
        Get a sampler instance by UUID

        :param uuid: unique id (expected to be uuid4)
        :return: sampling scheduler instance
        """
        for instance in cls.instances:
            if instance.uuid == uuid:
                return instance
        # todo consider supporting concatenated uuids
        raise KeyError(f'The uuid "{uuid}" does not match any SamplingScheduler instances.')
    ...
    def _sequence_loop(self):
        """
        Sequence loop that watches time points and triggers actions at the appropriate times
        """
        while True:
    ...
    def add_default_action(self,
                           action: Union[str, callable, SchedulerAction],
                           # location: int = None,  # todo add support for order specification (location)
                           ):
        """
        Adds a callable action to the list of actions

        :param action: action method to execute on triggering
        """
        # perform callable check
        ...
        if action in self._default_actions:
            raise ValueError(f'The action {action} is already in the list of actions')

        self._default_actions.append(
            SchedulerAction.register_action(action)
        )
        for tp in self._time_points:
            if tp.triggered is False:
                tp.add_action(action)

    def remove_default_action(self,
                              action: Union[str, callable, SchedulerAction],
                              ) -> callable:
        """
        Removes the specified action from the list of actions

        :param action: action to remove
        :return: removed action
        """
        action = SchedulerAction.register_action(action)
        if action not in self._default_actions:
            raise ValueError(f'The action {action} is not in the list of actions')
        for tp in self._time_points:
            if tp.triggered is False:
                tp.remove_action(action)
        return self._default_actions.remove(action)
    ...
    def get_time_point_return(self,
        ...
        if key is None:
            ...
            tp = ActionTimePoint.get_instance(key)
