...
def _selectedmany(current, selected="selected", comparator=operator.contains):
    """
...
class Widget(object):
    ...
    def __init__(self, **kwargs):
        ...
        self.attrs = dict(self.default_attrs, **kwargs)
...
class Checkbox(_Input):
    ...
    def __init__(self, *args, **kwargs):
        self.default_checked = kwargs.pop(
...
class IndexChoiceMapper(ChoiceMapper):
    ...
    def choice_map(self, choices):
        ...
        return dict(
            (ustr(ix), item) for ix, item in enumerate(build_list(choices))
        )
...
class SingleChoiceWidget(Widget):
    ...
    def configure(self, fieldname, raw, value, choices=None, required=True):
        ...
        return super(SingleChoiceWidget, self).configure(
            fieldname, raw, value, choices, required
        )
...
class Select(SingleChoiceWidget):
    ...
    def render_container(self, items, **kwargs):
        return Tag.select(
            items, name=self.fieldname, **dict(self.attrs, **kwargs)
        )
    ...
    def render_items(self, choices, **kwargs):
        ...
        for ix, v, l in choices:
            if ix is ChoiceMapper.OPTGROUP:
                ...
                yield Tag.option(l, value=ix, selected=selected(v))
...
class WidgetFirstMultWidget(MultiWidgetRenderingStrategy):
    ...
    def item(self, widget, labeltext, id):
        return Tag(widget, " ", self.label(labeltext, id), " ")

    def optgroup(self, choices, label):
        return Tag.fieldset(
            Tag.legend(label), self._strip_trailing_space(choices)
        )
...
class LabelFirstMultiWidget(WidgetFirstMultWidget):
    ...
    def item(self, widget, labeltext, id):
        return Tag(self.label([labeltext, " ", widget], id), " ")
...
class RadioGroup(SingleChoiceWidget):
    ...
    def __init__(self, *args, **kwargs):
        self.input_attrs = {}
        self.rendering_strategy = kwargs.pop(
    ...
    def configure(self, fieldname, raw, value, choices, required):
        ...
        return super(RadioGroup, self).configure(
            fieldname, raw, value, choices, False
        )

    def label_textfirst(widget, labeltext):
        return [labeltext, " ", widget]
...
class CheckboxGroup(RadioGroup, MultipleChoiceWidget):
    ...
    def configure(self, fieldname, raw, value, choices, required):
        ...
        """
        return super(CheckboxGroup, self).configure(
            fieldname, raw, value, choices, False
        )
...
class Textarea(Widget):
    def render(self, **kwargs):
        return Tag.textarea(
            ustr(self.raw), name=self.fieldname, **dict(self.attrs, **kwargs)
        )
