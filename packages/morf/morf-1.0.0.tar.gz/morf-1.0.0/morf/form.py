...
def cleans(*fieldnames, **kwargs):
    ...
    def _cleans(func, fieldnames=fieldnames):
        register_validator(func, fieldnames, is_cleaner=True, **kwargs)
        return func
...
class Form(fields.FieldBase):
    ...
    def __init__(self, raw=None, bind=None, **kwargs):
    ...
    def _initfields(self):
        ...
        fields = sorted(
            [(name, copy(f)) for name, f in fields],
    ...
    def replace_field(self, name, field):
    ...
    def _get_validators(self):
        ...
        """
        cls_field_to_name = dict(
            (v, k) for k, v in getmembers(self.__class__, _isfield)
        )
        vs = []
        for __, func in getmembers(self):
            for order, fieldnames, opts in getattr(func, "__validates__", []):
                fieldnames = tuple(
                    cls_field_to_name[f] if _isfield(f) else f
                    for f in fieldnames
                )
            ...
            try:
                ...
                if opts.is_cleaner:
                    ...
                    if fieldc == 0:
                        ...
                        assert (
                            isinstance(result, tuple) and len(result) == fieldc
                        ), (
                ...
                message = (
                    e.message
                    or (associated and associated.invalid_message)
                    or "validator '{0!r}' failed but did not "
