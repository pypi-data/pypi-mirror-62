...
class RenderOptions(object):
    ...
    field_template = (
        "{{ errors }}{{ label }}" '<div class="control">{{ control }}</div>'
    )
    errors_template = (
        '<ul class="errors">'
    ...
    control_template = "{{ widget }}"

    __compiled_templates = {}
    ...
    def convert_templates(self):
        for name, s in getmembers(self):
            if name.endswith("_template") and not isinstance(s, Template):
                try:
                    compiled = self.__compiled_templates[s]
                except KeyError:
                    compiled = self.jinja2_environ.from_string(s)
                    self.__compiled_templates[s] = compiled
...
class FieldRenderer(Renderer):
    ...
    def __init__(self, field, render_options, parent=None):
        ...
        self._widget.configure(
            ...
            raw=field.raw,
            value=field.value,
            required=(field.default is Undefined),
    ...
    def render_control(self, **kwargs):
        return Markup(
            self.render_options.control_template.render(
                field=self._field, widget=self.render_widget(**kwargs)
            )
        )

    def render_widget(self, **kwargs):
        return self._widget.render(id=self.get_id(), **kwargs).tohtml(
            self.render_options.dialect
        )

    def render_label(self):
        return Markup(
            self.render_options.label_template.render(
                id=self.get_id(), label=self._field.label, field=self._field
            )
        )
    ...
    def render_errors(self):
        ...
        return Markup(
            self.render_options.errors_template.render(
    ...
    def render(self, **kwargs):
        return Markup(
            self.render_options.field_template.render(
...
class FormRenderer(Renderer, CompoundRendererMixin):
    """
    Renderer for a :class:`~morf.form.Form` object.
    """
    ...
    def pop(self, fieldname, default=_marker):
        """
        Return the named field, removing it from the internal list of
        fields to render.
        """
    ...
    def visible(self):
        """
        Return a copy of the :class:`~morf.render.FormRenderer`
        object limited to displaying only the visible fields.
        """
        return self._fork(
            name for name, field in self._items() if field.is_visible
        )
    ...
    def hidden(self):
        ...
        """
        return self._fork(
            name for name, field in self._items() if not field.is_visible
        )
...
class ListRenderer(FieldRenderer, CompoundRendererMixin):
    ...
    def render_row(self, field, **kwargs):
        return Markup(
            self.render_options.row_template.render(
                field=field.render(**kwargs)
            )
        )
...
class HiddenRenderer(FieldRenderer):
    ...
    def render(self, **kwargs):
        return self.render_widget(**kwargs)
