...
class FieldBase(object):
    ...
    def __getstate__(self):
        ...
        return dict(
            (k, copy(v) if isinstance(v, mutable_types) else v)
    ...
    def _set_fieldname(self, name):
        ...
        self.displayname = coalesce(
            self.displayname, self.label, make_displayname(name)
        )
...
class Field(FieldBase):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.empty_message = coalesce(
            empty_message, message, self.empty_message
        )
        self.invalid_message = coalesce(
            invalid_message, message, self.invalid_message
        )
        ...
        if hidden:
            self.widget = widgets.HiddenJSON()
    ...
    def get_choices(self):
        ...
        if isinstance(self._choices, (ustr, str)):
            ...
            raise AttributeError(
                "%r has no attribute %r" % (self._parent, self._choices)
            )
        else:
...
class Constant(Field):
    ...
    def bind_input(self, raw, validate=True, _internal=False):
        return super(Constant, self).bind_input(
            self.value, validate=validate, _internal=False
        )
...
class ListOf(Field):
    ...
    def bind_input(self, raw, validate=True, _internal=False):
        ...
        if not iterable(raw):
            raw = [raw]
        for item in raw:
            if self.trim_empty and self.template_field.is_considered_empty(
                item
            ):
                continue
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self._choices is None:
            raise AssertionError(
                "%r must be initialized with a " "choices argument" % (self,)
            )
...
class MultipleChoice(Choice):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self._choices is None:
            raise AssertionError(
                "MultipleChoice must be initialized with a " "choices argument"
            )
