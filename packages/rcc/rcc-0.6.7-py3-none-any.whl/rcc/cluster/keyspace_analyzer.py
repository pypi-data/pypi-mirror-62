...
async def analyzeKeyspace(redisUrl: str, timeout: int):
    ...
    for node in nodes:
        ...
        conf = await client.config_get('notify-keyspace-events')
        if conf[1]:
            print(f'{node.ip}:{node.port} current keyspace config: {conf[1].decode()}')
            confs.append(conf[1].decode())

        # Set the new conf
        await client.config_set('notify-keyspace-events', keyspaceConfig)
    ...
    try:
        ...
        for node, conf in zip(nodes, confs):
            ...
            await client.config_set('notify-keyspace-events', conf)
