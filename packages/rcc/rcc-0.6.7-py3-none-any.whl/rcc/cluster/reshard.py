...
async def migrateSlot(masterNodes, slot, sourceNode, destinationNode, dry):
    ...
    if len(keys) > 0:
        ...
        await sourceClient.migrate(
            destinationNode.ip, destinationNode.port, '0', str(timeout), *keys
        )
    ...
    try:
        ...
        for node in masterNodes:
            ...
            await client.cluster_set_slot(slot, 'NODE', destinationNode.node_id)
    ...
    # breakpoint()
    # await runClusterCheck(sourceNode.port)  # FIXME hack
...
def binPackingReshard(redis_urls, path, dry=False, nodeId=None):
    ...
    asyncio.run(binPackingReshardCoroutine(redis_urls, weights, dry, nodeId))
