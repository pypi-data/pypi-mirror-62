...
#+PROPERTY: header-args:ipython :shebang "#!/usr/bin/env python" :session dewax
#+PROPERTY: header-args:R :shebang "#!/usr/bin/env R" :session dewax
#
                ...
                for m in modes:
                    for s in symmetrize:
                        for I in set_diags:
                            ...
                            dewaxer = dewakss.DEWAKSS(adata, iterations=1, init_diag=I, set_diag=(I if I==0 else None), run2best=True, denoise_type=denoiset, decay=d, mode=m, symmetrize=s, verbose=False)
...
performance_data.to_csv(os.path.join(datadir, f"dewakss_optimal_parameter_all_data_{subset}_dt_{denoisetypes[0]}_hvg_{str(hvg)}_m_{metric}.tsv.gz"), sep='\t', compression='gzip')
...
#+begin_src ipython
adata.write(os.path.join(datadir, f"Jackson2019_denoised_data_{subset}_{metric}.h5ad"))
#+end_src
...
scpipe.base_computations(adata, npcs=n_comps, nneighbors=10, recompute_pca=True, groupby='leiden', use_raw=False)
adata.layers['Ms'] = sc.pp.scale(adata.layers['Ms'].toarray(), copy=True, zero_center=False)
#+end_src

#+name: plot-jackson-marker-genes-denoised-recomputed
#+begin_src ipython :results output drawer replace
fig = plt.figure(figsize=(9,8), constrained_layout=True)
ax = fig.subplots(2, 2).flatten()
...
fdir = os.path.join(figdir, "clustering")
fname = f"markergenes_denoised_recomputed_PCA_{key1}_"
fnames = scpl.save_figure(fig, fdir, fname=fname, dpi=150)
print_file = "[[file:" + fnames[0] + "]]"
print(print_file, sep=",", end="")
print("")
...
#+name: load-cell-phase-genes
#+begin_src ipython
phasedata = pd.read_csv(os.path.join(datadir.replace('simplify-dewakss', 'master'), 'STable6.tsv'), sep='\t')
...
metric = 'euclidean'
# neigbours = [10, 20, 50, 100, 200, 500]
neigbours = [10, 15, 20, 30, 40, 50, 60, 70, 80, 90, 100, 110, 120, 130, 140, 150, 175, 200, 225, 300, 400]
