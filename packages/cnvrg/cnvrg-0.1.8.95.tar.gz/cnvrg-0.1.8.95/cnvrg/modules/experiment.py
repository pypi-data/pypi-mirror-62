...
class Experiment(CnvrgJob, ChartsMixin):
    ...
    def init(project=None, **kwargs):
        print("in init")
        print(env_helper.get_current_job_type())
        if env_helper.get_current_job_type() == env_helper.EXPERIMENT and env_helper.get_current_job_id():
            return Experiment()
        if isinstance(project, str): project = Project(project)
    ...
    @staticmethod
    def trun(command=None, arguments: dict=None, grid: dict=None, title: str=None, project: Project=None, compute: str=None, output_dir: str=None, computes: List=None,
            ...
            notify_on_error: bool=False, notify_on_success: bool=False, **kwargs):
        ...
        if local:
            ...
            e = project.run_task(command, arguments=arguments, title=title, templates=computes, datasets=datasets,
                                 ...
                                 notify_on_error=notify_on_error, notify_on_success=notify_on_success, **kwargs)
