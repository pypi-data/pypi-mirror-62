...
class Button(GLXCurses.Widget):
    ...
    def _draw_the_good_button(self, color):
        ...
        try:
            ...
            self.subwin.addstr(
                self._y_offset,
                self._x_offset + int(len(self.button_border) / 2),
                message_to_display,
                color
            )
        except curses.error:
        ...
        try:
            ...
            self.subwin.insstr(
                self._y_offset,
                int(self._x_offset + (len(self.button_border) / 2) + len(message_to_display)),
                self.button_border[-int(len(self.button_border) / 2):],
                color
            )
        except curses.error:
    ...
    def _handle_mouse_event(self, event_signal, event_args):
        if self.sensitive:
            ...
            x_pos_start = (self._get_x_offset()) + len(self.button_border) + len(self.get_text()) - 1
            ...
            if that_for_me:
                ...
                if event == curses.BUTTON1_PRESSED:
                    if isinstance(GLXCurses.Application().has_default, GLXCurses.GLXCChildElement):
                        GLXCurses.Application().has_default.widget._set_state_prelight(False)
    ...
    def _check_position_type(self):
        ...
        if self.get_position_type() == GLXCurses.GLXC.POS_CENTER:
            ...
            if self.height / 2 <= 1:
                ...
                self._set_y_offset(GLXCurses.clamp_to_zero(int((self.height) / 2)))
            ...
            if self.height - 1 <= 1:
                ...
                self._set_y_offset(GLXCurses.clamp_to_zero(self.height - 1))
