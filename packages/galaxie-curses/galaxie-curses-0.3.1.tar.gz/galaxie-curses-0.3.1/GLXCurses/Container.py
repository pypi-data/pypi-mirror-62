...
class Container(GLXCurses.Widget):
    ...
    def __init__(self):
        ...
        self.resize_mode = GLXCurses.GLXC.RESIZE_PARENT
    ...
    def add(self, widget=None):
        ...
        :type widget: Widget
        :raise TypeError: if ``widget`` is not a instance of :class:`Widget <GLXCurses.Widget.Widget>`
        """
        ...
        child = GLXCurses.GLXCChildElement()
        child.widget = widget
        child.name = widget.name
        child.type = widget.glxc_type
        child.id = widget.id
        child.properties.position = 0
    ...
    def add_with_properties(self, widget=None, properties=None):
        ...
        :type properties: ChildProperty
        :raise TypeError: if ``properties`` is not a ChildProperty instance
        :raise TypeError: if widget is not a instance of GLXCurses.Widget
        """
        # Try to exit as soon of possible
        if not isinstance(widget, GLXCurses.Widget):
            raise TypeError("'widget' must be an instance of GLXCurses.Widget")
        if not isinstance(properties, GLXCurses.GLXCChildProperty):
    ...
    def set_resize_mode(self, resize_mode=GLXCurses.GLXC.RESIZE_PARENT):
        """
        ...
        if resize_mode not in GLXCurses.GLXC.ResizeMode:
            raise TypeError("'resize_mode' must a GLXC.ResizeMode")

        # make the work just in case
            ...
            if hasattr(self, 'children'):
                if bool(self.children):
                    for child in self.children:
                        # print('Search in: ' + str(child.widget.__class__.__name__) + ' ' + str(child.widget.id))
                        for detailed_signal, handler in list(child.widget.get_events_list().items()):
                            if handler[0] == callback:
                                # print('Found: ' + str(detailed_signal) + ' in ' + str(handler[0]))
                                handler[0](*callback_data)
                        # print('')
        else:
            if hasattr(self, 'child'):
                if bool(self.child):
                    # print('Search in: ' + str(self.__class__.__name__) + ' ' + str(self.id))
                    for detailed_signal, handler in list(self.child.widget.get_events_list().items()):
                        if handler[0] == callback:
                            # print('Found: ' + str(detailed_signal) + ' in ' + str(handler[0]))
                            handler[0](*callback_data)
                    # print('')
        ...
        if self.__class__.__name__ in GLXCurses.GLXC.CHILDREN_CONTAINER:
            if bool(self.get_children()):
                ...
                for children in self.get_children():
                    ...
                    count += 1
                if last_found is not None:
                    if self.get_children()[last_found].widget.__class__.__name__ in GLXCurses.GLXC.CHILDREN_CONTAINER:
            ...
            if bool(self.child):
                if self.child.widget == child:
                    if self.child.widget.__class__.__name__ in GLXCurses.GLXC.CHILDREN_CONTAINER:
    ...
    def set_focus_vadjustment(self, adjustment=None):
        ...
        if adjustment is not None:
            adjustment_info = GLXCurses.GLXCChildElement()
        ...
        if self.__class__.__name__ in GLXCurses.GLXC.CHILDREN_CONTAINER:
            if bool(self.get_children()):
                ...
                for children in self.get_children():
                    ...
                    count += 1
                if last_found is not None:
                    self.get_children()[last_found].properties = properties
        ...
        if self.__class__.__name__ in GLXCurses.GLXC.CHILDREN_CONTAINER:
            if bool(self.get_children()):
                ...
                for children in self.get_children():
                    ...
                    count += 1
                if last_found is not None:
                    setattr(self.get_children()[last_found].properties, property_name, value)
        else:
            if self.child is not None:
                if self.child.widget == child:
    ...
    def child_get_property(self, child, property_name=None):
        ...
        if self.__class__.__name__ in GLXCurses.GLXC.CHILDREN_CONTAINER:
            if bool(self.get_children()):
                ...
                if last_found is not None:
                    try:
                        return getattr(self.get_children()[last_found].properties, property_name)
                    except KeyError:
