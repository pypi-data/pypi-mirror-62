...
class Widget(GLXCurses.Object, GLXCurses.Area):
    def __init__(self):
        """
    ...
    def focus_on_click(self, focus_on_click=False):
        ...
        :raise TypeError: if ``focus_on_click`` is not a bool
        """
    ...
    @has_default.setter
    def has_default(self, has_default=False):
        """
        ...
        if not isinstance(has_default, bool):
            raise TypeError("'has_default' must be a bool type")
    ...
    @has_focus.setter
    def has_focus(self, has_focus=False):
        """
    ...
    def parent(self, parent=None):
        ...
        if not isinstance(parent, GLXCurses.Container) and not isinstance(parent, GLXCurses.Application):
    ...
    @sensitive.setter
    def sensitive(self, sensitive=True):
        """
    ...
    def unparent(self, widget=None):
        ...
        if widget.parent is not None:
            if hasattr(widget.parent, 'get_children'):
                if bool(widget.parent.get_children()):
                    count = 0
                    last_found = None
                    for children in widget.parent.get_children():
                        if widget == children['widget']:
                            last_found = count
                        count += 1
                    if last_found is not None:
                        widget.parent.get_children().pop(last_found)

            if hasattr(widget.parent, 'get_child'):
                if bool(widget.parent.child()):
                    if widget.parent.child().widget == widget:
                        widget.parent.set_child(None)
    ...
    def show(self, widget=None):
        ...
        # make the job
        if widget.flags['TOPLEVEL']:
            if not widget.flags['REALIZED']:
                widget.flags['REALIZED'] = True
            if not widget.flags['MAPPED']:
                widget.flags['MAPPED'] = True

        if not widget.visible:
            widget.visible = True

    def show_now(self, widget=None):
        """
        Shows a widget.
    ...
    def hide(self, widget=None):
        ...
        # make the job
        if widget.flags['TOPLEVEL']:
            if widget.flags['REALIZED']:
                widget.flags['REALIZED'] = False
            if widget.flags['MAPPED']:
                widget.flags['MAPPED'] = False
    ...
    def map(self, widget=None):
        ...
        # make the job only if necessary
        if not widget.flags['MAPPED']:
            widget.flags['MAPPED'] = True

    def unmap(self, widget=None):
        """
        This function is only for use in widget implementations.
    ...
    def unrealize(self, widget=None):
        """
    ...
    def get_parent_size(self):
        if self.parent:
            return self.parent.height, self.parent.width
        else:
            return self.height, self.width
        # return self.parent.curses_subwin.getmaxyx()

    def get_parent_origin(self):
        if self.parent:
            return self.parent.y, self.parent.x
        else:
            return self.y, self.x
        # return self.get_parent_origin()
        # return self.parent.curses_subwin.getbegyx()

    def adopt(self, orphan=None):
        if orphan is None:
            pass
    ...
    def override_color(self, color):
        self.style.get_attribute_states()['text']['STATE_NORMAL'] = str(color).upper()

    def override_background_color(self, color):
        self.style.get_attribute_states()['bg']['STATE_NORMAL'] = str(color).upper()
    ...
    def set_preferred_size(self, value):
        self.preferred_size = value

    def get_size(self):
        return self.subwin.getmaxyx()
    ...
    def set_width_request(self, width):
        if self.__width_request != width:
            self.__width_request = width
    ...
    def _draw_box(self):
        ...
        if self.subwin:
            ...
            # For future usage
            # self._draw_box_left_side(char='.')
            # self._draw_box_top(char='.')
            # self._draw_box_bottom(char='.')
            # self._draw_box_right_side(char='.')
            #
            # self._draw_box_upper_left_corner(char='.')
            # self._draw_box_upper_right_corner(char='.')
            # self._draw_box_bottom_left_corner(char='.')
            # self._draw_box_bottom_right_corner(char='.')
    ...
    def _draw_box_bottom(self, char=None):
        ...
        try:
            for x_inc in range(0 + 1, self.width - 1):
                self.subwin.delch(self.height - 1, x_inc)
                self.subwin.insch(
                    self.height - 1,
                    x_inc,
                    char,
    ...
    def _draw_box_right_side(self, char=None):
        ...
        for y_inc in range(0 + 1, self.height):

            try:
                self.subwin.delch(y_inc, self.width - 1)
                self.subwin.insch(
                    y_inc,
                    self.width - 1,
                    char,
    ...
    def _draw_box_upper_right_corner(self, char=None):
        ...
        try:
            self.subwin.delch(0, self.width - 1)
            self.subwin.insch(
                0,
                self.width - 1,
                char,
    ...
    def _draw_box_bottom_right_corner(self, char=None):
        ...
        try:
            self.subwin.delch(self.height - 1, self.width - 1)
            self.subwin.insch(
                self.height - 1,
                self.width - 1,
                char,
    ...
    def _draw_box_bottom_left_corner(self, char=None):
        ...
        try:
            self.subwin.delch(self.height - 1, 0)
            self.subwin.insch(
                self.height - 1,
                0,
                char,
