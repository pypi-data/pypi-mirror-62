...
class VuMeter(GLXCurses.Widget):
    def __init__(self):
        # Load heritage
    ...
    def draw_vumeter(self):
        ...
        max_log = 20 * np.log10(self.get_scale_max())

        allowed_height = int(self.preferred_height - (self.border_len / 2))

        scale_list_reverse = np.linspace(max_log, self.get_scale_min(), allowed_height)
        scale_list = np.linspace(self.get_scale_min(), max_log, allowed_height)
        ...
        try:
            ...
            if self.get_display_scale():
                ...
                for i in range(0, allowed_height, 3):
                    ...
                    value = np.negative(20 * np.log10(scale_list_reverse[i]))
                    if np.isinf(value):
            ...
            for i in range(0, allowed_height, 1):
                # self.vumeter_curses_subwin.addstr(
                #     allowed_height - i - height_de_merde,
                #     clamp_to_zero(int(self.border_len / 2) + space_for_scale),
                ...
                # )
                color = 'GREEN'
                if scale_list[i] <= 20 * np.log10(self.get_value()):
