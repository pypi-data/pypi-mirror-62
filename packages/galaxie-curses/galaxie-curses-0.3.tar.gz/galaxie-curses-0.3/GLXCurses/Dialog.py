...
class Dialog(GLXCurses.Window):
    ...
    @_x_offset.setter
    def _x_offset(self, _x_offset=None):
        if _x_offset is None:
            _x_offset = 0
        if type(_x_offset) != int:
            raise TypeError('"x_offset" must be int type')
        if self._x_offset != _x_offset:
            self.__x_offset = _x_offset
    ...
    @_y_offset.setter
    def _y_offset(self, _y_offset=None):
        if _y_offset is None:
            _y_offset = 0
        if type(_y_offset) != int:
            raise TypeError('"y_offset" must be int type')
        if self._y_offset != _y_offset:
            self.__y_offset = _y_offset
        ...
        if isinstance(GLXCurses.Application().active_window.widget, GLXCurses.Dialog):
            if len(GLXCurses.Application().children) > 1:
                if GLXCurses.Application().children[1]:
                    GLXCurses.Application().active_window_id = GLXCurses.Application().children[1].id
    ...
    def _update_preferred_position(self):
        self._y_offset = 0
        self._y_offset += int(self.height / 2)
        self._y_offset -= int(self.preferred_height / 2)

        self._x_offset = 0
        self._x_offset += int(self.width / 2)
        self._x_offset -= int(self.preferred_width / 2)
    ...
    def _get_estimated_preferred_width(self):
        ...
        if len(self._content_area.get_children()) > 0:
    ...
    def draw_widget_in_area(self):
        ...
        if self.subwin:
            ...
            if self.child is not None:
                ...
                self.child.widget.style.get_attribute_states()['text']['STATE_NORMAL'] = 'BLACK'
                self.child.widget.style.get_attribute_states()['bg']['STATE_NORMAL'] = 'WHITE'

                if self.decorated:
                    self.child.widget.x = self._x_offset + 2
                    self.child.widget.y = self._y_offset + 2
            ...
            if len(self._action_area) > 0:
                if self._y_offset + self.preferred_height + 2 - 3 <= self.height:
                    self._draw_action_area()

            if self.title:
                if self._y_offset + 1 >= 0:
                    self._draw_title()
    ...
    def _draw_action_area(self):
        ...
        if self._action_area:
            ...
            for button in self._action_area:
                ...
                button.widget.style = GLXCurses.Style()
                button.widget.style.get_attribute_states()['text']['STATE_NORMAL'] = 'BLACK'
                button.widget.style.get_attribute_states()['bg']['STATE_NORMAL'] = 'WHITE'
                if button.widget.has_default is True:
                    button.widget.x = cursor_position
                    button.widget.y = self._y_offset + self.preferred_height - 3
            ...
            for button in self._action_area:
                if isinstance(GLXCurses.Application().has_default, GLXCurses.Child):
    ...
    def _draw_background(self):
        for y_inc in range(self._y_offset, self._y_offset + self.preferred_height):
            for x_inc in range(self._x_offset, self._x_offset + self.preferred_width):
                try:
    ...
    def _draw_title(self):
        if self.width > 1:
            ...
            for x_inc in range(0, len(title_with_spacing)):
                try:
                    self.subwin.delch(self._y_offset + 1, pos + x_inc)
                    ...
                    self.subwin.insch(
                        ...
                        self.style.get_color_pair(
                            foreground=self.style.get_color_text('text_fg', 'STATE_SELECTED'),
                            background=self.style.get_color_text('base', 'STATE_NORMAL')
                        )
    ...
    def _draw_box_bottom(self, char=None):
        ...
        for x_inc in range(self._x_offset + 2, self._x_offset + self.preferred_width - 1):
            try:
                self.subwin.delch(self._y_offset + self.preferred_height - 2, x_inc)
                self.subwin.insch(
                    self._y_offset + self.preferred_height - 2,
                    x_inc,
                    char,
                    self.style.get_color_pair(
                        foreground=self.style.get_color_text('dark', 'STATE_NORMAL'),
    ...
    def _draw_box_top(self, char=None):
        ...
        for x_inc in range(self._x_offset + 2, self._x_offset + self.preferred_width - 2):
            try:
                self.subwin.delch(self._y_offset + 1, x_inc)
                self.subwin.insch(
                    self._y_offset + 1,
                    x_inc,
                    char,
    ...
    def _draw_box_right_side(self, char=None):
        ...
        for y_inc in range(0 + 2, self.preferred_height - 2):
            try:
                self.subwin.delch(self._y_offset + y_inc, self._x_offset + self.preferred_width - 2)
                self.subwin.insch(
                    self._y_offset + y_inc,
                    self._x_offset + self.preferred_width - 2,
                    char,
                    self.style.get_color_pair(
                        foreground=self.style.get_color_text('dark', 'STATE_NORMAL'),
                        background=self.style.get_color_text('base', 'STATE_NORMAL')
    ...
    def _draw_box_left_side(self, char=None):
        ...
        for y_inc in range(0 + 2, self.preferred_height - 2):
            try:
                self.subwin.delch(self._y_offset + y_inc, self._x_offset + 1)
                self.subwin.insch(
                    self._y_offset + y_inc,
                    self._x_offset + 1,
                    char,
    ...
    def _draw_box_upper_right_corner(self, char=None):
        ...
        try:
            self.subwin.delch(self._y_offset + 1, self.preferred_width + self._x_offset - 2)
            self.subwin.insch(
                self._y_offset + 1,
                self.preferred_width + self._x_offset - 2,
                char,
                self.style.get_color_pair(
                    foreground=self.style.get_color_text('dark', 'STATE_NORMAL'),
                    background=self.style.get_color_text('base', 'STATE_NORMAL')
    ...
    def _draw_box_upper_left_corner(self, char=None):
        ...
        try:
            self.subwin.delch(self._y_offset + 1, self._x_offset + 1)
            self.subwin.insch(
                self._y_offset + 1,
                self._x_offset + 1,
                char,
    ...
    def _draw_box_bottom_right_corner(self, char=None):
        ...
        try:
            self.subwin.delch(self._y_offset + self.preferred_height - 2, self._x_offset + self.preferred_width - 2)
            self.subwin.insch(
                self._y_offset + self.preferred_height - 2,
                self._x_offset + self.preferred_width - 2,
                char,
    ...
    def _draw_box_bottom_left_corner(self, char=None):
        ...
        try:
            self.subwin.delch(self._y_offset + self.preferred_height - 2, self._x_offset + 1)
            self.subwin.insch(
                self._y_offset + self.preferred_height - 2,
                self._x_offset + 1,
                char,
    ...
    def _draw_box_tee_pointing_right(self, char=None):
        ...
        try:
            self.subwin.delch(self._y_offset + self.preferred_height - 4, self._x_offset + 1)
            self.subwin.insch(
                self._y_offset + self.preferred_height - 4,
                self._x_offset + 1,
                char,
                self.style.get_color_pair(
                    foreground=self.style.get_color_text('dark', 'STATE_NORMAL'),
    ...
    def _draw_box_tee_pointing_left(self, char=None):
        ...
        try:
            self.subwin.delch(self._y_offset + self.preferred_height - 4, self._x_offset + self.preferred_width - 2)
            self.subwin.insch(
                self._y_offset + self.preferred_height - 4,
                self._x_offset + self.preferred_width - 2,
                char,
    ...
    def _draw_box_bottom_tee(self, char=None):
        ...
        for x_inc in range(self._x_offset + 2, self._x_offset + self.preferred_width - 2):
            try:
                self.subwin.delch(self._y_offset + self.preferred_height - 4, x_inc)
                self.subwin.insch(
                    self._y_offset + self.preferred_height - 4,
                    x_inc,
                    curses.ACS_HLINE,
