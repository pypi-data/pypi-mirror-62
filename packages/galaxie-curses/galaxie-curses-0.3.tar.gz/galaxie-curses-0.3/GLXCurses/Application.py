...
class Application(EventBus, GLXCurses.Area, GLXCurses.ApplicationMonitor, metaclass=Singleton):
    """
    :Description:
    ...
    def __init__(self):
        ...
        GLXCurses.ApplicationMonitor.__init__(self)
        # Hidden vars
        self.__style = None
        self.__parent = None
    ...
    def active_window(self, child=None):
        if child.widget is not None and not isinstance(child.widget, GLXCurses.Window):
            raise TypeError("'child' is not a GLXCurses.Window or None type")

        if child.id != self.active_window_id:
            self.active_window_id = child.id
    ...
    def active_window_id(self, glxcid=None):
        if glxcid is not None and type(glxcid) != str:
            raise TypeError('"glxcid" must be a str or None')
        if self.active_window_id != glxcid:
            self.__active_window_id = glxcid
    ...
    def add_window(self, window):
        ...
        child_to_add.widget = window
        child_to_add.type = window.glxc_type
        child_to_add.id = window.id
    ...
    def refresh(self):
        ...
        # if self.height > 1 and self.active_window:
        #     if isinstance(self.active_window, GLXCurses.Dialog):
        #         if len(self.children) >= 2:
        #             if self.children[1]:
        #                 self.children[1].widget.draw()
        #     else:
        ...
        if self.active_window:
            if isinstance(self.active_window.widget, GLXCurses.Dialog):
                if len(self.children) > 1:
                    if self.children[1]:
                        self.children[1].widget.x = self.x
                        self.children[1].widget.y = self.y
                        self.children[1].widget.width = self.width
                        self.children[1].widget.height = self.height
                        self.children[1].widget.draw()
    ...
    def draw(self):
        """
        Special code for rendering to the stdscr, but the Application have nothing to draw for it self.

        The role of the application is to permit Widget to be draw then Application impose sizes setting to children.
        """
        self._check_sizes()

        # Draw a background but that is not require
        # if self.subwin:
        #     self._draw_background()

        # Yes the application draw nothing by it self, that is the job of Widget's
        # If the application have to draw something by it self that is here :)
    ...
    # Internal
    def _set_area_sizes(self, parent_height=0, parent_width=0):
        ...
        if self.subwin is None:
            ...
            if self.width != subwin_width or self.height != subwin_height:
                try:
                    self.subwin.resize(self.height, self.width)
                except curses.error:
                    pass

            if self.y != curses_derwin_y or self.x != curses_derwin_x:
                try:
    ...
    def _draw_background(self):
        ...
        # for line in range(self.y, self.height - 1):
        #     for char in range(self.x, self.width - 1):
        #         try:
        #             self.stdscr.addch(
        #                 line,
        #                 char,
    ...
    def _check_sizes(self):
        ...
        self._set_area_sizes(parent_height=screen_height, parent_width=screen_width)
