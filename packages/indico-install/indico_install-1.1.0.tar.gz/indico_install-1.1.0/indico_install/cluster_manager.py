...
def get_services_yaml_from_tag(updraft_tag, apps_only=True):
    ...
    out = run_cmd(f"wget -qO- {remote_services_yaml}", silent=True)
...
class ClusterManager(object):
    ...
    def __init__(self, reconcile=False, input_yaml=None):
        ...
        if not self.cm_exists:
            if not input_yaml:
                secho(
                    f"Input (cluster) yaml not provided to initialize Cluster Manager",
                    fg="yellow",
                )
                return
            ...
            secho(f"Ignoring input yaml. Manager exists", fg="yellow")
    ...
    def clean_services(self):
        ...
        if "services" in self.cluster_config:
            services = {k: v for k, v in deepcopy(self.cluster_config["services"]).items() if v.get("values", {}).get("image")}
            unneeded = []
            for svc, val in self.cluster_config["services"].items():
                if "image" not in val.get("values", {}):
                    continue
                del val["values"]["image"]
                val["values"].pop("updated_at", None)
                val["values"].pop("updated_by", None)
        ...
        if "hash" in self.cluster_config["frontend"]:
            updates["frontend"] = {"hash": self.cluster_config["frontend"].pop("hash")}
        return updates

    def load_from_cluster(self, backup=False):
        """
        Use the version configmap on the cluster to populate this Tracker instance
        """
        conf = TRACKER_CONFIGMAP_BACKUP if backup else TRACKER_CONFIGMAP
    ...
    def edit_cluster_config(self, changes=None, version=None, replace=False):
        ...
        """
        if changes:
            self.cluster_config = changes if replace else merge_dicts(self.cluster_config, changes)
        if version:
    ...
    def lock(self, retries=15):
        ...
        run_cmd(f"kubectl label cm {TRACKER_CONFIGMAP} lock={self._lock}", silent=True)
        if needed_to_wait:
            self.load_from_cluster()
        return self._lock
    ...
    def to_configmap(self, backup=False):
        return {
            ...
            "kind": "ConfigMap",
            "metadata": {
                "name": TRACKER_CONFIGMAP_BACKUP if backup else TRACKER_CONFIGMAP,
            },
