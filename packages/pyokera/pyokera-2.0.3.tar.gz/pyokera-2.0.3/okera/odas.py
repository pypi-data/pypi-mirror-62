...
from okera._thrift_api import (
    TAssignAttributesParams, TAttribute, TAttributeValue, TCreateAttributesParams,
    TDeleteAttributesParams, TGetAttributesParams, TGetDatabasesParams,
    TGetDatasetsParams, TGetAttributeNamespacesParams, TGetPartitionsParams,
    TGetRegisteredObjectsParams, TGetTablesParams, TNetworkAddress, TPlanRequestParams,
    TRequestType, TExecDDLParams, TExecTaskParams, TFetchParams, TListFilesOp,
    TListFilesParams, TRecordServiceException, TRecordFormat, TTypeId,
...
class OkeraContext():
    def __init__(self, application_name, namespace, tz=pytz.utc, retries=3,
            initial_retry_delay_ms=2000, dialect=None):
...
class PlannerConnection():
    ...
    def create_attribute(self, namespace, key, if_not_exists=False):
        ...
        try:
            ...
            if ex.detail and 'duplicate key value' in ex.detail:
                if if_not_exists:
                    # TODO: this should be added to thrift API and done in the server
                    return
                else:
                    raise RuntimeError(
                        "Attribute %s.%s already exists." % (namespace, key))
            raise
    ...
    def __set_attribute(self, is_assign, namespace, key, db, dataset=None, column=None,
                        if_existence=False):
        attr = TAttribute()
    ...
    def assign_attribute(self, namespace, key, db, dataset=None, column=None,
        ...
        self.__set_attribute(True, namespace, key, db, dataset, column, if_not_exists)
    ...
    def unassign_attribute(self, namespace, key, db, dataset=None, column=None,
        ...
        self.__set_attribute(False, namespace, key, db, dataset, column, if_not_exists)
    ...
    def scan_as_pandas(self,
        ...
        if chosen_dialect == DIALECT_OKERA:
            ...
            results, colnames = self.__execute_presto_query(request, max_records)
    ...
    def scan_as_json(self,
        ...
        if chosen_dialect == DIALECT_OKERA:
            ...
            results, colnames = self.__execute_presto_query(request, max_records, to_dict=True)
            res = results
    ...
    def __execute_presto_query(self, query, max_records, to_dict=False):
        ...
        try:
            conn = prestodb.dbapi.connect(
                ...
                http_scheme="https",
                auth=prestodb.auth.BasicAuthentication(username, token),
...
class WorkerConnection():
    ...
    def exec_task(
            self, task,
            max_records=None, output_format=TRecordFormat.ColumnarNumPy):
...
def _convert_col_native(
    ...
    if schema[col_idx].type.type_id == TTypeId.STRING or \
        ...
        # For arrays, we skip however many olumns we skipped recursively,
        ...
        # Simlar to the array case, we need to get the key and value
        ...
        # For maps, we skip however many olumns we skipped recursively,
...
def _columnar_batch_to_python(schema, columnar_records, num_records,
    ...
    while col_idx < len(schema_cols):
        ...
        if output_format == TRecordFormat.ColumnarNumPy:
            ...
            # As part of processing complex types, we may have processed some extra columns,
  ...
  def __fixup_datum(datum):
    if isinstance(datum, datetime.datetime):
        ...
        for key, value in datum.items():
            datum[key] = JsonScanTask.__fixup_datum(value)
        return datum
    elif isinstance(datum, list):
        return [JsonScanTask.__fixup_datum(child) for child in datum]
    else:
        return datum.decode('utf-8') if isinstance(datum, bytes) else datum
    ...
    for r in range(0, num_records):
        ...
        for c in range(0, num_cols):
            if is_nulls[c][r]:
                continue
            datum = data[c][r]
            row[c] = JsonScanTask.__fixup_datum(datum)
