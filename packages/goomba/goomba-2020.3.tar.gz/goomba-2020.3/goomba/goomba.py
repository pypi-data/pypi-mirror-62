...
import os, sys, glob, time, operator, uuid, pickle, gzip, bz2, subprocess, datetime, random
...
def write_object(obj, path, compression="infer",  protocol=pickle.HIGHEST_PROTOCOL, *args):
    """
    Extensions:
    pickle ==> .pkl
    dill ==> .dill
...
def read_object(path, compression="infer"):
    path = format_path(path, str)
