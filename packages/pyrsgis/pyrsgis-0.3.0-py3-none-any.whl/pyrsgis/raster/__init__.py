...
def export(band, ds, filename='pyrsgis_outFile.tif', dtype='int', bands=1):
    ...
    if dtype == 'float':
        outdata = driver.Create(filename, col, row, nBands, raster_dtype['float32']) # option: GDT_UInt16, GDT_Float32
    elif dtype == 'int':
        outdata = driver.Create(filename, col, row, nBands, raster_dtype['byte']) # option: GDT_UInt16, GDT_Float32
    else:
