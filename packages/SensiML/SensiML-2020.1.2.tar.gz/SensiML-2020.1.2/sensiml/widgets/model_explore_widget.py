        ...
        if self.kp is None:
            ...
            for capture in self._widget_capture_data.get_selected_df()[name].values:
                ...
                if os.path.exists(results_file_path):
                    ...
                    self._widget_render_space.value = "The cache is cleaned for the model: " + self.kp.name + " and the file: "  + capture
                    
                else:
                    self._widget_render_space.value = "There is no cached data for the model: " + self.kp.name + " and the file: "  + capture
                
            if capture is None:
                self._widget_render_space.value = "Please select the file to clean the cache." 
    ...
    def _recognize_signal(self, capture_file, recognize=True):
        ...
        if self.kp.knowledgepack_description:
            ...
            ytick_labels = ["Unknown"] + [
                self.kp.class_map[x] for x in sorted(self.kp.class_map)
            ]
