...
class CppRunner(Runner):
    ...
    def test_functionality(self, config):
        ...
        util.execute([self.executable, output_file], self.tmp_dir, timeout=EXECUTABLE_TIMEOUT)
        try:
    ...
    def test_executable_functionality(self, config):
        ...
        output_file = os.path.join(self.tmp_dir, "result.json")
        util.execute([PY_RUNNER, self.tester_path, self.executable, output_file, get_eval_util_path()], self.tmp_dir, timeout=EXECUTABLE_TIMEOUT)
    ...
    def static_code_analysis(self, config):
        ...
        _, _, stderr = util.execute(['cppcheck', self.tested_path], self.tmp_dir)
    ...
    def _compile_in_tmp(self, files, pedantic=False, link=True, output=None):
        ...
        _, _, stderr = util.execute([param for param in run_params if param != ""], self.tmp_dir)
