...
class OCache:
    ...
    def add(self, path):
        ...
        self._files[path.stem] = Path(compile_cpp_to_o(path.absolute().as_posix(), cached_path, working_dir=path.parent.absolute().as_posix()))
...
class CppRunner(Runner):
    def __init__(self, tmp_files, run_args={}):
        ...
        self._test_lookup = {
            "compile": self.test_compile,
            "functionality": self.test_functionality,
            "functionality_executable": self.test_executable_functionality,
            "static": self.static_code_analysis,
            "comments": self.test_comments,
            "style": self.test_code_style
        }
    ...
    def test_functionality(self, config):
        ...
        _, stdout, stderr = util.execute([executable, output_file], self.tmp_dir, timeout=EXECUTABLE_TIMEOUT)
    ...
    def test_executable_functionality(self, config):
        ...
        output_file = os.path.join(self.tmp_dir, "result.json")
        _, stdout, stderr = util.execute([PY_RUNNER, config.tester_file.absolute().as_posix(), executable, output_file, get_eval_util_path()], self.tmp_dir, timeout=EXECUTABLE_TIMEOUT)
        if self.verbose: print(f"stdout:\n{stdout}\nstderr:\n{stderr}")
    ...
    def _get_executable_standalone(self, tested_path):
        ...
        if tested_path in self.compiled_o_files:
            ...
            if not compiled:
                if self.verbose: print(f"Failed to compile {tested_path}\n{output}")
                return None
            self.compiled_o_files[tested_path] = tested_o
        
        compiled_executable = os.path.join(self.tmp_dir, f"{Path(self.tested_path).stem}.{EXECUTABLE_EXTENSION}")
        output, compiled = self._compile_in_tmp([tested_o], output=compiled_executable)
        if not compiled:
            if self.verbose: print(f"Failed to link {tested_o}\n{output}")
            return None
    ...
    def _get_executable_tester(self, tested_path, tester_path):
        ...
        compiled_executable = os.path.join(self.tmp_dir, f"{Path(tested_path).stem}_{Path(tester_path).stem}.{EXECUTABLE_EXTENSION}")
        ...
        if not compiled:
            if self.verbose: print(f"Failed to link {tested_o, tester_path, eval_util_o}\n{output}")
            return None
