...
def _make_beam_pipeline_args(json_beam_pipeline_args      )              :
...
def _sanitize_underscore(name      )        :
  """Sanitize the underscore in pythonic name for markdown visualization."""
  return name.replace('_', '\\_')


def _render_channel_as_mdstr(input_channel                 )        :
...
def _render_artifact_as_mdstr(single_artifact                   )        :
...
def _dump_ui_metadata(component                              ,
                      execution_info                          )        :
