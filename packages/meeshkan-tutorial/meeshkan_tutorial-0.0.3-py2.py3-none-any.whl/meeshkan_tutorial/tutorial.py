...
def make_pokemon_request(path):
    req = request.Request('https://pokeapi.co%s' % path, headers={
        'user-agent': 'python'
    })
...
for x, path in enumerate(PATHS):
    ...
    req, res = make_pokemon_request(path)
    writer.write(HttpExchange(
        request=RequestBuilder.from_urllib_request(req),
        response=ResponseBuilder.from_http_client_response(res))
    )

sink.seek(0)
with open('__meeshkan__/recordings.jsonl', 'w') as pokefile:
    pokefile.write(''.join([x for x in sink]))
sink.close()
'''
...
def starting_server():
    bar = FillingCirclesBar('Starting server', max=50)
    for i in range(50):
        time.sleep(0.1)
        bar.next()
    bar.finish()
...
def closing_server():
    bar = FillingCirclesBar('Closing server', max=30)
    for i in range(30):
        time.sleep(0.1)
        bar.next()
    bar.finish()
...
def cli():
    ...
    m_print("##############################")
    m_print("")
    m_print("First, let's record a bit of server traffic. We've written a file to `__meeshkan__/api_calls.py` to make our recordings.  Meeshkan expects recordings to be in the http-types format, so we'll use that.")
    m_print("")
    m_print("Open up `__meeshkan__api_calls.py`. You'll see that we call the API 33 times using Meeshkan as a forward proxy.")
    m_print("")
    if not os.path.exists("__meeshkan__"):
        os.mkdir("__meeshkan__")
    with open('__meeshkan__/api_calls.py', 'w') as fi:
        fi.write(API_CALLS)
    m_input("After you've checked out `__meeshkan__/api_calls.py`, press ENTER to execute the script!")
    m_print("")
    m_print("##############################")
    m_print("")
    ...
    m_print("")
    subprocess.call("meeshkan build -i __meeshkan__/recordings.jsonl -o __meeshkan__/replay -m replay", shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    building()
    m_print("")
    ...
    m_print("")
    m_input("Now, let's use this spec to create a server that serves back our recordings.  Press ENTER to boot up the mock server.")
    m_print("")
    p = subprocess.Popen("meeshkan mock -s __meeshkan__/replay -r".split(" "), stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    starting_server()
    m_print("")
    m_print("##############################")
    m_print("")
    ...
    m_print("..............................")
    m_print("")
    m_print("It's the exact same response we got back from the Pokemon API.  Pretty cool, huh?  You can try the same thing. From curl, Postman or your web browser, try calling endpoints like http://localhost:8000/api/v2/ability/ or http://localhost:8000/api/v2/type/2/.  When doing so, make sure to set the following headers:")
    m_print("")
    print("""{
    "Host": "pokeapi.co",
    "X-Meeshkan-Scheme": "https"
    ...
    m_print("")
    subprocess.call("meeshkan build -i __meeshkan__/recordings.jsonl -o __meeshkan__/gen -m gen", shell=True, stderr=subprocess.PIPE, stdout=subprocess.PIPE)
    m_print("")
    building()
    m_print("")
    m_print("Done.  In __meeshkan__/gen/, you'll see a new OpenAPI spec.")
    m_print("")
    m_input("Now, let's see use this spec to create some _synthetic_ data.  Press ENTER to reboot the mock server on port 8000.")
    m_print("")
    p = subprocess.Popen("meeshkan mock -s __meeshkan__/gen -r".split(" "), stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    m_print("##############################")
    m_print("")
    starting_server()
    m_print("")
    ...
    m_print("")
    m_input("From curl, postman or your web browser, try calling http://localhost:8000/api/v2/pokemon/\{id\}/ , where `\{id\}` is _any_ positive integer. And when doing so, make sure to set following two headers:")
    m_print("")
    print("""{
    "Host": "pokeapi.co",
    "X-Meeshkan-Scheme": "https"
    ...
    m_input("Lastly, please open the file `merge_specs.py` that we just created in the Meeshkan directory. It's a script that merges together the two OpenAPI specs - replay and gen - created by Meeshkan.  After you've looed at it, press ENTER to execute it.")
    m_print("")
    m_print("$ python __meeshkan__/merge_specs.py")
    m_print("")
    subprocess.call('python __meeshkan__/merge_specs.py', shell=True)
    m_print("Done.  In `__meeshkan__/both/`, you'll see an OpenAPI spec that combines _both_ the fixutres from `__meeshkan__/replay/openapi.json` and the synthetic spec from `__meeshkan__/replay/openapi.json`.")
    m_print("")
    ...
    m_print("")
    m_print("If you have a moment, please fill out our post-tutorial survey on https://meeshkan.typeform.com/to/FpRakX.  In addition to helping us improve Meeshkan, it will help us improve this and other tutorials.")
    m_print("")
