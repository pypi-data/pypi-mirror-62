...
class LowRankBlockDiag(object):
    ...
    def regularized_solve(self, value, lambd=0.):
        ...
        D_inv = self._D.regularized_inverse(lambd)

        #print('inverting S')
        internal = np.linalg.inv(self._S)

        #print('computing  V@A^(-1)@V.T')
        #temp = np.zeros(self._V.T.shape)
        temp = D_inv @ self._V.T

        internal += self._V @ temp

        #print('inverting internal')
        internal = np.linalg.inv(internal)

        result = D_inv @ value

        second_part = np.array(result)
        second_part = (self._V.T @ (internal @ (self._V @ second_part)))

        result -= D_inv @ second_part
