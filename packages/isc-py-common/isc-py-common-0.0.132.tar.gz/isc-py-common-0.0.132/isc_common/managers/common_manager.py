        ...
        if isinstance(remove_fields, list):
            ...
            for remove_field in remove_fields:
                if delAttr(data, remove_field) == True:
                    deleted = True
            if deleted == True:
                request.json.setdefault('data', data)
...
class CommonManager(Manager):
        ...
        if isinstance(data, dict):
            ...
            for key in data.keys():
                if not key.startswith('_'):
                    res.setdefault(key, data.get(key))
        return res
    ...
    def get_prp(self, data, propsArr):
        if isinstance(data, dict):
        ...
        if self._dataIsArray(data):
            ...
            for data_item in data:
                ...
                if isinstance(data_item, dict):
                    if function:
                        ...
                        if props > -1:
                            cloned_data.setdefault('props',props)
            ...
            if function:
                ...
                if request.get_id():
                    ...
                    _data.setdefault('id',new_item.id)
        return _data
