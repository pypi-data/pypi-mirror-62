...
class DSResponse__Messages_Confirm_reading(DSResponse_UploadFile):
    ...
    def __init__(self, request):
        ...
        try:
            message = Messages.objects.get(guid=chat_data.guid,
                                           to_whom_id=chat_data.user_id,
                                           state__in=[
                                               chat_data.message_state_new_id,
                                               chat_data.message_state_delivered_id
                                           ])
            ...
            WebSocket.send_message(host=chat_data.host, port=chat_data.port, channel=chat_data.channel, message=message, logger=logger)
