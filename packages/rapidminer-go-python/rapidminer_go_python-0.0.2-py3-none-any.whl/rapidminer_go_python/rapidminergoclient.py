...
class RapidMinerGoClient():
    ...
    #return: bestModel
    def determine_best_model(self, features):
        ...
        print('Exiting determine_best_model function with BestModel:'+str(self.__bestModel)+' param:'+str(self.__maxGain))
    ...
    def resultPerformancePercentageView(self, features):
        #print('Inside determine_best_model function')
        jsonList=[]
        listmodels = []
        model_types = map(lambda x: x['type'], self.__last_execution)
    ...
    def quick_automodel(go_url, inputdata, label):
        LABEL_ATTRIBUTE = label
        # To refresh the changes made in AutoModelWeb
       # To get the AMW instance
        # connection(go_url, gouser, gopassword)

        data = json_normalize(inputdata)

        # dataframe to json
        responseJSON = self.add_dataFrame(data)
        dataId = responseJSON[DATA_ID]

        modelingTask = self.create_modeling_task(dataId)
        modelingTaskID = modelingTask[DATA_ID]

        # setting label
        self.set_label(modelingTaskID, LABEL_ATTRIBUTE)
        # self.set_class_interest(modelingTaskID, 'Yes', 'No')
        # jsonVal = self.set_cost_matrix(modelingTaskID, [[1, -1], [-1, 1]])
        # print('TaskID:' + modelingTaskID)

        # Initiating model training
        self.start_execution(modelingTaskID)
        flag = True
        print('ExecutingModel...')
        # Loops till all the task are completed
        while (flag):
            flag = False
            r = self.get_modeling_execution(modelingTaskID)
        ...
        result = self.get_execution_result(modelingTaskID)
        ...
        bestModel = self.determine_best_model(features)

        # Deploying the best model
        global depID
        depID = self.deploy_model(modelingTaskID, bestModel)
        status = 'Failed'
        if str(depID) != '':
            status = 'Success'
