from . import gdal_datasets as gds
from box import Box
import numpy as np
from . import logger
import cv2
...
class Vectorizer(object):
	...
	def get_resized_array(array,scale_factor):
		resized_data=cv2.resize(array,tuple(np.round(np.array(array.shape)*scale_factor).astype(int))[::-1],interpolation=cv2.INTER_CUBIC)
		return resized_data
	...
	def get_saved(vectors: gpd.GeoDataFrame,vector_path):
		if 'DN' in vectors.columns: property_getter=lambda s:dict(DN=np.round(s.DN,decimals=5))
		else: property_getter=lambda s:None
