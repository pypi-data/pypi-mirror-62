...
class Simulator(ABC):
    ...
    FILE_EXTENSION = ".sim"
    HEALTH_CHECK_PREFIX = "HealthCheck:"
    ...
    def get_simulator_health_check_timeout():
        """Get the user input setting for the health check timeout.

        :return: The time lapse between health checks.
        :rtype: int
        """
        count = os.getenv("AZUREML_SIMULATOR_HEALTH_CHECK_TIMEOUT")
        if not count:
            print("Unable to find user setting using default of 5 seconds.")
            count = 5
        elif not count.isdigit():
    ...
    def get_health_status(self):
        ...
        :rtype: [azureml.contrib.train.rl._rl_simulator.HealthStatus, time_stamp].
        """
        ...
        try:
            with portalocker.Lock(_filename, "r") as f:
                ...
                for file_line in f:
                    if Simulator.HEALTH_CHECK_PREFIX in file_line:
                        status = file_line.split(Simulator.HEALTH_CHECK_PREFIX)[1]
    ...
    def update_health_status(self, status):
        """Update RL with the latest simulator health status given an id.

          :param status: Latest health update.
          :type: azureml.contrib.train.rl._rl_simulator.HealthStatus.
          """
        try:
            _file_contents = "{0}:{1}\n".format(self.ip_address, self.port) + \
                             "{0}{1}".format(Simulator.HEALTH_CHECK_PREFIX, status.name)

            Simulator._write_to_file(self, _file_contents)
        except Exception as _e:
            print("Send health update exception {0}".format(_e))
            raise Exception("Health Update Exception.")
    ...
    def _get_filename(self):
        ...
        file = str(self.id) + Simulator.FILE_EXTENSION
