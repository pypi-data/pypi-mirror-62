...
class Player( GenericInterface.GenericPlayer ):
	...
	def __init__( self, sound, device=None, stream=None, bufferLengthMsec=None, minLatencyMsec='auto', fs=None, resample=False, verbose=None, **kwargs ):
		"""
		Args:
			...
			bufferLengthMsec (float, None):
				...
				the first call will make any difference). Larger
				buffer lengths lead to larger playback latencies---
				usually larger latency is undesirable, but if your
				latency is *too* small (which is rarely the case,
				but can happen for example in macOS's Core Audio
				API) then your sound might stutter audibly when
				the CPU is busy or Python is performing some
				multi-threaded action.
				
			minLatencyMsec (float, None, 'auto'):
				Optionally specify the value that the environment
				variable `PA_MIN_LATENCY_MSEC` should take during
				PortAudio initialization (`None` means the variable
				should be blank, reverting to the library default,
				whereas `'auto'` means audiomath makes its own
				intelligent default). If the PortAudio library has
				already been initialized (a `Stream`, `Player`or
				`Recorder` has been created, or the list of
				host APIs or devices has been queried) then this
				has no effect.
		...
		PORTAUDIO.Initialize( minLatencyMsec=minLatencyMsec )
		...
		if not alreadyInitialized:
			pass # the logic below has been moved into _wrap_portaudio
			#if deviceRecord is None: deviceRecord = GetDeviceInfo()[ deviceIndex ]
			#if bufferLengthMsec is None and sys.platform.lower().startswith( 'darwin' ) and deviceRecord[ 'hostApi' ][ 'name' ].lower().startswith( 'core audio' ):
	...
	def minLatencyMsec( self ):
		return self.__stream.minLatencyMsec
...
class Recorder( GenericInterface.GenericRecorder ):
	...
	__stream = __verbose = None
	def __init__( self, seconds, device=None, stream=None, bufferLengthMsec=None, fs=None, start=True, loop=False, verbose=None, filename=None, **kwargs ):
		"""
		Args:
			seconds (float, Sound):
		...
		if SINGLE_INPUT_STREAM:
			...
			if not isinstance( stream, Stream ):
				...
				else: stream = Stream( device=stream, mode='i', verbose=verbose, bufferLengthMsec=bufferLengthMsec, sampleRate=fs )
			#if alreadyInitialized and stream and stream is not SINGLE_INPUT_STREAM: raise RuntimeError( 'cannot create multiple Streams' )
			self.__stream = stream if stream else SINGLE_INPUT_STREAM if alreadyInitialized else Stream( mode='i', verbose=verbose, bufferLengthMsec=bufferLengthMsec, sampleRate=fs )
			if not alreadyInitialized: SINGLE_INPUT_STREAM = self.__stream
		else:
			if stream is None: stream = DEFAULT_INPUT_DEVICE
			self.__stream = stream if isinstance( stream, Stream ) else Stream( device=stream, mode='i', verbose=verbose, bufferLengthMsec=bufferLengthMsec, sampleRate=fs )
		if self.__stream.inputDevice and not DEFAULT_INPUT_DEVICE: DEFAULT_INPUT_DEVICE = self.__stream.inputDevice[ 'index' ]
			
		stream = self.__stream
		super( Recorder, self ).__init__( seconds=seconds, fs=stream.fs, nChannels=stream.nInputChannels, sampleFormat=stream.sampleFormat[ 'numpy' ], start=start, loop=loop, filename=filename, **kwargs )
