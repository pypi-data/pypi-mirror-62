...
try: from builtins import raw_input as _input # Python 2 (for some reason just referencing `raw_input` directly gave a NameError on Python 2.7.15 on linux...)
...
class Prompt( object ):
	...
	def __call__( self ):
		try: return _input( self.prompt )
			...
			return ''
def WaitFor( condition, finalRecordingStatus=None ):
	"""
	...
	argument may be:
		...
		any callable:
			...
			call `condition()` and wait for it to
...
class GenericRecorder( thing ):
	...
	def MakeRecording( cls, space=60, prompt=None, verbose=None, cut=True, nChannels=None, filename=None, **kwargs ):
		...
		Examples::
		    ...
		    # a `Sound` instance.
		"""
		r = cls( seconds=space, start=True, verbose=verbose, filename=filename, **kwargs )
