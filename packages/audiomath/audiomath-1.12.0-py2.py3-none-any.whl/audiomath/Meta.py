...
def Bang( cmd, shell=False, stdin=None, raiseException=False ):
...
def GetRevision():
	...
	if rev.startswith( '@' ):
		...
		if all( x in repoSubdirectories for x in [ '.git', 'python' ] ): # then we're probably in the right place
			dotGitDir = os.path.join( possibleRepo, '.git' )
			out = ' '.join(
				stdout.strip()
				for cmd in [
					'git "--git-dir=%s" log -1 "--format=%%h %%ci"' % dotGitDir,
					'git "--git-dir=%s" describe --always --all --long --dirty=+ --broken=!' % dotGitDir,
				] for errorCode, stdout, stderr in [ Bang( cmd ) ] if not errorCode
			)
			if out: rev = 'git ' + out 
