...
Performance, as at 20200208:
	...
	- On Windows 10, there's no advantage in terms of absolute latency, at least
	  ...
	  using WDM-KS at 44100Hz. The main advantage to PTB, then, is the ability to
	  pre-schedule.
	...
	def __init__( self, sound, device=None, stream=None, latencyClass='DEFAULT', bufferLengthMsec=None, minLatencyMsec='auto', fs=None, resample=True, verbose=None, **kwargs ):
		...
		Args:
			...
			bufferLengthMsec (float, None):
				...
				TODO: Currently this is not implemented for the
				`PsychToolboxInterface` back-end and must be left
				at `None`. The parameter is included purely for
				compatibility with code written for the default
				`PortAudioInterface` back-end.
			...
			minLatencyMsec (float, None, 'auto'):
				...
				PortAudio initialization (NB: in the
				`PsychToolboxInterface` back-end this is most likely
				ignored and should be left alone---the parameter is
				included purely for compatibility with code written
				for the default `PortAudioInterface` back-end.
