...
if opts.glitch_test:
	openShell = opts.shell
	parameters = opts.glitch_test.replace( ':', '+' ).split( '+' )
	fs = bufferLengthMsec = ''
	...
	bufferLengthMsec = None if bufferLengthMsec.lower() == 'none' or opts.ptb else 'auto' if not bufferLengthMsec else float( bufferLengthMsec )
	tone = am.ToneTest( fs=fs, bufferLengthMsec=bufferLengthMsec, verbose=False, playing=True, device=opts.device )
...
if opts.test:
	openShell = opts.shell
	player = am.Player( am.TestSound( list( range( opts.test ) ) ), device=opts.device )
