...
# when evaluating, just use the generate function, which will default to top_p sampling with temperature of 1.
initial = torch.tensor([[0]]).long()
sample = model.generate(initial, 100, temperature=1., filter_thres = 0.9) # (1, 275, 20000) - now use your favorite sampling strategy and repeat
```
