...
class EntityMatchingModel(CogniteResource):
    def __init__(self, model_id=None, status=None, cognite_client=None):
...
class EntityMatcher:
    ...
    def fit(self, X: List[str]):
        ...
        while data["status"] != "Completed":
            ...
            time.sleep(1)
        return EntityMatchingModel(model_id=model_id, status=data["status"], cognite_client=self.client)
