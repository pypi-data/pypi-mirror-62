...
def print_conf(master):
    print_master_conf(master)
    print_slave_conf(master.get_slaves())


def print_master_conf(master):
    if master and isinstance(master, Master) and master.is_configured():
...
if __name__ == "__main__":
    ...
    if CFG.slave_type() == CFG.TYPE_VIRTUAL:
        slave_reader = VirtualSlaveReaderThread(slave_message_queue)
    elif CFG.slave_type() == CFG.TYPE_REAL:
        slave_reader = SlaveReaderThread(slave_message_queue)
