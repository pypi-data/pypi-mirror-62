...
from tracker_master.util import Util
...
class MasterConfigure(ZabbixClient):
    ...
    def configure(self, master, slaves, force_create=False):
        ...
        logger.info('#### Init Zabbix Master configuration ####')
        logger.info('## Configure host group %s' % self.host_group)
        group_id = self.get_host_group_id(self.host_group)
        if group_id is None:
            if force_create:
                group_id = self.create_host_group(self.host_group)
            else:
                logger.warn('Host group %s not exists' % self.host_group)
                return
        if group_id:
            self.host_group_id = group_id
            logger.info('## Zabbix host group %s configured' % self.host_group_id)
        else:
            logger.error('Host group creation error')

        logger.info('## Looking for template: %s ' % Fields.ZBX_DEVICE_MONITORING_TEMPLATE_NAME)
        t_id = self.get_template_id(Fields.ZBX_DEVICE_MONITORING_TEMPLATE_NAME)
        if t_id:
            logger.info('## Zabbix template %s configured' % self.host_group_id)
            self.master_template_id = t_id
        else:
            logger.error('Master template is missed')
            return

        logger.info('## Configure host %s' % self.host)
        host_id = self.get_host_id(self.host)
        if host_id is None:
            if force_create:
                host_id = self.create_master_host(self.host, self.host)
            else:
                logger.error('Host creation error')
                return
        if host_id:
            self.host_id = host_id
        logger.info('## Zabbix host %s configured' % self.host)

        if not slaves or len(slaves) == 0:
            logger.warning('Device list is empty')
            return False
        # Configure devices
        logger.info('Find %s devices' % len(slaves))
        slave_items = self.get_slave_configure()

        for slave_idx, slave in slaves.items():
            logger.info('Configure %s%s' % (Fields.ZBX_SLAVE_NAME_PREF, slave_idx))
    ...
    def create_host_if_not_exists(self, host_name):  # todo duplicated with __create_group_if_not_exists
        ...
        if host_id is None:
            # todo Critical
            logger.critical('Host %s not initialized' % host_name)
            return False

        logger.info('Host %s configured' % host_name)
        return host_id
