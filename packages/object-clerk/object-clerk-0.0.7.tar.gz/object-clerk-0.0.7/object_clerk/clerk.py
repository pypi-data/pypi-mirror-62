...
from object_clerk.exceptions import (
    ...
    ObjectClerkServerInternalException,
)
...
# Retry config for checksum failures
CHECKSUM_RETRY_CONFIG = {"tries": 3, "delay": 1}

# Transfer Configuration of the multipart upload size
...
class ObjectClerk:
    ...
    @mutate_client_exceptions
    def _upload_object(self, bytes_obj: bytes, bucket: str, object_key: str) -> None:
        """
        ...
        self.client.upload_fileobj(
            BytesIO(bytes_obj), Bucket=bucket, Key=object_key, Config=TRANSFER_CONFIG
        )
    ...
    def upload_object(
        ...
        retry_call(
            self._upload_object,
            fkwargs={"bytes_obj": bytes_obj, "bucket": bucket, "object_key": object_key},
            **self.retry_config,
        )
        if verify_checksum:
    ...
    @mutate_client_exceptions
    def _list_objects(self, bucket: str, max_keys: int, prefix: str) -> Tuple[Optional[List[dict]], Optional[bool]]:
        """
    ...
    def list_objects(self, bucket: str, max_keys: int = 1000, prefix: str = None) -> Tuple[Optional[List[dict]], Optional[bool]]:
        """
