...
def run_deep_model(X_train, y_train, X_val, y_val, n_input, n_output, n_units,
                   d_rate, n_epoch, n_batch, score_type):

    dldict = lambda: defaultdict(dldict)
    dl_dict = dldict()
    ...
    try:
        ...
        dl_dict[name]['model'] = model
        dl_dict[name]['forecast'] = pred
        ...
        score_val = np.inf
    dl_dict[name][score_type] = score_val
