####################################################################################
####                     Auto Time Series Early Alpha v0.1.5                    ####
####                              Python 3 Version                              ####
####                    Conceived and Created by Ram Seshadri                   ####
####                       Further Developed by Kevin Chen                      ####
####                            All Rights Reserved                             ####
####################################################################################
...
def make_ts_magic(data, ts_column, sep=',', target=None, univarate=True, timezone=None,
    ...
    ####                               Auto Time Series                             ####
    ####                                Version 0.1.5                               ####
    ####            Conceived and Created Auto_TimeSeries by Ram Seshadri           ####
    ####                      Further Developed by Kevin Chen                       ####
    ####                            All Rights Reserved                             ####
    ####################################################################################
    ##################################################################################################
    ...
    modeldict = lambda: defaultdict(modeldict)
    ...
    if model_type.lower() in ['prophet','all']:
        ...
        model_dict[name]['forecast'] = forecast_df['yhat'].values
        ...
        #     score_val = np.inf
        model_dict[name][score_type] = score_val
    if model_type.lower() in ['stats', 'all']:
        ...
        # try:
        #     model_dict[name]['model'], model_dict[name]['forecast'], rmse, norm_rmse = \
        ...
        try:
            model_dict[name]['model'], model_dict[name]['forecast'], rmse, norm_rmse = build_arima_model(ts_df[target],
        ...
        # try:
        model_dict[name]['model'], model_dict[name]['forecast'], rmse, norm_rmse = build_sarimax_model(ts_df[target], stats_scoring, seasonality,
        ...
        if len(preds) == 0 or univarate:
            ...
            try:
                if df_orig.shape[1] > 1:
                    ...
                    model_dict[name]['model'], model_dict[name]['forecast'], rmse, norm_rmse = build_var_model(ts_df[[target]+preds],stats_scoring,
        ...
        if len(preds) == 0:
            ...
            try:
                if df_orig.shape[1] > 1:
                    ...
                    #                           score_type, verbose)
                    best = run_ensemble_model(X, y, 'TimeSeries', score_type, verbose)
                    bestmodel = best[0]
                    model_dict[name]['model'] = bestmodel
                    ### Certain models dont have random state => so dont do this for all since it will error
                    #best.set_params(random_state=0)
                    model_dict[name]['forecast'] = bestmodel.fit(train[preds],train[target]).predict(test[preds])
    ...
    if model_type.lower() in ['ml', 'dnn', 'deep', 'all']:
        ...
        model_dict = run_deep_model(model_dict, X_train, y_train, X_val, y_val,
                                    n_input, n_output, n_units, d_rate, n_epoch,
                                    n_batch, score_type)
    ...
    for key, val in model_dict.items():
        f1_stats[key] = model_dict[key][score_type]
    ...
    #print('    Best Model Forecasts: %s' %model_dict[best_model_name]['forecast'])
    print('    Best Model Score: %0.2f' % model_dict[best_model_name][score_type])
