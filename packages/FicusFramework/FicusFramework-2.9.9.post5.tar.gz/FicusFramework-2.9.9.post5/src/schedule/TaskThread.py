...
class TaskThread(threading.Thread):
    ...
    def push_trigger_queue(self, trigger_param):
        ...
        return ResultVO(code=SUCCESS_CODE,msg=f"success,执行器ip:{config.server_ip or config.find_host_ip()}:{config.server_port or 5000}",content="success")
        ...
        if self.handler is not None:
            try:
                if (self.trigger_log_id_dict is not None) and (self.trigger_log_id_dict.get(log_id) is self.handler):
                    ...
                    self.__update_task_status_to_finished(log_id, ResultVO(FAIL_CODE, "任务不在当前执行队列中,可能执行器被重启.手动强制停止"), True, 1)
                self.handler.task_logger = None
            except Exception as e:
                FrameworkHandlerLogger.log_warn(f"kill Handler 失败,", e)
