...
class JPGFileProfile(FileProfile):
        ...
        for k, v in piexif.TAGS.items():
            if k in ['0th', '1st', 'Exif', 'GPS', 'Interop']:  # only filtering on those
                for kk, vv in v.items():
                    lc_vv = vv['name'].lower()
                    element = {'ifd': k, 'idx': kk, 'original': vv['name'], 'fieldname': f'{k}_{vv["name"]}'}
    ...
    def add_field(self, field):
        ...
        if field.fieldname.lower() in self.lc_kw_dict:
            ...
            for f in self.lc_kw_dict[field.fieldname.lower()]:
                ...
                new_field.fieldname = ExifTagStr(f['fieldname'],
                                                 is_exif=True,
                                                 ifd=f['ifd'],
                                                 idx=f['idx'],
                                                 original=f['original'])
    ...
    def read_field(self, state, record, fieldname):
        ...
        if exif_tag:
            ifd, idx = getattr(fieldname, '_ifd', None), getattr(fieldname, '_idx', None)
            try:
