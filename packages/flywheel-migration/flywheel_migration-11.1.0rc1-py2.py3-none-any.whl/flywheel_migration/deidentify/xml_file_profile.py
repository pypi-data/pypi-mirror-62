...
class XMLFileProfile(FileProfile):
    ...
    def add_field(self, field):
        ...
        def deidentify_list(field, profile, state, record):
            ...
            for el in els:
                fieldname = XPathStr(record.tree.getpath(el), is_xpath=True, original=field.fieldname)
    ...
    def read_field(self, state, record, fieldname):
        ...
        original = fieldname if not getattr(fieldname, '_original') else getattr(fieldname, '_original')
