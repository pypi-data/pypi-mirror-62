...
class PublicHolidaysOffline(CountryOrRegionTimePublicData):
    ...
    .. remarks::
        ...
        .. code-block:: python
            ...
            end_date = datetime.today()
            start_date = datetime.today() - relativedelta(months=1)
            PublicHolidaysOffline.env = environ.PandasEnv()
            hol_df = PublicHolidaysOffline().filter(environ.PandasEnv(), start_date, end_date)
    ...
    def __init__(
        ...
        super(PublicHolidaysOffline, self).__init__(cols=cols, enable_telemetry=enable_telemetry)
        if enable_telemetry:
    ...
    def filter(self, env, min_date, max_date):
        ...
        ds = super(PublicHolidaysOffline, self).filter(env, min_date, max_date)
