...
from .dataaccess._nyc_tlc_green_blob_info import NycTlcGreenBlobInfo
...
class NycTlcGreen(TimePublicData, TabularOpenDatasetFactory):
    ...
    .. remarks::
        ...
        .. code-block:: python
            ...
            end_date = parser.parse('2018-06-06')
            start_date = parser.parse('2018-05-01')
            NycTlcGreen.env = environ.PandasEnv()
            nyc_tlc_df = NycTlcGreen().filter(environ.PandasEnv(), start_date, end_date)
    ...
    _blobInfo = NycTlcGreenBlobInfo()

    data = BlobParquetDescriptor(_blobInfo)

    time_column_name = 'lpepPickupDatetime'
    ...
    def __init__(
        ...
        super(NycTlcGreen, self).__init__(cols=cols, enable_telemetry=enable_telemetry)
        self.limit = limit
        if enable_telemetry:
    ...
    def filter(self, env, min_date, max_date):
        ...
        ds = super(NycTlcGreen, self).filter(env, min_date, max_date)
