...
def write_txt(name_list, MyName="被冻结的光", fileDir="itchat.pkl"):
    ...
    try:
        ...
        with ThreadPoolExecutor(max_workers=len(name_list) + 1) as executor:
            executor.submit(relogin, fileDir)
...
def login(fileDir):
    ...
    itchat.dump_login_status(fileDir)

def relogin(fileDir):
    while(True):
        itchat.load_login_status(fileDir=fileDir)
...
if __name__ == '__main__':
    ...
    login(statusDir)
    write_txt(name_list=["妈妈", "caloi"], fileDir=statusDir)
