...
class Device_iCloud_Account(Device_Base):
    def __init__(
        ...
        self.account_username = Property_String(
            node, id="account", name="Accout User Name"
        )
        node.add_property(self.account_username)

        self.trusted_device = Property_String(
            node, id="trusteddevice", name="Trusted Device"
        )
        node.add_property(self.trusted_device)

        self.connection_status = Property_String(
            node, id="connection", name="iCloud Connection Status"
        )
        ...
        self.verification_code = Property_String(
            node,
            id="verificationcode",
            name="Verification Code",
            settable=True,
            set_value=icloud_account.send_verification_code,
        )
