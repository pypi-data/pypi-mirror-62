...
class ReversibleUnit(AssemblyUnit):
    ...
    **Attributes**
     ...
     * def_rev (DynamicConstraint): definition of the reversible constraint
     * def_rev_c (DynamicConstraint): definition of the reversible
     constraint in the case where only the consumption is fixed
     * def_rev_p (DynamicConstraint): definition of the reversible constraint
     in the case where only the production is fixed
    ...
    def __init__(self, time, name, pmin_cons=1e-5, pmax_cons=1e+5,
        ...
        if p_cons is None and p_prod is None:
            self.def_rev = DynamicConstraint(
            ...
            self.def_rev_c = DynamicConstraint(
                exp_t='{0}_p[t] - (1 - {1}_u[t]) * {2} <= 0'.format(
            ...
            self.def_rev_p = DynamicConstraint(
                exp_t='{0}_p[t] - (1 - {1}_u[t]) * {2} <= 0'.format(
                    self.production_unit.name, self.consumption_unit.name,
                    pmax_prod),
                t_range='for t in time.I',
