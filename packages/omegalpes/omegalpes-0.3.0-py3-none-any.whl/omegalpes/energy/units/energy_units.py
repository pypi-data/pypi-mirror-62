...
class EnergyUnit(OptObject):
            ...
            if isinstance(co2_out, (int, float)):
                ...
                if len(co2_out) != self.time.LEN:
                    raise IndexError(
                        ...
                        "CO2 emissions have a size of {1}".format(self.time.LEN,
                                                                  len(co2_out)))
                else:
                    self.calc_co2_emissions = DynamicConstraint(
    ...
    def add_operating_time_range(self, operating_time_range=[[int, int]]):
        """
        DEPRECATED : please use set_operating_time_range
        Add a range of hours during which the energy unit can be operated
        ...
        import warnings
        deprecated_msg = "DEPRECATED: add_operating_time_range is deprecated" \
                         " and will be removed in omegalpes version 0.4 " \
                         "Please use set_operating_time_range that allows to" \
                         " use hour and minutes in the operating_time_range " \
                         "definition"
    ...
    def minimize_exergy_destruction(self, weight=1):
    ...
    def minimize_exergy(self, energy_unit=None, weight=1):
        ...
        if hasattr(self, 'exergy'):
            self.min_exergy = Objective(name='min_exergy',
                                             exp='lpSum({0}_exergy[t] '
                                                 'for t in time.I)'.
                                             format(energy_unit.name), weight=weight,
                                             parent=energy_unit)
        else:
            raise ValueError("You should initialize exergy calculations "
                             "on the unit named {0} before trying to "
    ...
    **Attributes**
    ...
    def __init__(self, time, name: str, p: list or dict or pd.DataFrame,
        ...
        EnergyUnit.__init__(self, time=time, name=name, p=p, p_min=p_min,
                            p_max=p_max, e_min=e_tot, e_max=e_tot,
