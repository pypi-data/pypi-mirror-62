    ...
    **Attributes**
    ...
    def get_value_with_date(self):
        ...
        if isinstance(self, Quantity):
            print(self.value)
            if isinstance(self.value, dict):
                print(self.parent.time)
                print(self.value.values())

                df = pd.DataFrame(data=list(self.value.values()),
                                  index=self.parent.time.DATES,
                                  columns=['Value in {}'.format(self.unit)])
                return df
        ...
        if self.parent:
            if self not in self.parent._external_constraints_list:
                ...
                Warning("the external constraint {} should have a parent "
                        "otherwise the constraint won't be added to the parent "
...
class DailyDynamicConstraint(DynamicConstraint):
    """
    **Description**
