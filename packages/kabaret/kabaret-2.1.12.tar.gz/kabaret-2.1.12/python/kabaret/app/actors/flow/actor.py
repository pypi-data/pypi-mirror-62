...
class LS(Cmd):
    ...
    def _get_relation_infos(self, parent, relation):
        # We used to avoid accessing the related object here to have
        # a late as possible lazy instantiation. But Action.allow_context()
        # broke the rule, and we want a way to get the ui infos
        # from the Object so:
    ...
    def get_actions(cls, o, rel_path, without_dialog_only, context):
        ...
        for relation in o._mng.relations():
            ...
            if issubclass(relation_related_type, flow.Action):
                ...
                ui_config = relation.get_ui(action)
...
class Flow(Actor):
    ...
    def get_project_root(self, project_name, reload_first=False):
        ...
        self.log(
            '[Flow]',
            'Loaded Flow for Project', project_name, 
            'in', '%.2f seconds'%(time.time()-t,)
        )

        self.log(
            '[Flow]',
            'Creating Project (+ValueStore):', 
            project_name
        )
        t = time.time()
