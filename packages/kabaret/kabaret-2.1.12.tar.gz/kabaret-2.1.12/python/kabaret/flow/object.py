...
class _Relation(object):
    ...
    _next_relation_index = 0

    _GROUP_CONTEXT = []

    @classmethod
    def _generate_relation_index(cls):
        # tricky stuff to order relations in declaration order [part 1]
        # /!\ be sure to use _Relation class (not cls)
        _Relation._next_relation_index += 1
        return cls._next_relation_index

    @classmethod
    def reset_relation_indexes(cls):
        # /!\ be sure to use _Relation class (not cls)
        _Relation._next_relation_index = 0
    ...
    def __init__(self, related_type):
        ...
        self.index = self.__class__._generate_relation_index()
        self.group = None
...
class ObjectType(type):
    ...
    def __new__(cls, class_name, bases, class_dict):
        ...
        _Relation.reset_relation_indexes()
        ...
        for base in bases:
            try:
                ...
                if relation_indexes:
                    min_relation_index = max(
                        min_relation_index, max(relation_indexes))

        for n, o in class_dict.items():
            if isinstance(o, _Relation):
                o.name = n
                o.index += min_relation_index
                if o.group is None:
                    # set default if no group defined
