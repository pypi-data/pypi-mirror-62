...
if os.getenv("TREEBEARD_ENVIRONMENT") == "development":
    env = "development"
else:
    env = "production"
...
def push(file: IO):
    ...
    click.echo(f"Secrets are available in any notebook you run, fetch using: ")
    click.echo(f"from treebeard.secrets import get_secrets")
    click.echo(f"password = get_secrets()['password']")
    s = file.read()
    try:
