...
def _detect(env):
    ...
    product = env.WhereIs('product') or env.WhereIs('product')
    if product:
        return product
...
def generate(env):
    ...
    env.SetDefault(
        ...
        HEXCOM = '$PRODUCT image $HEXFLAGS $SOURCE -e $TARGET -x',
        HEXCOMSTR = '',

        # product image $(MK_GENERATED_IMGS_PATH)/images.zip -i $(MK_GENERATED_IMGS_PATH)/data -l -p
        IMAGECOM = '$PRODUCT image $IMAGEFLAGS ${IMAGE} -i ${PATH} -l -p',
        IMAGECOMSTR = '',
    )

    env.Append(BUILDERS = {
        'Product': _image_builder,
        'Hex': _hex_builder,
    })
