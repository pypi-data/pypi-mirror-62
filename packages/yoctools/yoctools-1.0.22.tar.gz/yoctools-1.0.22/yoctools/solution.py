...
class Solution:
    ...
    def set_solution(self, all_components):
        ...
        for component in [self.chip_component, self.board_component, self.solution_component]:
            if component:
                for k, v in component.hw_info.__dict__.items():
                    if v:
        ...
        if solution_hw_info.cpu_name:
            cpu_set_variable(solution_hw_info.cpu_name, solution_hw_info.arch_name)

        if solution_hw_info.cpu_id:
            self.variables.set(solution_hw_info.cpu_id.upper(), 1)

        if solution_hw_info.vendor_name:
            self.variables.set('VENDOR', solution_hw_info.vendor_name.upper(), True)

        if solution_hw_info.chip_name:
            self.variables.set('CHIP', solution_hw_info.chip_name.upper(), True)

        if solution_hw_info.board_name:
            self.variables.set('BOARD', solution_hw_info.board_name.upper(), True)
        ...
        self.global_includes.append(os.path.join(self.solution_component.path, 'include'))
