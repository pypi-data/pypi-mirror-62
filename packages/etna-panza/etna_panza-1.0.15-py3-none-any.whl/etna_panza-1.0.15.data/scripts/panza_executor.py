...
with open("/moulinette/result.json", 'w') as f:
    if job_failure is not None:
        feedback = {
            "messages": ["something went wrong when executing the moulinette, please report it"],
            "points": 0
        }
        result = {"errors": {"message": str(job_failure)}, "feedback": feedback}
    else:
        result = {"feedback": {"messages": global_failures, "points": global_points}}
