...
def parse_date_time(datetime_string, timezone=None):
    ...
    #         datetime_string += 'Z'
    #     return datetime.strptime(datetime_string, '%Y-%m-%dT%H:%M:%SZ').replace(tzinfo=pytz.UTC)
    # else:
    #     return datetime.strptime(datetime_string, '%Y-%m-%dT%H:%M:%S')

def parse_prev_flight(flight):
    prev_f_number = prev_f_iata = None
    ...
    return prev_f_number, prev_f_iata
