...
class AutoMLRun(Run):
    ...
    def continue_experiment(
        ...
        :raises: :class:`azureml.train.automl.exceptions.ScenarioNotSupportedException`
        """
        ...
        if not isinstance(experiment_timeout_hours, float) and experiment_timeout_hours is not None:
            raise ConfigException("experiment_timeout_hours expected to be 'float' or 'None'. received '{0}'"
                                  .format(type(experiment_timeout_hours)))
    ...
    def get_output(self,
        ...
        :raises: :class:`azureml.train.automl.exceptions.OptionalDependencyMissingException`
    ...
    def _download_automl_model(self, curr_run: Run) -> SKLearnPipeline:
        ...
        try:
            ...
            raise ClientException(str(e)) from None
    ...
    def _log_traceback(self, exception: BaseException, settings: Optional[AzureAutoMLSettings] = None) -> None:
        ...
        logger = _logging.get_azureml_logger(automl_settings=settings,
                                             parent_run_id=self._parent_run_id,
                                             child_run_id=self.run_id,
                                             parent_run_uuid=self._run_dto.parent_run_uuid,
                                             child_run_uuid=self._run_dto.run_uuid)
