...
from automl.client.core.common.constants import SupportedModelNames
...
def _local_managed(experiment: Experiment,
    ...
    logger = _logging.get_azureml_logger(automl_settings=settings_obj, parent_run_id=None, child_run_id=None)
...
class AutoMLConfig(object):
    ...
    :param task: The type of task to run. Values can be 'classification', 'regression', or 'forecasting'
        depending on the type of automated ML problem to solve.
    :type task: str or azureml.train.automl.constants.Tasks
    :param path: The full path to the Azure Machine Learning project folder.
    :type path: str
    :param iterations:
        The total number of different algorithm and parameter combinations
        to test during an automated ML experiment.
    :type iterations: int
    ...
    :param lag_length: How many rows of historical data to include when featurizing time series data.
    :type lag_length: int
    ...
    :param enforce_time_on_windows:
        Flag to enforce time limit on model training at each iteration on Windows.
    ...
    :param model_explainability:
        Flag to enable/disable explaining of best AutoML model at the end of all AutoML training iterations.
    ...
    :type whitelist_models: list(str)
        ...
        or list(azureml.train.automl.constants.SupportedModels.Regression) for regression task,
        or list(azureml.train.automl.constants.SupportedModels.Forecasting) for forecasting task
    :param enable_onnx_compatible_models: Flag to enable or disable enforcing the ONNX-compatible models.
    :type enable_onnx_compatible_models: bool
    :param time_column_name: The name of your time column. Used when forecasting to specify the datetime
        column in the input data used for building the time series and inferring its frequency. When task
        type is forecasting, this parameter is required.
    :type time_column_name: str
    :param max_horizon: The maximum desired forecast horizon in units of time-series frequency.
    ...
    :param drop_column_names: The names of columns to drop.
        drop_column_names is available for forecasting tasks.
        To customize drop columns for classification and regression tasks, please refer to FeaturizationConfig.
    ...
    :param enable_voting_ensemble: Flag to enable/disable VotingEnsemble iteration.
        For more information about ensembles, see `Ensemble configuration <https://docs.microsoft.com/
        azure/machine-learning/how-to-configure-auto-train#ensemble>`_.
    ...
    :type validation_data: pandas.DataFrame or azureml.core.Dataset
        or azureml.data.dataset_definition.DatasetDefinition or azureml.data.TabularDataset
    :param label_column_name: The name of the label column. (Applicable to training_data & validation_data parameters)
    ...
    :param weight_column_name: The name of the sample weight column. Automated ML supports a weighted column
        ...
        (Applicable to training_data & validation_data parameters)
    ...
    def __init__(self,
                ...
                if whitelist_models is None:
                    ...
                    if all_models is not None:
                        ...
                        if set(all_model_names).issubset(set(blacklist)):
                            err_message = "Can not find models to train, all available models are in blacklist"
                elif set(whitelist_models).issubset(set(blacklist)):
                        err_message = "Can not find models to train, all whitelisted models are also in blacklist"

                if err_message is not None:
                    raise ConfigException(err_message)
        ...
        try:
            ...
            self.user_settings["scenario"] = constants.Scenarios.SDK
            pass
    ...
    def _validate_config_settings(self, workspace: Optional[Workspace] = None) -> None:
        ...
        if (has_training_input_pandas_obj and self.user_settings['spark_context'] is not None):
            raise ConfigException("Pandas objects are not supported on Spark cluster. "
                                  "Please provide all dataset objects.")

        if has_single_dataset_input and has_cv_split_indices:
            raise ConfigException("Currently 'cv_splits_indices' option is not supported when the input data "
                                  "is combined. Please remove cv_splits_indices or separate the data into Features "
                                  "dataset and Label column.")
        ...
        if compute_target != constants.ComputeTargets.LOCAL:
            ...
            if not data_script_provided and \
                ...
                raise ConfigException(
                    "You must provide data with `azureml.dataprep.Dataflow`, `azureml.core.Dataset`,"
