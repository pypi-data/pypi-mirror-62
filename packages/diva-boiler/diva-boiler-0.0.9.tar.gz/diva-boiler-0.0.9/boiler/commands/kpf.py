...
def load(types, geom, activities, validate, prune, convert):
    ...
    output = {'summary': summarize_activities(list(activity_map.values()))}  # type: ignore

    if validate:
        warnings, errors = validate_activities(activity_map.values())
        output['error'] = web_serialization.serialize_validation_errors(errors)
        output['warning'] = web_serialization.serialize_validation_errors(warnings)
