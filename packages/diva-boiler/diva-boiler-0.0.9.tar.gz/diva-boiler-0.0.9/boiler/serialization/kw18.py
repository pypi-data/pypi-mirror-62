...
from boiler import BoilerSession
...
class KW18FileSet(NamedTuple):
    ...
    def read_actor_types(self) -> Dict[int, ActorType]:
        ...
        return {int(row[TypesRow.ID]): ActorType(row[TypesRow.TYPE]) for row in rows}
...
def find_activity_code(activity_type: ActivityType):
    for code, activity in all_activity_codes.items():
        if activity == activity_type:
            return code
    raise ValueError(f'no numerical code exists for {activity_type.value}')
