...
class Plum:
    ...
    def dump(self):
        ...
        """
        dump = Dump()
        # arguments: fmt, buffer, offset(0), args, kwargs, parents(None), dump
...
def _pack_value_with_format(fmt, buffer, offset, value, parents, dump):
    # pylint: disable=too-many-statements,too-many-branches,
    # pylint: disable=too-many-locals,too-many-nested-blocks

    if isinstance(fmt, PlumType):
        offset = fmt.__pack__(buffer, offset, value, parents, dump)
            ...
            for key, subfmt in fmt.items():
                try:
                    ...
                    offset = _pack_value_with_format(
                        subfmt, buffer, offset, item, parents,
...
def _pack_with_format(fmt, buffer, offset, args, kwargs, parents, dump):
    # pylint: disable=too-many-branches,too-many-arguments
    try:
        if isinstance(fmt, PlumType):
            if args:
                offset = fmt.__pack__(
                    buffer, offset, args[0], parents, dump.add_record(cls=fmt) if dump else dump)
            ...
            offset = _pack_value_with_format(fmt, buffer, offset, args, parents, dump)
            ...
            offset = _pack_value_with_format(fmt, buffer, offset, kwargs, parents, dump)
            ...
            # generate exception
            offset = _pack_value_with_format(
                fmt, buffer, offset, args[0] if args else kwargs, parents, dump)
...
def _unpack(fmt, buffer, offset, prohibit_excess, dump):
    ...
    try:
        if isinstance(fmt, PlumType):
            items, offset = fmt.__unpack__(
                buffer, offset, None,  # None -> no parents
                dump.add_record() if isinstance(dump, Dump) else dump)
            ...
            if extra_bytes:
                if dump:
                    ...
                    for i in range(0, len(extra_bytes), 16):
                        dump.add_record(
                            separate=separate, value=value, memory=extra_bytes[i:i+16])
                        value = ''
                        separate = False
