...
def kagra_llmerged(scope, name, rse, rse_attrs, proto_attrs):
    """
    Map the GWF files according to the KAGRA aggregated llmerged schema.

    Low-latency frames at ICRR are aggregated into 4096 frame files and placed
    in a directory named for the detector:

    O3:H-H1_llmerged-1266847744-4096.gwf ->
    H1/H-H1_llmerged-1266847744-4096.gwf
    ...
    detector, _, _, _ = match.groups()

    detector_pretty = detector[0] + '1'

    return "%s/%s" % (detector_pretty, name)


RSEDeterministicTranslation.register(kagra_llmerged)
...
def ligo_legacy(scope, name, rse, rse_attrs, proto_attrs):
    ...
    if dataset == 'V1Online' and scope != 'O3':
        ...
        return '%s/%s/%s/%s' % (detector_pretty, dataset_pretty, dir_hash,
                                name)

    elif detector == 'V' and dataset == 'raw' and scope in ['O2', 'O3']:
        detector_pretty = detector[0] + '1'
        return "%s/%s/%s/%s/%s" % (scope, dataset, detector_pretty, dir_hash,
                                   name)
    ...
    return "%s/%s/%s/%s/%s" % (scope, dataset_pretty, detector_pretty,
                               dir_hash, name)
...
if __name__ == '__main__':
    ...
    def test_mapping(algorithm, scope, name, pfn):
        ...
        if algorithm.__name__ not in ['ligo_legacy', 'kagra_llmerged']:
    ...
    test_mapping(kagra_llmerged, "O3", "H-H1_llmerged-1266847744-4096.gwf",
                 "H1/H-H1_llmerged-1266847744-4096.gwf")
