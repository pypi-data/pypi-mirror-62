...
class AbstractGeneratorXml(object):
    ...
    def create_xml(self, certificado_file_cer, certificado_file_pem):
        self.generate_xml()
        if self.global_data.get('provider') == 'solucion_factible':
            self.set_certificado(certificado_file_cer)
            self.set_sello(certificado_file_pem)
    ...
    def set_certificado(self, certificado_file_cer):
        """
    ...
    def set_sello(self, certificado_file_pem):
        try:
            ...
            cadena_original = self.get_cadena_original(self.get_xml())
...
class GeneratorXmlThreeDotThree(AbstractGeneratorXml):
    ...
    def generate_xml(self):
        ...
        dom.setAttribute('TipoDeComprobante', self.data.get('tipo_comprobante'))
        ...
        if self.tipo_objeto == 'I':
            ...
            dom.setAttribute('TipoCambio','1')  # 'Opcional solo cuando moneda es distinta de MXN'
            dom.setAttribute('MetodoPago', self.data.get('metodo_pago'))
            # Campos que solo en ingreso no son fijos
            dom.setAttribute('Moneda','MXN')  # 'Peso Mexicano'
        ...
        elif self.tipo_objeto == 'N':
            dom.setAttribute('FormaPago',self.data.get('forma_pago'))
            dom.setAttribute('CondicionesDePago','Contado')
            dom.setAttribute('TipoCambio','1')
            dom.setAttribute('MetodoPago','PUE')
            dom.setAttribute('Moneda','MXN')
    ...
    def get_pagos(self):
        try:
            ...
            fechaPago = str(self.data.get('fecha_pago').isoformat()).split('.')[ 0 ] + 'T00:00:00'
            ...
            for cfdi in doctos:
                ...
                docto.setAttribute('MonedaDR','MXN')
