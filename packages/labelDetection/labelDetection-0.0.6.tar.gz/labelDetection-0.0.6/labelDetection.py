...
import ensemble.mainModel
import ensemble.testTimeAugmentation
import ensemble.mainTTA
import notebooks.DDTecnhiques
...
class MainWindow(QMainWindow, WindowMixin):
    ...
    ## Support Functions ##
    def detectStoma(self):
        Dialog = QtWidgets.QDialog()
        ui = predict.tipoModelo.Ui_Dialog()
        ui.setupUi(Dialog)
        x = Dialog.exec_()
        ...
        if ui.genera:
            ...
            model = ensemble.mainModel.selectModel(self,ui.rb_1,ui.rb_2, ui.rb_3,ui.rb_4, ui.rb_5, ui.rb_6, ui.rb_7, ui.rb_8, ui.rb_9)
    ...
    def ensembleMethod(self):
        Dialog = QtWidgets.QDialog()
        ui = ensemble.ensembleOptions.Ui_Dialog()
        ui.setupUi(Dialog)
        x = Dialog.exec_()
        # El proceso principal se queda esperando hasta que el dialogo se cierra.
        # Una vez que el dialogo se ha cerrado podemos entrar a uno de los parámetros
        ...
        if ui.genera:
            ...
            tecnhiques = ensemble.mainModel.selectTecniques(self,ui.avg, ui.bila,ui.blur,ui.chanhsv,ui.chanlab,ui.crop,ui.drop,ui.elas,ui.histo,
                                  ui.vflip,ui.hflip,ui.hvflip,ui.gamma,ui.blurGa,ui.gaunoise, ui.invert,ui.median, ui.none,
                                  ui.raiseB, ui.raiseGreen, ui.raiseHue, ui.raisered, ui.raisesatu, ui.raiseval,
                                  ui.resize, ui.rot10, ui.rot90, ui.rot180, ui.rot270, ui.saltpe, ui.sharpen, ui.sift,
                                  ui.shear, ui.trans)
            model = ensemble.mainModel.selectModel(self,ui.yolo,ui.ssd, ui.fast,ui.yoMx,ui.rcnn, ui.reti, ui.efi, ui.fsaf, ui.fcos)
            option = ensemble.mainModel.selectOption(ui.aff, ui.con, ui.una)
            if model is not None:
                box = QDialog()
                box.setWindowFlags(box.windowFlags() | QtCore.Qt.CustomizeWindowHint)
                box.setWindowFlags(box.windowFlags() & ~QtCore.Qt.WindowCloseButtonHint)
    ...
    def generarExcel(self):
        generaExcel(os.path.dirname(ustr(self.filePath)))
        path = os.path.dirname(ustr(self.filePath)) if self.filePath else '.'
        QMessageBox.about(self, "Information", "Excel finished and save in "+ path)
    ...
    def trainModel(self):
        ...
        if ui.genera:
            path = os.path.dirname(ustr(self.filePath)) if self.filePath else '.'
            tecnhiques = ensemble.mainModel.selectTecniques(self, ui.avg, ui.bila, ui.blur, ui.chanhsv, ui.chanlab, ui.crop,
    ...
    def setClean(self):
        ...
        self.actions.ensemble.setEnabled(True)
...
def get_main_app(argv=[]):
    ...
    # Usage : labelDetection.py image predefClassFile saveDir
