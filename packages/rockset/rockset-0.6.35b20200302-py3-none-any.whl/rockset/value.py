...
class ValueCodec(object):
    ...
    def add_custom(self, type_id, py_type, encoder, decoder):
        if not isinstance(py_type, type):
            raise TypeError('Custom types must be standard Python types '
                            '(metaclasses not supported)')
        if py_type in _standard_types:
            raise TypeError('May not register custom type for Python standard '
                            'type {}'.format(py_type))
    ...
    def encode(self, x):
        ...
        if isinstance(x, list):
            return Value(array_value=ArrayValue(
                value=(self.encode(v) for v in x)))
        if isinstance(x, dict):
            return Value(object_value=ObjectValue(
                fields=(Field(key=k, value=self.encode(v))
                    for k, v in x.items())))
        enc = self._custom_encoders.get(type(x))
        if enc:
            return Value(custom_value=CustomValue(
                type=enc.type_id,
                encoded=enc.encoder(x)))
    ...
    def _decode_custom(self, x):
        ...
        if not dec:
            raise KeyError('No custom decoder registered for type id '
                           '{}'.format(cv.type))
        obj = dec.decoder(cv.encoded)
        if type(obj) != dec.py_type:
            raise TypeError('Custom decoder produced a Python object of type '
