...
class SQLSelect(object):
    ...
    def _enforce_build_order(self, build_order, coalesce=False):
        ...
        return (
            ...
            .add_from([Symbol('('), self, Symbol(')'), Symbol(' as '),
                self.default_alias()], alias=self.default_alias())
    ...
    def sqlexpression(self, **kwargs):
        ...
        if self._join_list:
            ...
            for (join_type, relation, predicate) in self._join_list:
                ret += '    {} {} '.format(
                    _sqlexp(join_type, **kwargs),
                    _sqlexp(relation, **kwargs))
                if predicate:
...
class FieldIntOpQuery(FieldOpQuery):
    def __init__(self, field, v, sqlop):
        super(FieldIntOpQuery, self).__init__(field,
            checked_decode(v, int), sqlop)


class FieldFloatOpQuery(FieldOpQuery):
    def __init__(self, field, v, sqlop):
        super(FieldFloatOpQuery, self).__init__(field,
            checked_decode(v, float), sqlop)


class FieldStringOpQuery(FieldOpQuery):
    def __init__(self, field, v, sqlop):
        super(FieldStringOpQuery, self).__init__(field,
            checked_decode(v, str), sqlop)
...
class FieldDateTimeOpQuery(FieldOpQuery):
    def __init__(self, field, v, sqlop):
        ...
        super(FieldDateTimeOpQuery, self).__init__(field,
            checked_decode(v, dt_types), sqlop)


class FieldBaseRefOpQuery(FieldOpQuery):
    def __init__(self, field, v, sqlop):
        super(FieldBaseRefOpQuery, self).__init__(field,
            checked_decode(v, BaseRef), sqlop)
...
class FieldIsNullQuery(FieldUnaryOpQuery):
    def __init__(self, field):
        super(FieldIsNullQuery, self).__init__(field,
            post_sqlop='IS NULL')


class FieldIsNotNullQuery(FieldUnaryOpQuery):
    def __init__(self, field):
        super(FieldIsNotNullQuery, self).__init__(field,
            post_sqlop='IS NOT NULL')
...
class FieldIsDefinedQuery(FieldUnaryOpQuery):
    ...
    def sqlexpression(self, **kwargs):
        return _sqlexp(
            FieldIsNotNullQuery(self.field) & FieldOpQuery(
                self.field, Symbol('NULL_VALUE'), 'IS DISTINCT FROM'
            ), **kwargs
        )
...
class FieldNestedQuery(FieldExistsQuery):
    def __init__(self, field, field_alias, nested_query, alias):
        ...
        alias = alias or 'uq'
        inner_query = (Q(field_unnest, alias=alias)
            .where(nested_query)
            .select(Literal(1)))
...
class JoinQuery(Query):
    ...
    def sqlbuild(self, sqlsel):
        sqlsel = self.child.sqlbuild(sqlsel)
        return sqlsel.add_join(self.right_query, self.join_predicate,
            self.join_type)
...
class AggFieldRef(BaseRef):
    def __init__(self, agg_op, field, allow_root=False, root_field=None):
        ...
        if (not allow_root and
            hasattr(field, '_is_root_field') and
...
for (pyop, sqlop) in (('__lt__', '<'), ('__le__', '<='), ('__gt__', '>'),
    ('__ge__', '>=')):
