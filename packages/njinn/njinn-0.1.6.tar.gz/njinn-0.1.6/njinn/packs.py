...
def prepare_actions(config):
    ...
    for (key, value) in actions.items():
        ...
        if "platform" in value:
            action["platform"] = value["platform"]
...
def clone_repository(repository_url, path, branch):
    ...
    logger.info("Cloning repository from %s to %s" %
                (repository_url, path))

    repo = git.Repo.clone_from(repository_url, path, branch=branch)
    commit_hash = repo.heads[branch].commit.hexsha
    logger.info("HEAD is at %s", commit_hash)
    return commit_hash
...
def zip_dir(source, target_temp_dir):
    ...
    with ZipFile(zip_filename, mode='w') as zip_file:
        for repo_file in files:
            ...
            zip_file.write(repo_file, 
                arcname=os.path.relpath(repo_file, source))
    return zip_filename
...
def install(repository_url, branch, username, password, api):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        with tempfile.TemporaryDirectory() as target_temp_dir:
            ...
            pack_conf_name = "pack.yaml" if os.path.exists(
                f"{temp_dir}/pack.yaml") else "pack.yml"
            ...
            try:
                with open(pack_conf_path, 'r') as pack_conf:
                    logger.info("Loading the configuration file: %s" %
                                pack_conf_name)
                    config = yaml.safe_load(pack_conf)

                    pack_data = prepare_pack(config, repository_url, branch, commit_hash)
                    ...
                    res = r.put(bundle_url, files={ "bundle":(zip_basename, open(zip_filename, "rb")),
                    ...
                    if config_schemes_data:
                        ...
                        for config_scheme in config_schemes_data:
                            ...
                            create_or_update(config_schemes_url,
                                            config_scheme, config_full_name)

                    actions_url = base_api + '/actions'
                    for action in actions_data:
                        action_full_name = ".".join(
                            [pack_full_name, action['name']])
