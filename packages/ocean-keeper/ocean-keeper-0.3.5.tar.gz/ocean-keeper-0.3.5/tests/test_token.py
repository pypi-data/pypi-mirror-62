...
def test_token_transfer():
    ...
    token = Token.get_instance()
    balance0 = token.get_token_balance(test_address)
    balance1 = token.get_token_balance(publisher_account.address)
    print(f'transfer source balance: {balance1}')
    if balance1 < 10:
        Dispenser.get_instance().request_tokens(10, publisher_account)
        balance1 = token.get_token_balance(publisher_account.address)
        print(f'transfer source balance after request_tokens: {balance1}')

    try:
        token.transfer(test_address, 3, publisher_account)
    except ValueError as err:
        print(f'Transfer tokens failed: {err}')
        Dispenser.get_instance().request_tokens(10, publisher_account)
        token.transfer(test_address, 3, publisher_account)

    new_balance = token.get_token_balance(test_address)
    assert new_balance == (balance0 + 3)
