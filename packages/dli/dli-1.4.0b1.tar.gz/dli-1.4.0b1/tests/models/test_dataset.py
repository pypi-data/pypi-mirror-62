...
class TestInstanceDownload:
    ...
    def test_instance_download_all_should_refer_to_download_function(
        ...
        result = instance.download_all(path)

        instance.download.assert_called_once_with(path)
        assert result == 'file'

    def test_instance_download(self, client, monkeypatch, instance):
        ...
        with tempfile.TemporaryDirectory() as target_dest:
            ...
            assert sorted(os.listdir(os.path.join(target_dest, 'prefix'))) == [
                'test.000.csv', 'test.001.csv', 'test.002.csv'
            ]


class TestDatasetModule:
    def test_display_dataset(
            self, capsys, test_client, package_request,
            dataset_request, instance_request
    ):

        test_client._session.get.return_value.json.side_effect = [
            package_request, dataset_request, instance_request,
        ]
    ...
    def test_retrieve_datasets(self, test_client, dataset_request_v1):
        test_client._session.get.return_value.json.side_effect = [
            dataset_request_v1,
            {
                'data': {
    ...
    def test_datasets_called_twice(self, test_client,
    ...
    def test_retrieve_instance_given_package_dataset(
        self, test_client,
        package_request, dataset_request, instance_request
    ):
        test_client._session.get.return_value.json.side_effect = [
