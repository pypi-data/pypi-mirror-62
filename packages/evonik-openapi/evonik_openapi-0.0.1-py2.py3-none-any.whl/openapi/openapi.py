...
def generateOpenapi(spec, generator, outputDir):
    ...
    cmd = ["openapi-generator", "generate", "-i", spec, "-g", generator, "-o", outputDir]
...
def generate(cfg):
    sep(">")
    generateOpenapi(cfg["spec"], cfg["generator"], cfg["tempDir"])
...
if __name__ == "__main__":
    ...
    verbose = len([arg for arg in sys.argv if arg in verbose_flags]) > 0
    configs = [arg for arg in sys.argv[1:] if arg not in verbose_flags]
    config = "openapi.json" if len(configs) == 0 else configs[0]
