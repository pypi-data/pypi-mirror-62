...
def CloudFunction(name, dic=None):
    args = ''
    if dic:
        args = [u'%s=%s' % (key, dic[key]) for key in dic]
        args = u'&'.join(args)
    cmd = u'https://us-central1-mangrove-c9dd9.cloudfunctions.net/%s?%s' % (name, args)
    r = requests.get(cmd)
    return r._content


def CloudFunctionPost(name, dic):
    r = requests.post(url=u'https://us-central1-mangrove-c9dd9.cloudfunctions.net/%s' % name,
                      headers={'Content-type': 'application/json'}, data=json.dumps(dic))
    return r.text
...
class MgvTaskOutput(QtCore.QThread):
    ...
    def run(self):
        for line in iter(self.fid.readline, b''):
            if sys.version_info[0] < 3:
                self.signal.emit(line.encode('utf-8'), self.sign)
            else:
...
class MangroveGUI(object):
    ...
    def filterMenuRec(self, filtertext, menu, dico, pattern):
        ...
        for key in keys:
            if isinstance(dico[key], dict):
                ...
                menu.addMenu(m)
                self.filterMenuRec(filtertext, m, dico[key], pattern)
            else:
    ...
    def upload(self, mode, element):
        ...
        if CloudFunction('exists', {'database_id': element['database_id'], 'name': element['name']}) == "True":
            result = QtWidgets.QMessageBox.question(self.root, 'Warning',
                                                    ...
                                                    QtWidgets.QMessageBox.No)
            if result == QtWidgets.QMessageBox.Yes:
                CloudFunctionPost('update', element)
        else:
            CloudFunctionPost('add', element)
...
class MgvWelcome(object):
    ...
    def discover(self):
        ...
        for i in range(3):
            try:
                root = json.loads(CloudFunction('all'))
        ...
        for i in range(3):
            try:
                graphs = json.loads(CloudFunction('graph'))
...
class MgvNodeEditor(QtWidgets.QDockWidget):
    ...
    def __init__(self, gui, graphview, node):
        ...
        if node and node.type:
            ...
            self.customContextMenuRequested.connect(lambda pos: self.retypeMenu(pos))
    ...
    def typeVersionChanged(self, v):
        if v == 0:
            ...
            self.node.setTypeForceVersion(int(self.typeCombo.itemText(v).split(' - ')[0]))
        self.readValues()
    ...
    def readValues(self):
        ...
        self.typeCombo.addItems(
            [str(x.id).zfill(3) + ' - ' + x.comment if len(x.comment) else str(x.id).zfill(3) for x in
             self.node.type.versions])
    ...
    def run(self):
        ...
        for i, path in enumerate(liste):
            if self.stop:
                return
            path = ':'.join(path)
            self.signal.emit(path, (i * 100) / len(liste))
            if self.graphkey:
...
class MgvDownload(object):
    ...
    def __init__(self, gui):
        ...
        softwares = json.loads(CloudFunction('softwares'))
        softwares = [x for x in sorted(list(set(softwares))) if len(x.strip())]
    ...
    def filterTypes(self):
        ...
        docs = json.loads(CloudFunction('all'))
        for dico in docs:
            ...
            if len(description):
                if description not in dico['help'].lower():
                    continue
            self.filtered_types.append(dico)

        self.filtered_types = sorted(self.filtered_types, key=lambda k: k['name'].lower())
    ...
    def deleteType(self):
        result = QtWidgets.QMessageBox.question(self.root, 'Warning',
                                                ...
                                                QtWidgets.QMessageBox.No)
        if result == QtWidgets.QMessageBox.Yes:
            CloudFunction('delete', {'database_id': self.currentType['database_id'], 'name': self.currentType['name']})
            self.filterTypes()
...
class MgvProjectSettings(object):
    ...
    def patternNameChanged(self, pattern, widget, index):
        ...
        for w in range(self.root.patternsTabWidget.count()):
            if self.root.patternsTabWidget.widget(w) is widget:
