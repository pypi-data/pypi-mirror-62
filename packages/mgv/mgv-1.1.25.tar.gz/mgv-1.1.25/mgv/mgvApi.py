...
def changeWrapper():
    ...
    localSettingsFile = os.getenv('MGVSETTINGS')
    if localSettingsFile is None:
...
def noBytes(dico):
    if isinstance(dico, dict):
        if len(dico.keys()) == 2 and 'data' in dico.keys() and 'type' in dico.keys():
            dico = ''.join([chr(x) for x in dico['data']])
        else:
            liste = list(dico.keys())
            for key in liste:
    ...
    try:
        ...
        for i, output_text in execute(cmd):
            if isString(output_text):
                if not textEdit:
                    if i == 0:
                        print(output_text[:-1], file=sys.__stdout__)
...
class MgvGraph(object):
    ...
    def __init__(self, pattern=None, path=None, uuid=None, template_name='', name='', load=True):
        ...
        self.groups = []
        self.name = name
        if not len(name) and pattern is not None and path is not None:
            self.name = pattern.convertGraphName(path)
        user = os.getenv('USER')
        if not user:
    ...
    def _linksFromString(self, graphview=None):
        """Replace links from string uuid references to
        link objects.
        """
        for node in self.nodes:
