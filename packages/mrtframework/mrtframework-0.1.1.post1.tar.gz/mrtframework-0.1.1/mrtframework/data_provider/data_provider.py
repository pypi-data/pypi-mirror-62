...
class DataProvider:
    ...
    def write(self, pubs: List[Publication]):
        ...
        if isinstance(self.papers, pymongo.collection.Collection):
            ...
            self.papers.update({pub.id: pub for pub in pubs})
            write_json_to_filepath({pid: pub.to_json() for (pid, pub) in self.papers.items()}, self.write_source, source_type='data_sources')
