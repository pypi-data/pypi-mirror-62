...
def get_or_create_custom_metric_descriptor(client, name, description='', display_name=''):
    # try to get
    try:
        client.fetch_metric_descriptor(name)
        return True
    except NotFound:
...
def send_metric(name,
    ...
    get_or_create_custom_metric_descriptor(
        MONITORING_CLIENT, name, description=description, display_name=display_name
    )
