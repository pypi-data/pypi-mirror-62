...
class GetData(dict):
    ...
    def _api_try(self, page):
        ...
        while tries < self.options['api_retry']:
            ...
            sleep((tries+1)**2)
            tries += 1
        response['error_msg'] = str(response['error_msg'])
        return response, resp_headers
    ...
    def _api_call(self, page):
        ...
        if not str(response['status_code']).startswith("2"):
            response['error_msg'] = self.get_error(req)
        else:
            try:
    ...
    def get_error(req):
        try:
            error_msg = json.dumps(req.json())
        except ValueError:
            error_msg = req.text
        # Attempt to Read Naapi Error
        try:
            error_msg = json.loads(error_msg)["ErrorMsg"]
        except (KeyError, ValueError):
            pass
        try:
            error_msg = json.loads(error_msg)["message"]
        except (KeyError, ValueError):
            pass
        return error_msg
