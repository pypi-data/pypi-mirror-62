...
class AbstractMessageChannel(ABC):
    ...
    def _read_response(self) -> dict:
        self._ensure_process()
        string = None
        while string is None and self._process.poll() is None:
            try:
                line = self._response_queue.get_nowait()
            except Empty:
                # If _response_queue is empty recheck while conditions to see if _process.returncode has been set.
                continue
            string = line.decode()
            if len(string) == 0 or string[0] != '{':
                ...
                string = None
        if string is None:
            log.error("Engine crashed from fatal exception during execution.")
            raise RuntimeError("Engine crashed from fatal exception during execution.")
