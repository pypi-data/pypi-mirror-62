...
class API:
  ...
  def argslist (self, part):
    ...
    types = "required int float uuid email none list string boolean dict".split ()
    alls = {}
    for idx, params in enumerate (requirements [:-1]):
      if not params:
        continue
      for param in params:
        if param not in alls:
          alls [param] = []
        alls [param].append (types [idx])

    for param, cond in requirements [-1].items ():
