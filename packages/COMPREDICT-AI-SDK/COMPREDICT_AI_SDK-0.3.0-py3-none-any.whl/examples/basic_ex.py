...
client = api.get_instance(token="5a444bc396e41e780ad2b356e0b69632ddcaaaae", callback_url=callback_url, ppk=ppk,
                          passphrase=passphrase, url="http://localhost:8800/api/v1")
client.fail_on_error(option=False)

body_algorithm = client.get_algorithm("observer")

df = pd.read_csv("test_observer.csv", index_col=None)

with open("test_observer.json") as f:
    data = json.load(f)

run = body_algorithm.run(data, file_content_type="application/json", compression="bz2")
# print(run)
