...
class PUML_Category:
    ...
    def relative_filename(self):
        return f'$AWS_PLANTUML\\{self.name}.puml'

    @property
    def include(self):
        #return '!include {}\n'.format(pu_cat.filename('dist'))
    ...
    def write(self):
        ...
        with open(self.filename('dist'), 'w') as cat:
            for i in self.items():
                output = str(i)
                cat.write(output)

        with open(self.filename('test'), 'w') as f:
            def w(text):
                f.write(text + '\n')

            w('@startuml\n')
            w('!$AWS_PLANTUML="{}"'.format(self.puml.OUT_PATH + '\\dist'))
...
class PlantUML:
    def __init__(self, OUT_PATH=None, AWS_ICON_PATH=None):
        self.OUT_PATH = OUT_PATH or os.getenv('PLANTUML_AWS')
        if self.OUT_PATH:
            print(f'Writing output to %PLANTUML_AWS% = "{self.OUT_PATH}"')
        else:
            raise Exception('No output path specified. Set environment variable PLANTUML_AWS to the directory that the PlantUML code shoul be written to')
    ...
    def source_file(self, name):
        ...
        return os.path.join(self.OUT_PATH, 'dist', name+'.puml')
