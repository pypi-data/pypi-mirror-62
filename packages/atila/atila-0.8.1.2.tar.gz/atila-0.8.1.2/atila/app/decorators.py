...
class Decorators:
    ...
    def _validate_param (self, params, required, ints, floats, emails, uuids, nones, lists, strings, booleans, dicts, **kargs):
        params = params or {}
        ...
        if lists:
            for each in lists:
                ...
                if not isinstance (val, (str, list, tuple)):
                    return 'parameter {} should be list or comma/bar delimtered string'.format (each)
        ...
        if dicts:
            for each in dicts:
                ...
                if isinstance (val, dict):
                    ...
                    return 'parameter {} should be object'.format (each)

        if booleans:
            for each in booleans:
                try: val = params [each]
                except KeyError: continue
                if val in ('yes', 'true'): val = True
                elif val in ('no', 'false'): val = False
                if val in (True, False):
                    params [each] = val
                else:
                    return 'parameter {} should be boolean'.format (each)
    ...
    def require (self, scope = 'ARGS', required = None, ints = None, floats = None, uuids = None, emails = None, nones = None, lists = None, strings = None, booleans = None, dicts = None, **kargs):
        def decorator(f):
            ...
            def wrapper (was, *args, **kwargs):
                ...
                more_info = self._validate_param (validatable_args, required, ints, floats, emails, uuids, nones, lists, strings, booleans, dicts, **kargs)
                if more_info:
                    return was.response.adaptive_error ("400 Bad Request", 'missing or bad parameterin {}'.format (scope_), 40050, more_info)
