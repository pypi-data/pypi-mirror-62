...
class Executor:
	...
	def __call__ (self):
		...
		try:
			...
			if html_minify and current_app.config.get ('MINIFY_HTML') and len (content) == 1 and isinstance (content [0], str) and self.was.response.get_header ('content-type') in ('text/html', None):
		...
		if current_app.debug and current_app.expose_spec and request.channel.addr [0].startswith ("127.0.0.") and isinstance (content [0], API):
