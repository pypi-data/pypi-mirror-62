...
class WAS (wsgiappservice.WAS):
    ...
    def make_uid (self, s):
        return base64.encodestring (md5 (s.encode ()).digest ()) [:-3].decode ().replace ('/', '-').replace ('+', '_')
    ...
    def csrf_token (self):
        ...
        if self.CSRF_NAME not in self.session:
            self.session [self.CSRF_NAME] = hex (random.getrandbits (64)) [2:]
    ...
    def verify_csrf (self, keep = False):
        token = self.request.get_header ('x-csrf-token', self.request.args.get (self.CSRF_NAME))
        if not token:
            return False
