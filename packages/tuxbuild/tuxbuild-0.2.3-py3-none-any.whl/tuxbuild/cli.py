...
def wait_for_object(build_object):
    ...
    state_items = [
        ...
        {
            "state": "building",
            "message": click.style("🚀 Building: ", fg="cyan", bold=True),
        },
    ]
...
@click.option(
    "--toolchain",
    required=True,
    help="option to choose between gcc-8, gcc-9, clang-8 and clang-9",
)
@click.option(
    "--kconfig-allconfig",
    required=False,
...
@click.option("--git-repo", required=True, help="Git repository")
@click.option("--git-ref", required=True, help="Git reference")
@click.option("--target-arch", required=False, help="Target architecture")
@click.option(
    "--kconfig",
    required=False,
    multiple=True,
...
def build_set(
    ...
    for b in build_list:
        ...
        b["kconfig_allconfig"] = params.get(
            "kconfig_allconfig", b.get("kconfig_allconfig", None)
        )
