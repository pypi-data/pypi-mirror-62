...
from ehealthkd.score import compute_metrics, subtaskA, subtaskB
...
def evaluate_one(
    ...
    scenario1 = dict(
        evaluate_scenario(scenario1_submit, scenario1_gold, 1)
    )
    scenario2 = dict(
        evaluate_scenario(scenario2_submit, scenario2_gold, 2)
    )
    scenario3 = dict(
        evaluate_scenario(scenario3_submit, scenario3_gold, 3)
    )
...
def filter_best(results):
    ...
    for user, submits in results.items():
        ...
        best[user] = dict(
            scenario1=best1, scenario2=best2, scenario3=best3
        )
    ...
    if single:
        ...
        for subfolder in submits.iterdir():
            ...
            users[submits.name].append(
                evaluate_one(subfolder, scn1_gold, scn2_gold, scn3_gold,)
            )
    else:
        for userfolder in submits.iterdir():
            ...
            for subfolder in userfolder.iterdir():
                ...
                if not run.exists():
                    raise ValueError(
                        "Directory {0} not found. Did you mean to use --single? Check --mode option.".format(run)
                    )
                users[userfolder.name].append(
                    evaluate_one(subfolder.name, run, scn1_gold, scn2_gold, scn3_gold,))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--single",
        metavar="SUBMITS",
        help="if passed, then SUBMITS points to a single participant's name with submission folders inside, otherwise SUBMITS points to a folder (./../evaluation/submissions/) with many participants, each with submission folders inside.",
    )
    parser.add_argument(
        "--best",
        action="store_true",
    ...
    parser.add_argument(
        ...
        default="../evaluation/submissions",
    )
