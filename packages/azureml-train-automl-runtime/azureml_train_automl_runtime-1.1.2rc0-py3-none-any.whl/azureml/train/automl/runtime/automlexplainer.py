...
from azureml.automl.core.model_explanation import _convert_explanation, ModelExpGithubNBLink
...
from azureml.train.automl.exceptions import OptionalDependencyMissingException, ConfigException, SystemException
...
def retrieve_model_explanation(child_run):
    ...
    logging.warning("retrieve_model_explanation() will be deprecated. " +
                    ...
                    ModelExpGithubNBLink + " to compute explanations for AutoML models and consuming them.")
    try:
...
def explain_model(fitted_model, X_train, X_test, best_run=None, features=None, y_train=None, **kwargs):
    ...
    msg = "explain_model() will be deprecated. Please use the workflow described at the link " + \
          ModelExpGithubNBLink + " to compute explanations for AutoML models."
    logging.warning(msg)
    logger.warning(msg)
    run_id = ""
    try:
