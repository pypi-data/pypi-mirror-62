...
class AutoMLStep(PipelineStep):
    ...
    .. remarks::
        ...
        This class is deprecated. Use the :class:`azureml.pipeline.steps.automl_step.AutoMLStep` class.
    """
    ...
    def __init__(self, name, automl_config, inputs=None, outputs=None, script_repl_params=None,
        ...
        :param automl_config: An AutoMLConfig that defines the configuration for this AutoML run. The
            file path specified in the ``data_script`` configuration parameter must end with a file named
            "get_data.py".
        ...
        :type version: str
        :param hash_paths: A list of paths to hash when checking for changes to the step contents.
        ...
        """
        warnings.warn(
            "The AutoMLStep in this pacakge is deprecated and will soon be removed from this pacakge. Please "
            "use the AutoMLStep in azureml-pipeline-steps.")
        if name is None:
            raise ConfigException('name is required', has_pii=False)
        if not isinstance(name, str):
            raise ConfigException('name must be a string', has_pii=False)

        if automl_config is None:
            raise ConfigException('automl_config is required', has_pii=False)
        if not isinstance(automl_config, AutoMLConfig):
            raise ConfigException('Unexpected automl_config type', has_pii=False)
    ...
    def create_node(self, graph, default_datastore, context):
        ...
        if self._script_name is not None:
            ...
            hash_paths.append(self._automl_config.user_settings.get("data_script"))
    ...
    def _get_automl_settings(self, context):
        ...
        compute_target = self._automl_config.fit_params.get('compute_target')
        if compute_target is None:
            raise ConfigException('compute_target is not provided to AutoMLConfig', has_pii=False)
        try:
            settings['MLCComputeType'] = compute_target.type
        except AttributeError as e:
            raise ConfigException('compute_target is not an object of ComputeTarget.', has_pii=False) from e
    ...
    def _get_runconfig_as_dict(self, run_config=None):
        ...
        if not isinstance(run_config, RunConfiguration):
            raise ConfigException('run_configuration is required', has_pii=False)
            ...
            if isinstance(arg_value, PipelineOutputTabularDataset):
                if self._passthru_automl_config:
                    raise ConfigException('passthru_automl_config should be set to False when constructing the '
                                          'AutoMLStep to use PipelineOutputTabularDataset.', has_pii=False)
...
class AutoMLStepRun(AutoMLRun):
    ...
    .. remarks::
        ...
        This class is deprecated. Use the :class:`azureml.pipeline.steps.automl_step.AutoMLStepRun` class.
    """
    ...
    def __init__(self, step_run):
        ...
        """
        warnings.warn(
            "The AutoMLStepRun in this pacakge is deprecated and will soon be removed from this pacakge. Please "
            "use the AutoMLStepRun in azureml-pipeline-steps.")
    ...
    def get_default_metrics_output(self):
        """
        Return default metrics output of the current run.
    ...
    def get_default_model_output(self):
        """
        Return default model output of the current run.
