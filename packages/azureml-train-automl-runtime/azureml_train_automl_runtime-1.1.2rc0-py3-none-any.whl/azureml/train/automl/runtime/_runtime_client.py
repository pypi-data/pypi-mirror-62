...
from automl.client.core.common.exceptions import (
    ClientException, DataException, ErrorTypes, MissingValueException, ServiceException)
...
from azureml.train.automl.utilities import friendly_http_exception, _get_package_version, _raise_exception
from . import _automl
...
class RuntimeClient:
    ...
    def _create_parent_run_for_local(self,
        ...
        if not existing_run:
            ...
            try:
                ...
                self.azureautomlclient._raise_feature_unavailable_exception(e)
                _raise_exception(e)
    ...
    def _fit_local(self, X=None, y=None, sample_weight=None, X_valid=None, y_valid=None, sample_weight_valid=None,
        ...
        try:
            ...
            if self.azureautomlclient._is_any_childruns_succeed(self.azureautomlclient.current_run):
                try:
                ...
                raise ModelFitException("All child runs failed.")
            ...
            self.azureautomlclient._fail_parent_run(error_details=e, logger=self.azureautomlclient.logger)
            self.azureautomlclient.current_run.add_properties({
                'error_code': getattr(e, 'error_type', ErrorTypes.Unclassified),
                'failure_reason': getattr(e, 'error_code', ErrorTypes.Unclassified)
            })
        except Exception as e:
            self.azureautomlclient._console_writer.println("Exception thrown. Terminating runs.")
            self.azureautomlclient._fail_parent_run(error_details=e, logger=self.azureautomlclient.logger)
            self.azureautomlclient.current_run.add_properties({
                'error_code': getattr(e, 'error_type', ErrorTypes.Unclassified),
                'failure_reason': getattr(e, 'error_code', ErrorTypes.Unclassified)
    ...
    def _fit_iteration(self,
        ...
        try:
            ...
            if fit_output.errors:
                ...
                if err_type == 'model_explanation' and isinstance(exception_obj, ImportError):
                    ...
                    errors.append('Run {} failed with exception.'.format(task_dto.childrun_id))
        ...
        self._update_internal_scores_after_iteration(score, model_name)
    ...
    def _update_internal_scores_after_iteration(self, score, model_name):
