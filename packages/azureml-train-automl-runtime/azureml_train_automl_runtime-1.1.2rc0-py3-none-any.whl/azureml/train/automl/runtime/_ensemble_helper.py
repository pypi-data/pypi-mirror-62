...
class EnsembleHelper(object):
    ...
    def __init__(self,
        ...
        if automl_settings is None:
            raise ConfigException("automl_settings parameter should not be None")

        if ensemble_run_id is None:
            raise ConfigException("ensemble_run_id parameter should not be None")

        if experiment_name is None:
            raise ConfigException("experiment_name parameter should not be None")

        if subscription_id is None:
            raise ConfigException("subscription_id parameter should not be None")

        if resource_group_name is None:
            raise ConfigException("resource_group_name parameter should not be None")

        if workspace_name is None:
            raise ConfigException("workspace_name parameter should not be None")
        ...
        if hasattr(self._automl_settings, "ensemble_download_models_timeout_sec"):
            ...
            self._download_models_timeout_sec = \
                self._automl_settings.ensemble_download_models_timeout_sec  # type: ignore
        else:
