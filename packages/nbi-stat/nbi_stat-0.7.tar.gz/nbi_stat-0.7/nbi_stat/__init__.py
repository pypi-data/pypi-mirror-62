...
# 
# Generated 2020-03-04 23:21:09.206942 UTC
...
def _west_merge(ma,cva,w1a,w2a,wa,
    ...
    from numpy import less_equal, abs, ones, zeros, ndim, outer, multiply, argmax
    ...
    def upd(num,den,base=0):
        if ndim(den) == 0 or len(den) == 1:
            return num / den if den != 0 else base * ones(num.shape)
        
        msk    = den != 0
        a      = base * ones(num.shape)
...
def chi2nu(x,y,f,theta,delta=None,deltax=None,df=None,df_step=None):
    ...
    if deltax is not None and len(deltax) == len(delta):
        d2 = effective_variance(x,deltax,f,theta,d2,df,df_step)
...
def plot_fit_table(p,ep,nsig=1,
    ...
    from matplotlib.pyplot import gca
    from numpy import floor, log10, abs, ndim, sqrt, diagonal
    ...
    for pi, (pv, pe, po) in enumerate(zip(p, ep, pars)):
        ...
        te             =  fr"${re[0]:.{ndig}f}{rp}$"   if pe is not None else ''
    ...
    if table:
        ...
        if ochi2:
            if isinstance(ochi2,bool):
                chisqnu = chi2nu(x,y,f,p,delta,xdelta,df)
            else:
...
See also
--------
plot_fit_table, plot_fit_func, fit, lsq_fit
"""
...
def maximize_llh(f,data,p0,tomax,*args,kw={},**kwargs):
    ...
    with errstate(all='ignore'):
        opt = minimize(tomin, p0, *args, **kwargs)
...
if __doc__ is not None:
    ...
    __doc__ += \
"""
2020-03-04 23:21:09.207942 UTC
"""
