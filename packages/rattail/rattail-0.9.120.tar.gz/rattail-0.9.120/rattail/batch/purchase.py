...
class PurchaseBatchHandler(BatchHandler):
    ...
    * ``rattail.enum.PURCHASE_BATCH_MODE_COSTING``
    ...
    def should_populate(self, batch):
        """
        Overrides default logic as follows:

        If the new batch is either "receiving" or "costing" mode, and the batch
        has an original "purchase" attached, then returns ``True`` so that the
        new batch is populated from this purchase.  Otherwise returns ``False``.
        """
    ...
    def populate(self, batch, progress=None):
        """
        Logic to populate the batch is as follows:

        The :attr:`~rattail.db.model.batch.purchase.PurchaseBatch.order_quantities_known`
        attribute is set to ``True`` for the batch, since population will only
        happen for "receiving" and "costing" batches which have an original
        purchase attached.

        If the batch is a "truck dump child" and does not yet have a receiving
        date, it is given the same one as the parent batch.

        A new data row is then added to the batch, for each line item on the
        original purchase.
        """
    ...
    def make_row_from_invoice(self, batch, invoice_row):
        ...
        assumed to be a :class:`~rattail.db.model.batch.vendorinvoice.VendorInvoiceRow` object
        as obtained from an invoice parser.
        """
        row = model.PurchaseBatchRow()
