...
########### load ราคาย้อนหลัง ######################
def loadHistData(symbol,start,end="",Volume=False,OHLC=False):
    """
    ...
    if(isinstance(symbol, list)):
      print(symbol)
      qs = crateQueryString(symbol,Volume=Volume,OHLC=OHLC)
      k = qs.replace(".bk","")
      symbol = k.split(",")
      #if(Volume==True):
      #  tmpsymbol = []
...
def listShareholders_Preprocess(symbol,eventdate):
  ...
  for i in k[0].find_all("tr"):
   ...
   Shares  = (tr[2].text.strip()) 
   Percent = (tr[3].text.strip())
    
   dsSymbol.append(symbol)   
   dsName.append(name)
   dsShares.append(Shares)
   dsPercent.append(Percent) 
...
#%config InlineBackend.figure_format ='retina'
def RelativeStrength_hist(dateStart,setUni,plotState):
 symbol = ["tdex"] 
 ...
 busketDS = getMemberOfIndex(sx.indexMarket.SET50.value)   
 ...
 for i in range(cnt):
    ...
    df = loaddata.get(qs,start=dateStart)
    df.columns = [symbol]
 ...
 if(plotState):   
  ...
  for i in range(10):
   for j in range(5):  
    ...
    ax = tmpRSX_rank[symbol].plot(ax=axes[i,j],figsize=(20,30),title=symbol) 
...
def findCASH(s):
 symbol = s
 #load data
 r = requests.get("https://www.set.or.th/set/factsheet.do?symbol="+symbol+"&ssoPageId=3&language=th&country=TH")
 r.content
 soup = BeautifulSoup(r.content, "lxml")  
...
def commonsizeBS(symbol):
 r = requests.get("https://www.set.or.th/set/companyfinance.do?type=balance&symbol="+symbol+"&language=th&country=TH")
 r.content
 soup = BeautifulSoup(r.content, "lxml") 
...
def commonsizeIS(symbol):
   r = requests.get("https://www.set.or.th/set/companyfinance.do?type=income&symbol="+symbol+"&language=th&country=TH")
   r.content
   soup = BeautifulSoup(r.content, "lxml") 
...
def getFinanceRatioProcess(symbol): 
   headers = {"User-Agent": "Mozilla/5.0 (Windows NT 6.1; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/58.0.3029.110 Safari/537.36"}
   r = requests.get("https://www.set.or.th/set/factsheet.do?symbol="+symbol+"&language=en&country=US",headers)
   r.content
   soup = BeautifulSoup(r.content, "lxml") 
...
def getBalanceSheet(symbol):  
 symbolMofi = symbol.replace("&","%26") 
 headers = {"User-Agent": "Mozilla/5.0 (Windows NT 6.1; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/58.0.3029.110 Safari/537.36"}   
 r = requests.get("https://www.set.or.th/set/factsheet.do?symbol="+symbolMofi,headers)
 r.content
 soup = BeautifulSoup(r.content, "lxml") 
...
def evperebitda(symbol):
  if type(symbol) is str:
    r = requests.get("https://quotes.wsj.com/TH/XBKK/"+symbol+"/financials")
    r.content
    soup = BeautifulSoup(r.content, "lxml")  
  ...
  if type(symbol) is list:
   ...
   for i in range(len(symbol)):
     symbol_ = symbol[i]
     r = requests.get("https://quotes.wsj.com/TH/XBKK/"+symbol_+"/financials")
     r.content
     soup = BeautifulSoup(r.content, "lxml")  
     g_data = soup.find_all("table", {"class" : "cr_dataTable cr_sub_valuation"})
...
def getProfitAbility(symbol):   
 ...
 if type(symbol) is list:
  ...
  for i in range(len(symbol)):
    tmp = symbol[i].lower()    
    r = requests.get("https://quotes.wsj.com/TH/XBKK/"+tmp+"/financials")
    r.content
    soup = BeautifulSoup(r.content, "lxml")  
    g_data = soup.find_all("table", {"class" : "cr_dataTable cr_sub_profitability"})
...
def getMemberOfIndex(indexType):
 ...
 r = requests.get("https://marketdata.set.or.th/mkt/sectorquotation.do?sector="+symbol+"&language=th&country=TH",verify=False,headers=headers)
 r.content
 soup = BeautifulSoup(r.content, "lxml")  
...
def getLastPrice(symbol,date="now",ordersymbol=False):
 if type(symbol) is str: 
   r = requests.get("https://marketdata.set.or.th/mkt/stockquotation.do?symbol="+symbol+"&ssoPageId=1&language=th&country=TH")
   r.content
   soup = BeautifulSoup(r.content, "lxml")  
   ...
   ds = []     
   for i in symbol:
     r = requests.get("https://marketdata.set.or.th/mkt/stockquotation.do?symbol="+i+"&ssoPageId=1&language=th&country=TH")
     r.content
     soup = BeautifulSoup(r.content, "lxml")  
     g_data = soup.find_all("table", {"class" : "table table-hover table-set col-3-center table-set-border-yellow"})
     s = g_data[0].find_all("td")[5].text.strip()  
...
def listStockInSector(sample_in_sector):
 headers = {"User-Agent": "Mozilla/5.0 (Windows NT 6.1; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/58.0.3029.110 Safari/537.36"}   
 r = requests.get("http://www.settrade.com/C04_08_stock_sectorcomparison_p1.jsp?txtSymbol="+sample_in_sector,headers=headers)
 r.content
 soup = BeautifulSoup(r.content, "lxml")  
...
def getdata(symbol,returntype="ar"):
 ...
 datasource = []   
 headers = {"User-Agent": "Mozilla/5.0 (Windows NT 6.1; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/58.0.3029.110 Safari/537.36"}      
 r = requests.get("http://www.settrade.com/C04_06_stock_financial_p1.jsp?txtSymbol="+symboltmp,headers)
 r.content
 soup = BeautifulSoup(r.content, "lxml")  
