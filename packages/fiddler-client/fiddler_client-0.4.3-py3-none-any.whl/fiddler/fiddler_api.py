...
class FiddlerApi:
    ...
    def __init__(self, url: str, org_id: str, auth_token: str,
                 verbose=False):
        ...
        try:
            ...
            LOG.warning(f'API CHECK FAILED: Able to connect to {self.url}, '
    ...
    def _call(self,
        ...
        with contextlib.ExitStack() as context_stack:
            ...
            LOG.debug(f'running api call as {request_type} request\n'
                      f'to {endpoint}\n'
            ...
            if is_get_request:
                ...
                if files is not None:
                    ...
                    opened_files = {
                        fpath.name: context_stack.enter_context(
                            fpath.open('rb'))
                        for fpath in files
                    }
                    '''
        ...
        if res.status_code == 401:
            error_msg = (f'API call to {endpoint} failed with status 401: '
        ...
        if stream:
            return self._process_streaming_call_result(res, endpoint,
                                                       raw_request_info)
        return self._process_non_streaming_call_result(
            res, endpoint, raw_request_info)

    @staticmethod
    def _raise_on_status_error(
            response: requests.Response,
            endpoint: str,
            raw_request_info: str):
        ...
        try:
            ...
            try:
                failure_message = response_payload.get(
                    'message', 'Unknown')
                failure_stacktrace = response_payload.get(
                    'stacktrace')
                error_msg = (
                    f'API call failed.\n'
                ...
                error_msg = (f'API call to {endpoint} failed.\n'
    ...
    def _process_non_streaming_call_result(
        ...
        log_msg = (f'API call to {endpoint} succeeded.\n'
    ...
    def _query_dataset(self, dataset_id: str, fields: List[str],
                       max_rows: int, split: Optional[str] = None,
                       stream=True, sampling=False, sampling_seed=0.0):
        query = dict(fields=fields, limit=max_rows)
        if sampling:
            query['sampling'] = True
            query['sampling_seed'] = sampling_seed
        if split is not None:
            query['source'] = f'{split}.csv'
        path = ['dataset_query', self.org_id, dataset_id]
        return self._call(path, query, stream=stream)

    def get_dataset(self, dataset_id: str, max_rows: int = 1_000,
                    splits: Optional[List[str]] = None, sampling=False,
                    dataset_info: Optional[DatasetInfo] = None,
                    include_fiddler_id=False, stream=True) \
        ...
        def get_df_from_split(split):
            dataset_rows = self._query_dataset(
                dataset_id, fields=dataset_info.get_column_names(),
                max_rows=max_rows, split=split, sampling=sampling,
                stream=stream)
            return df_from_json_rows(dataset_rows, dataset_info,
                                     include_fiddler_id=include_fiddler_id)

        if splits is None:
            use_splits = [os.path.splitext(filename)[0]
    ...
    def get_slice(self, sql_query: str, project: Optional[str],
                  columns_override: Optional[List[str]] = None, stream=True)\
            -> pd.DataFrame:
        ...
        payload = dict(sql=sql_query, project=project)
        ...
        if not slice_info['is_slice']:
            raise RuntimeError('Query does not return a valid slice. '
                               'Query: ' + sql_query)
    ...
    def _upload_dataset_csvs(self, dataset_id: str,
                             csv_file_paths: List[pathlib.Path],
        ...
        return self._call(path, json_payload=payload, files=csv_file_paths)
    ...
    def upload_dataset(self,
        ...
        inferred_info = DatasetInfo.from_dataframe(dataset.values(),
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for name, df in dataset.items():
                ...
                LOG.info(f'[{name}] dataset upload: '
            ...
            self._upload_dataset_csvs(dataset_id, csv_paths,
                                      split_test=split_test,
                                      dataset_info=new_schema)

        # trigger dataset upload
        LOG.info(f'[{dataset_id}] dataset upload: '
                 f'triggering import of dataset')
        res = self._db_import(dataset_id)
        return res

    def run_model(self, project_id: str, model_id: str,
        ...
        payload = dict(data=df.to_dict(orient='records'))
    ...
    def run_explanation(
        ...
        deserialize_fn = (MulticlassAttributionExplanation.from_dict
                          if is_multiclass
                          else AttributionExplanation.from_dict)
        ex_objs = [deserialize_fn(explanation_dict)
                   for explanation_dict in explanations_list]
        if len(ex_objs) == 1:
            return ex_objs[0]
        else:
            return ex_objs
    ...
    def run_feature_importance(
        ...
        if dataset_splits is not None and len(dataset_splits) > 1:
            raise NotImplementedError('Unfortunately, only a single split is '
    ...
    def create_model(self, project_id: str, dataset_id: str,
                     target: str,
        ...
        payload = dict(dataset=dataset_id, model_name=model_id,
                       source=source, inputs=features, targets=[target])
        return self._call(path, json_payload=payload)

    def upload_model_sklearn(
            self, model, info: ModelInfo, project_id: str, model_id: str,
        ...
        assert ' ' not in model_id, \
            'The model identifier should not contain whitespace'
        ...
        with tempfile.TemporaryDirectory() as tmp:
            pickle_path = pathlib.Path(tmp) / 'model.pkl'
    ...
    def upload_model_custom(
            self, artifact_path: pathlib.Path, info: ModelInfo,
        ...
        with tempfile.TemporaryDirectory() as tmp:
            tarfile_path = pathlib.Path(tmp) / 'files'
            shutil.make_archive(base_name=str(pathlib.Path(tmp) / 'files'),
                                format='tar',
                                root_dir=str(artifact_path),
                                base_dir='.')
            LOG.info(f'[{model_id}] model upload: uploading custom model from'
                     f' artifacts in {str(artifact_path)} tarred to '
                     f'{str(tarfile_path)}')
            return self._call(endpoint_path, json_payload=payload,
                              files=[pathlib.Path(tmp) / 'files.tar'])
    ...
    def trigger_model_predictions(self, project_id: str, model_id: str,
                                  dataset_id: str):
        ...
        return self._call(['dataset_predictions', self.org_id, project_id],
                          payload)
